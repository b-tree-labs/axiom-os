...
class ArtifactBackend(Protocol):
    ...
    def find_by_name(
        self, kind: str, name: str, include_deleted: bool = False
    ) -> list[Artifact]: ...
    def list_all(
    ...
    def find_by_name(
        ...
        return sorted(
            (
                ...
                and (include_deleted or not a.deleted)
            ),
...
class SQLiteBackend:
    ...
    def find_by_name(
        ...
        """Keyed (kind, name) lookup — rides idx_artifacts_kind_name.

        Replaces the list-then-filter scan paths (ADR-087 P1): same
        row set and ordering (non-deleted by default, ``created_at``
        ascending), O(rows-for-name) instead of O(rows-for-kind).
        """
        clauses = ["kind = ?", "name = ?"]
    ...
    def mark_deleted(self, artifact_id: str, reason: str | None) -> None:
        # `UPDATE ... WHERE id = ?` matches zero rows for an id that is not
        # there, commits happily, and returns None — so this backend used to
        # accept a delete the in-memory backend rejects with KeyError. Tests run
        # on in-memory and installs run on SQLite, which made "deleting a
        # missing artifact raises" a check that could never fail where it
        # mattered. rowcount is the only evidence the statement had an effect.
        cur = self._conn.execute(
...
class ArtifactRegistry:
    ...
    def find_by_name(
        ...
        """Keyed (kind, name) lookup: non-deleted by default,
        ``created_at`` ascending. The scan-free path for fragment-id
        resolution (ADR-087 P1)."""
        return self._backend.find_by_name(
            kind, name, include_deleted=include_deleted
        )
    ...
    def version_chain(self, *, kind: str, name: str) -> list[Artifact]:
        ...
        return self._backend.find_by_name(kind, name, include_deleted=True)

    def latest(self, *, kind: str, name: str) -> Artifact | None:
        """Most recent non-deleted artifact for (kind, name), or None."""
        live = self._backend.find_by_name(kind, name, include_deleted=False)
