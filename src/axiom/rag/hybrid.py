...
def rerank_score(
    *,
    ...
    authority_corpus: str | None = None,
    authority_pattern: re.Pattern[str] | None = None,
    extra_code_pattern: re.Pattern[str] | None = None,
) -> float:
    """Heuristic rerank score for one fused candidate.

    Higher = more value-bearing, on-topic, authoritative. Shared by the hybrid
    stack (``_rerank``) and the store-abstracted ``retriever.retrieve`` so both
    order candidates by the SAME signals: query-term ``coverage``, a
    ``value_intent`` bonus when a value-seeking query hits a chunk carrying
    numbers, a ``corpus_authority`` bonus, and normalized ``rrf``. With no query
    terms it degrades to the RRF order. See :class:`RerankWeights`.
    """
    if not query_terms:
        return rrf / max_rrf if max_rrf else 0.0
    body = text or ""
    coverage = len(
        query_terms & set(fts_terms(body, extra_code_re=extra_code_pattern))
    ) / len(query_terms)
    value = 1.0 if (value_query and _VALUE_TOKEN_RE.search(body)) else 0.0
    authoritative = (authority_corpus is not None and corpus == authority_corpus) or (
        authority_pattern is not None
        and (
            bool(authority_pattern.search(source_path or ""))
            or bool(authority_pattern.search(source_title or ""))
        )
    )
    rrf_norm = rrf / max_rrf if max_rrf else 0.0
    return (
        weights.coverage * coverage
        + weights.value_intent * value
        + weights.corpus_authority * (1.0 if authoritative else 0.0)
...
def _rerank(
    ...
    def score(c: _Candidate) -> float:
        ...
        return rerank_score(
            ...
            value_query=value_q,
            weights=w,
            authority_corpus=cfg.authority_corpus,
            authority_pattern=pattern,
            extra_code_pattern=cfg.extra_code_pattern,
        )
