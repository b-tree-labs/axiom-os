...
def _col(row: object, key: str, default: Any) -> Any:
    """Read ``key`` from a sqlite3.Row, returning ``default`` when absent/NULL."""
    try:
...
def _sqlite_nats(raw: object) -> tuple[str, ...] | None:
    """Parse the SQLite ``allowed_nationalities`` JSON-array column (NULL =
    unrestricted) into a tuple, mirroring the Postgres TEXT[] shape."""
    if not raw:
        return None
    try:
        vals = json.loads(raw)  # type: ignore[arg-type]
...
class SQLiteRAGStore:
    ...
    def __init__(self, database_url: str) -> None:
        if database_url.startswith("sqlite:///"):
            ...
            raw = database_url
        # `sqlite:///~/.axi/rag.db` must land in $HOME, not create a literal
        # `~` directory relative to the current working directory.
        self._db_path = str(Path(raw).expanduser())
    ...
    def search(
        ...
        if query_embedding is not None and self._vec_available and self._vec_table_created:
            ...
            vec_sql = (
                ...
                " c.access_tier, c.classification, c.allowed_nationalities"
                " FROM chunks_vec v"
            ...
            for r in filtered[:limit]:
                ...
                results.append(
                    SearchResult(
                        ...
                        corpus=r["corpus"],
                        access_tier=_col(r, "access_tier", "public"),
                        classification=_col(r, "classification", "unclassified"),
                        allowed_nationalities=_sqlite_nats(_col(r, "allowed_nationalities", None)),
                    )
                )
        ...
        if not results and query_text.strip():
            fts_sql = (
                ...
                f" c.access_tier, c.classification, c.allowed_nationalities"
                f" FROM chunks_fts f"
