...
def _load_env() -> None:
    ...
    # Fall back to whichever local candidate ACTUALLY connects.
    #
    # RAG used to build a URL from the stored AXIOM_PG_PASSWORD and use it
    # unconditionally. Nothing else on the platform honours that secret —
    # `axi status`, `infra.db`, signals, update and test all resolve through
    # AXIOM_DB_URL or the shared default — so when the two disagreed, RAG was
    # the only component that could not reach the database. It disagreed
    # here: the compose file seeds POSTGRES_PASSWORD only on FIRST init, so a
    # volume created before the secret was generated keeps the older
    # password, and the secret drifts silently.
    #
    # Resolving by connecting rather than by guessing survives that drift in
    # either direction — a stale secret, or a stale default after someone
    # rotates the real password.
    if not os.environ.get("DATABASE_URL"):
        chosen = _first_reachable_url()
        if chosen:
            os.environ["DATABASE_URL"] = chosen


def _candidate_urls() -> list[tuple[str, str]]:
    """(label, url) pairs to try, in order of how authoritative they are."""
    candidates: list[tuple[str, str]] = []
    explicit = os.environ.get("AXIOM_DB_URL")
    if explicit:
        candidates.append(("AXIOM_DB_URL", explicit))
    try:
        ...
        if pg_pass:
            candidates.append(
                ("stored AXIOM_PG_PASSWORD",
                 f"postgresql://axiom:{pg_pass}@localhost:5432/axiom_db")
            )
    except Exception:  # noqa: BLE001
        pass
    try:
        ...
        candidates.append(("platform default", DEFAULT_DB_URL))
    except Exception:  # noqa: BLE001
        pass
    return candidates


def _first_reachable_url() -> str:
    """Return the first candidate that accepts a connection, else "".

    Labels are logged, never URLs: a connection string carries a password.
    """
    import logging
    ...
    for label, url in _candidate_urls():
        ...
        try:
            ...
            log.debug("rag: %s did not connect: %s", label, str(exc).splitlines()[0][:80])
            continue
        log.debug("rag: using the database from %s", label)
        return url
    if tried:
        log.debug("rag: no candidate connected (tried: %s)", ", ".join(tried))
    return ""


def _get_store():
    # Route through the factory so every scheme it supports is reachable
    # from the CLI. This used to construct RAGStore (psycopg2) directly, so
    # the shipped zero-infra SQLite backend died with `invalid dsn` on a
    # `sqlite:///` URL (sandbox audit 2026-09-18, gap 4).
    ...
    if not url:
        print(
            ...
            f'  Zero-infra:  {_brand_cli()} settings set rag.database_url "sqlite:///~/.axi/rag.db"\n'
            f'  PostgreSQL:  {_brand_cli()} settings set rag.database_url "postgresql://..."\n'
            "  Or:          export DATABASE_URL=<either of the above>",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _print_storage(store) -> None:
    """Corpus size, printed by default rather than behind a flag.

    Size was discovered as a node outage because no number was in front of
    anyone. A figure available only to someone who thinks to ask for it is the
    same as no figure. Never raises: a status report must not fail because one
    of its sections could not be gathered.
    """
    from axiom.rag.storage_stat import collect, render

    try:
        conn = getattr(store, "conn", None) or getattr(store, "_conn", None)
        if conn is None:  # SQLite and remote peers report no pg storage figures
            return
        ...
        print(f"  (storage figures unavailable: {type(exc).__name__})")
        print()
...
def cmd_status(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        print()
        _print_storage(store)
        from axiom.infra.cli_format import SQUARE, Column, table, terminal_width

        rows = [
            (
                corpus.replace("rag-", ""),
                str(s["documents_by_corpus"].get(corpus, 0)),
                str(s["chunks_by_corpus"].get(corpus, 0)),
            )
        ...
        for line in table(
            rows,
            [
                Column("Corpus", wrap=True),
                Column("Docs", align="right"),
                Column("Chunks", align="right"),
            ],
            ...
            try:
                ...
                if not candidates:
                    print(
                        ...
                        f"  Download: {_brand_cli()} update --community-rag",
                        file=sys.stderr,
                    )
                    sys.exit(1)
...
def cmd_sync(args: argparse.Namespace) -> None:
    ...
    if target == "org":
        ...
        print(
            ...
            "\n"
            f"  {_brand_cli()} rag index runtime/knowledge/org-corpus/ --corpus rag-org\n"
            "\n"
            "Configure your server path in runtime/config/rag.toml.\n"
            f"Automated sync ({_brand_cli()} rag sync org --auto) will be available in v2."
        )
    else:
...
def cmd_verify(args: argparse.Namespace) -> None:
    ...
    icon = {Status.PASS: Glyph.OK, Status.WARN: Glyph.WARN, Status.FAIL: Glyph.FAIL}
...
def main(argv: list[str] | None = None) -> None:
    ...
    # WARNING by default, not INFO. basicConfig sets the ROOT level, so INFO
    # published anything INFO-level from anywhere in the process onto a user's
    # search results — five "Provider loaded" lines from the LLM gateway
    # before the first hit. Diagnostics are what `-v` is for; the command's
    # own output is what the user asked for.
    ...
    # basicConfig sets the ROOT LOGGER's level, and that is not what decides
    # whether an axiom record reaches the console. `axiom_logging` sets the
    # `axiom` logger to DEBUG on purpose — "handlers filter; root accepts
    # all" — which is true of the ring and incident handlers it installs, but
    # level filtering happens at the ORIGINATING logger. So every axiom DEBUG
    # record is created, propagates to root, and prints through any console
    # handler that has no level of its own. The handler needs one.
