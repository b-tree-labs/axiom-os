...
def evaluate_grounding_scores(
    scored_sources: Sequence[tuple[float | None, str]],
    threshold: GroundingThreshold = _DEFAULT_THRESHOLD,
) -> GroundingDecision:
    """Evaluate grounding from raw ``(score, source_id)`` pairs.

    The score-and-source primitive under :func:`evaluate_grounding`. Any
    retrieval-result type — the memory ``Citation`` here, the rag serving
    ``RetrievedChunk`` (``similarity`` / ``source_path``), a future one —
    can be gated by projecting it to ``(score, source_id)`` pairs, so this
    domain-agnostic gate never has to depend on that type. A ``None`` score
    is treated as ``0.0`` so a retriever that doesn't emit scores cannot
    silently bypass ``min_top_score`` / ``min_avg_score``.
    """
    citation_count = len(scored_sources)
    ...
    scores = [float(s) if s is not None else 0.0 for s, _ in scored_sources]
    top_score = max(scores)
    avg_score = sum(scores) / len(scores)
    distinct_sources = len({src for _, src in scored_sources})
...
def evaluate_grounding(
    ...
    Thin adapter over :func:`evaluate_grounding_scores` for the memory
    ``Citation`` type. Returns a ``GroundingDecision`` whose ``grounded``
    flag is True only when every threshold metric is satisfied;
    ``rationale`` lists the failed metric names in a stable order so
    downstream code can parse it deterministically if needed.
    """
    return evaluate_grounding_scores(
        [(c.score, c.source_id) for c in citations], threshold
    )
