...
def test_subscriptions_reach_retrieval_from_the_tenant_record(tmp_path) -> None:
    ...
    save_tenant(
        TenantProfile(site="acu-flowloop", subscriptions=("msr-literature", "reactor-general")),
        state_dir=tmp_path,
    )
    save_tenant(
        TenantProfile(site="netl-triga", subscriptions=("reactor-general",)), state_dir=tmp_path
    )
    catalog = [
        "acu-flowloop:uploads",
        "netl-triga:docs",
    ...
    acu = corpora_for_tenant("acu-flowloop", catalog, state_dir=str(tmp_path))
    netl = corpora_for_tenant("netl-triga", catalog, state_dir=str(tmp_path))
    assert "msr-literature" in acu
    assert "msr-literature" not in netl  # a TRIGA is not a molten-salt loop
    assert "reactor-general" in acu and "reactor-general" in netl
    assert "netl-triga:docs" not in acu
...
def test_a_tenant_without_a_record_still_reads_its_own(tmp_path) -> None:
    ...
    got = corpora_for_tenant(
        "tamu-flowloop", ["tamu-flowloop:docs", "shared"], state_dir=str(tmp_path)
    )
    assert set(got) == {"tamu-flowloop:docs", "shared"}
...
def test_a_subscription_to_a_corpus_that_does_not_exist_is_ignored(tmp_path) -> None:
    ...
    save_tenant(TenantProfile(site="acu", subscriptions=("gone-away",)), state_dir=tmp_path)
    assert corpora_for_tenant("acu", ["acu:docs"], state_dir=str(tmp_path)) == ["acu:docs"]
