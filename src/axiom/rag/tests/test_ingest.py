...
from .conftest import ingest_body
...
def test_embedding_failure_does_not_commit_or_record_checksum(tmp_path, monkeypatch):
    ...
    f = tmp_path / "big.md"
    # Real-shaped prose, not "content paragraph. " * 40. Repeated filler is now
    # refused by the ingestion guard before embedding is ever attempted, so the
    # old body made this test pass through a path it was not testing.
    f.write_text("# Doc\n\n" + ingest_body(paragraphs=6))
