...
def test_create_store_threads_remote_signature_from_env(monkeypatch):
    """A peer that enforces auth needs a real X-Signature; create_store threads
    it (and the node id) from the environment so a secret never rides the URL."""
    monkeypatch.setenv("AXIOM_RAG_REMOTE_SIGNATURE", "peer-secret")
    monkeypatch.setenv("AXIOM_RAG_REMOTE_NODE_ID", "laptop-42")
    store = create_store("https://peer:8767")
    assert store._signature == "peer-secret"
    assert store._node_id == "laptop-42"


def test_create_store_remote_defaults_when_env_absent(monkeypatch):
    """No env → the store's dev-mode defaults, unchanged (back-compat)."""
    monkeypatch.delenv("AXIOM_RAG_REMOTE_SIGNATURE", raising=False)
    monkeypatch.delenv("AXIOM_RAG_REMOTE_NODE_ID", raising=False)
    store = create_store("https://peer:8767")
    assert store._signature == "dev-mode"
    assert store._node_id == "local"
