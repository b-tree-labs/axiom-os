...
# Descriptive-statistic aggregates of a real tool-returned series are admissible
# (ADR-113 / spec-output-provenance-gate): sum, mean, min, max, std. NOT count —
# small integers collide with unrelated values and would launder a fabricated one.
def _aggs(vals: list[float]) -> set[float]:
    if len(vals) < 2:
        return set()
    n = len(vals)
    out = {sum(vals), sum(vals) / n, min(vals), max(vals)}
    try:
        out.add(statistics.pstdev(vals))  # population std, deterministic (ddof=0)
...
_MAX_TABULAR_ROWS = 5000  # runaway-input backstop


def _tabular_closure(text: str) -> set[float]:
    """Descriptive-stat aggregates of numeric series carried as NON-JSON text:
    CSV / delimited / tables (per column) and predominantly-numeric inline lists.
    Reads numbers only. A prose line (few numbers among many words) is ignored, so
    incidental numbers cannot be aggregated into false support."""
    out: set[float] = set()
    lines = [ln for ln in text.splitlines() if ln.strip()][:_MAX_TABULAR_ROWS]
    if not lines:
        return out
    # Columnar: split each row on delimiters (or whitespace); aggregate each column.
    rows: list[list[str]] = []
    for ln in lines:
        cells = _DELIM_RE.split(ln) if _DELIM_RE.search(ln) else ln.split()
    ...
    if len(rows) >= 3:
        ...
        for col in range(width):
            ...
            for r in rows:
                if col < len(r):
                    try:
                        colvals.append(float(r[col]))
                    except ValueError:
                        pass
            if len(colvals) >= 2:
                out |= _aggs(colvals)
    # Inline: a single line that is PREDOMINANTLY numbers is a series
    # ("peaks: 2.0, 4.0, 6.0"); a prose line (ratio < 0.6 or < 3 numbers) is not.
    for ln in lines:
        toks = _DELIM_RE.sub(" ", ln).split()
        nums: list[float] = []
        for tok in toks:
            try:
                ...
                pass
        if len(nums) >= 3 and len(nums) >= 0.6 * len(toks):
            out |= _aggs(nums)
    return out


def _corpus_numbers(texts: Iterable[str]) -> set[float]:
    """Parse every numeric token in the corpus, then augment with descriptive-stat
    aggregates (sum/mean/min/max/std) of the numeric SERIES it carries — in any
    format: JSON arrays (bare, or a list-of-dicts per key) and CSV/table/inline
    numeric series. An answer that honestly aggregates a tool series ("total
    $12.00" over three values) is then supported even though the total appears
    nowhere verbatim — while a value unrelated to the evidence still matches
    nothing. It does NOT admit `count`, and does not re-derive fits/calculus
    (those must be tool-produced and appear as tool-result values)."""
    nums: set[float] = set()
    for t in texts:
        if not t:
            continue
...
def _json_array_closure(node: object, out: set[float] | None = None) -> set[float]:
    """Descriptive-stat aggregates of numeric arrays in tool-result JSON: a bare
    numeric list, and each numeric column of a list-of-dicts (grouped by key)."""
    if out is None:
        out = set()
    if isinstance(node, dict):
        ...
        bare = [
            float(x) for x in node if isinstance(x, (int, float)) and not isinstance(x, bool)
        ]
        if len(bare) >= 2:
            out |= _aggs(bare)
