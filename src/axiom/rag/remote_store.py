...
class RemoteRetrievalStore:
    ...
    def close(self) -> None:
        """No-op — mirrors ``connect``; requests are stateless per call.

        Present so CLI handlers that uniformly ``finally: store.close()``
        work against any store the factory returns.
        """
        return None
