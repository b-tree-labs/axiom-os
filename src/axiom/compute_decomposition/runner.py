...
class SubprocessDispatcher:
    ...
    def _make_runner(self):
        ...
        #
        # This used to set AXI_STATE_DIR here, permanently, on the parent
        # process. `TaskStore` is handed an explicit `base_dir` so it never
        # needed it; the SUBPROCESS does, and that is scoped around the spawn
        # in `dispatch_one` alongside PYTHONPATH. Setting it here leaked: it is
        # the highest-priority input to `get_user_state_dir()`, so every later
        # caller in the process resolved the user state dir to this run's temp
        # directory. In a test session that is every subsequent test.
    ...
    def dispatch_one(self, chunk: Chunk) -> ChunkResult:
        ...
        # Both variables are for the CHILD. Scope them to the spawn and put
        # the parent's environment back exactly as it was — PYTHONPATH already
        # did this; AXI_STATE_DIR did not, and that asymmetry was the bug.
        _spawn_env = {
            "PYTHONPATH": new_pythonpath,
            "AXI_STATE_DIR": str(self.state_dir),
        }
        _prior = {k: os.environ.get(k) for k in _spawn_env}
        os.environ.update(_spawn_env)
        try:
