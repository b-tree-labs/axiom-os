...
def test_the_gate_bundle_obeys_the_same_cache_contract_as_every_surface(fake_dist):
    """One UI infra means one caching contract. The gate's entry document
    revalidates; its content-hashed assets cache forever."""
    c = _spa_client(fake_dist)
    index = c.get("/gate/login")
    assert "no-cache" in index.headers.get("cache-control", "")
    asset = c.get("/gate/assets/app.js")
    assert "immutable" in asset.headers.get("cache-control", "")


def test_a_gate_rebuild_reaches_the_browser_without_a_restart(fake_dist):
    """The index was read once at router-build time, so a rebuilt gate
    bundle was invisible until the process restarted."""
    c = _spa_client(fake_dist)
    assert "/gate/assets/app.js" in c.get("/gate/login").text
    (fake_dist / "index.html").write_text(
        '<!doctype html><html lang="en"><head><meta charset="utf-8">'
        '<script type="module" crossorigin src="/gate/assets/app-v2.js"></script>'
        "</head><body></body></html>",
        encoding="utf-8",
    )
    assert "/gate/assets/app-v2.js" in c.get("/gate/login").text
