...
def test_adduser_duplicate_needs_force(ctx: SkillContext, tmp_path: Path):
    ...
    forced = ctx.registry.invoke("gate.adduser", {**base, "force": True, "role": ["admin"]}, ctx)
...
def test_resetpw_generates_when_omitted(ctx: SkillContext, tmp_path: Path):
    f = _accounts(tmp_path)
    ctx.registry.invoke(
        "gate.adduser", {"email": "g2@ut.example", "password": PW, "accounts_file": f}, ctx
    )
    r = ctx.registry.invoke("gate.resetpw", {"email": "g2@ut.example", "accounts_file": f}, ctx)
    assert r.ok
...
def test_resetpw_unknown_account_errors(ctx: SkillContext, tmp_path: Path):
    f = _accounts(tmp_path)
    ctx.registry.invoke(
        "gate.adduser", {"email": "known@ut.example", "password": PW, "accounts_file": f}, ctx
    )
    r = ctx.registry.invoke("gate.resetpw", {"email": "ghost@ut.example", "accounts_file": f}, ctx)
    assert not r.ok and "no such account" in r.errors[0]


def test_resetpw_missing_file_errors(ctx: SkillContext, tmp_path: Path):
    r = ctx.registry.invoke(
        "gate.resetpw", {"email": "x@ut.example", "accounts_file": _accounts(tmp_path)}, ctx
    )
    assert not r.ok
...
def test_list_hides_password_hash(ctx: SkillContext, tmp_path: Path):
    ...
    ctx.registry.invoke(
        "gate.adduser",
        {"email": "seen@ut.example", "password": PW, "role": ["operator"], "accounts_file": f},
        ctx,
    )
