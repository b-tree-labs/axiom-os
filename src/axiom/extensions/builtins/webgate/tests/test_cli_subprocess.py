...
def test_adduser_generates_and_prints_password(tmp_path: Path):
    f = str(tmp_path / "gate-users.json")
    r = _run(
        ["--accounts-file", f, "adduser", "gen@ut.example", "--role", "student"], state_dir=tmp_path
    )
    assert r.returncode == 0, r.stderr
...
def test_resetpw_round_trip(tmp_path: Path):
    ...
    rst = _run(
        ["--json", "--accounts-file", f, "resetpw", "rot@ut.example", "--password", "Brand-New-9"],
        state_dir=tmp_path,
    )
...
def test_adduser_duplicate_exits_nonzero(tmp_path: Path):
    f = str(tmp_path / "gate-users.json")
    args = ["--accounts-file", f, "adduser", "dup@ut.example", "--password", "Correct-Horse-9"]
...
def test_issue_list_revoke_api_key_round_trip(tmp_path: Path):
    ...
    lst = _run(["--json", "--keys-file", f, "list", "api-keys"], state_dir=tmp_path)
    assert lst.returncode == 0, lst.stderr
    ...
    rev = _run(["--keys-file", f, "revoke", "api-key", key_id], state_dir=tmp_path)
    assert rev.returncode == 0, rev.stderr

    lst2 = _run(["--json", "--keys-file", f, "list", "api-keys"], state_dir=tmp_path)
    assert _json.loads(lst2.stdout)["value"]["items"][0]["revoked_at"]


def test_issue_requires_scope_flag(tmp_path: Path):
    f = str(tmp_path / "gate-api-keys.json")
    r = _run(["--keys-file", f, "issue", "api-key", "--principal", "@svc:org"], state_dir=tmp_path)
...
def test_revoke_unknown_key_exits_nonzero(tmp_path: Path):
    f = str(tmp_path / "gate-api-keys.json")
    _run(
        ["--keys-file", f, "issue", "api-key", "--principal", "@svc:org", "--scope", "llm"],
        state_dir=tmp_path,
    )
    r = _run(["--keys-file", f, "revoke", "api-key", "ghost"], state_dir=tmp_path)
    assert r.returncode != 0
