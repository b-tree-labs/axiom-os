...
def test_issue_prints_token_once_and_stores_hash(ctx, tmp_path):
    r = _issue(ctx, tmp_path, principal="@svc:org", scopes=("llm", "rag:read"), name="chat backend")
...
def test_issue_requires_principal_and_scopes(ctx, tmp_path):
    r = ctx.registry.invoke(
        "gate.issue", {"resource": "api-key", "scope": ["llm"], "keys_file": _keys(tmp_path)}, ctx
    )
    assert not r.ok and "principal" in r.errors[0]

    r = ctx.registry.invoke(
        "gate.issue",
        {"resource": "api-key", "principal": "@svc:org", "keys_file": _keys(tmp_path)},
        ctx,
    )
...
def test_issue_without_keys_file_errors(ctx, monkeypatch):
    ...
    r = ctx.registry.invoke(
        "gate.issue", {"resource": "api-key", "principal": "@svc:org", "scope": ["llm"]}, ctx
    )
...
def test_revoke_is_immediate(ctx, tmp_path):
    ...
    r = ctx.registry.invoke(
        "gate.revoke", {"resource": "api-key", "key_id": key_id, "keys_file": _keys(tmp_path)}, ctx
    )
    assert r.ok, r.errors
    store.reload()
...
def test_revoke_unknown_key_errors(ctx, tmp_path):
    _issue(ctx, tmp_path)
    r = ctx.registry.invoke(
        "gate.revoke", {"resource": "api-key", "key_id": "nope", "keys_file": _keys(tmp_path)}, ctx
    )
...
def test_list_api_keys_hides_hashes_and_marks_revoked(ctx, tmp_path):
    ...
    ctx.registry.invoke(
        "gate.revoke",
        {"resource": "api-key", "key_id": a.value["key_id"], "keys_file": _keys(tmp_path)},
        ctx,
    )

    r = ctx.registry.invoke(
        "gate.list", {"resource": "api-keys", "keys_file": _keys(tmp_path)}, ctx
    )
    assert r.ok, r.errors
    items = r.value["items"]
    assert len(items) == 2
...
def test_list_defaults_to_accounts(ctx, tmp_path):
    ...
    r = ctx.registry.invoke("gate.list", {"accounts_file": str(tmp_path / "gate-users.json")}, ctx)
    assert r.ok and r.value["items"] == []
