...
def _shell(brand: LoginBrand, *, title: str, inner: str, status: int = 200) -> HTMLResponse:
    ...
    # An auth page carries per-attempt state (the error, the pre-filled
    # address, the next target). None of it may be replayed from a cache
    # — least of all a shared one.
...
def _first_time_hint(brand: LoginBrand, providers: list[OidcSignIn]) -> str:
    ...
    from .account_flows import self_signup_enabled

    if brand.login_hint:
        return f'<p class="alt">{html.escape(brand.login_hint)}</p>'
    if self_signup_enabled():
        return '<p class="alt">New here? <a href="/gate/signup">Create an account</a></p>'
    if brand.signup_url:
        return (
...
def _forgot_page(brand: LoginBrand) -> HTMLResponse:
    # Self-service account help: email reset + passwordless sign-in link.
    # Both post to enumeration-proof endpoints (account_flows) and show the
    # generic confirmation whatever the email. The admin contact stays as
    # the escalation footer.
    if brand.support_email:
        mail = html.escape(brand.support_email, quote=True)
        admin = (
            f'Still stuck? Contact <a href="mailto:{mail}?subject='
            f'{quote(brand.product_name + " password reset")}">your administrator</a>.'
        )
    else:
        admin = "Still stuck? Contact your administrator."
    inner = (
        f"{_brand_header(brand)}"
        "<h2>Reset your password</h2>"
        '<form data-post="/gate/forgot">'
        '<label for="fe">Email</label>'
        '<input id="fe" name="email" type="email" autocomplete="email" required>'
        '<button type="submit">Email me a reset link</button></form>'
        '<div class="or">or</div>'
        "<h2>Sign in without a password</h2>"
        '<form data-post="/gate/magic-link">'
        '<label for="me">Email</label>'
        '<input id="me" name="email" type="email" autocomplete="email" required>'
        '<button type="submit">Email me a sign-in link</button></form>'
        '<p class="msg" aria-live="polite"></p>'
        f'<p class="help">{admin}</p>'
        '<p class="alt"><a href="/gate/login">← Back to sign in</a></p>'
        "<script>for(const f of document.querySelectorAll('form[data-post]')){"
        "f.onsubmit=async(e)=>{e.preventDefault();"
        "const r=await fetch(f.dataset.post,{method:'POST',"
        "headers:{'Content-Type':'application/json'},"
        "body:JSON.stringify({email:f.querySelector('input').value})});"
        "const j=await r.json();document.querySelector('.msg').textContent=j.detail||'';};}"
        "</script>"
    )
...
def _inject_brand(index_html: str, brand: LoginBrand, sso: OidcSignIn | None = None) -> str:
    """Splice ``window.__AXIOM_GATE_BRAND__`` into the built ``index.html``
    before the module script boots (mechanics shared with every surface via
    ``http.spa`` — extracted when receipts became the second consumer)."""
    from axiom.extensions.builtins.http.spa import inject_bootstrap_global

    return inject_bootstrap_global(
        index_html, "__AXIOM_GATE_BRAND__", _spa_brand_payload(brand, sso)
    )
...
def build_webgate_router(
    ...
    *,
    ...
    def _branded_index() -> str:
        """Read per request, like every other appkit-shell surface: a
        rebuild that reaches the server must also reach the browser."""
        return _inject_brand(
            (spa_root / "index.html").read_text(encoding="utf-8"), the_brand, providers
        )
    ...
    if spa_active:
        ...
        from axiom.extensions.builtins.http.spa import (
            ASSET_CACHE_CONTROL,
            INDEX_CACHE_CONTROL,
        )
        ...
        async def login_page() -> HTMLResponse:
            return HTMLResponse(_branded_index(), headers={"cache-control": INDEX_CACHE_CONTROL})
        ...
        async def assets(path: str) -> Response:
            from axiom.extensions.builtins.http.spa import safe_asset_path

            target = safe_asset_path(assets_root, path)
            if target is None:
    ...
    def _account_set_session(response: Response, user: User, request: Request) -> None:
        token = issue_session_token(user, ttl=DEFAULT_SESSION_TTL, issuer=_issuer(request))
        _set_session(response, token, ttl=DEFAULT_SESSION_TTL)

    def _account_shell(*, title: str, inner: str, status: int = 200) -> HTMLResponse:
        return _shell(
            the_brand,
            title=f"{title} · {the_brand.product_name}",
            inner=f"{_brand_header(the_brand)}{inner}",
            status=status,
        )

    from .account_flows import register_account_routes

    register_account_routes(
        router,
        store=_store,
        issuer_of=_issuer,
        set_session=_account_set_session,
        shell=_account_shell,
        providers=lambda: list(providers),
        default_site=lambda: os.getenv("AXIOM_SITE") or None,
    )
