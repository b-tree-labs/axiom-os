...
def _parser() -> argparse.ArgumentParser:
    ...
    add.add_argument("--apply-bundle", action="store_true",
                     help="resolve --role through the registered role bundles "
                          "and store the union of their scopes on the account "
                          "(attributes.bundle_scopes)")
    ...
    add.add_argument("--site",
                     help="the site (tenant) this account belongs to — the "
                          "':context' of its principal (default: $AXIOM_SITE)")
    ...
    iss.add_argument("--scope", action="append", metavar="MOUNT[:VERB]",
                     help="grant a scope (repeatable): a composed-app mount, "
                          "optionally narrowed to read|invoke|access "
                          "(e.g. --scope llm --scope rag:read); required "
                          "unless --role + --apply-bundle supply the scopes")
    iss.add_argument("--role", action="append", metavar="ROLE",
                     help="with --apply-bundle: a registered role bundle "
                          "whose scopes the key receives (repeatable)")
    iss.add_argument("--apply-bundle", action="store_true",
                     help="resolve --role through the registered role bundles "
                          "and issue the key with the union of their scopes "
                          "(plus any explicit --scope grants)")
    iss.add_argument("--name", help="what this key is for (free-form note)")
    iss.add_argument("--site",
                     help="bind the key to a site (tenant): becomes, or must "
                          "equal, the principal's ':context' (default: $AXIOM_SITE)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = invoke_capability(ctx.registry, f"gate.{args.verb}", params, ctx, surface=CLI_SURFACE)
