...
from axiom.extensions.builtins.repo.base import RepoActivity, RepoInfo
from axiom.extensions.builtins.repo.config import SourceConfig, detect_sources, load_config
from axiom.extensions.builtins.repo.orchestrator import (
...
class TestConfigDetection:
    ...
    def _isolate_credential_vault(self):
        # detect_sources() consults the credential vault via has_credential() in
        # addition to the env vars. A developer's locally-stored gitlab/github
        # token would otherwise leak in and break the env-var-driven assertions
        # below (seen as detect_no_tokens returning a stray gitlab source). Pin
        # has_credential off so detection here is driven solely by the env vars
        # each test sets. store_credential resolution (#570) taught
        # has_credential to find bare stored tokens, which is what exposed this.
        with patch("axiom.infra.connections.has_credential", return_value=False):
            yield
    ...
    def test_detect_no_tokens(self):
        ...
        env.pop("GITHUB_TOKEN", None)
        # has_credential() reads the machine credential store — isolate it too,
        # or the test fails on any dev box with gitlab/github connections.
        with patch.dict(os.environ, env, clear=True), \
                patch("axiom.infra.connections.has_credential", return_value=False):
            sources = detect_sources()
            assert sources == []
...
class TestOrchestrator:
    ...
    def test_run_dry_run(self, tmp_path):
        ...
        with patch("axiom.extensions.builtins.repo.orchestrator._create_provider", return_value=mock_provider):
