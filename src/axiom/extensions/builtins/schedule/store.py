...
class SqlFireLog:
    ...
    def record_outcome(
        ...
        """Record how a fire ended, and WHEN.

        `_update` has taken `finished_at` all along; this method never offered
        it, so every row in `schedule_fire_log` carried a `started_at` and a
        NULL `finished_at` — permanently. NULL there is indistinguishable from
        "still running" and from "crashed", which is the question someone asks
        the morning after a nightly job.
        """
        self._update(
            schedule_id,
            outcome,
