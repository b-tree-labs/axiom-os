...
def _fire_one(defn: dict[str, Any], now: datetime, ctx: EngineContext) -> bool:
    ...
    if not proceed:
        ctx.fire_log.record_outcome(
            schedule_id, bucket, phash, "failed", None, reason, finished_at=ctx.now_fn()
        )
    ...
    for attempt in range(1, max(1, max_attempts) + 1):
        try:
            ...
            ctx.fire_log.record_outcome(
                ...
                # Read fresh rather than reusing `now`: the point of this
                # column is how long the run TOOK, and `now` is when the tick
                # began, before the executor ran.
                finished_at=ctx.now_fn(),
            )
