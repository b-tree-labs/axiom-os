...
def _cmd_check(args: argparse.Namespace, root: str) -> int:
    ...
    if args.format == "json":
        ...
        print(f"No AGENTS.md at {root} — nothing to check. Run `{_brand_cli()} context init`.")
        return 0
    else:
        _print_results(results)
    if core.has_drift(results):
        if args.format != "json":
            print(f"\ncontext files drifted from AGENTS.md — run `{_brand_cli()} context sync`")
        return 1
    return 0
...
def _cmd_sync(args: argparse.Namespace, root: str) -> int:
    if core.read_canonical(root) is None:
        print(f"No AGENTS.md at {root}. Run `{_brand_cli()} context init` first.")
        return 1
...
def _cmd_init(args: argparse.Namespace, root: str) -> int:
    ...
    if res.created_canonical:
        print(f"\nNext: edit AGENTS.md, then `{_brand_cli()} context sync`.")
    return 0
