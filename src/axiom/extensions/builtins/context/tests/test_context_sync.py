...
def test_four_targets_registered():
    by_name = {t.name: t for t in generators.TARGETS}
    assert set(by_name) == {"cursor", "junie", "copilot", "gemini"}
    assert by_name["gemini"].relpath == "GEMINI.md"
...
def test_markdown_targets_have_no_frontmatter():
    for name in ("junie", "copilot", "gemini"):
...
def test_sync_writes_all_target_files(tmp_path):
...
def test_sync_rewrites_when_canonical_changes(tmp_path):
    ...
    assert by.pop("claude") == "unchanged"  # the symlink follows automatically
    assert set(by.values()) == {"written"}
...
def test_check_detects_hand_edit_drift(tmp_path):
    root = _repo_with_canonical(tmp_path)
    core.sync(root, write=True)
    # A marked file gone stale (an old generation) is drift — repairable.
    (root / ".github/copilot-instructions.md").write_text(
        generators.TARGETS[2].render("# Old canonical\n"), encoding="utf-8"
    )
    # An unmarked hand-edit is a conflict — protected, but still gates.
    (root / ".junie/guidelines.md").write_text(
        "someone hand-edited this\n", encoding="utf-8"
    )
    results = core.check(root)
    by = {r.target: r.status for r in results}
    assert by["copilot"] == "drift"
    assert by["junie"] == "conflict"
    assert core.has_drift(results) is True
...
def test_findings_flag_drift_with_remediation(tmp_path):
    ...
    (root / ".cursor/rules/axiom.mdc").write_text(
        generators.TARGETS[0].render("# Old canonical\n"), encoding="utf-8"
    )  # drift (stale but marked)
    (root / "GEMINI.md").write_text("hand notes\n", encoding="utf-8")  # conflict
    codes = {f.code for f in core.findings(root)}
    assert codes == {"context.missing", "context.drift", "context.conflict"}
    assert any("axi context sync" in f.remediation for f in core.findings(root))
...
def test_cli_check_exit_code_subprocess(tmp_path):
    ...
    # Introduce drift (stale but marked → repairable) → exit non-zero.
    stale = generators.TARGETS[0].render("# Old canonical\n")
    (root / ".cursor/rules/axiom.mdc").write_text(stale, encoding="utf-8")
    bad = run("check")
    ...
    assert {r["target"] for r in payload} == {"claude", "cursor", "junie", "copilot", "gemini"}


# ---------------------------------------------------------------------------
# CLAUDE.md symlink management (sync/check own it, not just init)
# ---------------------------------------------------------------------------


def _root(tmp_path, agents_text=SAMPLE):
    (tmp_path / "AGENTS.md").write_text(agents_text, encoding="utf-8")
    return tmp_path


def test_check_claude_symlink_ok(tmp_path):
    root = _root(tmp_path)
    (root / "CLAUDE.md").symlink_to("AGENTS.md")
    by = {r.target: r for r in core.check(root)}
    assert by["claude"].status == "ok"


def test_check_claude_missing(tmp_path):
    root = _root(tmp_path)
    by = {r.target: r for r in core.check(root)}
    assert by["claude"].status == "missing"
    assert core.has_drift(core.check(root))


def test_sync_creates_claude_symlink(tmp_path):
    root = _root(tmp_path)
    by = {r.target: r for r in core.sync(root)}
    assert by["claude"].status == "created"
    claude = root / "CLAUDE.md"
    assert claude.is_symlink() and claude.readlink().name == "AGENTS.md"


def test_sync_repairs_wrong_symlink_target(tmp_path):
    root = _root(tmp_path)
    (root / "OTHER.md").write_text("x\n", encoding="utf-8")
    (root / "CLAUDE.md").symlink_to("OTHER.md")
    assert {r.target: r for r in core.check(root)}["claude"].status == "drift"
    assert {r.target: r for r in core.sync(root)}["claude"].status == "written"
    assert (root / "CLAUDE.md").readlink().name == "AGENTS.md"


def test_sync_upgrades_identical_regular_claude_to_symlink(tmp_path):
    root = _root(tmp_path)
    (root / "CLAUDE.md").write_text(SAMPLE, encoding="utf-8")
    assert {r.target: r for r in core.check(root)}["claude"].status == "ok"
    assert {r.target: r for r in core.sync(root)}["claude"].status == "written"
    assert (root / "CLAUDE.md").is_symlink()


def test_diverged_regular_claude_is_conflict_and_never_clobbered(tmp_path):
    root = _root(tmp_path)
    (root / "CLAUDE.md").write_text("# Different hand-written content\n", encoding="utf-8")
    assert {r.target: r for r in core.check(root)}["claude"].status == "conflict"
    assert core.has_drift(core.check(root))
    assert {r.target: r for r in core.sync(root)}["claude"].status == "conflict"
    assert (root / "CLAUDE.md").read_text(encoding="utf-8").startswith("# Different")
    codes = {f.code for f in core.findings(root)}
    assert "context.conflict" in codes


def test_reversed_convention_repo_is_left_alone(tmp_path):
    # Some repos hand-author CLAUDE.md and symlink AGENTS.md -> CLAUDE.md.
    # Managing the claude target there would create a symlink cycle.
    (tmp_path / "CLAUDE.md").write_text(SAMPLE, encoding="utf-8")
    (tmp_path / "AGENTS.md").symlink_to("CLAUDE.md")
    by = {r.target: r for r in core.check(tmp_path)}
    assert by["claude"].status == "ok"
    by = {r.target: r for r in core.sync(tmp_path)}
    assert by["claude"].status == "unchanged"
    assert not (tmp_path / "CLAUDE.md").is_symlink()


# ---------------------------------------------------------------------------
# hand-authored files at generated paths are protected
# ---------------------------------------------------------------------------


def test_unmarked_hand_authored_target_is_conflict_not_clobbered(tmp_path):
    root = _root(tmp_path)
    hand = "# My own Gemini notes\n\nPrecious.\n"
    (root / "GEMINI.md").write_text(hand, encoding="utf-8")
    assert {r.target: r for r in core.check(root)}["gemini"].status == "conflict"
    assert {r.target: r for r in core.sync(root)}["gemini"].status == "conflict"
    assert (root / "GEMINI.md").read_text(encoding="utf-8") == hand


def test_marked_drifted_target_is_still_rewritten(tmp_path):
    root = _root(tmp_path)
    stale = generators.TARGETS[0].render("# Old canonical\n")
    dest = root / generators.TARGETS[0].relpath
    dest.parent.mkdir(parents=True, exist_ok=True)
    dest.write_text(stale, encoding="utf-8")
    name = generators.TARGETS[0].name
    assert {r.target: r for r in core.check(root)}[name].status == "drift"
    assert {r.target: r for r in core.sync(root)}[name].status == "written"
