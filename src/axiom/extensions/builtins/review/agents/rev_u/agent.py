...
from axiom.extensions.builtins.review.agents.rev_u.passes._parse import degraded
        ...
        *,
        ...
        for pass_name in selected:
            ...
            if pass_module is None:
                ...
                all_findings.append(
                    degraded(pass_name, "no such review pass is registered")
                )
                continue
            try:
                ...
                # Continue with the remaining passes — but record that this one
                # did not run. Logging alone meant every pass could fail and the
                # FindingSet still read as "no problems found".
                log.warning("pass %r raised an exception: %s", pass_name, exc)
                all_findings.append(degraded(pass_name, str(exc)))
