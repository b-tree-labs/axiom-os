...
def _build_parser() -> argparse.ArgumentParser:
    ...
    act = sub.add_parser(
        "actions",
        help="Query the agent action-provenance ledger (#665). "
             "Filters AND-compose; most recent first.",
    )
    act.add_argument(
        "--agent",
        help="filter to one agent (e.g. 'tidy', 'rivet', 'plinth')",
    )
    act.add_argument(
        "--op-class",
        dest="op_class",
        help="filter by operation class (e.g. 'git.branch.delete')",
    )
    act.add_argument(
        "--outcome",
        choices=["proceeded", "refused", "failed"],
        help="filter by action outcome",
    )
    act.add_argument(
        "--since",
        help="time-window floor: Nm/Nh/Nd/Nw or ISO-8601",
    )
    act.add_argument(
        "--until",
        help="time-window ceiling: Nm/Nh/Nd/Nw or ISO-8601",
    )
    act.add_argument(
        "--text",
        help="substring match against candidate / name / rule / undo ref",
    )
    act.add_argument(
        "--limit",
        type=int,
    ...
    act.add_argument(
        "--verify",
        action="store_true",
        help="verify the EC-mode HMAC chain instead of listing records",
    )
...
def _emit(result: SkillResult, as_json: bool) -> int:
    ...
    if resource == "actions":
        ...
        if not items:
            ...
            return result.exit_code
        print(f"{count} action record(s) [backend={value.get('backend')}]:")
        for a in items:
            rule = f"  rule={a['refusing_rule']}" if a.get("refusing_rule") else ""
            undo = f"  undo={a['undo_ref']}" if a.get("undo_ref") else ""
            ...
            print(
                ...
                f"{a['agent']:<8}  "
                f"{a['op_class']:<28}  "
                f"{a['candidate']}{rule}{undo}{dry}"
            )
        return result.exit_code

    if resource == "actions_verify":
        chained = value.get("chained")
        if not chained:
            print("OK: no chain key configured — records are plain "
                  "(standard mode); nothing to verify")
        elif value.get("ok"):
            print(f"OK: HMAC chain intact [backend={value.get('backend')}]")
        else:
            print(f"FAIL: HMAC chain broken at record {value.get('broken_at')} "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = invoke_capability(
            ctx.registry, f"audit.{args.verb}", params, ctx, surface=CLI_SURFACE
        )
