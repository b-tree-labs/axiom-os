...
class Rule:
    ...
    """Dispatch-surface scope (ADR-114 §3). ``None`` matches any surface
    (additive default). A string (``"mcp"``/``"cli"``/``"agent_tool"``, or
    ``"*"``) matches only envelopes carrying that ``surface`` — so a rule can
    require approval for a tool *on mcp* without gating the CLI. A surface-scoped
    rule never matches a surface-less envelope (fail-closed on scope)."""
    disposition: Disposition = "permit"
    priority: int = 0
    ttl: datetime | None = None
    roles: frozenset[str] = frozenset()
    """Match only actors holding at least one of these roles (ADR-084). Empty
    = any actor. Read from ``env.actor_context``; an envelope with no actor
    context never matches a role-scoped rule (fail-closed)."""
    ...
    def matches(self, env: ActionEnvelope, now: datetime | None = None) -> bool:
        ...
        if self.actor_pattern != "*" and self.actor_pattern != env.actor.handle:
            return False
        if self.roles:
            actor_ctx = getattr(env, "actor_context", None)
            held = set(actor_ctx.roles) if actor_ctx is not None else set()
            if not (self.roles & held):
                return False
        ...
        if self.federation_origin_pattern is None:
            ...
            if env.federation_origin != self.federation_origin_pattern:
                return False
        if self.surface_pattern is not None and self.surface_pattern != "*":
            # A surface-scoped rule matches only that surface, and never a
            # surface-less envelope (fail-closed on scope).
            if getattr(env, "surface", "") != self.surface_pattern:
                return False
        return True
