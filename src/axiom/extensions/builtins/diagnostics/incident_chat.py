    ...
    *,
    ...
    def _responder(question: str, ctx: dict) -> str:
        ...
        if not state["seeded"]:
            ...
            state["seeded"] = True
        try:
            # NOT raw. Two reasons, and the second is the one that matters.
            #
            # `raw` sends only the current user turn — no history — so seeding
            # the incident preamble ONCE meant every turn after the first had
            # neither the preamble nor the conversation. The incident context
            # evaporated after question one.
            #
            # And `raw` empties the system prompt, which is where the content
            # safeguards live. An incident conversation is the last place to
            # run a model without them.
            reply = a.turn(msg, stream=False)
        except Exception:
            return interview(question, ctx)
