...
def _bonsai_matcher(event: dict) -> Diagnosis | None:
    ...
    if "bonsai-1.7b.gguf" in blob or "bonsai-1.7b" in blob or "bonsai-local" in blob:
        return Diagnosis(
            ...
            summary=(
                f"{_brand_cli()} chat fell through to the legacy `bonsai-local` provider "
...
def _local_llamafile_down_matcher(event: dict) -> Diagnosis | None:
    ...
    return Diagnosis(
        ...
        remedy=(
            ...
            f"Qwen llamafile: `{_brand_cli()} setup model qwen` (downloads the GGUF if "
...
def _state_dir_permission_matcher(event: dict) -> Diagnosis | None:
    ...
    if not is_permission:
        return None
    has_state_dir = project_dir_name() in blob or LEGACY_PROJECT_DIR in blob
    if not has_state_dir:
        return None
    return Diagnosis(
        pattern_id="state-dir-permission-denied",
        summary=(
            f"Axiom's user state directory (`~/{project_dir_name()}/`) is not "
...
def _module_import_error_matcher(event: dict) -> Diagnosis | None:
    ...
    return Diagnosis(
        ...
        remedy=(
            ...
            f"`{_brand_cli()} ext lint` to surface manifest/code mismatches across all "
