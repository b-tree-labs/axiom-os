...
def pending_path(state_dir: Path | None = None) -> Path:
    """Resolve the pending-diagnoses log path.

    Accepts an explicit `state_dir` for tests; otherwise resolves under the
    PLATFORM state dir rather than the branded one.

    A diagnosis describes a failure of this install on this machine, not a
    failure of a product. Filing it under whichever alias was typed split it
    in half: `axi` warned about three pending diagnoses while `neut` — same
    install, same user, same failures — reported none, because it was reading
    `~/.neut` instead of `~/.axi`.
    """
...
def _adopt_branded_diagnoses() -> None:
    """Move diagnoses filed under a brand home into the shared one.

    Records written before the paths were unified are still real failures
    somebody should see, so they are merged rather than stranded. Deduped by
    fingerprint, and the brand-local file is removed once its contents are
    safely in the shared one — leaving it would resurrect the split on the
    next read.
    """
    try:
        ...
        if branded.resolve() == get_platform_state_dir().resolve():
            return
        legacy = branded / "agents" / "triage" / PENDING_FILENAME
        ...
        if adopted:
            ...
            with shared.open("a") as fh:
                fh.write("\n".join(adopted) + "\n")
        legacy.unlink()
        log.debug("cli_listener: adopted %d diagnoses from %s", len(adopted), legacy)
    except Exception as exc:  # noqa: BLE001
        log.debug("cli_listener: adopting branded diagnoses failed: %s", exc)
