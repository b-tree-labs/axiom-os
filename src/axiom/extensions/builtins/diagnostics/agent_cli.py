...
def _cmd_pending(args: argparse.Namespace) -> int:
    ...
    for d in pending:
        ...
        print()
    print(f"Acknowledge with: {_brand_cli()} triage clear <fingerprint>")
    print(f"Or all at once:   {_brand_cli()} triage clear --all")
    return 0
...
def _cmd_clear(args: argparse.Namespace) -> int:
    ...
    if args.clear_all:
        removed = cli_listener.clear_pending(state_dir=None, fingerprint=None)
        if removed:
            print(f"Cleared {removed} pending diagnos{'is' if removed == 1 else 'es'}.")
        else:
            print("Nothing to clear — no pending diagnoses.")
        return 0
    if not args.fingerprint:
        print("Provide a fingerprint or use --all.")
        print(f"List pending with: {_brand_cli()} triage pending")
        return 2
    removed = cli_listener.clear_pending(state_dir=None, fingerprint=args.fingerprint)
    if not removed:
        # Say so rather than reporting a success that did not happen.
        print(f"No pending diagnosis with fingerprint {args.fingerprint}.")
        print(f"List pending with: {_brand_cli()} triage pending")
        return 1
