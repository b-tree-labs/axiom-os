...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=f"{_brand_cli()} install",
        description="Run environment setup steps from runtime/config/install.toml",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
Examples:
  {_brand_cli()} install                  Auto-detect environment and run pending steps
  {_brand_cli()} install --env hpc        Run hpc environment steps
  {_brand_cli()} install --list           Show step status without running anything
  {_brand_cli()} install --force          Re-run all steps (ignore completion state)
  {_brand_cli()} install --step connect-llm-hpc   Run one step by id
""",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Finalize: register manifest-declared [[extension.schedule]] cadences into
    # PULSE (spec-schedule-consumer-seam §6). Idempotent + soft-fail: an absent
    # PULSE database or a bad block is skipped, never aborts install.
    _finalize_register_schedules()
...
def _finalize_register_schedules() -> None:
    """Register manifest-declared ``[[extension.schedule]]`` cadences into PULSE.

    Soft-fail: PULSE not installed, no database, or a malformed block all skip
    quietly (the schedules re-register on the next install once the cause is
    fixed — registration is idempotent by ``(extension, action)``).
    """
    try:
        from axiom.extensions.builtins.schedule.install import sync_manifest_schedules
        from axiom.extensions.discovery import discover_extensions
    except ImportError:
        return

    try:
        result = sync_manifest_schedules(discover_extensions())
    except Exception as exc:  # noqa: BLE001 — never abort install over scheduling
        print(f"  ⚠ Schedule registration skipped: {exc}")
        return

    if not (result.registered or result.errors):
        return  # nothing declared, or all already present — stay quiet
    print()
    print("  Scheduled cadences")
    print("  " + "─" * 50)
    for tag in result.registered:
...
def _finalize_register_agents() -> None:
    ...
    if failed:
        ...
        for name in failed:
            print(f"    {_brand_cli()} agents start {name}")
...
def _finalize_install_shim() -> None:
    ...
    try:
        ...
        if result.conflict:
            ...
            print(f"    run `{_brand_cli()} install-shim --force` to repoint")
        elif result.written:
