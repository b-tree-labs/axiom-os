...
def test_install_records_and_surfaces_app_type(env):
    """ADR-110: a connection declares its app-type (auth_mode/tenancy/scope), and
    `axi connector` surfaces it so a fleet of registrations reads as legible types."""
    res = reg.install(
        {
            **env,
            "connector": "ai.axiom.connector.slack",
    ...
    assert res.ok
    assert res.value["auth_mode"] == "delegated_user"
    assert res.value["tenancy"] == "multi"
    assert res.value["capability_scope"] == "user-own"

    st = reg.status({**env, "name": "slack-ops"}, ctx=None)
    row = st.value["connections"][0]
    assert row["auth_mode"] == "delegated_user"
    assert row["tenancy"] == "multi"
    assert row["capability_scope"] == "user-own"


def test_install_defaults_app_type_when_unspecified(env):
    res = reg.install(
        {
            **env,
            "connector": "ai.axiom.connector.slack",
            "name": "slack-svc",
            "owner": "@ben",
            "secret_ref": "kubernetes://axiom-data/svc",
        },
        ctx=None,
    )
    assert res.value["auth_mode"] == "unspecified"
    assert res.value["tenancy"] == "single"
