...
def _setup_with_auth_method_choice(conn: Connection, registry: ConnectionRegistry) -> int:
    ...
    if method_type == "browser":
        ...
        if conn.credential_file:
            ...
            creds = get_user_state_dir() / "credentials"
            print(f"  Path: {creds}/{conn.credential_file}")
        print()
        return 0
    else:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=f"""
Examples:
  {_brand_cli()} connect                    List all connections
  {_brand_cli()} connect github             Set up GitHub connection
  {_brand_cli()} connect --check            Health check all connections
  {_brand_cli()} connect teams --clear      Remove Teams credentials
  {_brand_cli()} connect --json             Machine-readable output
""",
    )
