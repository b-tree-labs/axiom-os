...
def register_builtin_connectors(fabric: ConnectorFabric | None = None) -> ConnectorFabric:
    ...
    for fn in (slack_connector_descriptor, teams_connector_descriptor,
               ...
               email_connector_descriptor, reddit_connector_descriptor,
...
def reddit_connector_descriptor() -> ConnectorDescriptor:
    """Reddit as a research signal source — OAuth2 app-only, public read.

    No official Reddit MCP server exists; community wrappers add a
    supply-chain seam while needing the same credentials, so the
    integration is native and reaches harnesses via the composed axiom
    MCP. Credentials are a free 'script' app (id + secret) — never a
    user password; app-only auth reads public content and can write
    nothing.
    """
    return ConnectorDescriptor(
        name="ai.axiom.connector.reddit",
        version="0.1.0",
        title="Reddit (research source)",
        description="Read-only research access to public Reddit via OAuth2 "
        "app-only: search, threads, comment harvest — market/user-signal "
        "collection for analysis pipelines.",
        artifact_class=ArtifactClass.CONNECTOR,
        kind="research_source",
        transport="https",
        connection_ref="reddit",
        provider_entry="axiom.extensions.builtins.connect.research_sources:RedditSource",
        env=[
            EnvVar("REDDIT_CLIENT_ID", "Script app client id", is_required=True, is_secret=True,
                   where="reddit.com → prefs → apps → create app (type: script) → the id under the app name",
                   url="https://www.reddit.com/prefs/apps"),
            EnvVar("REDDIT_CLIENT_SECRET", "Script app secret", is_required=True, is_secret=True,
                   where="same app card → 'secret'",
                   url="https://www.reddit.com/prefs/apps"),
            EnvVar("REDDIT_USER_AGENT", "Descriptive UA per Reddit API rules",
                   is_required=False),
        ],
        availability=Availability.AVAILABLE,
        setup=SetupSpec(
            install_kind="developer_token",
            summary="Create a free Reddit 'script' app, paste id + secret; "
            "verification runs a real search.",
            needs=("client id", "client secret"),
            urls={
                "Create the app": "https://www.reddit.com/prefs/apps",
                "API rules (user agent)": "https://support.reddithelp.com/hc/en-us/articles/16160319875092",
            },
        ),
    )


def x_connector_descriptor() -> ConnectorDescriptor:
    """X (Twitter) as a research signal source — API v2 bearer.

    No official X MCP server exists. Honest constraint carried in the
    descriptor: meaningful search requires a PAID developer tier; the
    free tier's read quota does not support research use.
    """
    return ConnectorDescriptor(
        name="ai.axiom.connector.x",
        version="0.1.0",
        title="X (research source)",
        description="Read-only research access to public X posts via API v2 "
        "recent search (app bearer token). Requires a paid developer tier "
        "for meaningful read volume.",
        artifact_class=ArtifactClass.CONNECTOR,
        kind="research_source",
        transport="https",
        connection_ref="x",
        provider_entry="axiom.extensions.builtins.connect.research_sources:XSource",
        env=[
            EnvVar("X_BEARER_TOKEN", "App bearer token (API v2)", is_required=True, is_secret=True,
                   where="developer.x.com → Projects & Apps → your app → Keys and tokens → Bearer Token",
                   url="https://developer.x.com/en/portal/dashboard"),
        ],
        availability=Availability.AVAILABLE,
        setup=SetupSpec(
            install_kind="developer_token",
            summary="Create an X developer app (search needs a paid tier), "
            "paste the bearer token; verification runs a real search.",
            needs=("bearer token", "paid developer tier for search volume"),
            urls={
                "Developer portal": "https://developer.x.com/en/portal/dashboard",
                "API tiers": "https://developer.x.com/en/products/x-api",
            },
        ),
    )
