...
_MARK = {
    "available": Glyph.OK,
    "planned": Glyph.ABSENT,
    "deprecated": Glyph.WARN,
}
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog=f"{_brand_cli()} connector", description="Manage connectors (Registry Fabric)")
...
def _resolve_or_die(fabric, name: str) -> str | None:
    ...
    print(msg + f"  (run `{_brand_cli()} connector list`)", file=sys.stderr)
    return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.verb == "list":
        ...
        # The legend is built from the same map the rows are, so it cannot
        # drift from them — this line still read "● available ○ planned"
        # after the marks themselves had changed.
        print(
            f"\n  Axiom connectors  ({_MARK['available']} available"
            f"  {_MARK['planned']} planned)\n"
        )
        rows = [
            (
                _MARK.get(e["availability"], "?"),
                "on" if e["enabled"] else "off",
                e["title"],
                e["name"],
            )
            for e in sorted(
                entries, key=lambda x: (x["availability"] != "available", x["name"])
            )
        ]
        # `{title:<18}` is a guess that holds only while every title is short.
        # The identifier trailing it has nothing to keep it on the line.
        for line in table(
            rows,
            [Column(""), Column("state"), Column("connector"), Column("id", wrap=True)],
            width=terminal_width(reserve=4),
            headers=True,
            border=SQUARE,
        ):
            print(line)
        print(f"\n  {_brand_cli()} connector show <name> · install <name> · enable <name> · disable <name>\n")
        return 0
    ...
    if not res.ok:
        ...
        if remedy:
            ...
            print(f'\n  → Not a known error. Ask AXI to diagnose:\n'
                  f'      {_brand_cli()} chat "connector {full} configure failed: {err[:160]}"\n', file=sys.stderr)
        return 1
    v = res.value
    ...
    if d.setup and d.setup.install_kind == "app_manifest":
        ...
        if "app_id" in v:
            ...
            for i, (label, url) in enumerate(v.get("token_urls", {}).items(), start=2):
                print(f"  {i}. {label}: {url}")
            print(f"\n  Then: paste tokens into the {secret_ref} binding + `{_brand_cli()} connector enable {name}`.\n")
        else:
            ...
            print(f"\n  After the app exists: install + invite the bot, paste tokens, `{_brand_cli()} connector enable {name}`.\n")
        return 0
    res = reg.install(
