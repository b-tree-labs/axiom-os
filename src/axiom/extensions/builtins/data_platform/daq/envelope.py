...
#: What a row's value *is*. This is the canonical definition — `skills.enroll`
#: imports it rather than keeping a second copy, because two lists of the
#: allowed provenance classes would eventually disagree and the row would be
#: accepted by one gate and rejected by the other.
#:
#: ``simulated`` names synthetic data: values a generator produced with no
#: referent in any real system. It is deliberately not folded into
#: ``predicted``, which means a model's statement *about a real system's
#: state*. A simulator's output is not a worse prediction, it is a different
#: kind of thing, and conflating them is how a demo stream ends up averaged
#: into a comparison between two real loops.
#:
#: Its absence is what put synthetic flow-loop rows into silver stamped
#: ``measured``: there was no honest value available, so they took the default.
SOURCE_CLASSES: tuple[str, ...] = ("measured", "predicted", "estimated", "simulated")

#: Classes that must name what produced them. A simulator is a model like any
#: other: a synthetic row nobody can attribute to a generator version cannot be
#: reproduced, compared or retired.
MODELLED_CLASSES: tuple[str, ...] = ("predicted", "estimated", "simulated")
...
class SignalEnvelope:
    ...
    def __post_init__(self) -> None:
        ...
        # The other half of that rule. Without it the contract only caught the
        # harmless direction — a record that names a model and admits it — and
        # let the damaging one through: output that names a model and calls
        # itself measured anyway. A consumer cannot tell that apart from a
        # sensor reading, which is the whole point of the field.
        if self.source_class == "measured" and self.model_ref:
            raise ValueError(
                "source_class 'measured' cannot carry a model_ref: a record "
                f"produced by {self.model_ref} is not a measurement"
            )
        if self.seq < 0:
