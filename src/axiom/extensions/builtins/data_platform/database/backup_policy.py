...
class BackupPolicy:
    ...
    Usually the wrong knob, and wrong *silently*. pg_dump will dump a view
    whose underlying table sits in an excluded schema, and the restore then
    fails on that view — after the artifact has passed every structural check.
    Seen on a live node: `bronze.documents` selects from `public`, and all
    seven `gold` views select from `silver`, so a scoped dump produced an
    83 MB file that existed, was fresh, parsed its TOC, and could not restore.
    Reach for ``exclude_table_data`` instead."""
    exclude_table_data: list[str] | None = None
    """Tables whose DATA is skipped while their DEFINITION is kept.

    The right lever for a database dominated by a few enormous but
    re-derivable tables: every object restores, the bulk simply comes back
    empty. Accepts pg_dump patterns (``public.reactor_timeseries_*``)."""
    timeout_s: int = 600
    """How long pg_dump may run. A big database needs more than the default,
    and a dump killed by its own timeout must fail loudly rather than leave a
    truncated artifact that looks like a backup."""
    validate_restore: bool = True
    """Whether the scheduled validation actually RESTORES the newest artifact.

    Defaults to true, and that default is the point. Without it
    ``data.backup_validate`` runs four structural checks — exists, fresh,
    non-empty, TOC parses — and never adds a restore check at all. Not a FAIL,
    not a skip: absent, from a report that then reads as complete.

    All four passed on an artifact that could not be restored. The scratch
    restore was the only thing that caught it, and it was the one part nothing
    scheduled ever ran."""
    scratch_dsn: str | None = None
    #: Name of a disposable database on the SAME server, for the restore check.
    #: Preferred over ``scratch_dsn``: a DSN carries a password, and this file
    #: is read back and printed by `axi data backup-policy`. Given a name, the
    #: DSN is derived from the live one at use time and never persisted.
    scratch_db: str | None = None
    """Disposable database the restore check restores INTO.

    With ``validate_restore`` on and this unset, the check FAILS rather than
    skipping — a node that turns on backups without somewhere to prove them is
    told so, loudly, instead of collecting green reports about files nobody has
    restored."""
    offbox: str = "none"
...
def save_backup_policy(policy: BackupPolicy, *, state_dir: Path | None = None) -> Path:
    ...
    if policy.exclude_table_data is not None:
        rendered_excludes = ", ".join(_toml_str(t) for t in policy.exclude_table_data)
        lines.append(f"exclude_table_data = [{rendered_excludes}]")
    lines.append(f"timeout_s = {policy.timeout_s}")
    lines.append(f"validate_restore = {'true' if policy.validate_restore else 'false'}")
    if policy.scratch_dsn:
        lines.append(f"scratch_dsn = {_toml_str(policy.scratch_dsn)}")
    if policy.scratch_db:
        lines.append(f"scratch_db = {_toml_str(policy.scratch_db)}")
...
def load_backup_policy(*, state_dir: Path | None = None) -> BackupPolicy | None:
    ...
    return BackupPolicy(
        ...
        exclude_table_data=(
            [str(t) for t in blob["exclude_table_data"]]
            if blob.get("exclude_table_data") is not None
            else None
        ),
        timeout_s=int(blob.get("timeout_s", BackupPolicy.timeout_s)),
        validate_restore=bool(blob.get("validate_restore", BackupPolicy.validate_restore)),
        scratch_dsn=(str(blob["scratch_dsn"]) if blob.get("scratch_dsn") else None),
        scratch_db=(str(blob["scratch_db"]) if blob.get("scratch_db") else None),
