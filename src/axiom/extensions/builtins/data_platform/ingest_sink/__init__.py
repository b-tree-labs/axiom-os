...
from .tabular import PushRowBatch, TabularIngestResult, TabularIngestSink
...
# -- tabular push (ADR-001): the row-lane peers of the above ---------------


def build_tabular_writer_for_config(config: ConnectorConfig) -> TabularBronzeWriter:
    """The ONE TabularBronzeWriter construction site — same bronze root + rules +
    disposition + tier the pull/CDC tabular path uses, so a pushed row batch
    lands identically to a fetched one (no split brain)."""
    rules = load_rules_file(config.provenance_rules_file) if config.provenance_rules_file else []
...
def tabular_sink_for_connector(
    connector_name: str,
    *,
    state_dir: Path | None = None,
) -> TabularIngestSink:
    """Build a :class:`TabularIngestSink` from a connector's config."""
    config = load_connector(connector_name, state_dir=state_dir)
    return TabularIngestSink(writer=build_tabular_writer_for_config(config))


def make_connector_tabular_sink_resolver(
    *,
    state_dir: Path | None = None,
) -> Callable[[str], TabularIngestSink]:
    """A cached per-connector resolver for the tabular push front door
    (``/ingest/rows``). Raises ``KeyError`` for an unknown connector so the
    surface fails loudly (422)."""
    cache: dict[str, TabularIngestSink] = {}

    def resolve(connector_name: str) -> TabularIngestSink:
        sink = cache.get(connector_name)
        if sink is None:
            try:
                sink = tabular_sink_for_connector(connector_name, state_dir=state_dir)
            except FileNotFoundError as exc:
                raise KeyError(connector_name) from exc
            cache[connector_name] = sink
        return sink
