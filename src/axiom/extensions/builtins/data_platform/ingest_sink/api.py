...
def build_tabular_ingest_router(
    ...
    *,
    ...
    def ingest_rows(req: TabularIngestRequest, request: Request) -> dict:
        if not req.source:
            raise HTTPException(status_code=422, detail="source is required")
        ...
        return asdict(target.ingest_rows(req.source, push_batches))

    return router


def create_tabular_ingest_app(sink: TabularIngestSink) -> FastAPI:
    """Standalone app exposing only the row-lane ingest endpoint."""
    app = create_app(
