...
_VERBS = (
    ...
    "list",
    "troubleshoot",
    # Productized backups (the DB-deployment lifecycle's data concern):
    "backup",
    "backup_validate",
    # Reserved: restore is deliberately interactive-only for now (the
    # dangerous path has no skill); the verb exists so the authz surface
    # is already carved when it ships.
    "restore",
    # Scheduled dispatch — the orchestrator writes one receipt per
    # PULSE-fired skill invocation.
    "dispatch",
)
...
def _build_context():
    ...
    try:
        ...
        _log.warning(
            ...
            f"without receipt-writing. Run `{_brand_cli()} audit healthcheck` to "
            "diagnose.",
            exc,
        )
