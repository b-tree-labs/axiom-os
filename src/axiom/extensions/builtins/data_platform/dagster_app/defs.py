...
def _build_definitions(*, registry=None):
    ...
    @asset(
        name="silver_signals_conform",
        group_name="silver",
        description="Bronze→silver conform pass: every registered connector's rows become "
        "canonical silver.signals. Site comes from the connector registry (the same source the "
        "register --site flag writes); the conform-run CLI verb is this same pass without the "
        "Dagster runtime.",
    )
    def silver_signals_conform(context):
        from ..conformance.runner import conform_verdict, run_conform

        dsn = os.environ.get(_RAG_DSN_ENV) or os.environ.get("DATABASE_URL")
        if not dsn:
            raise ValueError(f"{_RAG_DSN_ENV} / DATABASE_URL unset — cannot conform to silver")
        bronze_root = os.environ.get(_BRONZE_ROOT_ENV, "/var/lib/axiom/bronze")

        stats = run_conform(bronze_root=bronze_root, dsn=dsn)
        verdict = conform_verdict(stats, strict=True)
        context.add_output_metadata(
            {
                "rows_in": stats.get("rows_in", 0),
                "rows_out": stats.get("rows_out", 0),
                "errored": stats.get("errored", 0),
                "unmapped_connectors": ", ".join(stats.get("unmapped_connectors", [])) or "(none)",
                "unknown_schema": str(stats.get("unknown_schema", {})),
            }
        )
        # A skipped connector or an unknown schema is a silent drop — fail the
        # run so the Dagster failure surfaces it, exactly as the CLI exits non-zero.
        if not verdict.ok:
            raise Exception("conform funnel not clean: " + " | ".join(verdict.messages))
        return stats.get("rows_out", 0)

    conform_job = define_asset_job("dp1_conform_job", selection=[silver_signals_conform])
    conform_schedule = ScheduleDefinition(
        name="dp1_conform_schedule",
        job=conform_job,
        cron_schedule="*/30 * * * *",
        default_status=DefaultScheduleStatus.STOPPED,
    )

    assets: list = [silver_signals_conform]
    jobs: list = [conform_job]
    sensors: list = []
    schedules: list = [conform_schedule]
    ...
    return Definitions(assets=assets, jobs=jobs, sensors=sensors, schedules=schedules)
