...
def test_session_dir_env_flows_into_params(monkeypatch, tmp_path):
    _clear(monkeypatch)
    monkeypatch.setenv("AXIOM_BOX_SESSION_DIR", str(tmp_path / "sess"))
    monkeypatch.setenv("DP1_BOX_FOLDER_ID", "5")
        ...
        json.dumps(
            {
                ...
                "client_secret": "s",
                "refresh_token": "r",
                "token_store": str(tmp_path / "x.json"),
            }
        ),
    )
    monkeypatch.setenv("DP1_BOX_FOLDER_ID", "5")
...
def test_connector_tomls_are_primary_source(monkeypatch, tmp_path):
    ...
    assert specs == [SourceSpec("box", "dept-archive", "rag-org", {"folder_id": "228326101313"})]
...
def test_connector_flows_through_registry_for_non_box_kind(monkeypatch, tmp_path):
    ...
    assert specs == [SourceSpec("fake", "my-fake", "rag-org", {"endpoint": "x"})]
...
def test_build_writer_falls_back_to_env_writer_with_spec_tier(monkeypatch, tmp_path):
    ...
    writer = _build_writer(
        SourceSpec(
            kind="box", name="unregistered", default_tier="rag-internal", params={"folder_id": "1"}
        )
    )
