...
flowchart TD
    ...
    P -->|"ensure_backup_cadences()<br/>startup, or `--apply`"| O
    O --> E
    E --> X
    X --> B
    X --> V
...
## The other cadence the orchestrator arms

`ensure_freshness_cadence` runs at the same startup and registers a daily
`data.ingest_freshness` sweep (09:00 UTC). It is **not** gated on the backup
policy and needs no arming: the defect it closes is a partner stream going
quiet for four months while nothing said so, and a check an operator has to
remember to enable is that defect with an extra step.

It passes `alert=True`, which the skill honours only from the schedule. A human
running `axi data ingest-freshness` sees the same report and pages nobody —
inspecting freshness should not wake anyone up.

## Arming it

`enabled = true` in `~/.axi/plinth/backup_policy.toml` is the intention. A PULSE
schedule row is the effect. They are different facts, and they can disagree
silently: `ensure_backup_cadences` runs at orchestrator **startup**, and nothing
re-runs it when the file changes. Editing the policy and walking away leaves a
file that says the backup is on and a scheduler with nothing in it.

So arm it in one step and confirm in the same breath:

```
axi data backup-policy --apply
```

which projects the policy onto PULSE and then reports both halves:
...
`axi data backup-policy` on its own is read-only and answers the question that
matters: *will a backup actually run tonight?* It reports an error when the two
halves disagree — enabled with no rows, only one of the two cadences active, or
rows still active after a disable. An unreadable schedule store reports
`unknown` rather than guessing `not armed`.
