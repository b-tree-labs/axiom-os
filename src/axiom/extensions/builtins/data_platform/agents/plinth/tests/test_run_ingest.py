...
def test_run_ingest_populates_and_persists_generic_funnel(tmp_path: Path, monkeypatch):
    """The pull job builds the generic IngestRunReport funnel and persists it
    via the injected RunStore — proving the primitive is wired job-side."""
    from axiom.extensions.builtins.data_platform import rag_embed as re_mod
    from axiom.extensions.builtins.data_platform.agents.plinth.skills.run_ingest import run_ingest
    ...
    # The funnel is attached to the returned report …
    assert report.funnel is not None
    assert report.funnel["job_kind"] == "pull"
    assert report.funnel["source"] == "box-reports"
    assert report.funnel["status"] == "succeeded"
    stages = {s["stage"]: s for s in report.funnel["funnel"]}
    assert stages["discovered"]["entered"] == 1
    assert stages["fetched"]["advanced"] == 1
    assert stages["indexed"]["advanced"] == 1

    # … and persisted in the run store (single source of truth).
    persisted = run_store.recent()
    assert len(persisted) == 1
    assert persisted[0]["run_id"] == report.funnel["run_id"]
