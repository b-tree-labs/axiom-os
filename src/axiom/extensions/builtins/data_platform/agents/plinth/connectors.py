...
class ConnectorConfig:
    ...
    bronze_root: str
    site: str | None = None
    """The tenant this connector's rows belong to.

    Conformance needs a connector→site mapping to attribute rows, and a
    connector already knows its own name — so it carries its own site rather
    than being looked up in a second file that can disagree with this one. An
    unset site means unattributable: conformance skips the connector and says
    so, rather than guessing (ADR-023-A1 §A1.5(2), fail closed)."""
