...
def _resolve_dsn(params: dict[str, Any]) -> str | None:
    ...
    # No longer fails when no env name is set: resolve_dsn ends at the URL the
    # platform itself uses, so this backs up the database the process is
    # already connected to. Refusing here while `axi db migrate` succeeded on
    # the same node is how a nightly cadence gets scheduled and never runs.
    #
    # The source is reported in actions_taken — backing up the platform default
    # because nothing was configured is worth seeing in a nightly log.
...
def run(params: dict[str, Any], ctx: SkillContext) -> SkillResult:
    ...
    from .._dsn import dsn_source

    # Named, never printed: a DSN carries a password. Which SOURCE answered is
    # the part worth seeing in a nightly log.
    actions: list[str] = [
        f"database: {'explicit dsn= param' if params.get('dsn') else dsn_source()}"
    ]
    with _authz.action(
        verb="backup",
        ...
        result = create_backup(
            ...
            exclude_table_data=policy.exclude_table_data,
