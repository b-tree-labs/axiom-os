...
**Owner:** `axi data ingest` · invocable through SkillRegistry (ADR-056)
...
See `ingest.py` for the function body. CLI verb `axi data ingest` is a thin
...
through `ctx.registry.invoke("data.ingest", params, ctx)`.

## Inputs / Outputs

See the function docstring at `axiom.extensions.builtins.data_platform.skills.ingestun`.
