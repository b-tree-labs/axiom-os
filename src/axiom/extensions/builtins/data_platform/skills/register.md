...
**Owner:** `axi data register` · invocable through SkillRegistry (ADR-056)
...
See `register.py` for the function body. CLI verb `axi data register` is a thin
...
through `ctx.registry.invoke("data.register", params, ctx)`.

## Inputs / Outputs

See the function docstring at `axiom.extensions.builtins.data_platform.skills.registerun`.
