...
**Owner:** `axi data diagnose` · invocable through SkillRegistry (ADR-056)
...
See `diagnose.py` for the function body. CLI verb `axi data diagnose` is a thin
...
through `ctx.registry.invoke("data.diagnose", params, ctx)`.

## Inputs / Outputs

See the function docstring at `axiom.extensions.builtins.data_platform.skills.diagnoseun`.
