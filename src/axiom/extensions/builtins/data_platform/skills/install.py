...
def run(params: dict[str, Any], ctx: SkillContext) -> SkillResult:
    ...
    for binary in ("helm", "kubectl"):
        if shutil.which(binary) is None:
            errors.append(f"{binary!r} not found on PATH — install it before `{_brand_cli()} data install`")
    if errors:
    ...
    actions.append(
        ...
        f"`{_brand_cli()} data register <name> <kind> ...` (kinds: `{_brand_cli()} data list kinds`)."
    )
    return SkillResult(
        ok=True,
        value={
