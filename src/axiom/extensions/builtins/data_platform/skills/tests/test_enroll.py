...
# ---------------------------------------------------------------------------
# `simulated` — synthetic data needs a name of its own.
#
# Found live on the node: three flow-loop sites each carried a `loop.instrument`
# stream whose rows were stamped `source_class=measured` while their own
# `schema_ref` read `flowloop-sim/instrument-v0`. The record contradicted
# itself. The averages across the three agreed to five significant figures over
# the same seventeen minutes — one generator, three seeds — so a partner would
# have seen our simulator sitting beside their real instrument, both claiming to
# be measurements.
#
# The cause was that the vocabulary had nowhere honest to put it: `measured`,
# `predicted`, `estimated`. Synthetic data is none of those, so it took the
# default.
# ---------------------------------------------------------------------------
...
name = "flowloop-sim"
site = "acu-flowloop"
schema_ref = "flowloop-sim/instrument-v0"
source_class = "simulated"
model_ref = "flowloop-sim@0.3.1"
"""


def test_simulated_is_an_allowed_source_class():
    source, errors = parse_manifest(SIM_MANIFEST)
    assert errors == [], f"a simulated source should enroll cleanly: {errors}"
    assert source["source_class"] == "simulated"


def test_simulated_must_name_the_generator_that_produced_it():
    """A synthetic row nobody can attribute to a generator cannot be retired.

    Same rule as `predicted` and `estimated`, and for the same reason: when the
    simulator changes, you need to know which rows came from the old one.
    """
    manifest = SIM_MANIFEST.replace('model_ref = "flowloop-sim@0.3.1"\n', "")
    _, errors = parse_manifest(manifest)
    assert any("model_ref" in e for e in errors), (
        "a simulated source with no model_ref was accepted"
    )


def test_simulated_is_not_measured():
    """The distinction is the point, not a formality.

    `measured` still rejects a model_ref, so a source cannot claim both to be a
    measurement and to name what generated it.
    """
    manifest = SIM_MANIFEST.replace('source_class = "simulated"', 'source_class = "measured"')
    _, errors = parse_manifest(manifest)
    assert any("measurement has no model" in e for e in errors)


def test_one_vocabulary_not_two():
    """enroll and the ingest envelope must agree on what is admissible.

    They held separate copies of this tuple. Two lists is one edit away from a
    source that enrolls but whose rows are refused at ingest — or worse, the
    reverse, where enrollment rejects what ingest would happily have stamped.
    """
    from axiom.extensions.builtins.data_platform.daq import envelope
    from axiom.extensions.builtins.data_platform.skills import enroll

    assert enroll.SOURCE_CLASSES is envelope.SOURCE_CLASSES
    assert enroll.MODELLED_CLASSES is envelope.MODELLED_CLASSES
    assert "simulated" in envelope.SOURCE_CLASSES
