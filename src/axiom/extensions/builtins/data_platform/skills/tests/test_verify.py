...
def test_check_python_deps_pass_when_all_importable():
    with patch(
        "axiom.extensions.builtins.data_platform.skills.verify._try_import", return_value=True
    ):
...
def test_check_box_auth_fail_no_credentials(monkeypatch):
    ...
    c = check_box_auth()
    assert c.status == Status.FAIL
    assert "BOX_JWT_CONFIG" in (c.remediation or "") or "BOX_DEVELOPER_TOKEN" in (
        c.remediation or ""
    )
...
def test_check_pg_dsn_reports_which_source_answered(monkeypatch):
    """It used to FAIL when no env name was set. It cannot any more.

    `resolve_dsn` always answers, ending at the platform's own URL, so the old
    branch would be a check that can never fire — and a check that cannot fail
    is not evidence. What is still worth reporting is where the answer came
    from: running against the built-in default because nothing was configured
    is materially different from running against a DSN somebody set.
    """
    from axiom.extensions.builtins.data_platform.skills import verify as verify_mod

    for name in ("DP1_RAG_DSN", "DATABASE_URL", "AXIOM_DB_URL"):
        monkeypatch.delenv(name, raising=False)
    monkeypatch.setattr(verify_mod, "_pg_probe", lambda dsn: False)
    check = verify_mod.check_pg_dsn()
    assert check.status is verify_mod.Status.FAIL
    assert "platform default" in check.detail

    monkeypatch.setenv("DP1_RAG_DSN", "postgresql://u:pw@h/db")
    monkeypatch.setattr(verify_mod, "_pg_probe", lambda dsn: True)
    ok = verify_mod.check_pg_dsn()
    assert ok.status is verify_mod.Status.PASS
    assert "DP1_RAG_DSN" in ok.detail
    assert "pw" not in ok.detail, "never put a credential in a check detail"
