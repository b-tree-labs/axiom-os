...
class TestDsnResolution:
    def test_no_env_dsn_uses_the_platforms_own_database_and_says_so(self, tmp_path, monkeypatch):
        """This test used to assert the opposite, and the opposite was the bug.

        `axi data backup` refused on a node where `axi db migrate upgrade head`
        had just succeeded, because migrate resolves through
        `axiom.infra.db.platform_db_url` (which defaults) and this read two env
        names and gave up. data.backup is what the nightly PULSE cadence
        invokes, so arming the schedule against it would have registered a row
        that fires at 2am, fails, and leaves a policy reading `enabled = true`
        with no backup behind it.

        The fallback is not a guess: it is the database the process is already
        connected to. It is reported, by SOURCE not by value — a DSN carries a
        password, and "ran against the platform default because nothing was
        configured" is worth seeing in a nightly log.
        """
        ...
        assert any("platform default" in a for a in result.actions_taken)

    def test_the_reported_source_never_contains_the_dsn_itself(self, tmp_path, monkeypatch):
        """A password must not reach a log line."""
        monkeypatch.setenv("DP1_RAG_DSN", "postgresql://u:sup3rsecret@h/db")
        target = tmp_path / "backups"
        save_backup_policy(BackupPolicy(target_root=str(target)), state_dir=tmp_path)
        result = backup_skill.run({"_dry_run": True}, _ctx(tmp_path))
        joined = " ".join(result.actions_taken) + " ".join(result.errors)
        assert "sup3rsecret" not in joined
        assert "DP1_RAG_DSN" in joined, "name the source variable, not its value"
