...
def test_get_json_429_raises_typed_RateLimited(tmp_path, monkeypatch):
    ...
    import requests
    monkeypatch.setattr(requests.Session, "get", fake_get)
    # _retry_on_429 now honors Retry-After with real sleeps before re-raising;
    # neutralize the sleeper so the budget exhausts instantly in the test.
    import axiom.infra.ratelimit as _rl
    monkeypatch.setattr(_rl, "sleep_for_retry", lambda *a, **k: None)
