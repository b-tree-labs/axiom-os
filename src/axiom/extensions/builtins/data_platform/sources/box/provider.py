...
class BoxSourceProvider:
    ...
    def add_register_args(self, subparser: argparse.ArgumentParser) -> None:
        ...
        subparser.add_argument(
            ...
            default=[],
            metavar="PATH",
            help="subtree (source_path prefix, e.g. '/Corpus/TRIGA_realtime_logging') this "
                 "source never walks; repeatable. For telemetry drops and other non-document "
                 "subtrees under the corpus root.",
        )
        subparser.add_argument(
            "--exclude-ext",
            action="append",
            default=[],
            metavar="EXT",
            help="file extension this source never catalogs (e.g. jpg, mp4); repeatable. "
                 "Photos and videos on a shared corpus root are not documents.",
        )
        subparser.add_argument(
            "--max-item-mb",
            type=int,
            default=None,
            metavar="MB",
            help="skip any file larger than this many MB at catalog time, before any fetch.",
        )
        subparser.add_argument(
            "--jwt-secret-ref",
    ...
    def params_from_args(self, args: argparse.Namespace) -> dict[str, str]:
        ...
        excluded = normalize_exclude_prefixes(getattr(args, "exclude_prefix", None) or [])
        if excluded:
            params["exclude_prefixes"] = ",".join(excluded)
        exts = normalize_extensions(getattr(args, "exclude_ext", None) or [])
        if exts:
            params["exclude_extensions"] = ",".join(exts)
        max_mb = getattr(args, "max_item_mb", None)
        if max_mb is not None:
            if max_mb < 1:
                raise ValueError("--max-item-mb must be >= 1")
            params["max_item_bytes"] = str(max_mb * 1024 * 1024)
        return params
    ...
    def construct(self, config: ConnectorConfig) -> BoxIngestSource:
        ...
        return BoxIngestSource(name=config.name, folder_id=folder_id, api_client=api,
                               exclude_prefixes=exclude_prefixes_from_params(config.params),
                               exclude_extensions=exclude_extensions_from_params(config.params),
                               max_item_bytes=max_item_bytes_from_params(config.params))
