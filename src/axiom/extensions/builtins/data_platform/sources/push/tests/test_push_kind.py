...
def test_register_with_site_attributes_the_connector(tmp_path):
    """A push connector registered with --site carries it, so conformance can
    attribute its rows to silver.signals (the console/ROM ingest path)."""
    ctx = SkillContext(registry=SkillRegistry(), state_dir=tmp_path, logger=logging.getLogger("t"))
    res = register_skill.run(
        {
            "name": "netl-triga-console",
            "kind": "push",
            "site": "ut-triga",
            "bronze_root": str(tmp_path / "bronze"),
            "default_disposition": "allow",
            "kind_params": {"schema_ref": "triga-console/serial-v1"},
        },
        ctx,
    )
    assert res.ok, res.errors
    assert res.value["site"] == "ut-triga"
    assert load_connector("netl-triga-console", state_dir=tmp_path).site == "ut-triga"
    # no unattributed warning when a site is present
    assert not any("WITHOUT a site" in a for a in res.actions_taken)


def test_register_without_site_is_refused_unless_explicitly_unmapped(tmp_path):
    """A siteless push connector is REFUSED (it would accept rows that
    conformance silently skips forever); --allow-unmapped is the explicit
    escape hatch and keeps the loud warning."""
    ctx = SkillContext(registry=SkillRegistry(), state_dir=tmp_path, logger=logging.getLogger("t"))
    params = {
        "name": "orphan-feed",
        "kind": "push",
    ...
    refused = register_skill.run(params, ctx)
    assert refused.ok is False
    assert any("--allow-unmapped" in e for e in refused.errors)

    res = register_skill.run({**params, "allow_unmapped": True}, ctx)
    assert res.ok, res.errors
    assert res.value["site"] is None
    assert any("WITHOUT a site" in a for a in res.actions_taken)
