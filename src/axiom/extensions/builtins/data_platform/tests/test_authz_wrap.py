...
class TestIngestSkillWrapsRunIngest:
    def test_ingest_wraps_run_ingest(self, captured_actions, monkeypatch, ctx):
        # Stand-in for the shape dispatcher so we don't need a real connector.
        class _Report:
        ...
        monkeypatch.setattr(ingest_skill, "run_connector_ingest", lambda *a, **kw: _Report())
...
class TestDiagnoseSkillWrapsKubectlPasses:
    def test_diagnose_wraps_namespace_action(
        ...
        # Extraction deps are checked locally before the cluster probes; force
        # the green branch so this test exercises only the kubectl/helm path.
        monkeypatch.setattr(
            diagnose_skill.verify, "missing_extraction_deps", lambda: []
        )
