...
def test_backup_without_dsn_fails_closed(tmp_path: Path):
    """End-to-end verb→skill dispatch: no DSN anywhere → clear error, rc 1."""
    r = _run(
        ["backup"],
        state_dir=tmp_path,
        env_extra={"DP1_RAG_DSN": "", "DATABASE_URL": ""},
    )
