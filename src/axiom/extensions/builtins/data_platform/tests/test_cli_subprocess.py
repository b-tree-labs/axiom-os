...
# The repo-under-test's src/ — prepended to PYTHONPATH so the subprocess
# imports THIS tree's axiom, not whatever checkout the venv's editable
# install points at (pytest's `pythonpath = ["src"]` only covers
# in-process imports).
_SRC = str(Path(__file__).resolve().parents[5])
...
def test_cli_help_lists_all_verbs(tmp_path: Path):
    ...
    for verb in (
        ...
        "unregister",
        "list",
        # The effect-side counterpart of `refresh` (ran vs landed); the
        # skill shipped in 0.57.0 with no operator-facing verb.
        "ingest-freshness",
    ):
...
def test_backup_help(tmp_path: Path):
    r = _run(["backup", "--help"], state_dir=tmp_path)
    assert r.returncode == 0, r.stderr
    for flag in ("--dsn", "--label", "--target-root", "--box-secret-ref"):
        assert flag in r.stdout


def test_backup_validate_help(tmp_path: Path):
    r = _run(["backup-validate", "--help"], state_dir=tmp_path)
    assert r.returncode == 0, r.stderr
    for flag in ("--target-root", "--max-age-hours", "--validate-restore",
                 "--scratch-dsn"):
        assert flag in r.stdout


def test_backup_without_env_dsn_names_the_source_it_used(tmp_path: Path):
    """End-to-end verb→skill dispatch with no DSN env set.

    This asserted rc 1 and "DSN" in stderr — the behaviour that stopped
    `axi data backup` working on a node where `axi db migrate` succeeded.
    data.backup is what the nightly cadence invokes, so that refusal meant a
    schedule could be armed and never produce a backup.

    It now resolves the platform's own database and says which source answered.
    Whether the dump itself succeeds depends on a reachable Postgres, so this
    asserts on the reported source rather than the exit code.
    """
    r = _run(
        ["backup"],
        state_dir=tmp_path,
        env_extra={"DP1_RAG_DSN": "", "DATABASE_URL": "", "AXIOM_DB_URL": ""},
    )
    combined = r.stdout + r.stderr
    assert "platform default" in combined, combined[:400]
    assert "no DSN" not in combined


def test_backup_validate_empty_root_reports_checks(tmp_path: Path):
    """No artifacts → rc 1 and the fail-closed check battery in --json."""
    r = _run(
        ["--json", "backup-validate", "--target-root", str(tmp_path / "none")],
        state_dir=tmp_path,
    )
    assert r.returncode == 1, r.stderr
    payload = _json.loads(r.stdout)
    checks = {c["name"]: c["status"] for c in payload["value"]["checks"]}
    assert checks["backup_exists"] == "FAIL"
