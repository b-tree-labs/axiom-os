...
def _parser() -> argparse.ArgumentParser:
    ...
    ing.add_argument("--workers", type=int,
    ...
    ref.add_argument("--overlap-minutes", type=int, default=60,
    ...
    reg.add_argument("--credential-ref",
    ...
    reg.add_argument("--promotion-map-file",
    ...
    act = sub.add_parser(
        "activity",
        help="What the RAG ingested per connector since a time ago "
             "(e.g. --since 24h | 7d | 90m).",
    )
    act.add_argument("--since", default="24h",
                     help="lookback window: 24h, 7d, 90m, 2w (default 24h)")
    ...
    bf.add_argument("--dry-run", action="store_true",
                    help="report the match rate without writing anything")
    bf.add_argument("--corpus",
                    help="corpus to backfill (default: the connector's default tier)")
    ...
    bak.add_argument("--dsn",
                     help="database DSN (default: DP1_RAG_DSN / DATABASE_URL)")
    bak.add_argument("--label", default="", help="label appended to the filename")
    bak.add_argument("--target-root",
                     help="artifact directory (default: BackupPolicy.target_root)")
    bak.add_argument("--box-secret-ref",
    ...
    bval.add_argument("--target-root",
                      help="artifact directory (default: BackupPolicy.target_root)")
    bval.add_argument("--max-age-hours", type=float,
                      help="staleness threshold in hours (default 26)")
    bval.add_argument("--validate-restore", action="store_true",
                      help="restore into a scratch database + row-count sanity")
    bval.add_argument("--scratch-dsn",
                      help="disposable database DSN for --validate-restore")
    bval.add_argument("--dsn",
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Hyphenated CLI verbs (backup-validate) map to snake_case skill names.
    result = ctx.registry.invoke(f"data.{args.verb.replace('-', '_')}", params, ctx)
