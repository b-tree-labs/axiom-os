...
_BASE_SETS = [
    ...
    # Required since chart 0.2.0: Langfuse v3 needs a Redis queue and S3 for event
    # payloads, and the templates `fail` without them so a broken deploy cannot
    # render. The blank-secrets test below passes NO sets and must keep failing.
    "redis.external.connectionString=redis://redis.example.internal:6379",
    "s3.external.endpoint=https://s3.example.internal",
    "s3.external.bucket=langfuse",
]
_EXTERNAL_PG_SETS = [
