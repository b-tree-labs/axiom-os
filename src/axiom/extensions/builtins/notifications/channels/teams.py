...
def _build_adaptive_card(
    *,
    ...
    mention_upns: list[str] | None = None,
) -> dict[str, Any]:
    ...
    """
    # Clean rendering: the summary IS the message. Metadata (recipient,
    # classification, receipt) belongs in logs/receipts, not the channel —
    # the FactSet table read as debug output to operators. Urgency shows as
    # color+weight only when it means something (high/urgent). Mentions, when
    # configured, are what make Teams raise real activity notifications —
    # without one, a webhook post never badges anyone.
    urgent = priority in ("high", "urgent")
    body: list[dict[str, Any]] = []
    entities: list[dict[str, Any]] = []
    mentions = mention_upns or []
    if mentions:
        tags = " ".join(f"<at>{u}</at>" for u in mentions)
        body.append({"type": "TextBlock", "text": tags, "wrap": True, "isSubtle": True})
        entities = [
            {
                "type": "mention",
                "text": f"<at>{u}</at>",
                "mentioned": {"id": u, "name": u.split("@")[0]},
            }
            for u in mentions
        ]
    body.append(
            ...
            **(
                {
                    "size": "medium",
                    "weight": "bolder",
                    "color": _URGENCY_COLOR.get(priority, "attention"),
                }
                if urgent
                else {}
            ),
...
class TeamsChannelAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        self._mention_upns = list(mention_upns or [])
    ...
    def deliver_sync(
        ...
        *,
        ...
        payload = _build_adaptive_card(
            ...
            classification=classification,
            priority=priority,
            mention_upns=self._mention_upns,
        )
        ...
        idem = (
            idempotency_key
            or f"axiom-receipt-{receipt_id}-{uuid.uuid5(uuid.NAMESPACE_URL, receipt_id)}"
        )
        headers = {
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            try:
                ...
                last_error = _strip_secret(f"{type(exc).__name__}: {exc}")
                last_status = None
            ...
            if status in _RECONNECT_STATUSES:
                ...
                return TeamsDispatchResult(
                    ok=False,
                    status_code=status,
                    error=_strip_secret(f"HTTP {status} (auth): {body[:200]}"),
                    retry_attempts=attempt - 1,
            ...
            if status == 429 or 500 <= status < 600:
                ...
                body = getattr(resp, "text", "") or ""
                last_error = _strip_secret(f"HTTP {status}: {body[:200]}")
                if attempt < _MAX_ATTEMPTS:
...
class TeamsChannelAdapterProvider:
    ...
    def build(self, config: dict[str, Any] | None = None) -> TeamsChannelAdapter:
        ...
        raw_mentions = cfg.get("mention_upns") or ""
        mention_upns = (
            [m.strip() for m in raw_mentions.split(",") if m.strip()]
            if isinstance(raw_mentions, str)
            else list(raw_mentions)
        )
        if not webhook_url:
            raise ValueError(
                "teams channel requires `webhook_url` in config — the "
                "Workflows-trigger URL (post-2026-05-22 surface); "
                f"resolve via `{_brand_cli()} secrets resolve teams-workflow-<workspace>`"
            )
