...
class InboxDispatchResult:
    ...
    error: str | None = None
    durable: bool = True
    """Whether the row it wrote outlives this process.

    False means the alert was 'delivered' into a store that is discarded at
    exit — true of the default in-memory store, which is what an adopter
    running a monitor off-node gets. The send still succeeded; nobody can
    read it. A caller that cannot tell those apart reports a lie."""
        ...
        *,
        ...
        try:
            ...
            return InboxDispatchResult(
                ok=True,
                row_id=row_id,
                durable=bool(getattr(self._store, "durable", False)),
            )
