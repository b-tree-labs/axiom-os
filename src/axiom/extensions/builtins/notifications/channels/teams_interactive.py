...
def _token_authority_url(tenant_id: str | None) -> str:
    """The token endpoint for a Bot Connector client-credentials grant.

    A **multi-tenant** bot authenticates against the shared ``botframework.com``
    authority. A **single-tenant** bot (``MicrosoftAppType = SingleTenant``, the
    default UT provisions) is *not* present in that directory — authenticating
    there fails with ``AADSTS700016: Application … was not found in the directory
    'd6d4…'`` — and must use its own tenant's authority. So: tenant authority
    when ``tenant_id`` is set, ``botframework.com`` otherwise.
    """
    authority = tenant_id if tenant_id else "botframework.com"
...
def _default_token_provider(
    *, app_id: str, app_password: str, tenant_id: str | None = None
    ...
    ``tenant_id`` selects the authority (see :func:`_token_authority_url`):
    single-tenant bots mint against their own tenant, multi-tenant against
    ``botframework.com``, both with the ``api.botframework.com`` scope. Never
    called in tests — a fake provider is injected — so the httpx import stays
    lazy; the authority choice itself is unit-tested via ``_token_authority_url``.
    """
    ...
    def _acquire() -> str:
        ...
        resp = httpx.post(
            _token_authority_url(tenant_id),
            data={
