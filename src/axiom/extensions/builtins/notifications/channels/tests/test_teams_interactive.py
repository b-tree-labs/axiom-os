...
def test_token_authority_single_tenant():
    # Single-tenant bot (the default UT provisions) authenticates against its
    # own tenant — verified live 2026-08-12: botframework.com gave AADSTS700016.
    from axiom.extensions.builtins.notifications.channels.teams_interactive import (
        _token_authority_url,
    )

    url = _token_authority_url("31d7e2a5-bdd8-414e-9e97-bea998ebdfe1")
    assert url == (
        "https://login.microsoftonline.com/"
        "31d7e2a5-bdd8-414e-9e97-bea998ebdfe1/oauth2/v2.0/token"
    )


def test_token_authority_multi_tenant_default():
    from axiom.extensions.builtins.notifications.channels.teams_interactive import (
        _token_authority_url,
    )

    assert (
        _token_authority_url(None)
