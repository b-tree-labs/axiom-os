...
class TestAdaptiveCardPayload:
    def test_payload_has_attachment_with_adaptive_card(self):
        ...
        # Clean rendering: the summary IS the message — no metadata FactSet
        # (that read as debug output in-channel; receipts live in logs).
        assert card["body"][-1]["text"] == "DP1 RAG ingest complete"
        assert not any(b.get("type") == "FactSet" for b in card["body"])
        # normal priority: no shouty styling
        assert "weight" not in card["body"][-1]

    def test_mentions_raise_activity_notifications(self):
        poster_resp = [(200, "ok", {})]
        adapter = TeamsChannelAdapter(
            webhook_url="https://prod-1.logic.azure.com/workflows/x/paths/invoke?sig=s",
            mention_upns=["op1@example.edu"],
            poster=_FakePoster(poster_resp),
        )
        adapter.deliver_sync(
            recipient="#general",
            receipt_id="r-2",
        ...
        assert ent["type"] == "mention"
        assert ent["mentioned"]["id"] == "op1@example.edu"
        assert "<at>op1@example.edu</at>" in card["body"][0]["text"]
...
class TestBodyText:
    def test_body_text_renders_below_the_summary(self):
        adapter = _adapter([(200, "ok", {})])
        adapter.deliver_sync(
            recipient="#general",
            receipt_id="r-9",
            classification=Classification.INTERNAL,
            priority="low",
            summary="deployed: site@v1",
            body_text="v0 -> v1 (2 changes)\n• fix a\n• fix b",
        )
        card = adapter._poster.calls[0]["json"]["attachments"][0]["content"]
        assert card["body"][-1]["text"].startswith("v0 -> v1")
        assert card["body"][-1]["wrap"] is True
        assert card["body"][-2]["text"] == "deployed: site@v1"

    def test_without_body_text_the_summary_stays_last(self):
        adapter = _adapter([(200, "ok", {})])
        adapter.deliver_sync(
            recipient="#general",
            receipt_id="r-10",
        ...
        card = adapter._poster.calls[0]["json"]["attachments"][0]["content"]
        assert card["body"][-1]["text"] == "just a summary"
