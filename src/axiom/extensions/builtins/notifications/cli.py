...
def _build_parser() -> argparse.ArgumentParser:
    ...
    alrt = sub.add_parser(
        "alert",
        help="Post a DURABLE alert to a recipient's inbox (the monitor path).",
    )
    alrt.add_argument("--recipient", required=True,
                      help="recipient principal (Matrix-style @name:context)")
    alrt.add_argument("--summary", required=True,
                      help="what happened, as the operator will read it")
    alrt.add_argument("--actor", default="@monitor:local",
                      help="which monitor is reporting (default @monitor:local)")
    alrt.add_argument("--link", default="",
                      help="URL for more information, or to act on it "
                           "(cmd+click in most terminals)")
    alrt.add_argument(
    ...
    alrt.add_argument(
        "--dedup-key", dest="dedup_key",
        help="suppress a repeat of this same alert for 24h — a monitor on a "
             "timer re-observes a standing condition every run, and without "
             "this the operator gets it again every run",
    )

    # ---- ack -----------------------------------------------------------
    ack = sub.add_parser(
        "ack", help="Acknowledge an alert — record that you have taken it up."
    )
    ack.add_argument("id", help="alert id (the short form shown in chat is enough)")
    ack.add_argument("--recipient", default="",
                     help="who is acknowledging (defaults to this machine's principal)")
    ...
    st = sub.add_parser(
        "setup",
        help="Configure a delivery channel durably (links mode when no URL).",
    )
    st.add_argument("channel", help="channel to configure (teams, slack, mattermost)")
    st.add_argument("--webhook-url", dest="webhook_url",
                    help="the vendor webhook URL; omit to print the clickable setup links")
    st.add_argument("--expect", help="override expected_channels (comma-separated)")
    st.add_argument("--tenant-id", dest="tenant_id", help="Teams tenant id (deep links)")
    st.add_argument("--group-id", dest="group_id", help="Teams team/group id (deep links)")
    st.add_argument("--channel-id", dest="channel_id", help="Teams channel thread id (deep links)")
    st.add_argument("--channel-name", dest="channel_name", help="Teams channel display name")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = invoke_capability(ctx.registry, skill, params, ctx, surface=CLI_SURFACE)
        # setup links-mode on a TTY: prompt for the paste-back right here —
        # input() takes the URL raw, so there is no shell-quoting fight and
        # the secret never lands in shell history.
        if (
            args.verb == "setup"
            and result.ok
            and isinstance(result.value, dict)
            and result.value.get("mode") == "links"
            and not args.json
            and sys.stdin.isatty()
        ):
            _emit(result, as_json=False)
            pasted = input("\nPaste the webhook URL here (Enter to skip): ").strip()
            if not pasted:
                return 0
            params["webhook_url"] = pasted
            result = invoke_capability(ctx.registry, skill, params, ctx, surface=CLI_SURFACE)
