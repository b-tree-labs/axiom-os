...
class DeliveryReceipt:
    ...
    error: str | None = None
    durable: bool = True
    """Whether the selected channel put this somewhere a person can still read.

    `outcome` answers "did the channel accept it"; this answers "did it survive
    the process". They come apart exactly once, and it is the case that matters
    most: the default in-memory inbox accepts every alert and keeps none, so a
...
class SendContext:
    ...
    # STILL in-memory, deliberately, and this is a known gap rather than a
    # choice: an alert sent in one process is invisible to every other reader.
    # Flipping it to the database-backed store fails, because an inbox row has
    # a NOT NULL foreign key to `delivery_receipts` and NOTHING PERSISTS
    # RECEIPTS — they live in `SendContext.receipts`, a dict. Making
    # notifications survive a process means persisting the receipt first; that
    # is a separate change, not a default swap.
    ...
    dedup_log: MutableMapping = field(default_factory=dict)
    """``(actor, dedup_key) → receipt_id``; per fabric §6.1 sliding window.

    A plain dict is process-local, which silently defeats suppression for any
    caller that forks per run. ``SendContext.default()`` substitutes a durable
    file-backed log; tests keep the dict.
    """
    ...
    def default(
        ...
        *,
        ...
        """
        ctx = cls()
        # Durable by default: the callers that most need suppression are shell
        # scripts on timers, and they fork per run. An in-memory log is empty
        # every time for exactly the callers dedup exists to protect.
        from axiom.extensions.builtins.notifications.dedup import FileDedupLog

        ctx.dedup_log = FileDedupLog()
    ...
    *,
    ...
    if dedup_key is not None:
        ...
        if key in ctx.dedup_log:
            prior_id = ctx.dedup_log[key]
            prior = ctx.receipts.get(prior_id)
            if prior is not None:
                return prior
            # Suppressed by a DIFFERENT process — the receipt lives in that
            # run's memory, not ours. Synthesize one rather than raising: the
            # caller asked "was this sent?", and the answer is yes.
    ...
    # external channel is admitted for) still lands somewhere, and an
    # external channel that is unconfigured or fails to deliver falls through
    # to it rather than dropping the alert. It is appended LAST so a
    # configured, admitted external channel is still preferred.
    #
    # "Somewhere" is only durable if the configured store is: the default is
    # in-memory, and a fall-through onto it accepts the alert and keeps it
    # exactly as long as the process lives. That is reported on the receipt
    # (``durable``) rather than hidden behind a green outcome.
    ...
    # Nothing delivered yet, so nothing has outlived anything.
    durable = False
    for name in attempt_names:
        ...
        if getattr(result, "ok", False):
            ...
            error = None
            durable = bool(getattr(result, "durable", True))
            break
