...
class TestOutboundReceiptPersistence:
    ...
    def _setup_schema(self):
        ...
        from axiom.infra.db import (
            ensure_schema, get_engine, normalize_extension_name, session_for,
        )

        # Use the normalized name so pytest-xdist's per-worker schema
        # suffix (set by tests/conftest.py from PYTEST_XDIST_WORKER)
        # is honored. Without this, worker A's TRUNCATE wipes worker B's
        # rows mid-test and the suite false-flakes under -n auto.
        # See feedback_prepush_xdist_false_positives.
        schema = normalize_extension_name("vault")
        engine = get_engine()
