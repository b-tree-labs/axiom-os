...
def test_missing_expiry_is_not_silently_treated_as_safe():
    ...
    The name was right and the assertion was not: this used to assert
    ``due == 0``, pinning the silent drop, on the reasoning that `secrets audit`
    surfaced it elsewhere. It does — on a surface nothing runs unattended. The
    verb that IS on the heartbeat reported "nothing due" and exit 0, so a
    credential with no recorded expiry stayed invisible until it expired and
    locked us out. A test whose name asserts safety while its body asserts the
    unsafe behaviour passes either way.

    It escalates now: not auto-rotated (there is no due date to rotate against),
    but red, and naming the command that fixes it.
    """
    res = _run([{"name": "no-expiry", "provider": "gitlab-pat", "auto_rotate": True}])
    assert res.value["due"] == 1
    assert res.ok is False
    assert res.value["rotated"] == []
    assert any("no-expiry" in e for e in res.errors)
