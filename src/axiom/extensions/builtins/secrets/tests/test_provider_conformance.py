...
def _factory_config(kind: str) -> dict:
    ...
    if kind == "keychain":
        # Inject a no-op runner so conformance never touches a real keychain.
        def _noop_runner(args, *, input=None):  # noqa: ANN001, A002
            return 1, "", "conformance"
        return {"name": "conformance", "runner": _noop_runner}
    if kind == "file":
        import tempfile
        return {
            "name": "conformance",
            "path": f"{tempfile.mkdtemp(prefix='axiom-secrets-conf-')}/v.json",
        }
    return {"name": "conformance"}
