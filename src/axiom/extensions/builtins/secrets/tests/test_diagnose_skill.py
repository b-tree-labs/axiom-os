...
def fresh_registry(monkeypatch):
    """Swap the registry's ConnectorRegistry for an isolated one."""
    saved = SecretStoreRegistry._reg
    SecretStoreRegistry._reg = ConnectorRegistry("secret store")
    try:
        SecretStoreRegistry.register(_GoodProvider)
        SecretStoreRegistry.register(_UnreachableProvider)
        yield SecretStoreRegistry
    finally:
        SecretStoreRegistry._reg = saved
