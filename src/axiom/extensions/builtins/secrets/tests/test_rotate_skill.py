...
class TestVendorWiring:
    """Vendor-API strategies become reachable once an admin_ref is supplied
    (#16). Without one the skill refuses clearly; with one it drives the built
    strategy and never leaks the minted value."""

    def test_sendgrid_without_admin_ref_refuses_clearly(self, store):
        r = rotate_skill.run(
            {"ref": "openbao://kv/sg", "strategy": "sendgrid", "force": True},
            _ctx(),
        )
        assert not r.ok
        assert "admin" in r.errors[0].lower()

    def test_sendgrid_with_admin_ref_rotates_and_hides_value(self, store, monkeypatch):
        ...
        class FakeSendGrid:
            def post(self, path, body):
                return {"api_key_id": "KID1", "api_key": "SG.new-secret"}
        ...
        # admin-cred resolution is covered in test_rotation_config; here we assert
        # the skill threads admin_ref through to a working vendor strategy.
        monkeypatch.setattr(
            rotate_skill,
            "build_vendor_strategy",
            lambda config, store_for: SendGridRotation(
                http=FakeSendGrid(), key_name=config.managed_name or "m"
            ),
        )
        r = rotate_skill.run(
            {
                "ref": "openbao://kv/sendgrid-api-key",
                "strategy": "sendgrid",
        ...
        assert r.value["strategy"] == "sendgrid"
        assert r.value["forced"] is True
        # never leaks the minted secret
        assert "value" not in r.value and "secret" not in r.value
        # the new key was staged through the store
        assert store.puts == [b"SG.new-secret"]
