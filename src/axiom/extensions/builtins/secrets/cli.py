...
def _parser() -> argparse.ArgumentParser:
    ...
    st.add_argument("--expires-at", default=None,
                    help=f"Expiry (YYYY-MM-DD) for `{_brand_cli()} secrets audit`.")
    ...
    disc = sub.add_parser(
        "discover",
        help="Find credential material living OUTSIDE the store (git remotes, "
             "credential files, env files). Reports fingerprints, never values.",
    )
    disc.add_argument("--workspace", default=None,
                      help="Root to scan for git remotes (default ~/Projects).")
    disc.add_argument("--env-root", default=None,
                      help="Root to scan for env files (default ~/.config).")
    disc.add_argument("--probe", action="append", dest="probes", default=None,
                      help="Limit to a probe (repeatable): git-remote, "
                           "git-credentials, env-file.")
