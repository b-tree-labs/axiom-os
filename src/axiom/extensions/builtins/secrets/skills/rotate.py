...
Strategies: ``provider-native`` (the backend rotates itself), ``hitl`` (a human
supplies the new value via ``--value`` or the interactive prompt), and the
vendor-API strategies (``sendgrid`` …) which mint at the vendor. A vendor
strategy needs an ``admin_ref`` — the ref of its admin credential — from which
``build_vendor_strategy`` builds the client through the store seam; without one
the skill returns a clear error rather than half-doing it.
"""
...
from ..rotation.config import RotationConfig, build_vendor_strategy
...
# Vendor-API strategies: reachable once an admin-credential ref is supplied
# (build_vendor_strategy resolves it through the store seam, never inlined).
_VENDOR_STRATEGIES = ("sendgrid",)
...
def _build_strategy(kind: str, params: dict, ctx: SkillContext):
    ...
    if kind in _VENDOR_STRATEGIES:
        ...
        if not admin_ref:
            raise RotationError(
                f"strategy {kind!r} is a vendor-API strategy; pass an admin "
                "credential ref (admin_ref) — e.g. the SendGrid parent key — so "
                "its client can be built from the SecretStore. The admin key is "
                "resolved through the store seam, never inlined."
            )
        config = RotationConfig(
            kind=kind,
            admin_ref=admin_ref,
            base_url=params.get("base_url"),
            managed_name=params.get("managed_name") or params.get("key_name"),
            scopes=tuple(params["scopes"]) if params.get("scopes") else None,
        )
        return build_vendor_strategy(config, store_for=_store_for)
    raise KeyError(kind)
...
def run(params: dict[str, Any], ctx: SkillContext) -> SkillResult:
    ...
    if not raw_ref:
        return SkillResult(ok=False, errors=[
            "rotate needs a SecretRef (e.g. openbao://kv/x) or a foreign "
            "credential name (e.g. hpc-gitlab-pat)",
        ])
    if "://" not in raw_ref:
        # Bare name → the #667 foreign-credential flow (KEEP-executed:
        # issuer RotationProvider + guarded_act + action-ledger journal).
        from .rotate_foreign import rotate_foreign

        return rotate_foreign(raw_ref, params, ctx)
    try:
    ...
    try:
        ...
        # vendor strategy without an admin_ref, or another build-time refusal
        return SkillResult(ok=False, errors=[str(exc)])
    except KeyError:
        known = ", ".join(_CLI_STRATEGIES + _VENDOR_STRATEGIES)
        return SkillResult(
            ok=False,
            errors=[f"unknown strategy {kind!r}; known: {known}"],
        )
