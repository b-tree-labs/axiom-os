...
def setup_postgresql() -> int:
    ...
    if not value:
        ...
        print(f"  Set later: {_brand_cli()} settings set rag.database_url postgresql://...")
        print()
        return 0
    ...
    print(f"  ✓ Saved rag.database_url to {project_dir_name()}/settings.toml")
    print()
    print("  Next steps:")
    print(f"    {_brand_cli()} rag index docs/          — index your documents")
    print(f"    {_brand_cli()} rag index runtime/       — index sessions + notes")
    print(f"    {_brand_cli()} chat                     — RAG context now active")
    print()
    return 0
...
def setup_pack_server() -> int:
    ...
    try:
        ...
        if len(packs) > 10:
            print(f"      ... and {len(packs) - 10} more ({_brand_cli()} rag pack list --remote --server {name})")
    except Exception as e:
    ...
    print()
    print(f"  Install a pack:   {_brand_cli()} rag pack install <pack-id> --server {name}")
    print(f"  List packs:       {_brand_cli()} rag pack list --remote --server {name}")
    print()
    return 0
