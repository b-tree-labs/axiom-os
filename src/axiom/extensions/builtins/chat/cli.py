...
def build_setup_hint(gateway: Gateway) -> str | None:
    ...
    if missing_env_vars:
        ...
        lines.append(
            f"    {cli_name} config --model small         # local LLM (gemma2-2b, ~1.6GB, no key)"
        )
        lines.append(
...
def run_repl(
    ...
    try:
        while True:
            ...
            # A leading "!" used to run the rest of the line as a shell
            # command with no approval gate, no permission check and no
            # audit: a write path to the installed software, reachable from
            # the chat surface, that the authority model grades as
            # unauthorized. It is gone. The line is not sent to the model
            # either: whoever typed it expected a shell, so say so.
            ...
            try:
                ...
                # The status line is emitted by the agent now, on the
                # completed-turn path, so every surface gets it and the tier
                # travels with it. Calling it here too printed it twice.

                # Post-turn advisor: OFFERS at most one dim tip; executes
                # nothing; swallows everything. Same hook as the TUI.
                turn_index += 1
                _advise_after_turn(agent, user_input, response, turn_started, turn_index)
            except KeyboardInterrupt:
...
def _render_repl_tip(text: str) -> None:
    print(f"  {_c(_Colors.DIM, '💡 ' + text)}")


def _advise_after_turn(
    agent: ChatAgent,
    user_input: str,
    response: str | None,
    started: float,
    ...
    """End-of-turn advisor call for the REPL. Never raises."""
    try:
...
BANG_ESCAPE_NOTICE = (
    "Shell commands are not run from chat. Use a terminal, or ask for an approved tool."
)


def _refuse_bash_escape() -> None:
    """Explain that ``!cmd`` no longer runs anything, and run nothing."""
    print(f"  {_c(_Colors.DIM, BANG_ESCAPE_NOTICE)}")


def _fullscreen_unavailable_reason() -> str | None:
    """Why the full-screen UI cannot start, or ``None`` when it can."""
...
def _is_fullscreen_available() -> bool:
    ...
    return _fullscreen_unavailable_reason() is None


REPL_FALLBACK_NOTICE = (
    "[chat] full-screen UI not started ({reason}); using the line REPL. "
    "Same tools, approvals and permissions apply."
)


def _announce_repl_fallback(reason: str) -> None:
    """Never drop to the REPL silently: say which surface is running and why."""
    print(REPL_FALLBACK_NOTICE.format(reason=reason), file=sys.stderr)
...
def main():
    ...
    fallback_reason = "--no-tui" if args.no_tui else _fullscreen_unavailable_reason()
    if fallback_reason is None:
        try:
            ...
            fallback_reason = f"full-screen UI failed: {_tui_err}"

    # Classic REPL fallback: same agent, same gate, same permissions.
    _announce_repl_fallback(fallback_reason)
