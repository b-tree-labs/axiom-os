...
class TestSessionPlane:
    ...
    def test_with_no_database_it_is_the_local_file_ledger(
        self, identity, tmp_path, monkeypatch
    ):
        """The file store is still correct for a laptop with nothing wired —
        and now says so. It used to be the ONLY answer this plane could give,
        which is the shape that cannot support a second surface.
        """
        monkeypatch.delenv("AXIOM_DB_URL", raising=False)
        monkeypatch.delenv("DATABASE_URL", raising=False)
        monkeypatch.setattr(
            "axiom.infra.orchestrator.session_backend.choose_session_backend",
            lambda default_url=None: __import__(
                "axiom.infra.orchestrator.session_backend", fromlist=["x"]
            ).BackendChoice(
                kind="files",
                local_only=True,
                shared=False,
                ...
                reason="no database configured; local testing only",
            ),
        )
        plane = planes.resolve_session()
        assert plane.kind == "local"
        ...
        assert "@ben:laptop" in plane.detail
        assert "local testing only" in plane.detail

    def test_a_localhost_database_is_local_not_served(self, identity, monkeypatch):
        """The distinction the plane exists to keep: configured is not
        reachable. Claiming `served` for one machine's database would make
        this look finished while no second surface could reach it."""
        monkeypatch.setenv("AXIOM_DB_URL", "postgresql://a:b@localhost:5432/axiom_db")
        plane = planes.resolve_session()
        assert plane.kind == "local"
        assert "no second surface" in plane.detail
        assert "b@localhost" not in plane.source

    def test_a_remote_database_is_served(self, identity, monkeypatch):
        monkeypatch.setenv("AXIOM_DB_URL", "postgresql://a:b@db.example.edu:5432/axiom")
        plane = planes.resolve_session()
        assert plane.kind == "served"
        assert "db.example.edu" in plane.source
...
class TestFormat:
    ...
    def test_columns_align(self):
        ...
        text = format_plane_report(rows, width=100)
        # Rows open with the border now, so find them by content rather than
        # by the first character.
        lines = [ln for ln in text.splitlines() if "domain" in ln or "person" in ln]
        lines = [ln for ln in lines if "served" in ln or "local" in ln]
        assert len(lines) == 2, f"expected two data rows, got {lines}"
        assert lines[0].index("served") == lines[1].index("local"), (
            f"columns diverge:\n{lines[0]}\n{lines[1]}"
        )

    def test_the_report_is_a_closed_grid(self):
        """Borders, and a rule under the header only.

        This asserted a rule between EVERY row, which is what was asked for
        first. Ben changed it after seeing a fourteen-row table run past forty
        lines: header-only, like the reference table. One `├` is now correct
        and two would be the regression.
        """
        rows = [
            PlaneStatus("domain", "served", "https://retrieval.example", "endpoint"),
            PlaneStatus("person", "local", "os-session @me:local", ""),
        ]
        lines = [ln for ln in format_plane_report(rows, width=100).splitlines() if ln.strip()]
        assert any(ln.strip().startswith("┌") for ln in lines), "no top edge"
        assert any(ln.strip().startswith("└") for ln in lines), "no bottom edge"
        rules = sum(1 for ln in lines if ln.strip().startswith("├"))
        assert rules == 1, f"expected exactly one rule, under the header; got {rules}"
