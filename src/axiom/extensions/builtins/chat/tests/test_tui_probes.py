...
# ---------------------------------------------------------------------------
# Wrapped-line indentation: continue under the text, the way Claude Code does
# ---------------------------------------------------------------------------


def _wrap(tui, text, width=60):
    return tui._word_wrap(text, width).split("\n")


def test_a_wrapped_paragraph_keeps_its_own_margin(tui_factory):
    """A paragraph is not a list, so its continuation is not indented.

    The wrapper used to add two spaces to every continuation line, which
    quietly turned running prose into something that looked like a hanging
    indent.
    """
    tui, _ = tui_factory(columns=80, rows=24)
    lines = _wrap(tui, "word " * 40)
    assert len(lines) > 1, "the fixture text has to actually wrap"
    for cont in lines[1:]:
        assert not cont.startswith(" "), f"paragraph continuation indented: {cont!r}"


def test_a_wrapped_bullet_continues_under_its_text(tui_factory):
    tui, _ = tui_factory(columns=80, rows=24)
    lines = _wrap(tui, "- " + "word " * 40)
    assert len(lines) > 1
    ...
    for cont in lines[1:]:
        assert cont.startswith("  ") and not cont.startswith("   "), (
            f"bullet continuation should align under the text: {cont!r}"
        )


def test_a_wrapped_numbered_item_aligns_past_its_number(tui_factory):
    """`1. ` is three columns and `10. ` is four, so a flat two is wrong."""
    tui, _ = tui_factory(columns=80, rows=24)
    one = _wrap(tui, "1. " + "word " * 40)
    ten = _wrap(tui, "10. " + "word " * 40)
    assert one[1].startswith("   ") and not one[1].startswith("    "), one[1]
    assert ten[1].startswith("    ") and not ten[1].startswith("     "), ten[1]


def test_a_nested_bullet_keeps_its_nesting(tui_factory):
    tui, _ = tui_factory(columns=80, rows=24)
    lines = _wrap(tui, "  - " + "word " * 40)
    assert lines[0].startswith("  - ")
...
def test_a_wrapped_quote_keeps_the_quote_marker(tui_factory):
    """Dropping the marker on the continuation ends the quote mid-sentence."""
    tui, _ = tui_factory(columns=80, rows=24)
    lines = _wrap(tui, "> " + "word " * 40)
    assert len(lines) > 1
    for cont in lines[1:]:
        assert cont.startswith("> "), f"quote continuation lost its marker: {cont!r}"
