...
class TestTheCorporaReachTheRetriever:
    ...
    def test_the_call_site_hands_over_a_failclosed_access_context(self, machine, headless, monkeypatch):
        """The call site now hands the retriever a fail-closed ``AccessContext``
        derived from the session principal.

        It used to pass nothing, on the reasoning that a context was inert
        without wired lookups. That is no longer true: ``retrieve`` now honors
        each chunk's OWN access metadata (the store projects it), so the context
        is a real control — a classified or higher-tier chunk is filtered where,
        with no context, it leaked. Per-principal tier ELEVATION still awaits the
        policy engine; the baseline is the safe floor, and ``tier_lookup`` stays
        unwired because the chunk's projected column is the source of truth.
        """
        from axiom.rag.retriever import AccessContext

        seen = _record_retriever_calls(monkeypatch)
        headless.turn(PROBE, stream=False)
        assert seen
        for call in seen:
            ctx = call.get("access_context")
            ...
            assert "tier_lookup" not in call  # chunk's own column is honored

    def test_the_failclosed_baseline_permits_public_unclassified(self):
        """The baseline the call site hands over still permits the common case:
        a public, unclassified chunk passes. Enforcement filters classified /
        higher-tier content, not ordinary content."""
