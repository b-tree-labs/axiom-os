...
def _raw_enabled(monkeypatch):
    """These exercise `raw`, so they must stand up a deployment that allows it.

    `raw` is gated at the capability now: it empties the system prompt, where
    the content safeguards live, so it is enabled per-deployment rather than
    requested per call. The refusal path is covered in
    tests/chaos/test_raw_bypass_gate.py.
    """
    from axiom.infra.raw_mode import RAW_ENV

    monkeypatch.setenv(RAW_ENV, "1")
