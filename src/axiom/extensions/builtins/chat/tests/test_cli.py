...
class TestFirstRunGuard:
    ...
    def test_no_providers_hint_includes_free_local_options_first(self):
        ...
        assert f"{cli_name} config --model small" in hint, (
            "Free local gemma option should appear in the no-providers hint"
        )
        ...
        local_pos = hint.index(f"{cli_name} config --model small")
        anthropic_pos = hint.index("ANTHROPIC_API_KEY")
        assert local_pos < anthropic_pos, (
...
class TestMaybeStartLocalLLM:
    ...
    def test_no_op_when_llamafile_not_provisioned(self):
        ...
        with patch("axiom.setup.llamafile.is_llamafile_installed", return_value=False):
            transitioned = maybe_start_local_llm(gw)
        assert transitioned is False
        gw._discover_local_llm.assert_not_called()
    ...
    def test_starts_provisioned_but_idle_llamafile(self):
        ...
        with (
            ...
            patch("axiom.setup.llamafile.start_llamafile", return_value=True) as start,
        ):
            transitioned = maybe_start_local_llm(gw)
    ...
    def test_already_running_just_reprobes(self):
        ...
        with (
            patch("axiom.setup.llamafile.is_llamafile_installed", return_value=True),
            patch("axiom.setup.llamafile.is_llamafile_running", return_value=True),
    ...
    def test_start_failure_falls_through_silently(self):
        ...
        with (
            patch("axiom.setup.llamafile.is_llamafile_installed", return_value=True),
            patch("axiom.setup.llamafile.is_llamafile_running", return_value=False),
            patch("axiom.setup.llamafile.start_llamafile", return_value=False),
        ):
            transitioned = maybe_start_local_llm(gw)
        assert transitioned is False
...
class TestNoShellEscape:
    """A leading ``!`` must never reach a shell, and must not go to the model."""

    def test_bang_input_runs_nothing_and_is_not_sent_to_the_model(self, capsys, monkeypatch):
        import subprocess

        from axiom.extensions.builtins.chat.cli import BANG_ESCAPE_NOTICE, run_repl

        def _boom(*args, **kwargs):  # pragma: no cover - the assertion is that it is never hit
            raise AssertionError(f"shell spawned: {args} {kwargs}")

        monkeypatch.setattr(subprocess, "run", _boom)
        monkeypatch.setattr(subprocess, "Popen", _boom)
        monkeypatch.setattr("os.system", _boom)
        monkeypatch.setattr(
            "axiom.extensions.builtins.chat.cli.detect_workspace_context", lambda: None
        )
        ...
        store.list_sessions.return_value = []
        render = MagicMock()
        input_prov = MagicMock()
        input_prov.prompt.side_effect = ["!echo pwned", EOFError()]

        run_repl(agent, store, stream=False, render=render, input_prov=input_prov)

        out = capsys.readouterr().out
        assert BANG_ESCAPE_NOTICE in out
        assert "pwned" not in out
        agent.turn.assert_not_called()  # run_repl's only path to the model


class TestExplicitReplFallback:
    """Dropping to the line REPL is never silent, and never changes permissions."""

    def test_reason_when_stdin_is_not_a_terminal(self, monkeypatch):
        from axiom.extensions.builtins.chat.cli import (
            _fullscreen_unavailable_reason,
            _is_fullscreen_available,
        )

        monkeypatch.setattr("sys.stdin", MagicMock(isatty=lambda: False))
        assert _fullscreen_unavailable_reason() == "stdin is not a terminal"
        assert _is_fullscreen_available() is False

    def test_no_reason_when_everything_is_present(self, monkeypatch):
        from axiom.extensions.builtins.chat.cli import _fullscreen_unavailable_reason

        monkeypatch.setattr("sys.stdin", MagicMock(isatty=lambda: True))
        monkeypatch.setattr("sys.stdout", MagicMock(isatty=lambda: True))
        assert _fullscreen_unavailable_reason() is None

    def test_fallback_is_announced_with_its_reason(self, capsys):
        from axiom.extensions.builtins.chat.cli import _announce_repl_fallback

        _announce_repl_fallback("stdout is not a terminal")
        err = capsys.readouterr().err
        assert "stdout is not a terminal" in err
        assert "Same tools, approvals and permissions apply" in err
