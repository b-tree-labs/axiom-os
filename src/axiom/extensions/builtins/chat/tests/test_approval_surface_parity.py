...
class TestParser:
    ...
    def test_legend_and_retry_name_the_same_keys(self):
        """``h`` (hold) joined the vocabulary; the retry must still offer all of them.

        The loop is the property. A re-prompt that names fewer outcomes than
        the legend is how a surface quietly stops offering one, and the retry
        is the message a confused operator reads most closely.
        """
        keys = [k for k, _ in APPROVAL_LEGEND]
        assert keys == ["a", "A", "r", "D", "h", "s"]
        for key in keys:
