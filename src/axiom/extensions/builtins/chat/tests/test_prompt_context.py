...
def fragment(name: str, content: str, layer: str = "capabilities") -> dict:
    """A fragment an extension is actually allowed to contribute.

    These tests are about discovery — that a contributor is called, handed its
    context, and skipped when it breaks — and the layer was incidental. It
    defaulted to `identity`, which extensions may no longer write: who the
    agent is belongs to the product, not to whichever package wrote a prompt.
    """
    return {
        "layer": layer,
        "name": name,
