...
class AnsiRenderProvider(RenderProvider):
    ...
    def render_welcome(
        ...
        if gateway is not None:
            ...
            if provider:
                ...
                model_seg = f"  {_c(_Colors.DIM, 'No LLM configured.')} " \
                             f"{_c(_Colors.DIM, f'Run `{_brand_cli()} config` to set up.')}"
        ...
        if model_seg and not model_seg.startswith("  "):
            header_text = (
                f"{_c(_Colors.BOLD + _Colors.ACCENT_BLUE, f'{_brand_cli()} chat')}"
    ...
    def render_citations(self, chunks: Sequence[Any]) -> None:
        ...
        if not entries:
            return
        print()
        print(f"  {_c(_Colors.DIM, 'Sources:')}")
        for e in entries:
            line = f"[{e['key']}] {e['title']}  {e['path']}"
            print(f"  {_c(_Colors.DIM, line)}")
        print()
