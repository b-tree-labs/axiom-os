...
def serving_scope(session=None, **overrides) -> "ChatScope":
    """A scope for a surface answering SOMEBODY OTHER THAN THE OPERATOR.

    Three declarations, all of them the difference between a private
    conversation and a disclosure:

    - ``index_transcript=False`` — this request's conversation is never
      written where a later request could retrieve it;
    - ``retrieval_corpora=SHARED_CORPORA`` — and so never
      :data:`OPERATOR_LOCAL_CORPUS`, which holds this machine's own chat
      transcripts and would otherwise reach the prompt of whoever asks;
    - ``include_operator_local_prompts=False`` — the operator's project file,
      personal context file and prompt library stay out of a stranger's
      prompt.

    They live here, in one factory, because they had lived in one surface's
    constructor. Any second serving surface had to remember all three, and the
    HTTP surface did not: it ran on the DEFAULT scope, where
    ``index_transcript`` is True. That was harmless only for as long as the
    surface never saved a session file, since the indexer reads the file — a
    protection made of an omission, which held until the day the surface
    started persisting conversations.
    """
    scope = ChatScope(
