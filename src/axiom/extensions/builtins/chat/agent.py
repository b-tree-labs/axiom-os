...
def _hold_for_later(action: Action) -> str | None:
    """Put a chat-refused action on the durable approval queue.

    Returns the action id, or ``None`` when the queue could not be written,
    which the caller reports as "nothing is waiting" rather than inventing an
    id. Telling somebody their action is queued when it is not is worse than
    telling them it failed: they wait instead of retrying.

    The path comes from ``default_approval_path`` so this is the same queue
    ``axi approve`` reads. Any other spelling produces a second queue that
    ...
    try:
        ...
        ApprovalGate(FileActionStore(default_approval_path())).record(action)
    except Exception:  # noqa: BLE001 — an unreachable queue is reported, not raised
        _log.exception("could not hold %s for later approval", action.action_id)
        return None
        ...
        try:
            ...
            for frag in produced or []:
                if frag.get("layer") and frag.get("name") and frag.get("content"):
                    frag.setdefault("source", getattr(ep, "name", "extension"))
                    fragments.append(frag)
        except Exception as exc:
            log.warning("prompt contributor %r failed: %s", getattr(ep, "name", ep), exc)

    # Bound what any one extension can be. Measured on a real install: 69 tools
    # across twelve families and a single contributor, which supplied three of
    # six prompt fragments including one in `identity` — so the assistant's
    # character came from the extension holding 7% of the tools, and it answered
    # telemetry questions with model-catalogue suggestions.
    from axiom.extensions.builtins.chat.prompt_fairness import (
        enforce_contribution_limits,
    )

    return enforce_contribution_limits(fragments)
...
def _local_principal() -> str:
    """The handle a locally-created conversation belongs to.

    A session with no principal is a conversation nobody owns: it cannot be
    resumed by its author on another surface, cannot be attributed once it is
    persisted, and has no inbox, so alerts have nowhere to arrive. Nothing set
    this before, so every terminal session was anonymous to everything outside
    the process.

    Falls back to empty rather than raising — an agent that will not start
    because it cannot name the user is worse than one whose conversation is
    unattributed.
    """
    try:
...
def set_default_scope(agent: Any, scope: ChatScope) -> None:
    """Install *scope* as the conversation this agent's unscoped calls run in.

    A surface that answers somebody other than the operator has to say so
    BEFORE its first turn, and the alternative was reaching into the instance
    dictionary from outside. `_active_scope` creates a default on first use,
    so this must run before any turn — the caller owns that ordering.
    """
    agent.__dict__[DEFAULT_SCOPE_ATTR] = scope
...
class ChatAgent:
    ...
    def __init__(
        ...
        self._default_scope = ChatScope(
            session=session or Session(principal_id=_local_principal()),
            permissions=(
    ...
    def _turn_impl(
        ...
        *,
        ...
        # --- self-knowledge fast path (deterministic, no model round trip) ---
        # A question about THIS node is answered from the live registries in
        # milliseconds — the same doctrine as the oversight courier
        # (deterministic to the words). Carrying the facts in the system
        # prompt is necessary but NOT sufficient: observed live, a small
        # model with a long tool list still wandered off and spent two
        # minutes. This runs BEFORE classification and budget validation
        # because it spends no model tokens and needs no retrieval.
        # Precision-first: an uncertain match returns None and the turn
        # proceeds normally, with the facts still in the prompt.
        if not raw:
            try:
                from axiom.extensions.builtins.status.selfanswer import (
                    answer_about_self,
                )

                _self_answer = answer_about_self(user_input)
            except Exception:  # noqa: BLE001 — never break a turn for this
                _self_answer = None
            if _self_answer:
                scope.session.add_message("user", user_input)
                scope.session.add_message("assistant", _self_answer)
                if stream:
                    from axiom.llm.gateway import StreamChunk

                    chunk = StreamChunk(type="text", text=_self_answer)
                    try:
                        if self._render:
                            self._render.stream_text(iter([chunk]))
                        elif self._renderer_callback:
                            self._renderer_callback(_self_answer)
                        if getattr(scope, "on_chunk", None):
                            scope.on_chunk(chunk)
                    except Exception:  # noqa: BLE001 — the answer still returns
                        _log.debug("self-answer render failed", exc_info=True)
                return _self_answer
        ...
        for _round in range(max_rounds):
            ...
            # A provider may accept `tools` and ignore it, leaving the model to
            # improvise the protocol: it writes a fenced block naming a tool and
            # the turn ends with no tool calls and a paragraph that reads like an
            # answer. Recover what is runnable — validated against the tool table,
            # never against the text — so this works for every extension's tools
            # without per-extension wiring.
            if not response.tool_use and response.text:
                from axiom.extensions.builtins.chat.tool_call_recovery import (
                    looks_like_a_tool_attempt,
                    recover_tool_calls,
                )

                recovered = recover_tool_calls(response.text, all_tools or {})
                if recovered:
                    _log.info(
                        "recovered %d tool call(s) the model wrote as text "
                        "(provider=%s model=%s)",
                        len(recovered), response.provider, response.model,
                    )
                    response.tool_use = recovered
                elif looks_like_a_tool_attempt(response.text, all_tools or {}):
                    # Nothing runnable — an invented tool, or a payload that
                    # will not parse. Serving this would describe an action
                    # that never happened, which is worse than saying so.
                    _log.warning(
                        "model attempted an unrunnable tool call "
                        "(provider=%s model=%s)", response.provider, response.model,
                    )
                    return self._unrunnable_tool_attempt(
                        response, all_tools or {}, scope=scope
                    )
            ...
            if not response.tool_use:
                ...
                self._emit_citations(scope=scope)
                self._emit_status(routing_tier=routing_tier)
    ...
    def _raw_turn(
        ...
        """
        # Checked HERE, at the capability, not at whichever transport asked.
        # An in-process caller is trusted today; the gate exists so that a new
        # way to reach this is not automatically a new way around governance.
        from axiom.infra.raw_mode import require_raw_allowed
    ...
    def _unrunnable_tool_attempt(
        ...
        *,
        ...
        """Say that the model tried to act and could not, naming what it tried.

        The alternative is printing its attempt, which reads as a result: the
        user takes away a tool name and arguments that were never executed and
        may not exist. Naming the real tools turns a dead end into the next
        step.
        """
        from axiom.extensions.builtins.chat.tool_call_recovery import _candidates

        attempted = sorted({name for name, _ in _candidates(response.text or "")})
        known = ", ".join(sorted(all_tools)[:12]) or "none are registered"
        message = (
            "I tried to look that up and could not: the model asked for "
            f"{', '.join(repr(a) for a in attempted) or 'a tool'}, which did not "
            "run. Nothing below was measured, so I am not going to report it as "
            f"though it was.\n\nTools actually available here: {known}."
        )
        if self._render:
            self._render.render_message("assistant", message)
        if scope is not None:
            scope.session.add_message("assistant", message)
        return message
        ...
        for tool_block in response.tool_use:
            ...
            if action.status == ActionStatus.PENDING:
                ...
                if choice == "A":
                    ...
                    # Hold: the only outcome that does not decide. The action
                    # goes to the durable queue and this turn ends without
                    # running it, so a person can answer from a terminal, a
                    # phone or a channel, minutes or hours later.
                    #
                    # scope.gate is process-local by design — it is this
                    # conversation's working set. Writing to the durable store
                    # as well is the whole point: an approval nobody can reach
                    # from another process is not deferred, it is lost.
                    ...
                    results.append(
                        (
                            ...
                            {
                                "error": (
                                    f"Held for human approval as {held}. "
                                    "Not run. Do not retry this call; say that "
                                    "it is waiting on a person and continue "
                                    "with anything that does not depend on it."
                                )
                                if held
                                else (
                                    "Could not be held for approval: the queue "
                                    "was unreachable. Nothing is waiting. Not run."
                                ),
                                "held_action_id": held,
                            },
                        )
            ...
            # Chat dispatches beneath invoke_capability (the turn loop needs the
            # gateway's full payload on its own per-agent bus), so it publishes
            # the shared content-free projection here instead. Without it a
            # capability used in a conversation never reaches the series that
            # CLI and MCP invocations land in, and chat — the surface this is
            # most worth proving out on — stays unmeasurable. Never raises.
            publish_capability_telemetry(
    ...
    def _emit_status(self, *, routing_tier: str | None = None) -> None:
        """Report the turn's model, tier, tokens and cost to the renderer.

        This lived in the terminal CLI and nowhere else, so a browser user
        could not see which model answered them, at which tier, or what it
        cost. Emitting from the agent gives every provider the same facts, the
        way ``_emit_citations`` does.

        Never raises: a status line is the least important thing in a turn and
        must not be able to cost the caller their answer.
        """
        try:
            if self._render is None:
                return
            usage = getattr(self, "usage", None)
            turns = getattr(usage, "turns", None) or []
            if not turns:
                return
            last = turns[-1]
            provider = getattr(getattr(self, "gateway", None), "active_provider", None)
            self._render.render_status(
                model=getattr(provider, "model", "") or "",
                tokens_in=getattr(last, "input_tokens", 0) or 0,
                tokens_out=getattr(last, "output_tokens", 0) or 0,
                cost=getattr(last, "cost", 0.0) or 0.0,
                tier=routing_tier,
            )
        except Exception:
            pass  # a status line must never break answering

    def _emit_citations(self, *, scope: ChatScope | None = None) -> None:
        """Hand this turn's retrieved chunks to the renderer, as sources.

        The retriever has always computed ``citation_key`` and the agent has
        always parked the chunks on ``scope.last_retrieved``; nothing consumed
        them, so no surface could say what grounded an answer. Every provider
        implements ``render_citations``, so this one call reaches the terminal,
        the TUI and the browser alike.

        An ungrounded turn emits nothing rather than an empty Sources heading,
        and a renderer that raises never costs the caller their answer — the
        same rule the provenance check and the audit follow.
        """
        try:
            scope = _active_scope(self, scope)
            retrieved = scope.last_retrieved or []
            if not retrieved or self._render is None:
                return
            self._render.render_citations(retrieved)
        except Exception:
            pass  # showing sources must never break answering
    ...
    def _rag_context(self, query: str, limit: int = 4, *, scope: ChatScope | None = None) -> str:
        ...
        try:
            ...
            # Enforce access at retrieval: a fail-closed context derived from the
            # session principal, instead of None (which skips the filter).
            principal = (
                getattr(getattr(scope, "session", None), "principal_id", "")
                or _local_principal()
            )
            retrieved = retrieve(
                store=store,
                ...
                limit=limit,
                access_context=access_context_for_principal(principal),
            )
    ...
    def _build_system_prompt(self, *, scope: ChatScope | None = None) -> str:
        ...
        # Layer 1 — identity ---------------------------------------------------
        base = _get_prompt_registry().resolve("agent_base")
        composer.add(
            "identity",
            name="agent_base",
        ...
        # The capability layer, generated from the live tool registry.
        #
        # What the model is told it can do has to come from what it can
        # actually do. The base preamble used to name a fixed set, so a
        # deployment that installed extensions got an assistant that denied
        # holding their tools — observed against a real node, which answered
        # that `telemetry_metrics` was "not available in the current
        # environment" while that tool was in the table and being offered.
        #
        # Building it here is what makes integration free for every extension,
        # including one a site ships that the platform has never heard of:
        # registering the tool is the whole of it.
        try:
            from axiom.extensions.builtins.chat.tool_summary import (
            ...
            composer.add(
                ...
                content=describe_available_tools(get_all_tools()),
                source="registry",
            )
        except Exception:  # noqa: BLE001 — orientation must never block a turn
            # WARNING, not debug. Losing this fragment is how the assistant
            # goes back to denying it holds tools it holds, and the first
            # version of this block named a layer that does not exist and said
            # nothing about it — the failure looked exactly like success.
            _log.warning(
                "the system prompt could not describe the registered tools; "
                "the assistant may deny holding tools it has",
                exc_info=True,
            )

        # Layer 2b — self-knowledge (the node knows itself, first person).
        # Composed fresh each turn; node_summary is cached (~5s) so this is
        # near-free — the founder's "wicked fast": common self-questions
        # (version, federation, site, what's installed) answer in ONE turn
        # with zero tool calls; node_describe is the drill-down.
        try:
            from axiom.extensions.builtins.status.selfknowledge import (
                self_prompt_fragment,
            )

            _selfk = self_prompt_fragment()
            composer.add(
                _selfk["layer"],
                name=_selfk["name"],
                content=_selfk["content"],
                source=_selfk["source"],
                required=_selfk["required"],
            )
        except Exception:  # noqa: BLE001 — a turn must compose even without it
            _log.warning(
                "the system prompt could not carry the node's self-knowledge; "
                "the assistant may speculate about its own configuration",
                exc_info=True,
            )

        # Layer 3 — grounding & honesty (Axiom default policy). Domain-agnostic:
        # prefer tools over recall, never fabricate, report tool failures. A
        # consumer layers more specific directives on top via domain_context
        # (e.g. "use these deterministic tools for a calculation").
        try:
            from axiom.extensions.builtins.chat.grounding_prompt import (
                grounding_policy_fragment,
            )

            _grounding = grounding_policy_fragment()
            composer.add(
                _grounding["layer"],
                name=_grounding["name"],
                content=_grounding["content"],
                source=_grounding["source"],
                required=_grounding["required"],
            )
        except Exception:  # noqa: BLE001 — a turn must compose even without it
            _log.warning("grounding policy fragment could not be added", exc_info=True)
        ...
        # Last in the cascade, because a local override is the most specific
        # thing in it: platform default, shipped example, extension
        # contribution, site config, then this. It wins over everything —
        # including an extension that believes it speaks for the whole system,
        # which is how a stale capability list went unchallenged for months.
        try:
            from axiom.infra.prompt_overrides import apply_overrides, load_overrides

            apply_overrides(composer, load_overrides())
        except Exception:  # noqa: BLE001 - an experiment must not break the chat
            _log.debug("prompt overrides not applied", exc_info=True)
