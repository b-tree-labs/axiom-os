...
def emit_invite_ready(
    ...
    sys.stdout.write(f"    {_brand_cli()} classroom join {encoded}\n")
    sys.stdout.flush()
    console.print()
...
def emit_need_coordinator_url(classroom_id: str) -> None:
    ...
    msg.append(
        f"  {_brand_cli()} classroom invite {classroom_id} "
...
def emit_serve_banner(
    ...
    out().print(
        ...
        f"[cyan]{_brand_cli()} classroom invite {classroom_id}[/]"
    )
    out().print()
...
def emit_status_empty() -> None:
    out().print(
        ...
        f"Create one with:[/] [cyan]{_brand_cli()} classroom invite <classroom_id> "
        "--coordinator-url URL[/]"
    )
