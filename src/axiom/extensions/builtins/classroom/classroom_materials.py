    ...
    def _add_bytes(
        ...
        *,
        ...
        if not content_path.exists():
            # Content-addressed + concurrency-safe: a UNIQUE temp per writer
            # (never a shared "<file_id>.tmp" — two workers racing on the SAME
            # content would collide on it, and the loser's os.replace would find
            # its temp already gone and raise FileNotFoundError). Atomically
            # renamed into place; if another writer won, the bytes are identical
            # (same hash) so the overwrite is harmless. Mirrors ``_save``.
            with tempfile.NamedTemporaryFile(
                dir=self._materials_dir,
                prefix=file_id + ".",
                suffix=".tmp",
                delete=False,
            ) as tf:
