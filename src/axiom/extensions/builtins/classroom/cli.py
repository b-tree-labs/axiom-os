...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if not existing_course:
        next_steps.append(
            f"{_brand_cli()} classroom prep corpus {classroom_id} --upload <file> --preview <query>"
        )
        next_steps.append(
            f"{_brand_cli()} classroom prep prompt {classroom_id} --set '...' --test '...'"
        )
    next_steps.extend([
        f"{_brand_cli()} classroom prep rag    {classroom_id} --mode course_only",
        f"{_brand_cli()} classroom prep lms    {classroom_id} --canvas-course <id> --fake",
        f"{_brand_cli()} classroom prep status {classroom_id}",
    ])
...
def _cmd_status(args: argparse.Namespace) -> int:
    ...
    if not args.classroom_id:
        print(f"usage: {_brand_cli()} classroom prep status <classroom_id>", file=sys.stderr)
        return 1
    ...
    if loaded is None:
        print(
            ...
            f"Run `{_brand_cli()} classroom prep init` first."
        )
        return 1
...
def _cmd_prompt(args: argparse.Namespace) -> int:
    ...
    if args.set is not None and args.test is not None:
        ...
        if not args.approve:
            ...
            print(f"  {_brand_cli()} classroom prep prompt {args.classroom_id} --approve")
...
def _print_classroom_state(
    ...
    ui.emit_checklist(
        ...
        blockers=blockers,
        next_command=(
            f"{_brand_cli()} classroom publish {classroom_id} --approver <you>"
            if ready else None
        ),
    )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_status.add_argument(
        ...
        help=f"One-line tmux status-right output (for #({_brand_cli()} classroom "
    ...
    p_me.add_argument(
        ...
        help="Retract a specific interaction so it stops surfacing in "
             ...
             f"`{_brand_cli()} classroom me <CID> --memory`.",
    )
...
def _cmd_brief(args: argparse.Namespace) -> int:
    ...
    if total_questions == 0:
        ui.out().print(
            ...
            f"asked, or they're running `{_brand_cli()} classroom ask --cite-only`.[/]"
        )
    else:
        ui.out().print(
    ...
    if not coordinator_url:
        ...
        if args.json:
            ...
            if result.valid:
                ...
                print(f"  {_brand_cli()} classroom join {args.invite[:20]}... --coordinator <URL>")
            else:
...
def _require_active(classroom_id: str) -> bool:
    ...
    if is_archived(classroom_id):
        ui.emit_error(
            ...
            f"{_brand_cli()} classroom prep from-demo --from {classroom_id}"
        )
        return False
    return True
...
def _cmd_invite(args: argparse.Namespace) -> int:
    ...
    if count == 1:
        ...
        for i, encoded in enumerate(encoded_list, start=1):
            ui.out().print(f"[dim]{i:>3}.[/]  {_brand_cli()} classroom join {encoded}")
        ui.out().print()
    return 0
    ...
    if not class_dir.is_dir():
        if args.json:
            ...
            ui.emit_error(
                ...
                f"  {_brand_cli()} classroom join <invite-from-your-instructor>"
            )
        return 1
...
def _cmd_evals(args: argparse.Namespace) -> int:
    ...
    if student_index_db.is_file():
        ...
        ui.emit_error(
            ...
            f"  Instructor: upload via  [cyan]{_brand_cli()} classroom prep corpus[/] first\n"
            f"  Student:   join via    [cyan]{_brand_cli()} classroom join <invite>[/] first"
        )
        return 1
    ...
    if report.failed:
        ui.out().print(
            ...
            f"[/][cyan]{_brand_cli()} classroom prep corpus[/]"
        )
...
def _cmd_briefs_generate(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        ui.emit_error(
            ...
            f"Run: {_brand_cli()} classroom invite {args.classroom_id} --coordinator-url URL"
        )
        return 1
    ...
    ui.emit_next_steps([
        f"{_brand_cli()} classroom briefs list {args.classroom_id}",
        f"{_brand_cli()} classroom briefs review {args.classroom_id} <student_id> --approve",
...
def _cmd_briefs_list(args: argparse.Namespace) -> int:
    ...
    if not rows:
        ui.emit_info(
            ...
            f"Run: {_brand_cli()} classroom briefs generate {args.classroom_id}"
        )
        return 0
    ui.emit_table(
...
def _cmd_briefs_review(args: argparse.Namespace) -> int:
    ...
    if brief is None:
        ui.emit_error(
            ...
            f"Generate first: {_brand_cli()} classroom briefs generate {args.classroom_id}"
        )
        return 1
    ...
    if not (args.approve or args.note):
        ui.out().print()
        ui.emit_next_steps([
            f"{_brand_cli()} classroom briefs review {args.classroom_id} {args.student_id} --approve",
            f"{_brand_cli()} classroom briefs review {args.classroom_id} {args.student_id} --note 'well done'",
        ], title="Approve or add a note")
    elif args.approve:
        ui.out().print()
        ui.emit_success(f"Approved. The student will see this on `{_brand_cli()} classroom me`.")
    return 0
...
def _show_memory_view(args: argparse.Namespace, class_dir: Path) -> int:
    ...
    if recents:
        ...
        ui.out().print(
            f"[dim]  {_brand_cli()} classroom me {args.classroom_id} "
            "--forget <id>[/]"
        )
    ...
    if data.get("question_count", 0) == 0:
        ...
        ui.emit_info(
            ...
            f"send via `{_brand_cli()} classroom ask`."
        )
    return 0
    ...
    try:
        ...
        if exc.code == 404:
            ui.emit_error(
                ...
                f"Run `{_brand_cli()} classroom me {args.classroom_id} --memory` "
...
def _cmd_quiz_broadcast(args: argparse.Namespace) -> int:
    ...
    if not pool:
        ui.emit_error(
            ...
            f"'{args.bank_preset}'. "
            f"Try `{_brand_cli()} classroom quiz broadcast {args.classroom_id} "
    ...
    ui.emit_info(
        f"Students will see it on their next `{_brand_cli()} classroom quiz pending`."
    )
    return 0
...
def _cmd_quiz_pending(args: argparse.Namespace) -> int:
    ...
    if role != "student":
        ui.emit_error(
            ...
            f"{_brand_cli()} classroom join <invite>"
        )
        return 1
    ...
    ui.out().print()
    ui.emit_next_steps([
        f"{_brand_cli()} classroom quiz take {args.classroom_id} <quiz_id>",
    ], title="To take one")
    return 0
...
def _cmd_ask_instructor(args: argparse.Namespace) -> int:
    ...
    if role != "student":
        ui.emit_error(
            ...
            f"Run: {_brand_cli()} classroom join <invite-from-your-instructor>"
        )
        return 1
    ...
    if not student_id or not base_url:
        ui.emit_error(
            ...
            f"Try rejoining: {_brand_cli()} classroom join <invite>"
        )
        return 1
    ...
    ui.emit_next_steps([
        f"{_brand_cli()} classroom threads {args.classroom_id}",
        f"{_brand_cli()} classroom reply {args.classroom_id} {data['thread_id']} \"<your follow-up>\"",
    ])
    return 0
...
def _cmd_ask_student(args: argparse.Namespace) -> int:
    ...
    ui.emit_info(
        ...
        f"`{_brand_cli()} classroom threads`."
    )
    return 0
...
def _cmd_threads(args: argparse.Namespace) -> int:
    ...
    ui.emit_error(
        ...
        f"Join with: {_brand_cli()} classroom join <invite>"
    )
    return 1
...
def _threads_student_view(args: argparse.Namespace) -> int:
    ...
    if not threads:
        ui.emit_info(
            ...
            f"`{_brand_cli()} classroom ask-instructor {args.classroom_id} \"<question>\"`."
        )
        return 0
    rows = [
        [
        ...
        if force is not None:
            ...
            if force not in policy.allowed_modes:
                ui.emit_error(
                    ...
                    "set. Add it first: "
                    f"{_brand_cli()} classroom modes {args.classroom_id} --allow {force},..."
                )
                return 1
        policy = ClassroomModePolicy(
...
def _cmd_publish(args: argparse.Namespace) -> int:
    ...
    if result.get("published"):
        ...
        ui.emit_next_steps([
            f"{_brand_cli()} classroom enroll {args.classroom_id} --instructor <you>",
        ])
        return 0
    ...
    ui.emit_blocked(
        ...
        suggestion=f"{_brand_cli()} classroom prep status {args.classroom_id}",
    )
    return 1
...
def _cmd_checkpoints_skip_defaults(args: argparse.Namespace) -> int:
    ...
    ui.emit_next_steps([
        f"{_brand_cli()} classroom prep checkpoints add {args.course_id} <checkpoint_id>",
...
def _emit_orientation() -> None:
    ...
    if not instructor_classes and not student_classes:
        ui.out().print(f"[bold]Welcome to {_brand_cli()} classroom.[/]")
        ui.out().print()
        ui.out().print("First time? Try the demo to see a working classroom in 60s:")
        ui.out().print(f"  [cyan]{_brand_cli()} classroom demo[/]")
        ui.out().print()
        ui.out().print("Or jump straight in:")
        ui.out().print(f"  [cyan]{_brand_cli()} classroom prep init --title \"...\" --instructor <you>[/]   (instructor)")
        ui.out().print(f"  [cyan]{_brand_cli()} classroom join <invite-from-instructor>[/]                          (student)")
        ui.out().print()
        ui.out().print(f"[dim]Full command list: {_brand_cli()} classroom --help[/]")
        return
    ...
    if instructor_classes:
        ...
        ui.out().print(f"  [cyan]{_brand_cli()} classroom status[/]                                   (one-line dashboard)")
        ui.out().print(f"  [cyan]{_brand_cli()} classroom brief {first} --instructor <you>[/]")
        ui.out().print(f"  [cyan]{_brand_cli()} classroom doctor {first}[/]                            (diagnose)")
        ui.out().print()
    ...
    if student_classes:
        ...
        ui.out().print(f"  [cyan]{_brand_cli()} classroom ask {first} \"<question>\"[/]")
        ui.out().print(f"  [cyan]{_brand_cli()} classroom me {first} --memory[/]                       (what's logged)")
        ui.out().print(f"  [cyan]{_brand_cli()} classroom doctor {first}[/]                            (diagnose)")
        ui.out().print()
