...
# ---------------------------------------------------------------------------
# Concurrent writes (regression: shared temp filename raced under xdist)
# ---------------------------------------------------------------------------


class TestConcurrentWrites:
    def test_concurrent_identical_uploads_do_not_race(self, tmp_path):
        """Many workers seeding the SAME content into the SAME store must not
        crash. Regression: `_add_bytes` wrote to a shared ``<file_id>.tmp``, so
        two workers racing on identical content collided — the loser's
        ``os.replace`` found its temp already gone and raised
        ``FileNotFoundError`` (seen intermittently in CI under ``-n auto``)."""
        import threading

        content = b"identical bytes uploaded by many workers at once\n"
        base = tmp_path / "shared"
        # separate store instances on ONE base_dir = separate workers/processes
        stores = [ClassroomMaterialsStore(base) for _ in range(12)]
        errors: list[Exception] = []
        ready = threading.Barrier(len(stores))

        def upload(store, i):
            try:
                ready.wait()  # release all writers at once to force the overlap
                store.add_bytes(content, filename=f"note-{i}.txt")
            except Exception as exc:  # noqa: BLE001 — the invariant IS "no raise"
                errors.append(exc)

        threads = [threading.Thread(target=upload, args=(s, i))
        ...
        assert not errors, f"concurrent identical upload raced: {errors[:2]}"
        # content-addressed: the bytes landed once, readable and correct
        fid = compute_file_id(content)
        assert (base / "materials" / fid).read_bytes() == content
        # and no orphaned temp files were left behind
        assert not list((base / "materials").glob("*.tmp"))
