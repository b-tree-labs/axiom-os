...
def prague_cohort(tmp_path):
    ...
    # Threaded, with daemon handler threads. A plain HTTPServer serves one
    # request at a time, so a single handler still in flight when the fixture
    # tears down leaves serve_forever unable to return to its poll loop and
    # server.shutdown() waits forever. Twelve students hitting one coordinator
    # is exactly the shape that produces an in-flight request at teardown.
