...
def run_diagnostics(classroom_id: str) -> DiagnosticReport:
    ...
    if role == "unknown":
        report.checks.append(CheckResult(
            ...
            hint=(
                f"If you're the instructor: `{_brand_cli()} classroom prep init "
                "--title \"...\" --instructor <you>`. "
                f"If you're a student: `{_brand_cli()} classroom join <invite>`."
            ),
        ))
        return report
...
def _check_identity(report: DiagnosticReport) -> None:
    ...
    if identity is None:
        report.checks.append(CheckResult(
            ...
            hint=(
                f"First `{_brand_cli()} classroom invite` or `{_brand_cli()} classroom join` "
...
def _check_classroom_artifact(
    ...
    if data is None:
        report.checks.append(CheckResult(
            ...
            hint=f"`{_brand_cli()} classroom prep init --title \"...\" --instructor <you>` to recreate.",
        ))
        return
    ...
    if state == "archived":
        report.checks.append(CheckResult(
            ...
            hint=f"`{_brand_cli()} classroom prep from-demo --from {classroom_id}`",
        ))
        return
    report.checks.append(CheckResult(
...
def _check_course_artifact(
    ...
    if not course_id:
        report.checks.append(CheckResult(
            ...
            hint=f"`{_brand_cli()} classroom prep status {classroom_id}` (self-heals course_selected)",
        ))
        return
...
def _check_cohort_registry(
    ...
    if not store.exists(classroom_id):
        report.checks.append(CheckResult(
            ...
            hint=f"`{_brand_cli()} classroom invite {classroom_id} --coordinator-url <url>` to mint the first invite (auto-creates the cohort).",
        ))
        return
    try:
    ...
    if not url:
        report.checks.append(CheckResult(
            ...
            status="warn",
            detail=detail,
            hint=f"Set the coordinator URL: `{_brand_cli()} classroom invite {classroom_id} --coordinator-url <url>`",
        ))
        return
    report.checks.append(CheckResult(
...
def _check_coordinator_materials(
    ...
    if not coord_dir.is_dir():
        report.checks.append(CheckResult(
            ...
            hint=f"`{_brand_cli()} classroom prep corpus {classroom_id} --upload <file>` to seed.",
        ))
        return
    ...
    if not entries:
        report.checks.append(CheckResult(
            ...
            hint=f"`{_brand_cli()} classroom prep corpus {classroom_id} --upload <file>`",
        ))
        return
    report.checks.append(CheckResult(
        name="coordinator_materials",
...
def _check_classroom_state(
    ...
    if course_ready and class_ready:
        ...
        report.checks.append(CheckResult(
            ...
            hint=(
                "" if published
                else f"`{_brand_cli()} classroom publish {classroom_id} --approver <you>`"
            ),
        ))
        return
    ...
    report.checks.append(CheckResult(
        ...
        hint=f"`{_brand_cli()} classroom prep status {classroom_id}` for the full checklist.",
    ))
...
def _check_membership_manifest(
    ...
    try:
        ...
        report.checks.append(CheckResult(
            ...
            hint=f"`{_brand_cli()} classroom join <invite-from-your-instructor>`",
        ))
        return
    report.checks.append(CheckResult(
        name="membership",
...
def _check_coordinator_reachable(
    ...
    try:
        ...
        report.checks.append(CheckResult(
            ...
            hint=(
                f"If the instructor isn't running `{_brand_cli()} classroom serve`, "
