...
def import_bundle(
    ...
    # Harness session-store members (bundle v3, ADR-098 D1). Validated
    # entirely in phase 1: inner tars must contain only regular files on
    # relative, home-confined paths — anything else (traversal, absolute
    # paths, links, devices) refuses the whole bundle before any write.
    home = Path(params.get("home") or Path.home())
    store_files: dict[str, list[tuple[str, bytes]]] = {}
    for name in members:
        if not (name.startswith("harness-store-") and name.endswith(".tar")):
            continue
        harness = name[len("harness-store-"):-len(".tar")]
        try:
            inner = tarfile.open(fileobj=io.BytesIO(members[name]))
            entries = inner.getmembers()
        except tarfile.TarError as exc:
            return deny(
                "store_invalid",
                f"harness store member {name!r} is not a valid tar: {exc}",
            )
        ...
        for entry in entries:
            ...
            if unsafe:
                return deny(
                    "store_unsafe_path",
                    f"harness store member {name!r} contains an unsafe "
                    f"entry {rel!r} (traversal/link/device) — refusing "
                    "the entire bundle",
                )
            files.append((rel, inner.extractfile(entry).read()))
    ...
    if dry_run:
        return SkillResult(
            ...
            value={
                ...
                "harness_stores": {
                    h: {"files": len(files)}
                    for h, files in store_files.items()
                },
    ...
    # Restore harness session stores into the destination home. Existing
    # files are never overwritten — a machine's own session state always
    # wins over an incoming replica (kept-both doctrine: the bundle still
    # holds the other copy).
    store_report: dict[str, dict[str, int]] = {}
    for harness, files in store_files.items():
        restored = 0
        skipped_existing = 0
        for rel, blob in files:
            ...
            restored += 1
        store_report[harness] = {
            "restored": restored, "skipped_existing": skipped_existing,
        }
