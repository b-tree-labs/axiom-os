...
def _coerce_int(value, name: str) -> tuple[int, dict | None]:
    """Coerce an MCP-transported integer argument.

    MCP clients see loose schemas and routinely send ``"50"`` for ``50``; a
    raw string reaching a slice used to surface as ``TypeError: slice
    indices must be integers`` deep in the read path. Returns
    ``(value, None)`` on success or ``(0, error_dict)`` on garbage.
    """
    try:
        return int(value), None
    except (TypeError, ValueError):
        return 0, {"error": f"argument {name!r} must be an integer"}
...
def show(*, principal_id: str | None = None, limit: int = 10) -> dict:
    ...
    limit, err = _coerce_int(limit, "limit")
    if err:
        return err
...
def search(
    *,
    ...
    """Filter a principal's fragments by tool / fact_kind / substring / time.

    The WHOLE ledger is scanned and ``limit`` caps the *matches* returned
    (newest first). Filtering used to happen after a newest-``limit`` scan
    window, so a ``tool=`` filter silently missed every fragment older than
    the newest 50 rows.
    ...
    from axiom.memory.session_capture import resolve_principal_id
    from axiom.memory.session_summary import list_fragments_by_principal
    from axiom.policy.action_ledger import parse_when
    ...
    try:
        ...
        return {"error": f"argument 'since': {exc}"}
    try:
        until_dt = parse_when(until)
    except ValueError as exc:
        return {"error": f"argument 'until': {exc}"}

    resolved = resolve_principal_id(principal_id)
    composition = _build_default_composition()
    fragments = list_fragments_by_principal(
        composition, resolved, limit=None,
    )

    def _frag_dt(f) -> datetime | None:
        raw = str(f.provenance.timestamp or "")
        try:
    ...
    def _matches(f) -> bool:
        ...
        if since_dt or until_dt:
            ts = _frag_dt(f)
            if ts is None:
                return False  # can't prove it's inside the window — exclude
    ...
    matched = [f for f in fragments if _matches(f)][:limit]
    return {
        "principal": resolved,
...
def secrets_rotate_trigger(
    *,
    ...
    result = invoke_capability(ctx.registry, "secrets.rotate", params, ctx, surface=MCP_SURFACE)
    return {
        "ok": result.ok,
        "value": result.value,
...
_TOOLS: list[Tool] = [
    ...
    Tool(
        ...
        description=(
            ...
            "substring match against turn text, or a since/until time "
            "window. Scans the whole ledger; limit caps the matches."
        ),
        input_schema={
            "type": "object",
            "properties": {
                ...
                "since": {
                    "type": "string",
                    "description": "Window floor: shorthand (Nm/Nh/Nd/Nw, "
                                   "e.g. '30d') or ISO-8601.",
                },
                "until": {
                    "type": "string",
                ...
                "limit": {
                    "type": "integer",
                    "description": "Maximum matching fragments to return, "
                                   "newest first (default 50). The whole "
                                   "ledger is scanned before this cap.",
                },
            },
        },
    ),
    ...
    Tool(
        ...
        description=(
            ...
            f"MCP: run `{_brand_cli()} secrets rotate <name>` in a terminal instead."
        ),
        input_schema={
            "type": "object",
            "properties": {
...
def _call_kw(fn, args: dict[str, Any]) -> Any:
    """Invoke ``fn`` with the subset of ``args`` it actually accepts.

    MCP clients routinely send extra fields (a guessed parameter, a
    harness-injected key). ``fn(**args)`` turns every such field into a
    raw ``TypeError``; dropping unknown keys keeps a read from failing
    over a field the tool never declared. Declared-but-invalid values
    still error cleanly inside the tool functions themselves.
    """
...
# ---------------------------------------------------------------------------
# Aggregated-surface adapters
# ---------------------------------------------------------------------------
#
# The root MCP server calls a tool handler with a single ``args`` dict; the
# functions above take keyword arguments. These adapters are the join, and they
# reuse ``_call_kw`` so an unknown field from a harness is dropped rather than
# raising — a read should not fail over a key the tool never declared.
#
# Credential tools have no adapter on purpose. They stay reachable through the
# CLI and this standalone server, but do not enter the surface every harness
# registers by default: making secrets tooling opt-out rather than opt-in is an
# escalation nobody asks for when the reason to connect is cross-tool memory.


def tool_memory_append(args: dict[str, Any]) -> Any:
    """Write a memory fragment. Aggregated-surface entry point."""
    return _call_kw(append, args or {})


def tool_memory_show(args: dict[str, Any]) -> Any:
    """Show one memory fragment. Aggregated-surface entry point."""
    return _call_kw(show, args or {})


def tool_memory_recent(args: dict[str, Any]) -> Any:
    """Most recent memory fragments. Aggregated-surface entry point."""
    return _call_kw(recent, args or {})


def tool_memory_search(args: dict[str, Any]) -> Any:
    """Search memory fragments. Aggregated-surface entry point."""
    return _call_kw(search, args or {})


def tool_memory_recall(args: dict[str, Any]) -> Any:
    """Recall over the memory corpus. Aggregated-surface entry point."""
    return _call_kw(recall, args or {})


def tool_actions_recent(args: dict[str, Any]) -> Any:
    """Recent entries from the action ledger. Aggregated-surface entry point."""
    return _call_kw(actions_recent, args or {})


def tool_actions_search(args: dict[str, Any]) -> Any:
    """Filtered search over the action ledger. Aggregated-surface entry point."""
    return _call_kw(actions_search, args or {})


_HANDLERS: dict = {
    "axiom_memory_append": lambda args: _call_kw(append, args),
    "axiom_memory_show": lambda args: _call_kw(show, args),
    "axiom_memory_recent": lambda args: _call_kw(recent, args),
    "axiom_memory_search": lambda args: _call_kw(search, args),
    "axiom_memory_recall": lambda args: _call_kw(recall, args),
    "axiom_actions_recent": lambda args: _call_kw(actions_recent, args),
    "axiom_actions_search": lambda args: _call_kw(actions_search, args),
    "axiom_secrets_list": lambda args: _call_kw(secrets_list, args),
    "axiom_vault_audit": lambda args: _call_kw(vault_audit, args),
    "axiom_secrets_rotate_trigger": lambda args: _call_kw(secrets_rotate_trigger, args),
}
