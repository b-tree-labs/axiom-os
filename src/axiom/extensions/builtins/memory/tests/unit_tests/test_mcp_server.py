...
def test_append_tool_input_schema_requires_tool_only():
    ...
    schema = append_tool.input_schema
...
# ---------------------------------------------------------------------------
# Read-path regressions (2026-09-02): filters must see the whole ledger, and
# transported arguments must coerce or error cleanly — never raise raw.
# ---------------------------------------------------------------------------


def _append_turn(tool: str, i: int) -> None:
    from axiom.extensions.builtins.memory import mcp_server
...
def test_search_tool_filter_reaches_past_default_scan_window(patched_composition):
    """A tool filter must match fragments older than the newest 50.

    Regression: search() used to fetch the newest ``limit`` fragments and
    THEN filter, so `tool="chatgpt"` returned [] whenever the 50 newest
    rows were all from another tool — even with chatgpt rows in the ledger.
    """
    from axiom.extensions.builtins.memory import mcp_server

    _append_turn("chatgpt", 0)          # oldest row in the ledger
    for i in range(55):
        _append_turn("claude-code", i + 1)

    result = mcp_server.search(principal_id="user@example.org", tool="chatgpt")
    assert [f["tool"] for f in result["fragments"]] == ["chatgpt"]


def test_search_limit_caps_matches_not_the_scan(patched_composition):
    from axiom.extensions.builtins.memory import mcp_server

    for i in range(3):
        _append_turn("chatgpt", i)
        for j in range(20):
            _append_turn("claude-code", 100 + i * 20 + j)

    result = mcp_server.search(
        principal_id="user@example.org", tool="chatgpt", limit=2,
    )
    assert len(result["fragments"]) == 2
    assert all(f["tool"] == "chatgpt" for f in result["fragments"])


def test_search_since_until_filter_and_validation(patched_composition):
    from axiom.extensions.builtins.memory import mcp_server

    _append_turn("claude-code", 0)

    hits = mcp_server.search(principal_id="user@example.org", since="1h")
    assert len(hits["fragments"]) == 1

    future = mcp_server.search(
        principal_id="user@example.org", since="2099-01-01T00:00:00+00:00",
    )
    assert future["fragments"] == []

    past = mcp_server.search(
        principal_id="user@example.org", until="2000-01-01T00:00:00+00:00",
    )
    assert past["fragments"] == []

    bad = mcp_server.search(principal_id="user@example.org", since="not-a-time")
    assert "error" in bad and "since" in bad["error"]


def test_read_tools_coerce_integer_arguments(patched_composition):
    """MCP transports untyped args; '2' must behave as 2 and garbage must
    come back as a clean error dict, never a raw TypeError from a slice."""
    from axiom.extensions.builtins.memory import mcp_server

    for i in range(3):
        _append_turn("claude-code", i)

    assert len(mcp_server.recent(principal_id="user@example.org", n="2")["fragments"]) == 2
    assert len(mcp_server.search(principal_id="user@example.org", limit="1")["fragments"]) == 1
    assert len(mcp_server.show(principal_id="user@example.org", limit="2")["fragments"]) == 2

    assert mcp_server.recent(principal_id="user@example.org", n="lots") == {
        "error": "argument 'n' must be an integer",
    }
    assert mcp_server.search(principal_id="user@example.org", limit="many") == {
        "error": "argument 'limit' must be an integer",
    }
    assert mcp_server.show(principal_id="user@example.org", limit="some") == {
        "error": "argument 'limit' must be an integer",
    }


def test_handlers_drop_unknown_arguments_instead_of_raising(patched_composition):
    from axiom.extensions.builtins.memory import mcp_server

    _append_turn("claude-code", 0)
    result = mcp_server._HANDLERS["axiom_memory_search"]({
        "principal_id": "user@example.org",
        "bogus_field": {"x": 1},
    })
...
def test_search_tool_schema_declares_time_window():
    from axiom.extensions.builtins.memory import mcp_server

    search_tool = next(
        t for t in mcp_server._TOOLS if t.name == "axiom_memory_search"
    )
