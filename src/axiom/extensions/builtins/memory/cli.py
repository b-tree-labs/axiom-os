...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_record = sub.add_parser(
        ...
        description=(
            ...
            f"path the MCP write tool and {_brand_cli()} chat use. Provide values "
    ...
    p_sweep = sub.add_parser(
        "capture-sweep",
        help="Sweep every harness on disk into the ledger (cross-tool capture).",
        description=(
            "One pass over Claude Code, Codex and Cursor transcripts, folding "
            "new conversation turns into the principal's ledger. Reports "
            "FRAGMENTS WRITTEN per tool — not files touched — and fails when "
            "the ledger's newest captured turn is older than the staleness "
            "threshold, which is the signal a watchdog should alert on."
        ),
    )
    p_sweep.add_argument(
        "--principal", default=None,
        help="Ledger owner (defaults to memory.default_principal).",
    )
    p_sweep.add_argument(
        "--claude-projects", default=None,
        help="Claude Code project root (default: ~/.claude/projects).",
    )
    p_sweep.add_argument(
        "--codex-sessions", default=None,
        help="Codex rollout root (default: ~/.codex/sessions).",
    )
    p_sweep.add_argument(
        "--cursor-path", action="append", default=None, dest="cursor_paths",
        help=(
            "Cursor state.vscdb to sweep (repeatable). Default: the "
            "per-platform Cursor stores that exist on this machine."
        ),
    )
    p_sweep.add_argument(
        "--lookback-days", type=int, default=None,
        help="Only sweep transcripts modified within N days (default 2).",
    )
    p_sweep.add_argument(
        "--max-ledger-age-hours", type=float, default=None,
        help="Staleness threshold before the sweep reports failure (default 24).",
    )
    p_sweep.add_argument(
        "--no-salience", action="store_true",
        help="Disable the salience gate and capture every turn verbatim.",
    )
    p_sweep.add_argument("--json", action="store_true", help="Emit JSON.")

    # ----- hook (at-turn capture) ---------------------------------------------
    p_hook = sub.add_parser(
        "hook",
        help="Capture the transcript named in a harness hook payload (at-turn).",
        description=(
            "Invoked by the harness on its own events (Claude Code Stop / "
            "SessionEnd), reading the hook payload and folding that "
            "session's new turns into the ledger. Runs inside the user's "
            "conversation, so it ALWAYS exits 0 and never raises: a memory "
            "backstop must not be able to break the session it observes."
        ),
    )
    p_hook.add_argument(
        "--payload-file", default=None,
        help="File holding the hook payload JSON (default: read stdin).",
    )
    p_hook.add_argument(
        "--unlink-payload", action="store_true",
        help="Delete the payload file after reading (temp-file callers).",
    )
    p_hook.add_argument(
        "--principal", default=None,
        help="Ledger owner (defaults to memory.default_principal).",
    )
    p_hook.add_argument("--json", action="store_true", help="Emit JSON.")

    # ----- hook-install / hook-uninstall ---------------------------------------
    p_hook_install = sub.add_parser(
        "hook-install",
        help="Register at-turn capture in the harness settings.",
        description=(
            "Adds Stop and SessionEnd hooks to ~/.claude/settings.json so "
            "capture happens when a turn completes rather than when a timer "
            "fires. Backs the file up first, refuses to rewrite settings it "
            "cannot parse, and preserves hooks installed by anything else."
        ),
    )
    ...
    p_hook_install.add_argument(
        "--axi", default=None,
        help="axi binary the hook should invoke (default: the pinned service venv).",
    )
    p_hook_install.add_argument("--json", action="store_true", help="Emit JSON.")

    p_hook_uninstall = sub.add_parser(
        "hook-uninstall",
        help="Remove at-turn capture hooks from the harness settings.",
    )
    p_hook_uninstall.add_argument(
        "--settings", default=None,
        help="Settings file to edit (default: ~/.claude/settings.json).",
    )
    p_hook_uninstall.add_argument("--json", action="store_true", help="Emit JSON.")

    # ----- backfill-history ---------------------------------------------------
    p_hist = sub.add_parser(
        "backfill-history",
        help="Recover prompts from ~/.claude/history.jsonl (survives transcript deletion).",
        description=(
            "Claude Code deletes session transcripts on a rolling "
            "cleanupPeriodDays window, but history.jsonl keeps every prompt "
            "the user typed. This recovers those prompts as PROMPT-ONLY "
            "turns (marked extra.capture_mode=prompt_only) for the periods "
            "whose transcripts are gone. Assistant responses from those "
            "sessions are unrecoverable. Prompts whose full exchange is "
            "already in the ledger are skipped, not duplicated. This is a "
            "backfill, not part of the recurring sweep."
        ),
    )
    p_hist.add_argument(
        "--principal", default=None,
    ...
    p_hist.add_argument(
        "--path", default=None,
        help="History file (default: ~/.claude/history.jsonl).",
    )
    p_hist.add_argument(
        "--reset", action="store_true",
        help=(
            "Clear previously backfilled prompt-only fragments first, so an "
            "improved read of the same history source replaces them instead "
            "of being skipped as already-captured."
        ),
    )
    p_hist.add_argument(
    ...
    p_hist.add_argument(
        "--no-salience", action="store_true",
        help="Disable the salience gate and recover every prompt.",
    )
    ...
    p_repair = sub.add_parser(
        "repair-hollow",
        help="Purge content-free chat_turn fragments so they can be recaptured.",
        description=(
            "Removes chat_turn fragments that carry neither user_input nor "
            "assistant_output. These are residue of the pair-wise transcript "
            "parser, and because ingest dedupes on source_uuid each one "
            "permanently blocked its turn from ever being captured. Purging "
            "them frees those uuids; re-run capture-sweep afterwards to "
            "recover the real conversation."
        ),
    )
    ...
    p_repair.add_argument(
        ...
        help=(
            "Also purge turns captured mid-answer (prompt present, "
            "assistant side empty), but only where the session transcript "
            "still exists so a re-sweep can recapture them whole."
        ),
    )
    p_repair.add_argument(
        "--dry-run", action="store_true",
    ...
    p_export.add_argument(
        "--harness-store", action="append", dest="harness_stores",
        default=None,
        help=(
            "Also carry a harness's native session store (transcripts, "
            "metadata, checkpoints) — repeatable; e.g. claude-code, codex. "
            "Fail-closed secret scan runs on every store file."
        ),
    )
    p_export.add_argument(
        "--store-exclude", action="append", dest="store_excludes",
        default=None,
        help=(
            "Home-relative glob to leave out of harness stores "
            "(repeatable) — the escape hatch when the secret scan refuses."
        ),
    )
    p_export.add_argument(
        "--home", default=None,
        help="Override the harness home directory (default: $HOME).",
    )
    p_export.add_argument(
        "--skip-fragment", action="append", dest="skip_fragments",
        default=None, metavar="ID",
        help=(
            "Leave a credential-flagged fragment out of the bundle "
            "(repeatable). Prefer vaulting or forgetting it; skips are "
    ...
    p_import.add_argument(
        "bundle", help=f"Path to a bundle produced by `{_brand_cli()} memory export`.",
    )
    p_import.add_argument(
    ...
    p_import.add_argument(
        "--home", default=None,
        help=(
            "Where harness session stores are restored (default: $HOME). "
    ...
    p_ingest = sub.add_parser(
        ...
        description=(
            ...
            f"via the same common path `{_brand_cli()} memory record` and the MCP "
    ...
    p_heartbeat = sub.add_parser(
        ...
        description=(
            ...
            f"{_brand_cli()} dr's heartbeat-freshness check uses the most-recent "
    ...
    p_hb_install = sub.add_parser(
        "heartbeat-install",
        help=f"Install the macOS launchd plist that runs `{_brand_cli()} memory heartbeat` hourly.",
        description=(
    ...
    p_reindex = sub.add_parser(
        ...
        description=(
            ...
            f"in the ledger but were never projected, so `{_brand_cli()} memory recall` / "
    ...
    p_port = sub.add_parser(
        "port",
        help="One-command onboarding: register MCP + absorb harness memory (+ optional bundle).",
        description=(
            "Compose the porting primitives into the single verb a new "
            "(or moved) node runs once: register the MCP server into every "
            "detected client, absorb each detected harness's native memory, "
            "and optionally import a signed export bundle. Every step is "
            "idempotent — safe to re-run. --refresh additionally rebuilds "
            "the recall corpus to repair index drift."
        ),
    )
    p_port.add_argument(
        "--principal", default=None,
        help=(
            "Owner of the ported memories. Falls back to the "
            "memory.default_principal setting when omitted."
        ),
    )
    p_port.add_argument(
    ...
    p_port.add_argument(
        "--harness", action="append", dest="harnesses", default=None,
        help=(
            "Explicit harness(es) to absorb (repeatable). Default: detect "
            "by store presence (claude-code, codex, gemini-cli, hermes)."
        ),
    )
    p_port.add_argument(
        "--home", default=None,
        help="Override the harness home directory used for detection + absorb.",
    )
    p_port.add_argument(
        "--root", action="append", dest="roots", default=None,
        help="Project root for hierarchy walks (repeatable).",
    )
    p_port.add_argument(
        "--bundle", default=None,
        help="Signed export bundle to import, assumed as --principal.",
    )
    p_port.add_argument(
        "--sessions-dir", dest="sessions_dir", default=None,
        help="Where bundle session checkpoints are restored (default: user state dir).",
    )
    p_port.add_argument(
        "--refresh", action="store_true", default=False,
        help="Force a recall-corpus reindex after absorb/import (drift repair).",
    )
    p_port.add_argument(
        "--dry-run", action="store_true", dest="dry_run", default=False,
        help="Report what every step would do; write nothing.",
    )
    p_port.add_argument("--json", action="store_true", default=False)
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"{_brand_cli()} memory show: {exc}", file=sys.stderr)
        return 1
...
def _cmd_capture_sweep(args: argparse.Namespace) -> int:
    """Thin ADR-056 wrapper: build the composition, invoke the sweep skill."""
    from axiom.memory.session_capture import resolve_principal_id

    from .skills.capture_sweep import capture_sweep

    try:
        principal = resolve_principal_id(args.principal)
    except ValueError as exc:
        print(f"{_brand_cli()} memory capture-sweep: {exc}", file=sys.stderr)
        return 1
    ...
    result = capture_sweep({
        ...
        "cursor_paths": args.cursor_paths,
        "lookback_days": args.lookback_days,
        "max_ledger_age_hours": args.max_ledger_age_hours,
        "salience_gate": not args.no_salience,
    }, None)
    ...
    v = result.value or {}
    print(_h1("Memory capture sweep"))
    print(f"- Principal: {v.get('principal', '')}")
    print(f"- Sources scanned: {v.get('files_scanned', 0)}")
    print(f"- Fragments written: {v.get('written', 0)}")
    for tool, count in sorted((v.get("written_by_tool") or {}).items()):
        filtered = (v.get("filtered_by_tool") or {}).get(tool, 0)
        print(f"    {tool}: {count} written, {filtered} filtered as noise")
    age = v.get("ledger_age_hours")
    if age is None:
        print("- Ledger age: no captured turns")
    else:
        print(f"- Ledger age: {age:.1f}h (threshold {v.get('max_ledger_age_hours')}h)")
    for err in result.errors:
        print(f"{_brand_cli()} memory capture-sweep: {err}", file=sys.stderr)
    return result.exit_code


def _cmd_hook(args: argparse.Namespace) -> int:
    """At-turn capture. ALWAYS returns 0 — see the skill docstring.

    A non-zero Stop hook is interpreted by the harness, so any failure
    here has to stay inside this function. Errors go to the capture log,
    never to the user's terminal.
    """
    payload: dict = {}
    try:
        ...
        if args.payload_file and args.unlink_payload:
            try:
    ...
    try:
        ...
        from .skills.capture_hook import capture_hook
        ...
        _log_hook_failure(f"{type(exc).__name__}: {exc}")
        return 0

    for err in result.errors:
        _log_hook_failure(err)
    if args.json:
        print(json.dumps(result.value or {}, indent=2))
    return 0


def _log_hook_failure(message: str) -> None:
    """Append a hook failure to the capture log, swallowing any error.

    The hook is silent by design; without this line a broken hook would
    be invisible, which is the failure mode this whole effort exists to
    stop.
    """
    try:
        from datetime import UTC, datetime

        log = Path.home() / ".axi" / "logs" / "memory-capture.log"
        log.parent.mkdir(parents=True, exist_ok=True)
        stamp = datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
        with open(log, "a") as fh:
            fh.write(f"{stamp} HOOK-FAIL: {message}\n")
    except Exception:
        pass


def _cmd_hook_install(args: argparse.Namespace) -> int:
    from .hook_install import install_capture_hook

    try:
        result = install_capture_hook(
            settings_path=args.settings, axi_path=args.axi,
        )
    except ValueError as exc:
        print(f"{_brand_cli()} memory hook-install: {exc}", file=sys.stderr)
        return 1

    if args.json:
        print(json.dumps(result, indent=2))
        return 0
    print(_h1("At-turn capture installed"))
    print(f"- Settings: {result['settings_path']}")
    print(f"- Backup: {result['backup']}")
    print(f"- Events: {', '.join(result['events'])}")
    print(f"- Command: {result['axi']} memory hook")
    print(_dim_italic("Restart Claude Code for the hooks to take effect."))
    return 0


def _cmd_hook_uninstall(args: argparse.Namespace) -> int:
    from .hook_install import uninstall_capture_hook

    try:
        result = uninstall_capture_hook(settings_path=args.settings)
    except ValueError as exc:
        print(f"{_brand_cli()} memory hook-uninstall: {exc}", file=sys.stderr)
        return 1

    if args.json:
        print(json.dumps(result, indent=2))
        return 0
    print(_h1(f"Removed {result['removed']} capture hook entry(ies)"))
    print(f"- Settings: {result['settings_path']}")
    return 0


def _cmd_backfill_history(args: argparse.Namespace) -> int:
    """Thin ADR-056 wrapper: recover prompts that outlived the transcripts."""
    ...
    try:
        ...
        print(f"{_brand_cli()} memory backfill-history: {exc}", file=sys.stderr)
        return 1

    path = args.path or str(default_history_path())
    if not os.path.exists(path):
        print(f"{_brand_cli()} memory backfill-history: no history file at {path}",
              file=sys.stderr)
        return 1
    ...
    print(_h1("Prompt-history backfill"))
    print(f"- Prompts scanned: {report['scanned']}")
    print(f"- Recovered: {report['written']}")
    print(f"- Already captured in full: {report.get('already_captured', 0)}")
    print(f"- Filtered as noise: {report.get('filtered', 0)}")
    print(f"- Already backfilled: {report['skipped']}")
    if args.dry_run:
        print(_dim_italic("Dry run — nothing written."))
    return 0


def _cmd_repair_hollow(args: argparse.Namespace) -> int:
    """Thin ADR-056 wrapper: build the composition, invoke the repair skill."""
    from axiom.memory.session_capture import resolve_principal_id

    from .skills.repair_hollow import repair_hollow

    try:
        principal = resolve_principal_id(args.principal)
    except ValueError as exc:
        print(f"{_brand_cli()} memory repair-hollow: {exc}", file=sys.stderr)
        return 1
    ...
    if args.json:
        print(json.dumps(result.value or {"errors": result.errors}, indent=2))
        return result.exit_code

    v = result.value or {}
    if v.get("dry_run"):
        print(_h1(f"Would purge {v.get('would_purge', 0)} fragment(s)"))
        print(f"- Hollow: {v.get('would_purge_hollow', 0)}")
        print(f"- Partial (recapturable): {v.get('would_purge_partial', 0)}")
    else:
        print(_h1(f"Purged {v.get('purged', 0)} fragment(s)"))
        print(f"- Hollow: {v.get('purged_hollow', 0)}")
        print(f"- Partial (recapturable): {v.get('purged_partial', 0)}")
    if v.get("partial_unrecoverable"):
        print(f"- Partial kept (transcript gone): {v['partial_unrecoverable']}")
    print(f"- Fragments kept: {v.get('kept', 0)}")
    print(f"- Source uuids freed for re-capture: "
          f"{v.get('released_source_uuids', 0)}")
    for err in result.errors:
        print(f"{_brand_cli()} memory repair-hollow: {err}", file=sys.stderr)
    return result.exit_code
...
def _cmd_forget(args: argparse.Namespace) -> int:
    ...
    if result.value is None:
        for err in result.errors:
            print(f"{_brand_cli()} memory forget: {err}", file=sys.stderr)
        return result.exit_code
...
def _cmd_export(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"{_brand_cli()} memory export: {exc}", file=sys.stderr)
        return 1
    ...
    result = export_bundle({
        ...
        "harness_stores": args.harness_stores,
        "store_excludes": args.store_excludes,
        "home": args.home,
    ...
    if not result.ok:
        for err in result.errors:
            print(f"{_brand_cli()} memory export: {err}", file=sys.stderr)
        return result.exit_code
...
def _cmd_import(args: argparse.Namespace) -> int:
    ...
    if not result.ok:
        for err in result.errors:
            print(f"{_brand_cli()} memory import: {err}", file=sys.stderr)
        return result.exit_code
...
def _cmd_absorb(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"{_brand_cli()} memory absorb: {exc}", file=sys.stderr)
        return 1
    ...
    if not result.ok:
        for err in result.errors:
            print(f"{_brand_cli()} memory absorb: {err}", file=sys.stderr)
        return result.exit_code
    ...
    if v["conflicts_queued"]:
        ...
        print(_dim_italic(f"  review with: {_brand_cli()} memory conflicts list"))
    if v["skipped"]:
...
def _cmd_conflicts(args: argparse.Namespace) -> int:
    ...
    if not result.ok:
        for err in result.errors:
            print(f"{_brand_cli()} memory conflicts: {err}", file=sys.stderr)
        return result.exit_code
...
def _cmd_dedup(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"{_brand_cli()} memory dedup: {exc}", file=sys.stderr)
        return 1
    ...
    if not result.ok:
        for err in result.errors:
            print(f"{_brand_cli()} memory dedup: {err}", file=sys.stderr)
        return result.exit_code
...
def _cmd_record(args: argparse.Namespace) -> int:
    ...
    if args.json_stdin:
        try:
            ...
            print(
                f"{_brand_cli()} memory record: invalid JSON on stdin: {exc}",
                file=sys.stderr,
            )
            return 1
        ...
        if not tool:
            print(
                f"{_brand_cli()} memory record: stdin event missing tool",
                file=sys.stderr,
            )
            return 1
        try:
            ...
            print(f"{_brand_cli()} memory record: {exc}", file=sys.stderr)
            return 1
        kwargs = dict(
        ...
        if not args.tool:
            print(
                f"{_brand_cli()} memory record: --tool is required (or use --json-stdin).",
                file=sys.stderr,
            )
            return 1
        try:
...
def _cmd_ingest(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"{_brand_cli()} memory ingest: {exc}", file=sys.stderr)
        return 1
    ...
    if not _os.path.exists(args.path):
        print(
            f"{_brand_cli()} memory ingest: file not found: {args.path}",
            file=sys.stderr,
        )
        return 1
...
def _cmd_heartbeat(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"{_brand_cli()} memory heartbeat: {exc}", file=sys.stderr)
        return 1
...
def _cmd_heartbeat_install(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        print(f"{_brand_cli()} memory heartbeat-install: wrote {result['plist_path']}")
...
def _cmd_heartbeat_uninstall(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        if result.get("removed"):
            print(f"{_brand_cli()} memory heartbeat-uninstall: removed {result['plist_path']}")
        else:
            print(
                f"{_brand_cli()} memory heartbeat-uninstall: nothing to remove "
                f"({result['plist_path']})"
            )
...
def _cmd_register_mcp(args: argparse.Namespace) -> int:
    ...
    # One interpreter for writing and for judging staleness: the registrar's
    # own resolution (pinned service venv when present, else sys.executable).
    expected = resolve_memory_server()[1]

    if not args.check:
        print(
            f"note: `{_brand_cli()} memory register-mcp` is superseded by `{_brand_cli()} mcp install`, "
        ...
        if args.check:
            ...
            for name, reg in TOOL_REGISTRARS.items():
                if not reg.detect():
                    ...
                    continue
                statuses[name] = reg.is_registered(expected_command=expected)
            ...
            if args.json:
                ...
                for name, s in statuses.items():
                    if s.get("registered") is None:
                        ...
                        print(f"         fix: {_brand_cli()} memory register-mcp --tool {name}")
    ...
    if args.tool is not None:
        ...
        if reg is None:
            ...
            print(
                f"{_brand_cli()} memory register-mcp: unknown tool '{args.tool}'. "
                f"Known: {known}",
                file=sys.stderr,
            )
            return 1
        ...
        if args.check:
            status = reg.is_registered(expected_command=expected)
            if args.json:
                print(json.dumps(status, indent=2))
            else:
                if not status.get("registered"):
                    print(
                        f"{_brand_cli()} memory register-mcp ({args.tool}): not registered "
                        f"({status.get('reason', 'unknown')})",
                        file=sys.stderr,
                    )
                    print(
                        f"  fix: {_brand_cli()} memory register-mcp --tool {args.tool}",
                        file=sys.stderr,
                    )
                elif status.get("stale"):
                    print(
                        f"{_brand_cli()} memory register-mcp ({args.tool}): registered "
                    ...
                    print(
                        f"{_brand_cli()} memory register-mcp ({args.tool}): ok "
        ...
        try:
            result = reg.register(python_path=expected)
        except NotImplementedError as exc:
            print(f"{_brand_cli()} memory register-mcp ({args.tool}): {exc}", file=sys.stderr)
            return 1
        if args.json:
            print(json.dumps(result, indent=2))
        else:
            ...
            if action == "unchanged":
                print(
                    f"{_brand_cli()} memory register-mcp ({args.tool}): already registered "
                    f"({result['command']})"
                )
            else:
                print(
                    f"{_brand_cli()} memory register-mcp ({args.tool}): {action} entry in "
    ...
    if args.check:
        status = is_axiom_memory_mcp_registered(expected_command=expected)
        if args.json:
            print(json.dumps(status, indent=2))
        else:
            if not status.get("registered"):
                print(
                    f"{_brand_cli()} memory register-mcp: not registered "
                    f"({status.get('reason', 'unknown')})",
                    file=sys.stderr,
                )
                print(f"  fix: {_brand_cli()} memory register-mcp", file=sys.stderr)
            elif status.get("stale"):
                print(
                    f"{_brand_cli()} memory register-mcp: registered with stale python "
                ...
                print(
                    f"{_brand_cli()} memory register-mcp: ok "
                    f"(command={status.get('command')})"
                )
        registered_ok = status.get("registered") and not status.get("stale")
        return 0 if registered_ok else 1
    ...
    if args.json:
        ...
        if action == "unchanged":
            print(
                f"{_brand_cli()} memory register-mcp: already registered "
                f"({result['command']})"
            )
        else:
            print(
                f"{_brand_cli()} memory register-mcp: {action} entry in "
                f"{result['config_path']} (command={result['command']})"
            )
    return 0
...
def _cmd_migrate(args: argparse.Namespace) -> int:
    if not args.backfill_scope:
        print(
            f"{_brand_cli()} memory migrate: nothing to do. Pass "
...
def _cmd_reindex(args: argparse.Namespace) -> int:
    ...
    if not args.all_principals:
        ...
        try:
            ...
            print(f"{_brand_cli()} memory reindex: {exc}", file=sys.stderr)
            return 1
    ...
    if not result.ok:
        for err in result.errors:
            print(f"{_brand_cli()} memory reindex: {err}", file=sys.stderr)
        return result.exit_code
...
def _cmd_port(args: argparse.Namespace) -> int:
    """Thin ADR-056 wrapper: invoke memory.port (composed onboarding)."""
    from axiom.memory.session_capture import resolve_principal_id

    from .skills.port import port

    try:
        principal = resolve_principal_id(args.principal)
    except ValueError as exc:
        print(f"{_brand_cli()} memory port: {exc}", file=sys.stderr)
        return 1
    ...
    v = result.value
    if v is not None:
        title = "Port dry run" if v["dry_run"] else "Ported"
        print(_h1(f"{title} — {v['principal']}"))
        print()
        mcp = v["mcp"] or {}
        if "error" in mcp:
            print(f"- MCP registration: FAILED ({mcp['error']})")
        else:
            for tool, r in (mcp.get("results") or {}).items():
                print(f"- MCP {tool}: {r.get('action', '?')}")
        for harness, hv in v["harnesses"].items():
            if "errors" in hv:
                print(f"- Absorb {harness}: FAILED ({'; '.join(hv['errors'])})")
            else:
                print(
                    f"- Absorb {harness}: {hv['imported']} imported, "
                    f"{hv['skipped_echo']} already present"
                )
        if not v["harnesses"]:
            print(_dim_italic("  no harness stores detected; nothing absorbed"))
        if v["bundle"] is not None:
            bv = v["bundle"]
            if "errors" in bv:
                print(f"- Bundle import: FAILED ({'; '.join(bv['errors'])})")
            else:
                n = bv.get("imported", bv.get("would_import", 0))
                print(f"- Bundle import: {n} fragment(s)")
        if v["reindex"] is not None:
            rv = v["reindex"]
            if "errors" in rv:
                print(f"- Recall reindex: FAILED ({'; '.join(rv['errors'])})")
            elif rv.get("action") == "skipped-dry-run":
                print("- Recall reindex: skipped (dry run)")
            else:
                print(f"- Recall reindex: {len(rv.get('principals', []))} principal(s)")
    if not result.ok:
        for err in result.errors:
            print(f"{_brand_cli()} memory port: {err}", file=sys.stderr)
    return result.exit_code
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.action == "capture-sweep":
        return _cmd_capture_sweep(args)
    if args.action == "repair-hollow":
        return _cmd_repair_hollow(args)
    if args.action == "backfill-history":
        return _cmd_backfill_history(args)
    if args.action == "hook":
        return _cmd_hook(args)
    if args.action == "hook-install":
        return _cmd_hook_install(args)
    if args.action == "hook-uninstall":
