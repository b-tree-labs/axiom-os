...
def _cmd_capture_sweep(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"axi memory capture-sweep: {exc}", file=sys.stderr)
        return 1
    ...
    for err in result.errors:
        print(f"axi memory capture-sweep: {err}", file=sys.stderr)
    return result.exit_code
...
def _cmd_hook_install(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"axi memory hook-install: {exc}", file=sys.stderr)
        return 1
...
def _cmd_hook_uninstall(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"axi memory hook-uninstall: {exc}", file=sys.stderr)
        return 1
...
def _cmd_backfill_history(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"axi memory backfill-history: {exc}", file=sys.stderr)
        return 1

    path = args.path or str(default_history_path())
    if not os.path.exists(path):
        print(f"axi memory backfill-history: no history file at {path}",
              file=sys.stderr)
        return 1
...
def _cmd_repair_hollow(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"axi memory repair-hollow: {exc}", file=sys.stderr)
        return 1
    ...
    for err in result.errors:
        print(f"axi memory repair-hollow: {err}", file=sys.stderr)
    return result.exit_code
...
def _cmd_port(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"axi memory port: {exc}", file=sys.stderr)
        return 1
    ...
    if not result.ok:
        for err in result.errors:
            print(f"axi memory port: {err}", file=sys.stderr)
    return result.exit_code
