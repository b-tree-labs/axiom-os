...
def _git_common_dir(path: Path) -> Path | None:
    """Absolute git-common-dir for `path`, or None if `path` isn't in a repo.

    All linked worktrees of one repo — and a bare repo plus every worktree
    attached to it — share a single common-dir. That makes it the stable key
    for collapsing 'the same logical repo' down to one entry.
    """
    rc, out = _run(["git", "rev-parse", "--git-common-dir"], cwd=path)
    if rc != 0 or not out.strip():
        return None
    p = Path(out.strip())
...
def _is_work_tree(path: Path) -> bool:
    rc, out = _run(["git", "rev-parse", "--is-inside-work-tree"], cwd=path)
    return rc == 0 and out.strip() == "true"


def discover_repos(root: Path) -> list[Path]:
    """Distinct git repos at/under `root`, one representative path per repo.

    Collapses by git-common-dir so a **bare** repo and all its linked
    worktrees resolve to a single logical repo (a real working tree is
    preferred as the representative, since ``find_stale`` needs one to
    identify the main worktree). Scans `root` itself plus its immediate
    children — the bare-repo + sibling-worktree workspace layout that the
    single-``--repo`` path is blind to (axiom-os#482).
    """
    root = root.resolve()
    candidates: list[Path] = [root]
    if root.is_dir():
        try:
            candidates += [c for c in sorted(root.iterdir()) if c.is_dir()]
        except OSError:
            pass
    ...
    for cand in candidates:
        ...
        if common is None:
            continue
        if common not in by_common:
            by_common[common] = cand
        elif _is_work_tree(cand) and not _is_work_tree(by_common[common]):
            # Upgrade a bare-only representative to a real working tree.
            by_common[common] = cand
    return list(by_common.values())


def find_stale_workspace(root: Path) -> list[StalenessVerdict]:
    """``find_stale`` across every repo discovered under `root`.

    When `root` is a single worktree this is just ``find_stale(root)`` (one
    repo's worktrees). When it's a workspace parent of many repos — including
    a bare repo with sibling worktrees — it sweeps them all and de-dupes by
    worktree path. This is the discovery fix for axiom-os#482: TIDY was
    scoping to one CWD repo and reporting 'No worktrees found' for the bare
    workspace root.
    """
    repos = discover_repos(root)
    if not repos:
        return find_stale(root)
    verdicts: list[StalenessVerdict] = []
    seen: set[Path] = set()
    for repo in repos:
        for v in find_stale(repo):
            rp = v.worktree.path.resolve()
            if rp in seen:
                continue
            seen.add(rp)
            verdicts.append(v)
