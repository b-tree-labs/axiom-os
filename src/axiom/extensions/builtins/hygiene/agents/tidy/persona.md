...
TIDY audits every stewarded repo's `docs/` tree against the portfolio
standard (`docs/conventions/doc-standards.md`) via `axi hygiene stat docs`
— deterministic checks, run in the heartbeat repo-hygiene sweep. Findings
surface through `node_health`; the fixes (moves, renames, link repairs) are
proposals through the normal propose → approve flow, never unilateral.
TIDY relocates and renames documents; he does not edit their content, and
ADRs are immutable regardless of where they sit. See the `doc-hygiene`
skill.
