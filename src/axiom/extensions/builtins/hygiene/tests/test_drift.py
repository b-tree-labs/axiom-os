...
def test_suggested_action_open_pr_far_behind_escalates_to_rebase(repo):
    """2026-07 audit: 'monitor-pr' regardless of drift let a PR\'d branch
    fall 100+ commits behind unnoticed. At OPEN_PR_BEHIND_THRESHOLD the
    open-PR path escalates to 'rebase-recommended'."""
    from axiom.extensions.builtins.hygiene.drift import (
        OPEN_PR_BEHIND_THRESHOLD,
        BranchPurpose,
        WorktreeDrift,
    ...
    r = WorktreeDrift(
        path=repo / "wt-behind",
        branch="feat/pr-way-behind",
        ahead=5,
        behind=OPEN_PR_BEHIND_THRESHOLD + 27,
        unpushed=0,
        dirty_files=0,
        last_commit_age_days=20,
        has_open_pr=True,
        pr_state="OPEN",
        purpose=BranchPurpose("feat/pr-way-behind", "way behind", [], []),
        recent_commit_subjects=[],
        top_changed_paths=[],
        drift_severity="stale",
        suggested_action="",
    ...
    assert suggest_action(r) == "rebase-recommended"
    # The rationale names the behind count so the human sees the why.
    r.suggested_action = "rebase-recommended"
    packet = render_decision_packet(r)
    assert str(r.behind) in packet.split("Suggested action:")[-1]


def test_suggested_action_open_pr_below_threshold_stays_monitor(repo):
    """Below the threshold the open-PR posture is unchanged: stand down."""
    from axiom.extensions.builtins.hygiene.drift import (
        OPEN_PR_BEHIND_THRESHOLD,
        BranchPurpose,
        WorktreeDrift,
    ...
    r = WorktreeDrift(
        ...
        branch="feat/pr-fine",
        ahead=5,
        behind=OPEN_PR_BEHIND_THRESHOLD - 1,
        unpushed=0,
        dirty_files=0,
        last_commit_age_days=2,
        has_open_pr=True,
        pr_state="OPEN",
        purpose=BranchPurpose("feat/pr-fine", "fine", [], []),
        recent_commit_subjects=[],
        top_changed_paths=[],
        drift_severity="moderate",
        suggested_action="",
...
def test_open_pr_behind_threshold_default():
    from axiom.extensions.builtins.hygiene.drift import OPEN_PR_BEHIND_THRESHOLD

    assert OPEN_PR_BEHIND_THRESHOLD == 50
