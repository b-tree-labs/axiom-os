...
def _stash_one(repo: Path, msg: str) -> str:
    """Create a change, stash it with `msg`, return the stash commit sha."""
    p = repo / f"stash-{msg}.tmp"
    p.write_text("wip\n")
    _git(repo, "stash", "push", "-q", "-u", "-m", msg)
    return _git(repo, "rev-parse", "stash@{0}")


class TestArchiveStashes:
    def test_archives_stash_to_branch_and_drops(self, repo, tmp_path):
        """The reversible conversion: an `archive/stash-<date>-<subject>`
        branch is created at the stash commit, then the stash is dropped."""
        from axiom.extensions.builtins.hygiene.branch_prune import (
            archive_stashes,
        )
        sha = _stash_one(repo, "old wip")

        result = archive_stashes(
            repo, state_dir=tmp_path / "state", dormancy_days=0
        )

        assert result.proceed is True
        assert len(result.pruned) == 1
        [branch] = result.pruned
        assert branch.startswith("archive/stash-")
        assert "old-wip" in branch
        # archive branch points at the stash commit
        assert _git(repo, "rev-parse", branch) == sha
        # stash entry gone
        assert _git(repo, "stash", "list") == ""

    def test_fresh_stash_not_archived(self, repo, tmp_path):
        """Dormancy gate: a stash younger than the threshold stays put."""
        from axiom.extensions.builtins.hygiene.branch_prune import (
            archive_stashes,
        )
        _stash_one(repo, "fresh wip")

        result = archive_stashes(
            repo, state_dir=tmp_path / "state", dormancy_days=7
        )

        assert result.pruned == []
        assert _git(repo, "stash", "list") != ""

    def test_dry_run_does_not_touch_stash(self, repo, tmp_path):
        from axiom.extensions.builtins.hygiene.branch_prune import (
            archive_stashes,
        )
        _stash_one(repo, "keep me")

        result = archive_stashes(
            repo, state_dir=tmp_path / "state", dormancy_days=0, dry_run=True
        )

        assert result.reason == "dry_run"
        assert len(result.would_prune) == 1
        assert _git(repo, "stash", "list") != ""

    def test_undo_restores_stash_entry(self, repo, tmp_path):
        from axiom.extensions.builtins.hygiene.branch_prune import (
            archive_stashes,
            undo_stash_archive,
        )
        sha = _stash_one(repo, "restore me")
        result = archive_stashes(
            repo, state_dir=tmp_path / "state", dormancy_days=0
        )
        [branch] = result.pruned

        assert undo_stash_archive(repo, branch) is True
        # stash entry is back, pointing at the original commit
        assert _git(repo, "rev-parse", "stash@{0}") == sha

    def test_over_limit_batch_downgrades_to_confirmation(self, repo, tmp_path,
                                                         monkeypatch):
        """D6.3: same volume guard as branch pruning, own op-class."""
        from axiom.extensions.builtins.hygiene.branch_prune import (
            archive_stashes,
        )
        monkeypatch.setenv("TIDY_GIT_STASH_ARCHIVE_MAX_PER_TICK", "1")
        _stash_one(repo, "one")
        _stash_one(repo, "two")
        ...
        # nothing dropped
        assert len(_git(repo, "stash", "list").splitlines()) == 2
...
# ---------------------------------------------------------------------------
# gh-PR-state merge confirmation (2026-09-02): a squash-merge whose review
# edits leave the branch tree un-subsumed lands the work but NOT the tree, so
# content_landed (tree-equality) says False. The persona already prescribes
# `gh pr` state as the merge-confirmation signal; this wires it in.
# ---------------------------------------------------------------------------


class TestMergedPrSignal:
    def test_merged_pr_makes_content_unmerged_branch_prunable(self, repo):
        from axiom.extensions.builtins.hygiene.branch_prune import list_merged_local
        # Not an ancestor, content NOT landed (the squash-with-review case).
        _unmerged_branch(repo, "feat/squashed")
        names = {
            b for b, _ in list_merged_local(
                repo, pr_merged=lambda r, b: b == "feat/squashed"
            )
        }
        assert "feat/squashed" in names

    def test_open_or_absent_pr_is_not_a_prune_signal(self, repo):
        from axiom.extensions.builtins.hygiene.branch_prune import list_merged_local
        _unmerged_branch(repo, "feat/wip")
        names = {b for b, _ in list_merged_local(repo, pr_merged=lambda r, b: False)}
        assert "feat/wip" not in names

    def test_default_check_absent_gh_adds_no_false_positive(self, repo, monkeypatch):
        # Default path: gh missing / no PR -> _pr_state_for_branch None -> not merged.
        from axiom.extensions.builtins.hygiene import branch_prune
        _unmerged_branch(repo, "feat/wip")
        monkeypatch.setattr(branch_prune, "_pr_state_for_branch", lambda r, b: None)
        names = {b for b, _ in branch_prune.list_merged_local(repo)}
        assert "feat/wip" not in names

    def test_default_check_merged_state_lists_it(self, repo, monkeypatch):
        from axiom.extensions.builtins.hygiene import branch_prune
        _unmerged_branch(repo, "feat/squashed")
        monkeypatch.setattr(branch_prune, "_pr_state_for_branch",
                            lambda r, b: "MERGED" if b == "feat/squashed" else None)
        names = {b for b, _ in branch_prune.list_merged_local(repo)}
        assert "feat/squashed" in names

    def test_prune_threads_pr_signal_end_to_end(self, repo):
        from axiom.extensions.builtins.hygiene.branch_prune import prune
        _unmerged_branch(repo, "feat/squashed")
        res = prune(repo, state_dir=repo / ".tidy-state", dry_run=True,
                    pr_merged=lambda r, b: b == "feat/squashed")
        assert "feat/squashed" in res.would_prune


class TestProposeMerged:
    def test_returns_merged_names_and_deletes_nothing(self, repo):
        from axiom.extensions.builtins.hygiene.branch_prune import propose_merged
        _merged_branch(repo, "feat/done")
        _unmerged_branch(repo, "feat/wip")
        names = propose_merged(repo)
        assert "feat/done" in names
        assert "feat/wip" not in names
        assert _git_rc(repo, "rev-parse", "--verify", "feat/done") == 0  # not deleted

    def test_honors_injected_pr_merged(self, repo):
        from axiom.extensions.builtins.hygiene.branch_prune import propose_merged
        _unmerged_branch(repo, "feat/squashed")
        names = propose_merged(repo, pr_merged=lambda r, b: b == "feat/squashed")
        assert "feat/squashed" in names
