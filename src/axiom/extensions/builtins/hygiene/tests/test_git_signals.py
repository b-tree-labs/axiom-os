...
class TestCheckDormantStashes:
    ...
    def test_default_dormancy_is_seven_days(self):
        """2026-07 audit: 60 days let stashes rot for two months before
        anyone heard about them. A stash is a parking spot, not storage —
        one week is the new dormancy policy."""
        from axiom.extensions.builtins.hygiene.git_signals import (
            DEFAULT_STASH_DORMANCY_DAYS,
        )
        assert DEFAULT_STASH_DORMANCY_DAYS == 7

    def test_remediation_instructs_archive_branch_conversion(self, repo):
        """The remediation is no longer 'review or drop' — dormant stashes
        should be converted to archive branches (visible, pushable,
        recoverable) via `branch_prune.archive_stashes`."""
        from axiom.extensions.builtins.hygiene.git_signals import (
            check_dormant_stashes,
        )
        _stash_one(repo, "remediate")

        [match] = check_dormant_stashes(repo, dormancy_days=0)
        assert "archive" in match.expected_value.lower()
        assert "branch" in match.expected_value.lower()
