...
# An OPEN PR used to mean "TIDY stands down" unconditionally — which let
# PR'd branches fall 100+ commits behind unnoticed (2026-07 audit). At
# this behind-count the open-PR path escalates to 'rebase-recommended'.
OPEN_PR_BEHIND_THRESHOLD = 50
...
def suggest_action(r: WorktreeDrift) -> str:
    ...
    if r.pr_state in ("MERGED", "CLOSED"):
        return "archive"
    if r.has_open_pr:
        # Standing down is only right while the PR is still mergeable in
        # practice; past the behind threshold the drift itself is the risk.
        if r.behind >= OPEN_PR_BEHIND_THRESHOLD:
...
def _action_rationale(r: WorktreeDrift) -> str:
    ...
    if a == "rebase-recommended":
        if r.has_open_pr:
            return (f"  Why: PR is OPEN but the branch is {r.behind} commits behind main "
                    f"(threshold {OPEN_PR_BEHIND_THRESHOLD}). Rebase before review drifts "
                    "further — reviewers shouldn't absorb the merge risk.")
