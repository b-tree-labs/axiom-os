...
Usage:
    ...
    axi hygiene stat git          Repo-wide git-state audit (branches, stashes, worktrees)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=f"{_brand_cli()} hygiene",
        description="TIDY — Autonomous Resource Steward",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
Examples:
  {_brand_cli()} hygiene status       # Show scratch space status
  {_brand_cli()} hygiene ls           # List all tracked entries
  {_brand_cli()} hygiene clean        # Sweep expired and orphaned entries
  {_brand_cli()} hygiene purge        # Delete all scratch entries
  {_brand_cli()} hygiene stat vitals       # Detailed vitals snapshot
  {_brand_cli()} hygiene diagnose     # LLM-powered diagnosis
""",
    )
    ...
    git_p = sub.add_parser(
        "git",
        help=(
            "Repo-wide git-state audit: stale/squash-landed branches, "
            "upstream-gone, dormant stashes, orphan worktrees, primary untracked"
        ),
    )
    git_p.add_argument(
        "--repo",
    ...
    git_p.add_argument("--json", action="store_true", help="Output as JSON")

    docs_p = sub.add_parser(
        "docs",
        help=(
            "Doc-standards audit per docs/conventions/doc-standards.md: "
            "loose root files, kind prefixes, casing, ADR collisions, "
            "retired folders, missing H1s, broken relative links"
        ),
    )
    docs_p.add_argument(
        "--repo",
    ...
    docs_p.add_argument("--json", action="store_true", help="Output as JSON")
...
def _cmd_clean(args) -> int:
    ...
    if getattr(args, "repo", False):
        ...
        if findings.get("stale_project_dir"):
            from axiom.infra.paths import get_project_state_dir

            state_dir = get_project_state_dir(REPO_ROOT)
            stale = findings["stale_project_dir"]
            print(f"\nStale {state_dir.name}/ items: {', '.join(stale)}")
            if not dry_run:
                import shutil

                for name in stale:
                    stale_path = state_dir / name
                    if stale_path.is_dir():
                        shutil.rmtree(stale_path, ignore_errors=True)
                    elif stale_path.is_file():
                        stale_path.unlink(missing_ok=True)
                    print(f"  Removed {state_dir.name}/{name}")
        ...
        if unauthed:
            ...
            for p in unauthed:
                if p["type"] == "gitlab":
                    ...
                    print(f"    or: {_brand_cli()} connect gitlab")
                elif p["type"] == "github":
...
def _cmd_retention(args) -> int:
    ...
    if not status["categories"]:
        ...
        print()
        print(f"  Run `{_brand_cli()} hygiene stat retention --dry-run` to preview cleanup")
        print(f"  Run `{_brand_cli()} hygiene stat retention --cleanup` to execute")
...
def _cmd_drift(args) -> int:
    ...
    print(
        f"\nRun `{_brand_cli()} hygiene stat drift --branch <name>` for the full decision packet "
...
def _cmd_git(args) -> int:
    """`axi hygiene stat git` — repo-wide git-state audit.

    Fans out over every `git_signals.check_*` via `audit_git` — the
    aggregator that makes the issue-#201 signals live at runtime
    (before it, the checks were unit-tested but never invoked; the
    ADR-046 detect-only anti-pattern). Report-only: prune candidates
    route through `axi hygiene list branches --prune`.
    """
    from pathlib import Path

    from .git_signals import audit_git
    from .node_health import Severity
    ...
    if getattr(args, "json", False):
        print(json.dumps(
            {"repo": str(repo), "findings": [f.to_dict() for f in findings]},
            indent=2,
        ))
        return 0

    print(f"TIDY Git Audit — {repo}")
    print("=" * 50)

    if not findings:
        print("  All checks passed — repo git state is clean.")
        return 0
    ...
    for f in findings:
        ...
        print(f"  [{icon}] {f.check}: {f.message}{fix}")
        if f.expected_value:
            print(f"         remediation: {f.expected_value}")
    ...
    if info:
        summary.append(f"{info} info")
    print()
    print(f"  Summary: {', '.join(summary)}")
    print(f"\nAuto-fixable branch candidates: `{_brand_cli()} hygiene list branches --prune`")
    print("(archived under refs/tidy-archive/ first — reversible).")
    return 0
...
def _cmd_docs(args) -> int:
    """`axi hygiene stat docs` — doc-standards audit.

    Fans out over every `doc_hygiene.check_*` via `audit_docs`. Report-only:
    the audit never moves files — fixes route through propose → approve.
    Standard: docs/conventions/doc-standards.md.
    """
    from pathlib import Path

    from .doc_hygiene import audit_docs
    from .node_health import Severity

    repo = Path(args.repo).expanduser().resolve()
    findings = audit_docs(repo)
    ...
    print(f"TIDY Doc-Standards Audit — {repo}")
    print("=" * 50)

    if not findings:
        print("  All checks passed — docs/ conforms to doc-standards.md.")
        return 0
    ...
    info = sum(1 for f in findings if f.severity == Severity.INFO)
    print()
    print(f"  Summary: {warnings} warnings, {info} info")
    print("\nStandard: docs/conventions/doc-standards.md")
    return 0
...
def _cmd_worktrees(args) -> int:
    ...
    # Workspace-aware: discovers every repo at/under `repo` (incl. a bare repo
    # with sibling worktrees), not just the one CWD repo. Fixes the
    # "No worktrees found" blindness on the bare-repo layout (axiom-os#482).
    verdicts = wt_mod.find_stale_workspace(repo)
    ...
    if not args.prune and not args.dry_run:
        print(f"\nRun `{_brand_cli()} hygiene list worktrees --prune` to remove stale worktrees,")
        print(f"or `{_brand_cli()} hygiene list worktrees --dry-run` to preview the exact commands.")
        return 0
...
def _cmd_branches(args) -> int:
    ...
    if not args.prune and not args.dry_run:
        ...
        print(f"\nRun `{_brand_cli()} hygiene list branches --prune{flag}` to delete these")
