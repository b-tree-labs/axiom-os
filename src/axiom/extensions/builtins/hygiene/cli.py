...
def build_parser() -> argparse.ArgumentParser:
    ...
    # `stat <resource>` (vitals|health|ci|drift|retention|git)
    stat_p = sub.add_parser("stat", help="Stat resource (vitals|health|ci|drift|retention|git).")
    stat_sub = stat_p.add_subparsers(dest="resource", required=True)
    for res in ("vitals", "health", "ci", "drift", "retention", "git"):
        if res in legacy_subs:
            legacy_p = legacy_subs[res]
