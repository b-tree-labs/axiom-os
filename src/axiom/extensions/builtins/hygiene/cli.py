...
def build_parser() -> argparse.ArgumentParser:
    ...
    # `stat <resource>` (vitals|health|ci|drift|retention|git|docs|capabilities)
    stat_p = sub.add_parser(
        "stat",
        help="Stat resource (vitals|health|ci|drift|retention|git|docs|capabilities).",
    )
    stat_sub = stat_p.add_subparsers(dest="resource", required=True)
    for res in ("vitals", "health", "ci", "drift", "retention", "git", "docs"):
        if res in legacy_subs:
            legacy_p = legacy_subs[res]
            stat_sub.add_parser(res, parents=[legacy_p], add_help=False,
                                conflict_handler="resolve")
    # Not a legacy verb — capability declaration drift across every installed
    # extension, with a paste-ready SkillSpec per finding.
    stat_sub.add_parser(
        "capabilities",
        help="Capabilities registered but NOT declared, so nothing can find them.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = invoke_capability(registry, f"hygiene.{args.verb}", params, ctx, surface=CLI_SURFACE)

    # Legacy `_cmd_*` handlers print as they go and return a bare result. Newer
    # skills stay SILENT and report through the result, which is what makes them
    # safe on MCP — this server speaks over stdio, so a stray print would land
    # in the protocol stream. Rendering here keeps that property while the CLI
