...
"""Repo hygiene checks — TIDY keeps the workspace tidy.
...
Run via: axi hygiene clean --repo
Or automatically during TIDY's periodic sweep.
"""
...
from axiom.infra.paths import get_project_state_dir, project_dir_name
...
# Expected project state directory contents (anything else is flagged as stale)
EXPECTED_PROJECT_DIR_ITEMS = {
    # Directories
...
def expected_root_dotdirs() -> set[str]:
    """Dotfiles/dotdirs at root that are always OK (with or without git).

    A function rather than a constant because the project state directory's
    name comes from the active branding, which a product registers before the
    CLI runs rather than before this module imports.
    """
    return {
        ".git", ".venv", ".claude", ".claude.example",
        ".github", ".vscode", project_dir_name(),
    }
...
def scan_repo_hygiene(root: Path) -> dict[str, Any]:
    ...
    # Check the project state directory for stale subdirectories
    state_dir = get_project_state_dir(root)
    if state_dir.exists():
        for item in state_dir.iterdir():
            if item.name not in EXPECTED_PROJECT_DIR_ITEMS:
                findings["stale_project_dir"].append(item.name)
...
def clean_clutter(root: Path, dry_run: bool = True) -> dict[str, int]:
    ...
    items — those require explicit `axi hygiene clean --repo` with user review.

    Returns counts of cleaned items.
    """
    findings = scan_repo_hygiene(root)
    cleaned = {"dirs": 0, "files": 0}
    # Note: stale_project_dir items are reported but NOT auto-cleaned here.
