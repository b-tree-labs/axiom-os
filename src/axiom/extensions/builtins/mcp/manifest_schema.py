...
class MCPToolDecl:
    ...
    ``input_schema_module`` is a dotted ``pkg.mod:attr`` path (or
    ``pkg.mod``, resolved via the conventional ``INPUT_SCHEMA`` attribute)
    whose target is a JSON Schema dict, a ``SkillSpec.inputs`` shape map, a
    callable returning either, a pydantic model, or an object exposing
    ``to_json_schema()``. ``input_schema`` is the same thing declared
    inline as a TOML table and wins when both are present. See
    ``mcp/schema_inference.py`` for the resolution rules.
    """
    ...
    side_effects: str = ""
    entry: str = ""
    input_schema: Mapping[str, Any] | None = field(default=None, hash=False)
...
def _probe_input_schema_module(target: str) -> str:
    """Empty string when ``target`` resolves to a schema; else the reason."""
    ...
    try:
        coerce_input_schema(load_schema_target(target))
    except Exception as exc:  # noqa: BLE001 — lint reports, never raises
...
def parse_mcp_block(manifest_path: Path) -> MCPExtensionConfig | None:
    ...
    for raw in mcp_block.get("tool", []) or []:
        ...
        raw_schema = raw.get("input_schema")
        tools.append(
            MCPToolDecl(
                name=tool_name,
                mcp_name=mcp_name,
                ...
                input_schema=dict(raw_schema) if isinstance(raw_schema, dict) else None,
            )
        )
...
def lint_mcp_block(manifest_path: Path) -> list[LintError | LintWarning]:
    ...
    for raw in mcp_block.get("tool", []) or []:
        if "input_schema" in raw and not isinstance(raw.get("input_schema"), dict):
            findings.append(
                LintError(
                    message=(
                        f"[[extension.mcp.tool]] {raw.get('name', '<unnamed>')!r}: "
                        "input_schema must be a table (inline JSON Schema or "
                        "inputs shape map)"
                    ),
                    path=manifest_path,
                )
    ...
    for tool in cfg.tools:
        if tool.input_schema_module and tool.input_schema is None:
            problem = _probe_input_schema_module(tool.input_schema_module)
            if problem:
                findings.append(
                    LintWarning(
                        message=(
                            f"[[extension.mcp.tool]] {tool.name!r}: input_schema_module "
                            f"{tool.input_schema_module!r} does not resolve ({problem}); "
                            "the tool will advertise the freeform schema"
                        ),
                        path=manifest_path,
                    )
                )
