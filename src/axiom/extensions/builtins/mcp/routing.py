...
async def gate_result_for_client(
    *,
    ...
    # window-sized chunks and withhold on the FIRST export-controlled verdict,
    # so the whole result is covered and verdicts stop depending on size.
    decision: RoutingDecision | None = None
    try:
        for _chunk in _gate_chunks(text):
            _verdict = router.classify(_chunk)
            if (
                _verdict is not None
                and _verdict.tier == RoutingTier.EXPORT_CONTROLLED
            ):
                decision = _verdict
                break
