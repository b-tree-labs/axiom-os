...
Hermes uses YAML (``mcp_servers``); OpenCode keys servers under ``mcp`` with
``type: local`` and the command as one argv array. Clients are declared in
:data:`TOOL_SPECS`; adding another is one row.

:data:`TOOL_SPECS` is the **single registrar matrix**: every harness's config
path, file format, detection rule and EC posture lives here and nowhere else.
Other MCP servers that need registering (the memory server's
``register_mcp`` module) reuse :func:`spec_for`, :func:`config_path_for`,
:func:`install_entry` and :func:`read_entry` with their own command/args
instead of re-implementing a file format, so the two can never drift.
...
def _xdg_config_home() -> Path:
    """``$XDG_CONFIG_HOME`` or ``~/.config`` on every platform — for clients
    (OpenCode) that ignore the OS-native root and always read XDG."""
    return Path(os.environ.get("XDG_CONFIG_HOME") or Path.home() / ".config")


def _opencode_config_path() -> Path:
    """OpenCode reads ``opencode.json`` *or* ``opencode.jsonc`` from its XDG
    dir; prefer whichever the user already has so we never fork their config."""
    root = _xdg_config_home() / "opencode"
...
class ToolSpec:
    name: str
    fmt: str  # "mcp_json" | "vscode_json" | "codex_toml" | "hermes_yaml" | "opencode_json"
...
def _present(
    *,
    ...
    """Detection rule: a home-relative config dir, a binary on PATH, or an
    absolute app-bundle path (GUI harnesses on macOS often have no CLI shim)."""

    def _check() -> bool:
        if any(_h(d).exists() for d in dirs):
            return True
        if any(shutil.which(b) is not None for b in bins):
            return True
        return any(Path(a).exists() for a in apps)
...
TOOL_SPECS: tuple[ToolSpec, ...] = (
    ...
    ToolSpec(
        ...
        _present(dirs=(".cursor",), bins=("cursor",), apps=("/Applications/Cursor.app",)),
    ...
    ToolSpec(
        "hermes", "hermes_yaml", "AXIOM_HERMES_CONFIG",
        lambda: _h(".hermes", "config.yaml"),
        _present(dirs=(".hermes",), bins=("hermes",)),
        # Hermes is provider-agnostic and could in principle be pointed at
        # the local ingress, but its data path is not verified here. The
        # registry rule is fail-closed, and being generous with this flag is
        # exactly how EC tool output reaches a public cloud.
        model_routable=False,
        ec_notes=(
            "UNVERIFIED: Nous Research Hermes; provider-agnostic and may be "
            "routable to the local ingress, but data path not confirmed. "
            "Treated non-EC pending verification."
        ),
    ),
    ToolSpec(
        # Verified against the published schema (https://opencode.ai/config.json):
        # servers live under ``mcp`` as ``{type: local, command: [argv...],
        # environment, enabled}``; the file is XDG on every platform and may be
        # ``.jsonc``. The earlier ``mcpServers`` row wrote a file OpenCode
        # never read.
        "opencode", "opencode_json", "AXIOM_OPENCODE_CONFIG",
        _opencode_config_path,
        _present(dirs=(".config/opencode", ".opencode"), bins=("opencode",)),
    ...
    ToolSpec(
        ...
        _present(bins=("code",), apps=("/Applications/Visual Studio Code.app",)),
        ec_notes=(
...
# Which top-level key each format keeps its servers under. Readers and
# uninstallers dispatch on this so no caller hardcodes a key.
SERVERS_KEY_BY_FMT: dict[str, str] = {
    "mcp_json": "mcpServers",
    "vscode_json": "servers",
...
def spec_for(tool: str) -> ToolSpec | None:
    """The matrix row for ``tool`` (``None`` when unknown)."""
    return _SPEC_BY_NAME.get(tool)
...
def config_path_for(spec: ToolSpec) -> Path:
    """The config file this spec writes: its env override, else its default."""
    return _config_path(spec)
...
def _load_json(path: Path) -> dict[str, Any]:
    """Parse ``path`` as a JSON object. Malformed JSON raises
    ``json.JSONDecodeError`` (a ``ValueError``) and a non-object document
    raises ``ValueError`` — in both cases *before* anything is written, so a
    config the user owns is never clobbered to add a server."""
...
def _load_yaml(path: Path) -> dict[str, Any]:
    """Parse ``path`` as a YAML mapping. Same contract as :func:`_load_json`:
    malformed YAML or a non-mapping document raises ``ValueError`` before
    anything is written."""
    import yaml
...
def _install_hermes_yaml(
    path: Path, server_name: str, command: str, args: list[str],
    env: dict[str, str], *, dry_run: bool,
) -> str:
    """Write the server into Hermes's ``config.yaml`` under ``mcp_servers``.

    Merges into any existing entry rather than replacing it. That matters
    for ``tools.include``: it is the allowlist keeping a third-party agent
    away from the KEEP credential tools, so a re-install that silently
    dropped it would be a privilege escalation shipped as an upgrade.
    """
    import yaml
    ...
    if not is_new and existing == entry:
        return "unchanged"
    if dry_run:
        return "would-add" if is_new else "would-update"
...
def _install_opencode(
    ...
    """Write the server into OpenCode's config under ``mcp``.

    OpenCode's local-server entry is ``{type: local, command: [argv...],
    environment, enabled}`` (schema: https://opencode.ai/config.json). Like
    the Hermes writer this merges into an existing entry so a user's
    ``enabled: false`` or ``timeout`` survives a re-install.
    """
    data = _load_json(path)
...
def _uninstall_hermes_yaml(path: Path, *, dry_run: bool) -> str:
    import yaml
...
def _install_one(
    spec: ToolSpec, server_name: str, command: str, args: list[str], env: dict[str, str],
    *, dry_run: bool, path: Path | None = None,
) -> str:
    path = path if path is not None else _config_path(spec)
    ...
    if spec.fmt == "hermes_yaml":
        return _install_hermes_yaml(
            path, server_name, command, args, env, dry_run=dry_run,
        )
    if spec.fmt == "opencode_json":
        return _install_opencode(path, server_name, command, args, env, dry_run=dry_run)
    return "unsupported"
...
def install_entry(
    ...
    *, dry_run: bool = False, path: Path | None = None,
    ...
    """Write one server entry in ``spec``'s format — the writer every MCP
    server shares. ``path`` overrides the spec's config path (tests, explicit
    ``--config`` flags). Returns the idempotent action word:
    ``added|updated|unchanged|would-add|would-update|unsupported``. Raises
    (``ValueError`` or a parser error) on a config it cannot parse, before
    writing anything."""
    return _install_one(spec, server_name, command, args, env, dry_run=dry_run, path=path)


def _plain(value: Any) -> Any:
    """Recursively turn tomlkit/yaml containers into plain dict/list/str."""
...
def read_entry(spec: ToolSpec, server_name: str, *, path: Path | None = None) -> dict[str, Any] | None:
    """Read back the entry ``server_name`` from ``spec``'s config file.

    Returns a plain dict normalised to ``{"command": str, "args": [...], ...}``
    regardless of format (OpenCode's argv array is split back into command +
    args), or ``None`` when the file or the entry is absent. Raises on a file
    that cannot be parsed, so callers can distinguish *missing* from
    *malformed*. This is what ``is_registered`` checks and doctor self-heal
    read: the same file the writer wrote, in the same format.
    """
    path = path if path is not None else _config_path(spec)
    if not path.exists() or not path.read_text().strip():
        return None
    key = SERVERS_KEY_BY_FMT[spec.fmt]
    if spec.fmt in ("mcp_json", "vscode_json", "opencode_json"):
        doc: Any = _load_json(path)
    elif spec.fmt == "codex_toml":
        import tomlkit

        doc = tomlkit.loads(path.read_text())
    elif spec.fmt == "hermes_yaml":
        doc = _load_yaml(path)
    else:
        return None
    ...
    if spec.fmt == "opencode_json":
        ...
        if isinstance(argv, list):
            entry["command"] = argv[0] if argv else ""
            entry["args"] = list(argv[1:])
        entry.setdefault("env", entry.get("environment", {}))
    return entry
...
def install(
    *,
    ...
    for tool in targets:
        ...
        except Exception as exc:  # noqa: BLE001 — a config we can't parse is a per-client outcome
            results[tool] = {
                "action": "error",
...
def _uninstall_one(spec: ToolSpec, *, dry_run: bool) -> str:
    ...
    if spec.fmt == "hermes_yaml":
        return _uninstall_hermes_yaml(path, dry_run=dry_run)
    if spec.fmt == "opencode_json":
        return _uninstall_json(path, "mcp", dry_run=dry_run)
    return "unsupported"
