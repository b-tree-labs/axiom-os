...
class _FakeStore:
    ...
    def access_tier_for(self, source_path: str) -> str:
        # A local store must report tiers (else the primitive fails closed);
        # these fixtures are public so they pass the default public filter.
        return "public"
...
def test_classified_chunk_is_filtered_by_access(monkeypatch):
    """MCP retrieval now enforces access: a classified chunk is dropped under the
    fail-closed principal context. Before this the handler passed no context, so
    retrieve() skipped the filter and the chunk leaked to any caller."""
    pub = _hit(path="docs/pub.md", text="public info")
    secret = SearchResult(
        source_path="docs/secret.md", source_title="Secret", chunk_text="secret info",
        chunk_index=0, similarity=0.9, combined_score=0.9, corpus="rag-internal",
        classification="classified",
    )
    store = _FakeStore([pub, secret])
    monkeypatch.setattr(pp, "_resolve_rag_store", lambda: store)
    monkeypatch.setattr(pp, "_embed_query", lambda q: None)
    out = asyncio.run(pp._rag_retrieve({"query": "info", "k": 5}))
    assert out["ok"] is True
    paths = [r["source_path"] for r in out["results"]]
    assert paths == ["docs/pub.md"]  # classified filtered; public passes
