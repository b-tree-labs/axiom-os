...
# ---- ADR-114 §1: allowed_principals threaded onto the built surface --------


def test_registry_tool_defaults_to_any_local(tmp_axiom_home, tmp_path):
    """A capability that declares no principals picks up the @*:local default."""
    reg = _reg_with(
        SkillSpec(
            name="alpha.ping",
    ...
    surface = _agg(reg, tmp_path).build()
    assert surface.allowed_principals["axiom_alpha__ping"] == ("@*:local",)


def test_registry_tool_honors_declared_principals(tmp_axiom_home, tmp_path):
    """A capability's declared allowed_principals reach the surface verbatim."""
    reg = _reg_with(
        SkillSpec(
            name="alpha.ping",
            fn=lambda p, c: SkillResult(ok=True),
            surfaces=("mcp",),
            side_effects=False,
            allowed_principals=("@ops:acme", "@ben:local"),
        )
    )
    surface = _agg(reg, tmp_path).build()
    assert surface.allowed_principals["axiom_alpha__ping"] == ("@ops:acme", "@ben:local")


def test_platform_tools_default_owner_only(tmp_axiom_home, tmp_path):
    """Platform primitives are owner-only (spec §8), not the @*:local general default."""
    from axiom.extensions.builtins.mcp.identity_gate import caller_owner_default

    surface = AggregationRegistry(extensions=[]).build()
    owner = (caller_owner_default(),)
    assert surface.allowed_principals["axiom_memory__compose"] == owner
    assert surface.allowed_principals["axiom_node__hooks_list"] == owner
    assert owner != ("@*:local",)  # genuinely stricter than the general default
