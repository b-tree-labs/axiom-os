...
def test_server_instructions_present(tmp_axiom_home):
    ...
    # Starts with, not equals: the handshake also carries the capability
    # discovery block (what this install has not been used for), which is an
    # ADDITION and must never displace the conventions. The block is empty
    # once every capability has been used, so equality holds again then.
