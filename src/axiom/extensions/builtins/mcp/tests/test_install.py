...
def _point_paths_at(tmp_path, monkeypatch):
    ...
    # Backstop: any client added to the registry later is isolated too. A
    # hand-listed map stops isolating the moment a client is added, and the
    # suite then reads and writes the developer's real IDE configs — which is
    # exactly what adding `hermes` did.
    for spec in inst.TOOL_SPECS:
        paths.setdefault(spec.env_override, tmp_path / f"{spec.name}.cfg")
...
# --- OpenCode: ``mcp`` key, ``type: local``, argv array (schema-verified) ----


def test_opencode_writes_mcp_local_entry(tmp_path, monkeypatch):
    """OpenCode's schema (https://opencode.ai/config.json) keys servers under
    ``mcp`` as ``{type: local, command: [argv...], environment, enabled}``;
    the earlier ``mcpServers`` row wrote a file OpenCode never read."""
    _point_paths_at(tmp_path, monkeypatch)
    r = inst.install(tools=["opencode"])
    assert r["results"]["opencode"]["action"] == "added"
    cfg = json.loads((tmp_path / "opencode.json").read_text())
    assert "mcpServers" not in cfg
    entry = cfg["mcp"][r["server"]]
    assert entry["type"] == "local"
    assert entry["command"] == [r["command"], *r["args"]]
    assert entry["environment"]["AXIOM_MCP_CLIENT"] == "opencode"
    assert entry["enabled"] is True
    assert inst.install(tools=["opencode"])["results"]["opencode"]["action"] == "unchanged"


def test_opencode_reinstall_keeps_user_toggles_and_uninstall_removes_only_ours(
    ...
    p.write_text(json.dumps({
        ...
        "mcp": {
            ...
            "axiom": {"type": "local", "command": ["/old"], "enabled": False, "timeout": 9000},
        },
    }))
    assert inst.install(tools=["opencode"])["results"]["opencode"]["action"] == "updated"
    cfg = json.loads(p.read_text())
    assert cfg["mcp"]["axiom"]["enabled"] is False  # the user's toggle survives
    assert cfg["mcp"]["axiom"]["timeout"] == 9000
    assert cfg["mcp"]["axiom"]["command"][0] == sys.executable
    assert cfg["$schema"] == "https://opencode.ai/config.json"

    assert inst.uninstall(tools=["opencode"])["results"]["opencode"]["action"] == "removed"
    after = json.loads(p.read_text())["mcp"]
    assert "other" in after and "axiom" not in after


def test_opencode_default_path_is_xdg_and_prefers_existing_jsonc(tmp_path, monkeypatch):
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path / "xdg"))
    monkeypatch.delenv("AXIOM_OPENCODE_CONFIG", raising=False)
    spec = inst.spec_for("opencode")
    assert inst.config_path_for(spec) == tmp_path / "xdg" / "opencode" / "opencode.json"
    (tmp_path / "xdg" / "opencode").mkdir(parents=True)
    (tmp_path / "xdg" / "opencode" / "opencode.jsonc").write_text("{}")
    assert inst.config_path_for(spec) == tmp_path / "xdg" / "opencode" / "opencode.jsonc"


# --- shared machinery every MCP server registers through --------------------


def test_read_entry_normalises_every_format(tmp_path, monkeypatch):
    _point_paths_at(tmp_path, monkeypatch)
    r = inst.install(all_tools=True)
    for spec in inst.TOOL_SPECS:
        entry = inst.read_entry(spec, r["server"])
        assert entry is not None, spec.name
        assert entry["command"] == r["command"], spec.name
        assert entry["args"] == r["args"], spec.name
        assert entry["env"]["AXIOM_MCP_CLIENT"] == spec.name
    assert inst.read_entry(inst.spec_for("cursor"), "nope") is None


def test_read_entry_absent_is_none_and_malformed_raises(tmp_path, monkeypatch):
    _point_paths_at(tmp_path, monkeypatch)
    for spec in inst.TOOL_SPECS:
        assert inst.read_entry(spec, "axiom") is None, spec.name
    for spec in inst.TOOL_SPECS:
        path = inst.config_path_for(spec)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("{ [ } ]\n")
        with pytest.raises(ValueError):
            inst.read_entry(spec, "axiom")


def test_install_reports_a_malformed_config_per_client_without_aborting(tmp_path, monkeypatch):
    _point_paths_at(tmp_path, monkeypatch)
    (tmp_path / "cursor-mcp.json").write_text("{ [ } ]\n")
    r = inst.install(tools=["cursor", "codex"])
    assert r["results"]["cursor"]["action"] == "error"
    assert r["results"]["cursor"]["detail"].startswith("JSONDecodeError")
    assert (tmp_path / "cursor-mcp.json").read_text() == "{ [ } ]\n"  # untouched
    assert r["results"]["codex"]["action"] == "added"  # the sweep went on


def test_spec_for_and_supported_tools_agree():
    for name in inst.supported_tools():
        assert inst.spec_for(name) is not None and inst.spec_for(name).name == name
    assert inst.spec_for("not-a-harness") is None
    assert set(inst.SERVERS_KEY_BY_FMT) == {s.fmt for s in inst.TOOL_SPECS}


def test_present_detects_config_dir_binary_or_app_bundle(tmp_path, monkeypatch):
    monkeypatch.setenv("PATH", str(tmp_path / "empty"))
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: tmp_path))
    app = tmp_path / "Applications" / "Thing.app"
    check = inst._present(dirs=(".thing",), bins=("thing",), apps=(str(app),))
