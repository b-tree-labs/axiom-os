...
extensions are installed, all wired to real services (``CompositionService``,
``NodeRegistry``, ``BriefingService``, ``HookRegistry``, and the RAG
retriever). ``axiom_rag__retrieve`` runs the real hybrid retrieve over the
node's configured store with a **public-tier access filter** (a broad surface
defaults to public and fails closed if a local store cannot report tiers;
per-principal tier expansion is a follow-up, and the MCP client-seam EC gate
runs downstream regardless).
...
PLATFORM_TOOL_NAMES: tuple[str, ...] = (
    ...
    "axiom_mcp__client_capabilities",
)
...
async def _memory_compose(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if cognitive_type == "episodic" and "event_time" not in content:
        content["event_time"] = datetime.now(UTC).isoformat()
...
async def _rag_retrieve(args: dict[str, Any]) -> dict[str, Any]:
    ...
    owns_hybrid = getattr(store, "does_own_hybrid", False)
    ...
    embedding = None if owns_hybrid else _embed_query(query)

    # Access filter. This is a BROAD surface. The context is derived through
    # the ONE seam (`access_context_for_principal`, G2) — today that seam
    # returns the fail-closed PUBLIC baseline for every principal, and when
    # the policy engine lands, per-principal elevation wires in there rather
    # than at each call site. A local store enforces it here with the store's
    # real per-path tier; a local store that cannot report tiers FAILS CLOSED
    # (refuse rather than leak). A server-side-hybrid (remote) store applies
    # its own filter, and the MCP client-seam EC gate still runs downstream
    # regardless.
    ...
    tier_lookup = None
    if not owns_hybrid:
        tier_fn = getattr(store, "access_tier_for", None)
        if not callable(tier_fn):
            return {
                ...
                "ok": False,
                "note": (
                    "this RAG store cannot report per-document access tiers; "
                    "refusing to retrieve unfiltered (fail-closed)."
                ),
            }

        def tier_lookup(path: str, _fn: Any = tier_fn) -> str:
            try:
                return _fn(path) or "restricted"  # unknown path -> deny, never public
            except Exception:  # noqa: BLE001 — any lookup error -> deny
                return "restricted"

        access_context = access_context_for_principal(_resolve_principal(args))
    ...
    return {
        ...
        "access_scope": "public" if access_context is not None else "store-managed",
        "results": results,
        "count": len(results),
        "ok": True,
    }
...
async def _mcp_client_capabilities(args: dict[str, Any]) -> dict[str, Any]:
    """Return the agent-harness × EC-routing capability matrix."""
    try:
        from axiom.extensions.builtins.mcp.install import client_capabilities
    except Exception as exc:  # noqa: BLE001
        return {"clients": [], "ok": False, "error": str(exc)}
    rows = client_capabilities()
    return {
        "ok": True,
        "clients": rows,
        "ec_routable": [r["client"] for r in rows if r["ec_routable"]],
        "not_ec_routable": [r["client"] for r in rows if not r["ec_routable"]],
        "legend": (
            "ec_routable = the harness's model can be put in-enclave (routed to "
            "the local ingress). EC tool output is withheld from clients that are not."
        ),
    }
...
_DESCRIPTIONS: dict[str, str] = {
    ...
    "axiom_mcp__client_capabilities": (
        "Chart of supported agent harnesses (clients) and whether each is OK to "
        "route to an export-controlled (EC) model. A harness is EC-routable iff "
        "its model can be put in-enclave; the MCP server withholds EC tool output "
        "from any client that is not."
    ),
}
...
_HANDLERS: dict[str, Callable[[dict[str, Any]], Awaitable[dict[str, Any]]]] = {
    ...
    "axiom_mcp__client_capabilities": _mcp_client_capabilities,
}
