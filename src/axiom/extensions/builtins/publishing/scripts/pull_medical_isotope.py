...
def main():
    state_path = get_project_state_dir(Path.cwd()) / 'docflow' / 'state.json'
    if not state_path.exists():
        print(f'Registry not found at {state_path}')
        return 1
    ...
    downloads = get_project_state_dir(Path.cwd()) / 'downloads'
    ...
    md = Path('docs/prds/prd-medical-isotope.md')
    media = Path('docs/prds/media/prd_medical-isotope')
    if not pandoc:
