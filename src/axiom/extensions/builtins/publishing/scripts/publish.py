...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = get_project_state_dir(repo_root) / "generated"
    state_path = get_project_state_dir(repo_root) / "publisher" / "publisher-state.json"
    ...
    try:
        # Run pandoc from the docs/prds directory so relative image paths resolve
        cmd = [
            pandoc,
            "-f", "markdown",
            "-t", "docx",
