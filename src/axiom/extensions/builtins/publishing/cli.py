...
Action verbs — produce or move artifacts:
    ...
    axi pub mirror add|sync|status|resolve  Coordinate a doc with a remote editor
...
def _all_source_docs(engine) -> list[str]:
    ...
    config_path = get_project_state_dir(REPO_ROOT) / "publisher" / "workflow.yaml"
...
def _add_draft_args(parser: argparse.ArgumentParser) -> None:
    """Register shared args for `draft` / `generate` subparsers."""
    parser.add_argument("file", help="Markdown source file")
    parser.add_argument(
        "--format",
        "--to",
        dest="format",
        choices=["docx", "pdf"],
        help="Output format (default: configured provider, usually docx). "
        "'pdf' renders a branded PDF via pandoc + WeasyPrint.",
    )
    # Generic branding overrides — these win over the doc's frontmatter.
    parser.add_argument(
        "--brand-color", dest="brand_color",
        help="Header/footer band + heading color (e.g. '#BF5700').",
    )
    parser.add_argument(
        "--brand-org", dest="brand_org",
        help="Org wordmark shown in the running header.",
    )
    parser.add_argument(
        "--brand-logo", dest="brand_logo",
        help="Path to a logo image rendered in the header (optional).",
    )
    parser.add_argument(
        "--brand-title", dest="brand_title",
        help="Override the document title used on the cover + header.",
    )
...
def cmd_draft(args: argparse.Namespace) -> None:
    ...
    # Collect generic branding overrides (color / org / logo / title).
    # These override the document's frontmatter for branded formats (PDF).
...
def cmd_mirror(args: argparse.Namespace) -> None:
    """Thin wrapper over the press.mirror_* skills (ADR-056)."""
    from pathlib import Path as _Path

    from axiom.infra.paths import get_project_state_dir
    from axiom.infra.skills import SkillContext

    from .skills import bind_default, mirror_sync

    ctx = SkillContext(
        registry=bind_default(),
        state_dir=get_project_state_dir(_Path.cwd()) / "publisher",
        logger=logging.getLogger("press.mirror"),
    )
    if args.mirror_command == "add":
        result = mirror_sync.add(
            {"local": args.local, "url": args.url, "name": args.name,
             "git_annotate": args.git_annotate}, ctx)
    elif args.mirror_command == "sync":
        result = mirror_sync.sync(
            {"name": args.name, "watch": args.watch_once}, ctx)
    elif args.mirror_command == "resolve":
        result = mirror_sync.resolve(
            {"name": args.name, "strategy": args.strategy}, ctx)
    else:
        result = mirror_sync.status({}, ctx)
    print(json.dumps({"ok": result.ok, "value": result.value,
                      "errors": result.errors}, indent=2, default=str))
    if not result.ok:
        raise SystemExit(1)
...
def cmd_check_links(args: argparse.Namespace) -> None:
    ...
    if not valid and not missing:
        ...
        print(f"  Run: {_brand_cli()} pub publish <file>")
        print(separator("=") + "\n")
        return
    ...
    if total > 0:
        ...
        if pct == 100:
            ...
            print(f"✗ Many links are broken. Run '{_brand_cli()} pub scan' for details.")
...
def cmd_diff(args: argparse.Namespace) -> None:
    ...
    if not changed:
        ...
        print(f"\n  To publish changed docs: {_brand_cli()} pub publish --all --changed-only")
    else:
        ...
        print(f"  {_brand_cli()} pub publish --all --changed-only")
...
def cmd_scan(args: argparse.Namespace) -> None:
    ...
    if untracked:
        ...
        for filename in sorted(untracked):
            print(f"  • {filename}")
        print(f"\n  To add: {_brand_cli()} pub onboard <doc_id> <filename> --folder <path>")
...
def cmd_onboard(args: argparse.Namespace) -> None:
    ...
    if not args.doc_id or not args.file:
        print(f"Usage: {_brand_cli()} pub onboard <doc_id> <file> [--folder <path>] [--url <sharepoint_url>]")
        sys.exit(1)
    ...
    if success:
        ...
        if args.url:
            ...
            print(f"\n💡 Tip: Set SharePoint URL with: {_brand_cli()} pub onboard <id> <file> --url <url>")
        print(separator("=", width=70) + "\n")
    else:
...
def cmd_review(args: argparse.Namespace) -> None:
    ...
    if not draft_path:
        if file_arg:
            ...
            print(f"Generate one with: {_brand_cli()} signal draft")
        sys.exit(1)
...
def cmd_overview(args: argparse.Namespace) -> None:
    ...
    if total_untracked > 0:
        print(f"  • {total_untracked} untracked documents — run '{_brand_cli()} pub scan' to see them")
    if without_urls > 0:
        print(f"  • {without_urls} documents missing SharePoint URLs — run '{_brand_cli()} pub onboard' to configure")
    if broken_links > 0:
        print(f"  • {broken_links} broken links — run '{_brand_cli()} pub check-links' to diagnose")
...
def cmd_standards(args: argparse.Namespace) -> None:
    ...
    result = invoke_capability(reg, "press.standards", params, None, surface=CLI_SURFACE)
    if not result.ok:
...
def cmd_do_standard(args: argparse.Namespace) -> None:
    ...
    result = invoke_capability(reg, "press.do_standard", params, None, surface=CLI_SURFACE)
    if not result.ok:
        for e in result.errors:
            print(f"ERROR: {e}", file=sys.stderr)
        sys.exit(1)
...
COMMANDS: dict[str, str] = {
    ...
    "mirror": "Coordinate a doc with a remote editor (web-canonical)",
}
...
def emit_deprecation_warning_if_alias(verb: str) -> None:
    ...
    if verb in DEPRECATED_VERB_ALIASES:
        ...
        print(
            f"{_brand_cli()} pub {verb}: deprecated; use `{_brand_cli()} pub {canonical}`. "
...
def get_parser() -> argparse.ArgumentParser:
    ...
    rev_parser.add_argument("--chat", action="store_true", help=f"Review conversationally via {_brand_cli()} chat")
    ...
    do_parser.add_argument("name", help=f"Standard name (see `{_brand_cli()} pub standards`)")
    ...
    # mirror — coordinate a document with a remote editor (web-canonical)
    mirror_parser = subparsers.add_parser(
        "mirror",
        help="Mirror a document between a remote editor and a local file",
    )
    mirror_sub = mirror_parser.add_subparsers(dest="mirror_command", required=True)
    m_add = mirror_sub.add_parser("add", help="Register a mirror")
    m_add.add_argument("local", help="Local mirror path")
    m_add.add_argument("--url", required=True, help="Remote share URL")
    m_add.add_argument("--name", help="Mirror name (default: file stem)")
    m_add.add_argument("--git-annotate", action="store_true",
                       help="Commit sync events with provenance trailers when the mirror is in a git repo")
    m_sync = mirror_sub.add_parser("sync", help="Reconcile mirrors with their remote editors")
    m_sync.add_argument("name", nargs="?", help="One mirror (default: all)")
    m_sync.add_argument("--watch-once", action="store_true",
                        help="Also detect and repair a stale-session flush")
    mirror_sub.add_parser("status", help="Show mirrors and last-known state")
    m_resolve = mirror_sub.add_parser(
        "resolve",
        help="Clear a blocked conflict by choosing a side (git-style)")
    m_resolve.add_argument("name", help="Mirror name")
    m_strat = m_resolve.add_mutually_exclusive_group(required=True)
    m_strat.add_argument("--theirs", action="store_const", const="theirs",
                         dest="strategy", help="Keep the incoming remote text")
    m_strat.add_argument("--ours", action="store_const", const="ours",
                         dest="strategy", help="Push your preserved local edit")
    m_strat.add_argument("--merged", action="store_const", const="merged",
                         dest="strategy",
                         help="Push whatever you hand-edited the mirror into")
...
def cmd_push(args: argparse.Namespace) -> None:
    ...
    if not args.path:
        print(f"Usage: {_brand_cli()} pub push <path> [--all] [--endpoint onedrive] [--headed]")
        print("\nExamples:")
        print(f"  {_brand_cli()} pub push docs/prds/prd_executive.md")
        print(f"  {_brand_cli()} pub push --all --endpoint onedrive --headed")
        return
...
def _cmd_push_batch(args, engine, draft, storage, headed, force):
    ...
    if single_path and not push_all:
        ...
        for config_path in [
            get_project_state_dir(REPO_ROOT) / "publisher" / "workflow.yaml",
            REPO_ROOT / ".publisher.yaml",
        ]:
            cfg = load_yaml(config_path)
            folders = cfg.get("source_dirs", cfg.get("folders", []))
            if folders:
    ...
    if storage == "box-browser":
        ...
        if not provider.has_session() and not headed:
            ...
            print(f"    {_brand_cli()} pub push --all --endpoint box-browser --headed\n")
            sys.exit(1)
        ...
        if not provider.has_session() and not headed:
            print("\n  No saved session. Run with --headed for first-time login:")
            print(f"    {_brand_cli()} pub push --all --headed\n")
            sys.exit(1)
...
def _docx_output_path(md_path: Path) -> Path:
    ...
    generated = get_project_state_dir(REPO_ROOT) / "generated"
    try:
        rel = md_path.parent.relative_to(REPO_ROOT)
        output_dir = generated / rel
    except ValueError:
...
def _generate_docx(md_path: Path) -> Path | None:
    ...
    # Mirror source folder structure: docs/prds/ → <dot-dir>/generated/requirements/
    generated = get_project_state_dir(REPO_ROOT) / "generated"
    try:
        rel = md_path.parent.relative_to(REPO_ROOT)
        output_dir = generated / rel
    ...
    ref_doc = get_project_state_dir(REPO_ROOT) / "publisher" / "reference.docx"
    if ref_doc.exists():
