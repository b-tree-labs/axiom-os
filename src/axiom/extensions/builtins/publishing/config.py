...
Loads configuration via a multi-tier discovery hierarchy:
  ...
  2. AXIOM_CONFIG environment variable
...
Project root discovery (for state files, registry, docs):
  1. AXIOM_ROOT environment variable
...
def _state_dir(root: Path) -> Path:
    ...
    Always uses the project state directory's ``publisher/`` subdirectory
    to avoid polluting the project root.
    """
    state_dir = get_project_state_dir(root) / "publisher"
...
def _discover_config_path() -> Path | None:
    ...
    1. AXIOM_CONFIG env var
    2. .publisher.yaml in CWD
    3. <dot-dir>/config.yaml in CWD
    4. ~/.config/<cli>/config.yaml
    """
    # Tier 1: env var
    env_config = os.environ.get("AXIOM_CONFIG")
    if env_config:
        p = Path(env_config)
        if p.exists():
            return p
    ...
    # Tier 2: <dot-dir>/publisher/workflow.yaml in project root
    candidate = get_project_state_dir(PROJECT_ROOT) / "publisher" / "workflow.yaml"
    if candidate.exists():
        return candidate

    # Tier 3: <dot-dir>/config.yaml in CWD
    candidate = get_project_state_dir(cwd) / "config.yaml"
    if candidate.exists():
        return candidate

    # Tier 4: user-level config
    from axiom.infra.branding import get_branding
    candidate = Path.home() / ".config" / get_branding().cli_name / "config.yaml"
    if candidate.exists():
        return candidate
