...
_SPECS: tuple[SkillSpec, ...] = (
    ...
    SkillSpec(
        ...
        allowed_tools=(),
        # mcp: external egress (upload + bus announcement), returned to MCP behind the
        # ADR-114 §3 effect gate — the built-in egress_write_mcp_approval floor proposes
        # (requires approval) this capability on the mcp surface only; CLI/agent unchanged.
        surfaces=("cli", "mcp", "agent_tool"),
    ...
    SkillSpec(
        ...
        allowed_tools=(),
        # mcp: external egress (writes the remote document), returned to MCP behind the
        # ADR-114 §3 effect gate (propose-on-mcp floor; see press.publish note above).
        surfaces=("cli", "mcp", "agent_tool"),
