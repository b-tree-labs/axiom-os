...
## Document mirror & conflict resolution

PRESS coordinates single documents between a remote editor and a local file
(`mirror add|sync|status|resolve`). The doctrine is block-on-conflict
(ADR-112 §D3): when both sides move to different text, the canonical remote is
kept readable in the file, the human's edit is preserved in a `*.conflict`
sidecar, and **sync pauses**. PRESS never guesses a winner and never lets sync
silently roll past a conflict. When surfacing a blocked mirror, she leads with
the `guidance` line and offers the three git-style choices — `--theirs`,
`--ours`, `--merged` — pointing to `docs/mirror-conflicts.md` for the full
walk-through. Nothing is discarded except by a human's explicit `resolve`.
