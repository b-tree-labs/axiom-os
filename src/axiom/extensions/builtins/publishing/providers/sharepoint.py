...
Features:
- Interactive device-code auth via MSAL (requires `DOCFLOW_CLIENT_ID` and `DOCFLOW_TENANT_ID`, or the project docflow/config.json)
- Download a shared document URL via Microsoft Graph `/shares/{shareId}/driveItem/content`
- Save .docx to the project `downloads/` directory and optionally convert to Markdown using `pandoc` if available

Usage:
    from axiom.extensions.builtins.publishing.providers.sharepoint import SharePointProvider
    p = SharePointProvider()
    p.pull(sharepoint_url, Path('docs/prds/mydoc.md'))
...
def _docflow_dir() -> Path:
    """Project-local docflow directory, resolved against the active branding."""
    return get_project_state_dir(Path.cwd()) / 'docflow'


def _config_path() -> Path:
    """SharePoint client credentials file."""
    return _docflow_dir() / 'config.json'


def _token_cache_path() -> Path:
    """MSAL serialized token cache."""
    return _docflow_dir() / 'token_cache.json'


def _downloads_dir() -> Path:
    """Where pulled .docx files land."""
    return get_project_state_dir(Path.cwd()) / 'downloads'

# MSAL adds the reserved scopes (openid, offline_access, profile) itself and
# refuses a scopes list that names them — refresh tokens still arrive.
SCOPES = [
    'Files.ReadWrite.All',
    'User.Read',
]
...
class SharePointProvider:
    ...
    def __post_init__(self):
        ...
        if not self.client_id or not self.tenant_id:
            raise RuntimeError(
                'Missing DOCFLOW_CLIENT_ID or DOCFLOW_TENANT_ID. '
                f'Add to {_config_path()} or env vars.'
            )
        ...
        if _token_cache_path().exists():
            try:
                self.token_cache.deserialize(_token_cache_path().read_text())
            except Exception:
    ...
    def pull(self, sharepoint_url: str, local_path: Path) -> Path:
        ...
        tmp_path = _downloads_dir() / docx_name
...
# ---------------------------------------------------------------------------
# Remote-editor endpoint (mirror engine transport) — see ../mirror.py
# ---------------------------------------------------------------------------

from collections.abc import Callable  # noqa: E402

from axiom.extensions.builtins.publishing.mirror import (  # noqa: E402
    RemoteDoc,
    VersionConflict,
)
...
@dataclass
class GraphEditorEndpoint:
    """A shared document as a versioned read/write endpoint over Graph.

    The transport (``session``) and credential (``token_getter``) are
    injected, so the class is testable without a network and usable with
    either the interactive MSAL flow (``from_share_url``) or any other
    token source. Writes carry ``If-Match`` with the eTag they expect, so
    a concurrent save on the remote side surfaces as
    ...
    def __post_init__(self):
        ...
        self._item = f"{GRAPH_BASE}/shares/{SharePointProvider._make_share_id(self.share_url)}/driveItem"

    @classmethod
    def from_share_url(cls, share_url: str) -> GraphEditorEndpoint:
        provider = SharePointProvider()

        def _token() -> str:
            # _acquire_token_interactive already returns the bearer string
            return provider._acquire_token_interactive()

        return cls(share_url=share_url, token_getter=_token)
    ...
    @staticmethod
    def _author_app(meta: dict) -> str:
        by = meta.get("lastModifiedBy", {})
        return (by.get("application", {}) or {}).get("displayName", "") or (
            by.get("user", {}) or {}
        ).get("displayName", "")

    def read(self) -> RemoteDoc:
        meta = self.session.get(
            f"{self._item}?$select=eTag,lastModifiedBy",
            headers=self._headers(),
        )
        meta.raise_for_status()
        m = meta.json()
        content = self.session.get(
            f"{self._item}/content", headers=self._headers()
        )
        content.raise_for_status()
        return RemoteDoc(
            text=content.text,
            version=m["eTag"],
            author_app=self._author_app(m),
        )

    def write(self, text: str, expected_version: str) -> str:
        r = self.session.put(
            f"{self._item}/content",
            headers=self._headers({"If-Match": expected_version}),
            data=text.encode("utf-8"),
        )
        if r.status_code == 412:
            raise VersionConflict(expected_version, "remote-changed")
        r.raise_for_status()
        return r.json()["eTag"]

    def versions(self, limit: int = 10) -> list[RemoteDoc]:
        r = self.session.get(
            f"{self._item}/versions?$top={limit}", headers=self._headers()
        )
        r.raise_for_status()
        return [
            RemoteDoc(
                text="",
                version=item.get("id", ""),
                author_app=self._author_app(item),
            )
