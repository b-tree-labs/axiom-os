...
Configuration:
- Environment variables OR the project state directory's docflow/config.json:
  {
    "box": {
      "client_id": "...",
      "client_secret": "...",
...
Usage:
    from axiom.extensions.builtins.publishing.providers.box import BoxProvider
    provider = BoxProvider()

    # Upload
    result = provider.upload(
        local_path=Path("docs/prds/my-prd.docx"),
...
def _config_path() -> Path:
    """Project-local Box credentials file, resolved against the active branding."""
    return get_project_state_dir(Path.cwd()) / "publisher" / "config.json"
    ...
    def __post_init__(self):
        ...
        if not self.client_id or not self.client_secret:
            raise RuntimeError(
                ...
                f"Add to {_config_path()} or set environment variables."
            )
