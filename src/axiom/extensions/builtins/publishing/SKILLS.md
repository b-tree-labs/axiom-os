...
### Document Mirror (web-canonical sync + conflict resolution)
- Keep one local file in step with a remote editor (SharePoint/OneDrive, or a shared-drive path via the local editor). Directionality follows the human: `axi pub mirror add|sync|status|resolve`.
- One side canonical per mode; every push carries the version it expects, so no edit is ever overwritten blind.
- **Block-on-conflict doctrine (ADR-112 §D3).** When both sides change to different text between syncs, the mirror lands the canonical remote text in the file (kept readable — never conflict markers), preserves your edit in a `*.conflict` sidecar, and **pauses sync until it is resolved**. This is git's essence made safe for a live byte-mirror.
- **Resolving.** `axi pub mirror resolve <name> --theirs|--ours|--merged` — keep the remote, push your preserved edit, or push a hand-merged file. A remote that moved on re-blocks rather than being clobbered. Full guide: `docs/mirror-conflicts.md`.
- When surfacing a blocked mirror to a human, lead with the `guidance` string `status`/`sync` return — it names where the edit was kept and the three choices.
