...
def build_parser() -> argparse.ArgumentParser:
    ...
    proj = sub.add_parser(
        "project-openwebui",
        help="Upsert a command catalog into an Open WebUI prompt library",
    )
    proj.add_argument(
        "--db", type=Path, required=True, help="Path to Open WebUI's webui.db"
    )
    proj.add_argument(
        "--catalog",
        type=Path,
        required=True,
        help="Path to the command-catalog markdown (### /cmd — Title sections)",
    )
    proj.add_argument(
        "--owner",
        default=None,
        help="User id to own the prompts (default: the first admin user)",
    )
    proj.add_argument(
        "--private",
        action="store_true",
        help="Do NOT grant public read (owner-only visibility)",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.action == "project-openwebui":
        return _cmd_project_openwebui(args)
    return 1
...
def _cmd_project_openwebui(args: argparse.Namespace) -> int:
    """Project a site's command catalog into an Open WebUI prompt library.

    Thin wrapper over the tested projector: parse the catalog, upsert the rows
    (valid JSON + is_active + public grant), report the counts.
    """
    from .openwebui_projector import parse_command_catalog, project_prompts

    if not args.catalog.exists():
        print(f"catalog not found: {args.catalog}", file=sys.stderr)
        return 2
    if not args.db.exists():
        print(f"Open WebUI database not found: {args.db}", file=sys.stderr)
        return 2

    entries = parse_command_catalog(args.catalog.read_text(encoding="utf-8"))
    if not entries:
        print(f"no `### /command — Title` sections found in {args.catalog}", file=sys.stderr)
        return 1
    try:
        res = project_prompts(
            args.db, entries, owner_user_id=args.owner, public=not args.private
        )
    except ValueError as exc:
        print(f"projection failed: {exc}", file=sys.stderr)
        return 2
    print(
        f"open-webui prompts: {res.inserted} inserted, {res.updated} updated, "
        f"{res.total} total"
    )
    return 0
