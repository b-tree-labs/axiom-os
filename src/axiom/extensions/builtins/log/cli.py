...
"""CLI handler for `axi log` — log inspection, verification, and management.

Usage:
    axi log tail [--follow] [--ec] [--level LEVEL] [--provider PROVIDER] [--n N]
    axi log verify [--table TABLE] [--since DAYS]
    axi log stats
    axi log backend
    axi log sinks
    axi log export [--since DAYS]
"""
...
def _cmd_tail(args: argparse.Namespace) -> int:
    ...
    if path is None:
        print(f"{_brand_cli()} log: routing_events log file not found", file=sys.stderr)
        return 1
...
def _cmd_verify(args: argparse.Namespace) -> int:
    try:
        ...
        print(f"{_brand_cli()} log verify: error calling AuditLog.verify_chain: {e}", file=sys.stderr)
        return 1
...
def _cmd_backend(_args: argparse.Namespace) -> int:
    try:
        ...
        print(f"{_brand_cli()} log backend: {e}", file=sys.stderr)
        return 1
    return 0
...
def _cmd_sinks(_args: argparse.Namespace) -> int:
    try:
        from axiom.infra.log_sinks import LogSinkFactory
        sinks = LogSinkFactory.available()
    except (ImportError, AttributeError, TypeError) as e:
        print(f"{_brand_cli()} log sinks: {e}", file=sys.stderr)
        return 1
...
def _cmd_export(args: argparse.Namespace) -> int:
    path = _routing_events_path()
    if path is None:
        print(f"{_brand_cli()} log export: routing_events log file not found", file=sys.stderr)
        return 1
...
def get_parser() -> argparse.ArgumentParser:
    """Build and return the argument parser for `axi log`."""
    parser = argparse.ArgumentParser(
        prog=f"{_brand_cli()} log",
        description="Log inspection, verification, and management",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
Examples:
  {_brand_cli()} log tail                      # Print last 20 routing events
  {_brand_cli()} log tail --follow             # Stream new events as they arrive
  {_brand_cli()} log tail --ec --n 50          # Last 50 EC-flagged events
  {_brand_cli()} log tail --level warning      # Events at WARNING level or above
  {_brand_cli()} log tail --provider llama-internal
  {_brand_cli()} log verify                    # Verify audit chain integrity
  {_brand_cli()} log verify --table routing    # Verify a specific table
  {_brand_cli()} log stats                     # Show backend info and counts
  {_brand_cli()} log backend                   # Print current backend name
  {_brand_cli()} log sinks                     # List active log sinks
  {_brand_cli()} log export                    # Dump all routing_events to stdout
  {_brand_cli()} log export --since 7          # Events from the last 7 days
""",
    )
...
def register(subparsers: argparse._SubParsersAction) -> None:
    ...
    Consumer CLI discovery calls this when noun="log" is declared in
    axiom-extension.toml and the dispatcher uses a register-based pattern.
    Adds a `log` sub-parser that delegates to the full `axi log` parser.
    """
...
def main(argv: list[str] | None = None) -> int:
    """Entry point called by the CLI dispatcher via mod.main()."""
    parser = get_parser()
