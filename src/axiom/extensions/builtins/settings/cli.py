...
from axiom.extensions.builtins.settings.reconcile import (
    AUTONOMY_KEY,
    reconcile_autonomy_registration,
)
...
def cmd_settings_list(
    ...
    if not visible:
        ...
        console.print(
            f"  Run [cyan]{_brand_cli()} settings --all[/cyan] to include unconfigured sections.\n"
        )
        return 0
    ...
    console.print()
    console.print(
        f"  [cyan]{_brand_cli()} settings <section>[/cyan]  [dim]drill in to view or edit[/dim]"
    )
    if not show_all:
        console.print(
            f"  [cyan]{_brand_cli()} settings --all[/cyan]      [dim]include unconfigured sections[/dim]"
        )
    console.print()
    return 0
...
def cmd_settings_view(view: SectionView) -> int:
    ...
    console.print()
    console.print(
        f"  [cyan]{_brand_cli()} settings {view.name} set <key> <value>[/cyan]  [dim]edit a key[/dim]"
    )
    console.print(
        f"  [cyan]{_brand_cli()} settings {view.name} reset <key>[/cyan]        [dim]remove an override[/dim]\n"
    )
    return 0
...
def get_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=f"{_brand_cli()} settings",
        description="View and edit axi configuration",
    )
    parser.add_argument(
        "--global", dest="global_scope", action="store_true",
        help="Operate on global settings (the user state directory's settings.toml)",
    )
...
def main():
    ...
    if args.cmd == "set":
        ...
        from .store import _PROJECT_SETTINGS_PATH, _get_global_settings_path

        target = (
            _get_global_settings_path() if scope == "global" else _PROJECT_SETTINGS_PATH
        )
        print(f"  {args.key} = {_fmt_value(value)}  →  {target}")
        # autonomy.enabled is enforced at OS-timer registration as well as at
        # runtime, so writing it is not enough to make it true. Re-run
        # registration here or the operator gets a setting that reads `true`
        # with nothing installed to act on it.
        if args.key == AUTONOMY_KEY:
            results = reconcile_autonomy_registration(bool(value))
            if results is not None:
                print("  registered the background service so heartbeats can dispatch")
        return
