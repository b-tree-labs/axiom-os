...
def build_parser() -> argparse.ArgumentParser:
    ...
    resume_p = sub.add_parser(
        "resume",
        help=f"Lift a pause sentinel set by `{_brand_cli()} rivet pause`",
    )
...
def _cmd_pause(args: argparse.Namespace) -> int:
    ...
    print(f"  resume with: {_brand_cli()} rivet resume --scope {args.scope}")
    return 0
...
def _cmd_status(args: argparse.Namespace) -> int:
    ...
    if status["all_green"]:
        ...
        print(f"Build issues detected. Run `{_brand_cli()} release check` for diagnostics.")
...
def _cmd_watch(args: argparse.Namespace) -> int:
    if getattr(args, "watch_kind", None) != "routine":
        print(f"usage: {_brand_cli()} rivet watch routine <trigger_id> --branch <name>")
        return 1
...
def _cmd_plan(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "format", "human") == "json":
        ...
        print(f"No release milestones. Run `{_brand_cli()} release plan --add <version>` to create one.")
    else:
        print("Release Plan:\n")
        for m in milestones:
