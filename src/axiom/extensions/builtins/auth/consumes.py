...
def _idp_for(decl: dict, tenant: Optional[str]):
    from axiom.extensions.builtins.auth.providers import get_idp

    # Resolve through the open IdP registry (ADR-075 §2): entra pulls a tenant,
    # google needs nothing, a registered issuer pulls its own config. No caller
    # hardcodes a provider list or a default.
    return get_idp(decl["idp"], tenant=tenant or decl.get("tenant"),
                   issuer=decl.get("issuer"))
