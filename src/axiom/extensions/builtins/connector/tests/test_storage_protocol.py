...
def test_stored_bytes_version_field_is_named_version_across_families():
    """ADR-112 §D4: the connector file-store's PutReceipt and the object-store's
    UploadResult name the 'which stored bytes' field identically — `version`.
    The two storage families stay separate; only the field name is shared. This
    guards the convergence against a drift back to `revision`."""
    import dataclasses

    from axiom.infra.storage import UploadResult

    put_fields = {f.name for f in dataclasses.fields(PutReceipt)}
    upload_fields = {f.name for f in dataclasses.fields(UploadResult)}
    assert "version" in put_fields, "PutReceipt must expose `version`"
    assert "revision" not in put_fields, "the old `revision` name must be gone"
    assert "version" in upload_fields, "UploadResult must expose `version`"

    # and it round-trips as a real field
    ref = FileRef(vendor="local", path="/x")
    assert PutReceipt(ref=ref, version="42").version == "42"
    assert PutReceipt(ref=ref).version is None  # optional: absent when unversioned
