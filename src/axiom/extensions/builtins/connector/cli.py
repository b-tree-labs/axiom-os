    ...
    if "connectors" in value and "reconnect_pending_count" in value:
        ...
        for c in connectors:
            ...
            if c.get("status_code"):
                note = f"status={c['status_code']}"
            if c.get("error"):
                note = f"{note}  ({c['error']})" if note else f"({c['error']})"
            rows.append((marker, c["connector"], str(c["observed_at"]), note))
        # `{connector:<14}` pads to a guess; an error message appended to the
        # end of a padded line runs off the terminal with nothing to catch it.
        for line in table(
            rows,
            [Column(""), Column("connector"), Column("observed"), Column("note", wrap=True)],
