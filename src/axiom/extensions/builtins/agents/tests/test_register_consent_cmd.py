...
def isolate(tmp_path, monkeypatch):
    ...
    # Both: consent_path resolves the PLATFORM directory, so patching only the
    # branded one leaves these tests reading the operator's real ~/.axi record.
...
def test_help_points_at_the_onoff_readme():
    from axiom.extensions.builtins.agents.cli import build_parser

    help_text = build_parser().format_help()
    assert "README.md" in help_text
    assert "on/off runbook" in help_text
