...
def register_all_daemon_agents() -> list[RegistrationResult]:
    ...
    """
    # Master autonomy gate. When OFF (the default), never write an OS timer —
    # so a fresh install registers nothing and macOS shows no Login-Items /
    # "Background Items Added" pop-up. A surviving timer from a prior install
    # is separately neutered at runtime (background_service_main). The
    # onboarding interview / `axi settings set autonomy.enabled true` flips it.
    if not autonomy_enabled():
        return [
            RegistrationResult(
                ...
                registered=False,
                started=False,
                error="skipped: autonomy disabled (set autonomy.enabled true to enable)",
            )
        ]
    ...
    try:
        ...
        results.append(
            RegistrationResult(
                ...
                else "background-service install or start returned False",
            )
        )
...
def _cleanup_legacy_launchd() -> list[RegistrationResult]:
    ...
    # Pass 2: every Background Service plist that is not the current brand's.
    #
    # Driven by a filesystem glob, NOT by the discovered portfolio list. A plist
    # left behind by a package that has since been *renamed or uninstalled* can
    # never appear in `portfolio_pkgs` — entry-points discovery only sees what is
    # installed now — so the previous package-enumerated pass could not reach it
    # and it double-dispatched forever. Observed in the wild: a
    # `com.axiom-os.background-service.plist` from the pre-rename package kept
    # ticking alongside `com.axiom-os-lm.…` for ~4 months, so every heartbeat ran
    # twice from two different interpreters.
    #
    # Exactly one Background Service may own this slot, so anything else matching
    # the pattern is stale by construction. The binary-name guard keeps the glob
    # from touching an unrelated vendor's `com.*.background-service.plist`.
    known_packages = _portfolio_package_names()
    for plist_path in sorted(launch_agents_dir.glob("com.*.background-service.plist")):
        if plist_path.name == current_bg_plist:
            continue  # the survivor
        if not _is_portfolio_background_service(plist_path, known_packages):
            continue  # not ours — leave it alone
        results.extend(_unload_and_remove_plist(plist_path))

    _ = portfolio_pkgs  # still used by pass 1
    _ = subprocess  # used inside the helper
    return results


def _portfolio_package_names() -> set[str]:
    """Package names of portfolio members visible right now.

    Only ever a *positive* signal. A package absent here may simply have been
    renamed — which is the very case this cleanup exists for — so absence
    proves nothing and is never on its own grounds for deletion.
    """
    from axiom.infra import branding as _branding

    names: set[str] = set()
    try:
        for member in _branding.discover_portfolio_members():
            pkg = getattr(member, "package_name", None)
            if pkg:
                names.add(str(pkg))
    except Exception:  # noqa: BLE001 - discovery is best-effort
        pass
    try:
        current = getattr(_branding.get_branding(), "package_name", None)
        if current:
            names.add(str(current))
    except Exception:  # noqa: BLE001
        pass
    return names


def _is_portfolio_background_service(plist_path, known_packages=None) -> bool:
    """Whether this Background Service plist is one of ours.

    Two independent positive signals, because neither covers the other's case:

    1. **The plist launches one of our wrapper binaries.** Read as bytes and
       matched on the executable name, so the check survives plistlib,
       XML-vs-binary format, and whichever interpreter installed it. This is
       the signal that catches a *renamed or uninstalled* package, whose name
       can no longer be discovered at all.

    2. **The label's package segment is a portfolio package we can see.** This
       is the signal that catches a stale plist left by another brand in the
       portfolio, whose body may not name a wrapper binary.

    Conservative when neither fires: an unreadable plist, or one belonging to
    an unrecognised vendor, is left alone rather than removed.
    """
    try:
    ...
    if b"Background-Service" in blob:
        return True

    name = getattr(plist_path, "name", "")
    prefix, suffix = "com.", ".background-service.plist"
    if not (name.startswith(prefix) and name.endswith(suffix)):
        return False
    package = name[len(prefix) : -len(suffix)]
    if known_packages is None:
        known_packages = _portfolio_package_names()
    return package in known_packages
...
def _make_background_service_manager():
    ...
    if binary_path is None:
        ...
        if fallback_path is not None:
            # DEBUG: on any install where the brand ships no wrapper of its
            # own this is the normal path, and it fired on every agents
            # command. The condition worth a warning is having no service at
            # all, which the status line already reports.
            log.debug(
    ...
    # The service_id has to name the service that is actually REGISTERED.
    # It was built from the active brand's package while the binary fell back
    # to the platform's, so `neut agents status` looked up
    # a service id under the consumer's package — never installed — and said "not
    # installed" plus "nothing will dispatch", on a machine where the Axiom
    # service was running and dispatching all six agents. Following the
    # binary keeps the identity and the thing it manages in agreement.
    pkg = get_branding().package_name
    if used_fallback:
        from axiom.infra.branding import BrandingConfig

        pkg = BrandingConfig.package_name
...
def autonomy_status_lines(autonomy_on: bool) -> list[str]:
    """The master gate, stated before anything else — in BOTH states.

    ``autonomy_registration_warning`` below shouts about the one incoherent
    pairing (on, but no timer). This line exists for the coherent-but-silent
    one: autonomy OFF while the screen reads "running", lists consented agents
    and their intervals, and shows last-run times — every line true, the whole
    implying that dispatch is happening. It is the first gate; consent is the
    second. A reader who does not know the first exists cannot explain why
    nothing has run for a month.
    """
    cli = _cli_name()
    if autonomy_on:
        return ["  Autonomy:  \033[32mON\033[0m — due agents below are dispatched"]
    return [
        "  Autonomy:  \033[33mOFF\033[0m — nothing dispatches, whatever "
        "consent or intervals say",
        f"             turn on with `{cli} settings --global set "
        "autonomy.enabled true`",
        "             global, not project: the service runs with no "
        "working directory",
    ]


def autonomy_registration_warning(autonomy_on: bool, service_status: str) -> str | None:
    """Warn when the setting says yes but nothing is registered to act on it.

    Autonomy is gated at runtime *and* at OS-timer registration. The pairing
    "enabled, but no timer" produces total silence: no dispatch, no error, no
    clue. That combination is the one state worth shouting about — every other
    pairing is coherent.
    """
    if autonomy_on and service_status == "not_installed":
        return (
            "autonomy is enabled but no background service is registered — "
            "nothing will dispatch. Run `axi agents register` to install the timer."
        )
    return None
...
def _cmd_status(args) -> int:
    ...
    autonomy_on = autonomy_enabled()
    for line in autonomy_status_lines(autonomy_on):
        print(line)

    warning = autonomy_registration_warning(autonomy_on, coord_info.status)
    if warning:
        print(f"  \033[33mWARNING:\033[0m   {warning}")
    print()
    ...
    for ext in agents:
        ...
        gated = approved is not None and ext.agent.is_registrable and ext.name not in approved
        note = "  \033[90m(not enabled)\033[0m" if gated else ""
        print(f"  {ext.name:<20} {startup:<10} {interval:<10} {last_run_str:<20}{note}")
...
def _cmd_info(args) -> int:
    ...
    name = getattr(args, "name", None)
    agents = [ext for ext in _discover_agent_extensions() if ext.agent and ext.agent.is_registrable]
    if name:
...
def _interactive_select(candidates: list[Extension]) -> tuple[list[str], bool]:
    ...
    print("Registering installs an OS task that survives reboots and runs on your host.")
    while True:
        raw = input("Enable which? [a]ll / [n]one / numbers e.g. 1,3 / ?N for details: ").strip()
        if raw.startswith("?"):
            target = raw[1:].strip()
...
def run_interactive_registration() -> int:
    ...
    """
    candidates = [
        ext for ext in _discover_agent_extensions() if ext.agent and ext.agent.is_registrable
    ]
    if not candidates:
...
def _cmd_unregister(args) -> int:
    ...
    for r in cleanup:
        print(
            f"  Cleaned up legacy unit: {r.agent_name[16:-1] if r.agent_name.startswith('<legacy-cleanup:') else r.agent_name}"
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=f"{_cli_name()} agents",
        description="Manage always-on agent services",
        epilog=(
            "Full on/off runbook (two gates, verify, field notes): "
            f"{Path(__file__).with_name('README.md')}"
        ),
    )
    ...
    info_p = sub.add_parser("info", help="Describe agents (what they do) before enabling them")
