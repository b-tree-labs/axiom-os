...
def test_maybe_default_authz_hook_dev_mode_permits(monkeypatch):
    """In dev mode ON A LOOPBACK BIND the default hook resolves a dev
    principal and permits — the dev convenience is loopback-only."""
    ...
    hook = ah.maybe_default_authz_hook(loopback=True)
    assert hook is not None
    decision = hook(_fake_request(headers={}))
    assert decision.allow is True


def test_maybe_default_authz_hook_dev_nonloopback_no_creds_is_none(monkeypatch):
    """Dev mode with no credentials and an unknown/network bind wires NOTHING
    — compose_app then refuses auth-required mounts (SRV-022). Dev is the
    default mode on an unconfigured box; a network serve must never inherit
    anonymous permit-all from a default."""
    from axiom.extensions.builtins.http import authz_hook as ah

    monkeypatch.setenv("AXIOM_MODE", "dev")
    monkeypatch.delenv("AXIOM_API_KEY", raising=False)
    monkeypatch.delenv("AXIOM_HTTP_API_KEYS", raising=False)
    monkeypatch.delenv(ah.API_KEYS_FILE_ENV, raising=False)

    assert ah.maybe_default_authz_hook() is None
    assert ah.maybe_default_authz_hook(loopback=False) is None


def test_maybe_default_authz_hook_dev_nonloopback_with_creds_requires_bearer(
        monkeypatch):
    """Dev mode + network bind + configured credentials: the hook wires the
    real resolvers with NO anonymous fallback — a bare request denies, the
    configured bearer passes."""
    from axiom.extensions.builtins.http import authz_hook as ah

    monkeypatch.setenv("AXIOM_MODE", "dev")
    monkeypatch.setenv("AXIOM_API_KEY", "sekrit-token")
    monkeypatch.delenv("AXIOM_HTTP_API_KEYS", raising=False)
    monkeypatch.delenv(ah.API_KEYS_FILE_ENV, raising=False)

    hook = ah.maybe_default_authz_hook(loopback=False)
    assert hook is not None
    anonymous = hook(_fake_request(headers={}))
    assert anonymous.allow is False
    authed = hook(_fake_request(
        headers={"authorization": "Bearer sekrit-token"}))
    assert authed.allow is True
