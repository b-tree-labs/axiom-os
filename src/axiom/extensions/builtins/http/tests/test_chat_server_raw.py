...
def _raw_bypass_enabled(monkeypatch):
    """These tests exercise the raw bypass, so they must declare it.

    The bypass is a property of the DEPLOYMENT, refused otherwise — it skips
    retrieval, the system prompt and tool routing, so an authenticated caller
    asking for it is not enough. Every test here therefore stands up a
    deployment that permits it; the refusal path is covered in
    tests/chaos/test_raw_bypass_gate.py.
    """
    monkeypatch.setenv(chat_server.RAW_BYPASS_ENV, "1")
