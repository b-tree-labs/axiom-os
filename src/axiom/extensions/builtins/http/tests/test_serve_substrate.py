...
def test_compose_auto_wires_default_authz_hook_in_dev(monkeypatch):
    """With ``auto_authz`` (the default), the dev anonymous convenience is
    LOOPBACK-ONLY: a loopback bind serves the auth-required mount permit-all;
    an unknown or network bind is fail-closed (dev is the default mode on an
    unconfigured box — a network serve must never inherit anonymous
    permit-all). ``AXIOM_SERVE_INSECURE`` stays the explicit escape, and
    production with no keys refuses on any bind."""
    from fastapi import APIRouter
    from fastapi.testclient import TestClient
    ...
    # Dev + loopback bind → adapter wires permit-all + dev principal → served.
    app_lo = compose_app(registry=_reg(), include_builtins=False,
                         bind_host="127.0.0.1")
    assert TestClient(app_lo).get("/secure").status_code == 200

    # Dev + unknown bind (no bind_host) → fail-closed refusal.
    app_unknown = compose_app(registry=_reg(), include_builtins=False)
    assert TestClient(app_unknown).get("/secure").status_code == 404, (
        "dev anonymous permit-all leaked to an unknown bind — fail-OPEN hole"
    )

    # Dev + network bind → fail-closed refusal.
    app_net = compose_app(registry=_reg(), include_builtins=False,
                          bind_host="0.0.0.0")
    assert TestClient(app_net).get("/secure").status_code == 404

    # Dev + network bind + explicit insecure opt-out → served (audited escape).
    monkeypatch.setenv("AXIOM_SERVE_INSECURE", "1")
    app_esc = compose_app(registry=_reg(), include_builtins=False,
                          bind_host="0.0.0.0")
    assert TestClient(app_esc).get("/secure").status_code == 200
    monkeypatch.delenv("AXIOM_SERVE_INSECURE", raising=False)

    # Production with no keys → adapter returns None → refusal on any bind.
