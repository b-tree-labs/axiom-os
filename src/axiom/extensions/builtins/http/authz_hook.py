...
from .actor import ActorResolver, build_actor_resolver, slug
...
class ResolvedCredential:
    ...
    """Verified claims behind the credential (a webauth session's, an API key's
    identity). What the actor resolver reads — never client-supplied headers."""
    posture: str = "service"
    """ADR-077 posture the credential proves: ``service`` for keys, ``attested``
    for a password session, ``sso`` for an IdP-backed one."""
...
def _handle_of(principal: object) -> str:
    """The ADR-020 handle for a resolved principal, or ``""``.

    Defensive: an identity provider that returns something unexpected must
    yield NO identity rather than a wrong one, since the value becomes the key
    a conversation is filed under.
    """
    for attribute in ("handle", "id", "name"):
...
def build_authz_hook(
    *,
    ...
    resolve_actor_context: ActorResolver | None = None,
) -> AuthzHook:
    ...
    ``resolve_actor_context`` (ADR-084 × ADR-103) turns the credential's
    verified claims into the ``ActorContext`` + ``SubjectContext`` that ride
    the envelope — roles, tenant, assurance, groups as contextual tuples — so
    ``decide()`` finally sees who is acting. Defaults to claims-only
    resolution; ``maybe_default_authz_hook`` wires the directory seam from
    the environment. It never raises; failure resolves to an actor with no
    roles.
    """
    actor_resolver = resolve_actor_context or build_actor_resolver()
    ...
    def hook(request: object) -> AuthzDecision:
        ...
        if isinstance(resolved, ResolvedCredential):
            ...
            credential_id, credential = resolved.credential_id, resolved
        else:
            principal, scopes, credential_id, credential = resolved, (), None, None
        actor = None
        try:
            ...
            if scopes:
                ...
                envelope = replace(
                    ...
                    capability=_scoped_capability(principal, patterns,
                                                  credential_id))
            actor, subject = actor_resolver(request, principal, credential)
            envelope = replace(envelope, actor_context=actor, subject=subject)
        ...
        # Attach the resolved principal + actor for downstream handlers.
        state = getattr(request, "state", None)
        if state is not None:
            state.principal = principal
            state.actor = actor
        if verdict.next_action_for_caller is NextAction.PROCEED:
            return AuthzDecision(allow=True, principal=_handle_of(principal))
...
SESSION_CONTEXT_ENV = "AXIOM_GATE_PRINCIPAL_CONTEXT"
"""The ``:context`` half of a session principal's ``@name:context`` handle —
the site/tenant a browser login belongs to. Default ``gate``."""


def _session_issuer(request: object) -> str | None:
...
def build_session_resolver(*, context: str = "gate",
                           issuer: str | None = None) -> PrincipalResolver:
    """Resolve the gate's ``webauth`` session cookie to a principal.

    The cookie is the ES256 session the ``webgate`` login (password or OIDC)
    minted on this node; verification is by signature + ``type=session``, so an
    access/refresh token cannot stand in. The principal handle is
    ``@<slug(sub)>:<context>`` and the credential carries the session's verified
    claims (``sub``, ``email``, ``roles``, ``idp``) plus a posture of ``sso``
    when an IdP backed the login, else ``attested``. No cookie → ``None``, so
    a chained bearer resolver gets its turn.
    """
    from axiom.webauth import session_from_cookies

    def resolve(request: object) -> ResolvedCredential | None:
        cookies = getattr(request, "cookies", None) or {}
        if not cookies:
            return None
        claims = session_from_cookies(cookies, issuer=issuer or _session_issuer(request))
        if not claims or not claims.get("sub"):
            return None
        handle = f"@{slug(str(claims['sub']))}:{slug(context)}"
        return ResolvedCredential(
            principal=_principal_from_handle(handle),
            scopes=(),
            credential_id=f"session:{claims['sub']}",
            claims=dict(claims),
            posture="sso" if claims.get("idp") else "attested",
        )

    return resolve


def chain_resolvers(*resolvers: PrincipalResolver) -> PrincipalResolver:
    """First resolver to return a credential wins; all ``None`` → ``None``.

    An exception in any resolver propagates — the hook turns it into a
    fail-closed deny rather than letting a later resolver quietly succeed.
    """

    def resolve(request: object):
        for r in resolvers:
            found = r(request)
    ...
    def resolve(request: object) -> Principal | ResolvedCredential | None:
        ...
        if token.startswith(TOKEN_PREFIX):
            ...
            return ResolvedCredential(
                ...
                claims={"sub": identity.principal, "key_id": identity.key_id,
                        "name": identity.name},
                posture="service",
            )
...
def maybe_default_authz_hook(*, loopback: bool = False) -> AuthzHook | None:
    """Wire the real authz engine + credential resolvers, or return ``None``.

    Returns ``None`` (so ``compose_app`` stays fail-closed and refuses
    auth-required mounts) when there is nothing safe to wire: no configured
    credentials (neither env-registry tokens nor an issued-keys file) — in
    production, staging, OR a dev serve that is not known to be loopback.

    The dev conveniences — an anonymous dev principal (and the permit-all
    ``DecideContext`` fallback when no engine is wired) — are gated on
    ``loopback=True``: dev mode is the *default* when ``AXIOM_MODE`` is
    unset, so an unset box binding a network interface must never serve
    auth-required mounts anonymously (SRV-022 stays fail-closed). Callers
    that know their bind address pass ``loopback`` accordingly
    (``compose_app(bind_host=...)`` classifies it); unknown means ``False``.
    A dev serve on a network bind with credentials configured wires the real
    resolvers with NO anonymous fallback — bearer required.
    """
    ...
    dev_open = dev and loopback
    registry = _load_token_registry()
    api_keys = _load_api_key_store()

    if not dev_open and not registry and api_keys is None:
        _LOGGER.warning(
            "authz seam not wired: no AXIOM_HTTP_API_KEYS / AXIOM_API_KEY / "
            f"{API_KEYS_FILE_ENV} configured and %s — auth-required mounts "
            "will be refused (fail-closed). Configure credentials to enable "
            "the gateway, or bind loopback for the dev convenience.",
            "not in dev mode" if not dev
            else "the bind is not loopback (dev anonymous access is "
                 "loopback-only)")
        return None
    ...
    if authz_ctx is None:
        ...
        # loopback serves anonymously; a network dev bind reaches here only
        # with credentials configured, so authn still gates every request.
        if not dev:
            return None
    ...
    anon = get_current_actor(dev_mode=True).handle if dev_open else None
    bearer = build_bearer_resolver(registry, api_keys=api_keys,
                                   anonymous_handle=anon)
    # A browser that logged in at the gate carries its session cookie to the
    # composed app too; honour it ahead of bearer so humans and keys share one
    # decision path. The bearer resolver's dev-anonymous fallback still only
    # applies when neither credential is present.
    session = build_session_resolver(
        context=os.environ.get(SESSION_CONTEXT_ENV, "gate") or "gate")
    from .actor import actor_resolver_from_env

    return build_authz_hook(
        resolve_principal=chain_resolvers(session, bearer),
        decide_fn=decide_fn,
        resolve_actor_context=actor_resolver_from_env(),
    )
