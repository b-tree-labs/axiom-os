...
def _default_manifest_dirs() -> list:
    """Every extension directory discovery knows about — builtins,
    project, user, and INSTALLED PACKAGES (the separate-instance
    consumer seam: a domain layer or appkit lands here via pip)."""
    from axiom.extensions.discovery import get_extension_dirs

    return list(get_extension_dirs())


def discovered_manifests(*search_dirs) -> list[dict]:
    """Raw manifest dicts for every discoverable extension.

    The serve path feeds these to :func:`discover_mounts` so a bare node
    composes manifest-declared service mounts from installed packages
    with no site-repo hand-registration. ``search_dirs`` overrides the
    default discovery roots (tests, embedders)."""
    from axiom.infra.toml_compat import tomllib

    dirs = list(search_dirs) if search_dirs else _default_manifest_dirs()
    manifests: list[dict] = []
    for d in dirs:
        root = Path(d)
        if not root.is_dir():
            continue
        # A root may BE an extension dir or CONTAIN extension dirs.
        candidates = [root, *sorted(p for p in root.iterdir() if p.is_dir())]
        for ext_dir in candidates:
            for name in ("axiom-extension.toml", "neut-extension.toml"):
                mf = ext_dir / name
                if mf.is_file():
                    try:
                        manifests.append(tomllib.loads(mf.read_text(encoding="utf-8")))
                    except Exception:
                        _LOGGER.warning("unparseable manifest skipped: %s", mf)
                    break
    return manifests
...
#: Hosts whose bind means "reachable from this machine only" — the only case
#: where the dev-mode anonymous convenience is safe to auto-wire (SRV-022).
_LOOPBACK_HOSTS = frozenset({"127.0.0.1", "::1", "localhost"})


def _is_loopback_host(host: str | None) -> bool:
    """True iff ``host`` is a loopback bind. Unknown (``None``/empty) is
    treated as NOT loopback — exposure must be proven, never assumed."""
...
def compose_app(
    *,
    ...
    no API keys configured, in any mode. Pass ``auto_authz=False`` to test
    or run the raw substrate without the default adapter.

    ``bind_host`` tells the adapter what interface the app will be served on.
    The dev-mode anonymous convenience (permit-all without credentials) is
    wired ONLY for a loopback bind (``127.0.0.1`` / ``::1`` / ``localhost``);
    an unknown (``None``) or network bind is fail-closed, because dev is the
    *default* mode on an unconfigured box and a network-exposed serve must
    never inherit anonymous permit-all from a default (the pre-fix hole
    behind the consumer-layer ``test_mount_refused_when_no_authz_hook``).
    Credentialed serves are unaffected: configured tokens/keys wire the real
    resolvers on any bind, with no anonymous fallback off-loopback.
    ...
    **Fail-closed authz (SRV-022).** A mount declaring ``requires_authz=True``
    ...
    if auto_authz and mw.authz is None:
        ...
        hook = maybe_default_authz_hook(loopback=_is_loopback_host(bind_host))
        if hook is not None:
    ...
    for spec in reg.specs(profile=profile):
        ...
        if not isinstance(spec.router, APIRouter):
            _LOGGER.error(
                "mount %s (%s) is not an APIRouter (%s) — skipped; other mounts unaffected",
                spec.prefix,
                spec.extension,
                type(spec.router).__name__,
            )
            continue
        try:
    ...
    _install_front_door(app, reg.specs(profile=profile))
    return app


#: Where "/" goes on a composed node. Deployment config (site env), e.g.
#: ``AXIOM_DEFAULT_SURFACE=/receipts/``.
DEFAULT_SURFACE_ENV = "AXIOM_DEFAULT_SURFACE"


def _install_front_door(app: FastAPI, specs) -> None:
    """Route "/" on the composed node — the front door.

    Every mount lives under a prefix, so the substrate's root was a JSON
    not_found — which is what a person met after any flow whose ``next``
    defaulted to "/" (the OIDC round-trip, found live 2026-09-24), and
    what a node's bare hostname showed. With ``AXIOM_DEFAULT_SURFACE``
    set, "/" redirects there; without it, "/" is an honest node index of
    the mounted prefixes (names only — every gated surface still gates).
    A consumer mount that already claimed "/" wins: we never shadow it.
    """
    from fastapi.responses import HTMLResponse, RedirectResponse

    for r in app.routes:
        if getattr(r, "path", None) == "/":
            return  # a mount owns the root; the front door is theirs

    target = os.environ.get(DEFAULT_SURFACE_ENV, "").strip()
    prefixes = sorted({s.prefix for s in specs if s.prefix and s.prefix != "/"})

    if target:
        if not target.startswith("/") or target.startswith("//"):
            _LOGGER.error(
                "%s=%r is not a local path — front door falls back to the index",
                DEFAULT_SURFACE_ENV,
                target,
            )
        else:
    ...
    rows = "".join(
        f'<li><a href="{p}/">{p}</a></li>' if not p.endswith("/") else f'<li><a href="{p}">{p}</a></li>'
        for p in prefixes
    )

    @app.get("/", include_in_schema=False)
    async def node_index() -> HTMLResponse:
        return HTMLResponse(
            "<!doctype html><html><head><meta charset='utf-8'>"
            "<title>Axiom node</title></head><body>"
            "<h1>Axiom node</h1><p>Mounted surfaces:</p>"
            f"<ul>{rows or '<li>(none)</li>'}</ul>"
            f"<p>Set {DEFAULT_SURFACE_ENV} to route this page to a surface.</p>"
            "</body></html>"
        )
