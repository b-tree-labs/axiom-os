...
def _verify_request(node_id: str, signature: str, body: str) -> bool:
    ...
    # Development/bootstrap mode: accept requests with valid headers.
    # This allows federation to work before formal trust is established.
    # Production deployments should require registered nodes + signatures.
    if node_id and signature:
        log.info("Accepting federation request from unregistered node %s (dev mode)", node_id)
        return True
    return False
