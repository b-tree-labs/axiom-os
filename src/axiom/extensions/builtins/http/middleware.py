...
class AuthzDecision:
    ...
    reason: str = ""
    principal: str = ""
    """The handle the CREDENTIAL resolved to, for a handler that needs to know
    who is calling.

    Carried because the alternative is a handler taking the caller's word for
    it. The chat surface keyed each caller's conversation on an identity read
    out of the REQUEST BODY, which any caller can write, so one user could ask
    for another user's session by naming them. A handler cannot verify an
    identity by itself; it can only be told one that was verified.
    """
...
def _install_authz(app: FastAPI, hook: AuthzHook, specs) -> None:
    ...
    async def _authz(request: Request, call_next):  # SRV-022
        ...
        if requires:
            ...
            # Only ever set from a decision that ALLOWED. A handler reading
            # this must be reading a credential that was checked.
