...
## `raw` — answering straight from the model
...
**`raw` is off unless the deployment enables it.** It skips every
governance layer the wrapper applies, so it is a property of the
deployment rather than of a request: an authenticated caller is not
thereby authorised to turn the wrapper off for a turn. Set
`AXIOM_ALLOW_RAW=1` on an instance stood up for benchmarking; anywhere
else the request is refused with `403` and every raw turn is logged
...
| Tool-use loop | up to the scope's round budget (`MAX_TOOL_ROUNDS` by default), and its turn deadline if it set one | single gateway call |
