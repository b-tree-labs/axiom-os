...
Quick Start:
    ...
    axi signal db migrate upgrade head

    # Check current revision
    axi signal db migrate current

    # Create a new migration (autogenerate from model changes)
    axi signal db migrate revision --autogenerate -m "add feature"

Programmatic Usage:
    from axiom.extensions.builtins.signals.migrations import run_migrations, check_migrations
...
def get_db_url() -> str:
    ...
    Routed through the platform resolver so the driver is named whatever the
    environment supplies. This is the call that broke ``main``: a bare
    ``postgresql://`` in CI's env made ``ensure_pgvector_extension`` raise
    ``ModuleNotFoundError: No module named 'psycopg'``, so ``CREATE
    EXTENSION vector`` never ran and the integration suite then failed on
    every vector column.
    """
    from axiom.infra.db import require_named_driver

    return require_named_driver(os.environ.get("AXIOM_DB_URL", DEFAULT_DB_URL))
...
def get_alembic_config():
    ...
    # Set absolute version_locations so Alembic finds scripts regardless of
    # where the process is invoked from (the relative "versions" in alembic.ini
    # may not resolve correctly when script_location is overridden).
...
def run_migrations(command: str, revision: str = "head", message: str = "", autogenerate: bool = False) -> bool:
    ...
    try:
        ...
        # NOT flattened into the False below. "this extension's dependency is
        # absent" and "this migration is broken" want opposite reactions from
        # the caller — skip versus stop the deploy — and a bool cannot carry
        # the difference.
        #
        # `axi db migrate upgrade head` already had the skip branch for this
        # (signals needs pgvector, which lives in the `signal` extra). It could
        # never fire, because the exception died here one frame down and
        # arrived as `False`. The v0.52.0 deploy failed on exactly that and
        # correctly refused to restart the node's services.
        raise
