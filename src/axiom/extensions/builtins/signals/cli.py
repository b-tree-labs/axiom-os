...
"""CLI handler for `axi signal` — program awareness.

User commands (shown in default help):
    axi signal brief                     Catch up on what happened
    axi signal media search <query>      Search recordings
    axi signal draft                     Generate weekly changelog
    axi signal status                    Check pipeline health

Pipeline commands (under `axi signal pipeline`):
    axi signal pipeline ingest --source voice   Process voice memos
    axi signal pipeline review                  Guided correction review
    axi signal pipeline review --transcripts    Transcript correction
    axi signal pipeline suggest --run           LLM signal-to-PRD matching
"""
        ...
        if match and self._valid_subcommands:
            ...
            if suggestions:
                ...
                if (len(suggestions) == 1
                    ...
                    try:
                        from axiom.setup.renderer import prompt_yn
                        corrected = suggestions[0]
                        if prompt_yn(f"Run `{_brand_cli()} signal {corrected}` instead?", default=True):
                            ...
                            sys.argv = fixed_argv
                            parser = get_parser()
                            args = parser.parse_args(fixed_argv[2:])  # skip "axi signal"
                            _dispatch(args)
                            self.exit(0)
...
from axiom import REPO_ROOT as _REPO_ROOT  # noqa: E402
...
def _brand_cli() -> str:
    ...
    These lines said "axi" unconditionally, so `neut signal status` announced
    itself as a different product than the one being run.
    """
    try:
...
def cmd_status(args: argparse.Namespace) -> None:
    ...
    print(f"{_brand_cli()} signal — status")
    print()
...
def _ingest_prd_comments(correlator) -> list:
    ...
    if not config_path.exists():
        ...
        config_path.write_text('{"folder_path": "/Documents/ConsumerDocs/PRDs", "days_back": 14}')
...
def _ingest_publisher(gateway, correlator) -> list:
    ...
    if not all_signals and not local_docs:
        ...
        print(f"  To register a PRD: {_brand_cli()} pub register --prd <id> --uri <sharepoint_url>")
...
def cmd_draft(args: argparse.Namespace) -> None:
    ...
    print(f"{_brand_cli()} signal — draft synthesis")
    print("-" * 40)
    ...
    if not all_signals:
        print(f"  No processed signals found. Run '{_brand_cli()} signal ingest' first.")
        return
...
def cmd_correct(args: argparse.Namespace) -> None:
    ...
    print()
    print(f"{_brand_cli()} signal — transcript correction")
    print("-" * 40)
    ...
    if args.path:
        ...
        if not transcripts:
            print(f"  No transcripts found. Run '{_brand_cli()} signal ingest --source voice' first.")
            return
...
def cmd_routes(args: argparse.Namespace) -> None:
    ...
    print("  ╰──────────────────────────────────────────────────────┤")
    print("\n  Commands:")
    print(f"    {_brand_cli()} signal routes --pending     Show queued signals")
    print(f"    {_brand_cli()} signal routes --deliver     Push to endpoints")
    print(f"    {_brand_cli()} signal routes --endpoint X  Show endpoint details")
    print()
...
def cmd_suggest(args: argparse.Namespace) -> None:
    ...
    if args.run:
        ...
        if not all_signals:
            print(f"  No signals found. Run '{_brand_cli()} signal ingest' first.")
            return
    ...
    print("\n  Commands:")
    print(f"    {_brand_cli()} signal suggest --run                 Run LLM matching")
    print(f"    {_brand_cli()} signal suggest --prd <id>            Filter by PRD")
    print(f"    {_brand_cli()} signal suggest --accept sig:prd      Accept suggestion")
    print(f"    {_brand_cli()} signal suggest --reject sig:prd      Reject suggestion")
    print()
...
def cmd_brief(args: argparse.Namespace) -> None:
    ...
    if args.topics:
        ...
        print("  ╰──────────────────────────────────────────────────────╯")
        print()
        print(f"  Usage: {_brand_cli()} signal brief \"Kevin\"")
        print(f"         {_brand_cli()} signal brief \"blockers\"")
        print(f"         {_brand_cli()} signal brief \"project alpha\"")
        print()
        return
    ...
    if sys.stdin.isatty() and sys.stdout.isatty():
        ...
        try:
            ...
            if prompt_yn("Enter chat?", default=True):
                from axiom.extensions.builtins.chat.entry import (
...
# Pipeline internals — accessible via `axi signal pipeline <cmd>`
...
def _build_custom_help() -> str:
    ...
    lines.append("")
    lines.append(f"Run '{_brand_cli()} signal pipeline' for advanced pipeline commands.")
    lines.append(f"Run '{_brand_cli()} signal <command> --help' for details.")
    return "\n".join(lines)
...
def _build_pipeline_help() -> str:
    ...
    lines = [
        "axi signal pipeline -- Advanced pipeline commands",
        "",
    ]
    for cmd, desc in PIPELINE_COMMANDS.items():
        lines.append(f"  {cmd:<14}{desc}")
    lines.append("")
    lines.append(f"Run '{_brand_cli()} signal pipeline <command> --help' for details.")
    return "\n".join(lines)
...
def _add_pipeline_subparsers(subparsers) -> None:
    ...
    subcommand group. This ensures `axi signal pipeline ingest` works the
...
def get_parser() -> SuggestingArgumentParser:
    ...
    # ── Legacy corrections parser (kept for `axi signal corrections --help`) ──
    # Still registered so existing scripts/docs referencing it get a hint
    corrections_parser = subparsers.add_parser(
        "corrections",
        help=f"(moved to: {_brand_cli()} signal pipeline review)",
    )
    ...
    correct_parser = subparsers.add_parser(
        "correct",
        help=f"(moved to: {_brand_cli()} signal pipeline review --transcripts)",
    )
    ...
    # ── Legacy db parser (promoted to axi db) ──
    subparsers.add_parser("db", help=f"(moved to: {_brand_cli()} db)")
...
def _dispatch(args: argparse.Namespace) -> None:
    ...
    # ── Legacy commands (still work, kept for backward compat) ──
        ...
        print(f"'{_brand_cli()} signal db' has moved to '{_brand_cli()} db'.")
        sys.exit(1)
        ...
        if once:
            ...
            changes = poll_onedrive_folder(_gb_od().product_name)
            if changes:
                for c in changes:
            ...
            from axiom.infra.branding import get_branding as _gb_od
            watch_onedrive(_gb_od().product_name, interval_seconds=interval)
...
def main():
    """CLI entry point for axi signal."""
    parser = get_parser()
    ...
    try:
        ...
        from axiom.infra.self_heal import attempt_recovery, emit_cli_error
        ...
        # Pin this bus as the process default so manifest-discovered
        # diagnostics/hygiene handlers publish through it. Per spec-hooks
        # §7 + §9, the registry attaches the doctor handlers to this bus
        # via the extension's [[extension.provides]] kind = "hook" blocks
        # — no boot-time bus.subscribe() ceremony.
        set_default_eventbus(bus)
        try:
            from axiom.extensions.contracts import parse_manifest
            from axiom.infra.hooks import HookBus
            from axiom.infra.hooks.registry import HookRegistry

            registry = HookRegistry(hookbus=HookBus(), eventbus=bus)
            from pathlib import Path as _P
            for ext_name in ("diagnostics", "hygiene"):
                manifest_path = (
                ...
                if manifest_path.exists():
                    try:
                        ...
                        registry.register_extensions([ext])
                    except Exception:
                        pass
        except ImportError:
            pass
        ...
        try:
            from axiom.infra.subscribers.gitlab_issues import gitlab_issue_handler
        ...
        if not recovered_args:
            print(f"\n  Error: {e}")
            print(f"  Run '{_brand_cli()} signal {args.command} --help' for usage.\n")
            sys.exit(1)
...
def _run_interactive_guided_review(
    ...
    if remaining > 0:
        ...
        print(f"Run: {_brand_cli()} signal corrections --guided")
...
def _print_session_summary(
    ...
    if training_added > 0:
        ...
        # Group by category
        categories = {}
        for _orig, _corr, cat in session_training:
...
def cmd_sources(args):
    ...
    if args.fetch:
        ...
        print(f"  (fetch logic depends on extractor - use `{_brand_cli()} signal ingest --source {source_name}`)")
        return
...
def cmd_db(args):
    ...
    if action == "stats":
        try:
            ...
            print(f"\nIs the database running? Try: {_brand_cli()} signal db up")
        ...
        try:
            ...
            if rag.backend_name != "pgvector":
                ...
                print(f"Make sure database is running: {_brand_cli()} signal db up")
                return
            ...
            print(f"\nMake sure database is running: {_brand_cli()} signal db up")
        ...
        if cmd == "check":
            ...
            if not status.get("connected"):
                ...
                print(f"   Is the database running? Try: {_brand_cli()} signal db up")
                return
            ...
            if status.get("up_to_date"):
                ...
                for rev in status.get("pending_revisions", []):
                    print(f"   - {rev}")
                print(f"\nRun: {_brand_cli()} signal db migrate upgrade head")
            ...
            try:
                ran = run_migrations("upgrade", revision)
            except ModuleNotFoundError as exc:
                # This extension's own dependency is absent. Name it and the
                # extra that carries it rather than printing a traceback at
                # someone who has not installed an optional feature.
                missing = getattr(exc, "name", None) or str(exc)
                print(
                    f"\n⏭️  Skipped — {missing!r} is not installed. "
                    "Run: pip install 'axiom-os-lm[signal]'"
                )
                ran = False
            else:
                if ran:
            ...
            if not message:
                ...
                print(f"Example: {_brand_cli()} signal db migrate revision -m 'add user table'")
                return
            ...
            print(f"Usage: {_brand_cli()} signal db migrate <command> [revision]")
            print()
            print("Commands:")
            ...
            print()
            print("Examples:")
            print(f"  {_brand_cli()} signal db migrate check")
            print(f"  {_brand_cli()} signal db migrate upgrade head")
            print(f"  {_brand_cli()} signal db migrate downgrade -1")
            print(f"  {_brand_cli()} signal db migrate revision -m 'add user preferences' --autogenerate")

    elif action == "config":
        print("\n⚙️  Database Configuration\n")

        db_url = os.environ.get("AXIOM_DB_URL", DEFAULT_LOCAL_URL)
        ...
        print("\n--- Environments ---")
        print(f"  Local:      {_brand_cli()} signal db up  (K3D)")
        print("  Staging:    Set AXIOM_DB_URL to staging PostgreSQL")
        print("  Production: Set AXIOM_DB_URL to production PostgreSQL")
        ...
        if not args.confirm:
            ...
            print("\nTo confirm, run:")
            print(f"  {_brand_cli()} signal db delete --confirm")
            return
        ...
        print(f"Usage: {_brand_cli()} signal db <command>")
        print()
        ...
        print()
        print("Quick Start:")
        print(f"  {_brand_cli()} signal db up              # Start local PostgreSQL")
        print(f"  {_brand_cli()} signal db migrate upgrade # Apply migrations")
        print(f"  {_brand_cli()} signal db stats           # Check it's working")
        print()
        print("Configuration:")
        print("  Local (default): postgresql://axiom:axiom@localhost:5432/axiom_db")
        print("  Remote: Set AXIOM_DB_URL environment variable")
...
def cmd_media(args):
    ...
    if action == "search":
        ...
        print(f"Usage: {_brand_cli()} signal media <search|play|discuss|stats|index|list>")
        print()
        print("Commands:")
        ...
        print()
        print("Examples:")
        print(f"  {_brand_cli()} signal media search heat exchangers --discuss")
        print(f"  {_brand_cli()} signal media discuss abc123 --summary")
        print(f"  {_brand_cli()} signal media discuss abc123 -q 'What was the decision?'")
...
def cmd_voice(args):
    ...
    if not all_profiles:
        ...
        print(f"  Run: {_brand_cli()} signal corrections --guided --play")
