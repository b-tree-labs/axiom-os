...
def build_security_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=f"{_brand_cli()} security",
        description="SECUR-T — federation security guardian",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
Examples:
  {_brand_cli()} security status
  {_brand_cli()} security alerts --all
  {_brand_cli()} security trust node-abc
  {_brand_cli()} security scan node-abc
""",
    )
...
def build_chaos_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=f"{_brand_cli()} chaos",
        description="Chaos test framework — federation resilience testing",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
Examples:
  {_brand_cli()} chaos list
  {_brand_cli()} chaos run content-injection
  {_brand_cli()} chaos run --all
  {_brand_cli()} chaos status
""",
    )
