...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=f"{_brand_cli()} nodes",
        description="Fleet view — monitor and manage Axiom nodes",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=f"""
Examples:
  {_brand_cli()} nodes add <node-name> <user>@<host>
  {_brand_cli()} nodes add local
  {_brand_cli()} nodes status
  {_brand_cli()} nodes list --json
""",
    )
...
def _cmd_add(args) -> int:
    ...
    print(
        f"  Re-run `{_brand_cli()} nodes add` after resolving, or the peer will be "
...
def _cmd_status(args) -> int:
    ...
    if not nodes:
        ...
        if getattr(args, "json", False):
            ...
            print(f"  Use `{_brand_cli()} nodes add <name> <user@host>` to register a node.")
            print(f"  Use `{_brand_cli()} nodes add local` to register this machine.")
        return 0
...
def _cmd_remove(args) -> int:
    if not getattr(args, "confirm", False):
        ...
        print(f"  {_brand_cli()} nodes remove {getattr(args, 'name', '<name>')} --confirm")
        return 1
