...
def _cmd_join(args) -> int:
    ...
    # NOT "join_requested". The handshake protocol does not exist, so nothing
    # was submitted anywhere — this printed a completed action and exited 0,
    # which is a report with no effect behind it, on a security-adjacent
    # surface, to a caller with no way to tell the difference.
    data = {
        "action": "join_unavailable",
        "target": url,
        "node_id": identity.node_id,
        "message": "No join request was sent: the handshake protocol is not implemented.",
    }

    if getattr(args, "json", False):
        print(json.dumps(data, indent=2))
    else:
        print(f"Nothing was sent to {url}.")
        print("  The federation handshake protocol is not implemented yet,")
        print("  so no join request exists and none is pending.")
        print(f"  Node ID: {identity.node_id}")
