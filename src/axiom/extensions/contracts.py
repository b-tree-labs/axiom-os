...
@dataclass
class ApiSurfaceDef:
    """A slice of ``/api/v1`` declared via `[[extension.provides]] kind = "api"`.

    The mechanism that lets the platform serve one unified API without the API
    shell depending on every verb. A verb declares the subpath it serves and the
    ``module:function`` that registers its routes; the ``webapp`` extension
    discovers the declaration and resolves the entry lazily at compose time.

    Deliberately declaration-first rather than import-first: an extension that
    is not installed is simply not discovered, and adding a verb needs no change
    to ``webapp``.
    """

    subpath: str  # joined onto /api/v1, e.g. "/chat"
    entry: str  # "module:function", called as f(router, subpath=...)
    description: str = ""
...
class Extension:
    ...
    api_surfaces: list[ApiSurfaceDef] = field(default_factory=list)
...
def parse_manifest(manifest_path: Path) -> Extension:
    ...
    # API surfaces — AEOS [[extension.provides]] blocks with kind="api".
    # A verb declares the slice of /api/v1 it serves; the webapp extension
    # discovers the declaration and resolves the entry lazily, so the API shell
    # never imports a verb and installing one verb does not drag in the rest.
    for prov in ext_section.get("provides", []):
        if prov.get("kind") != "api":
            continue
        ext.api_surfaces.append(
            ApiSurfaceDef(
