...
def list_sessions(principal_id: str | None = None) -> list[SessionMetadata]:
    """Return every *readable* session, optionally filtered by principal.

    The distinction matters. This used to say "all sessions" and skip anything
    it could not parse with a bare `continue`, so a truncated file vanished from
    the listing with no row, no warning and no trace — a complete-looking list
    quietly missing a session that is sitting right there on disk.
    """
    ...
    for f in sorted(_sessions_dir().glob("*.json")):
        ...
        try:
            ...
            log.warning(
                "session file %s could not be read (%s) and is missing from this "
                "listing — the session still exists on disk", f, exc,
            )
            continue
