...
def check_peer(
    *,
    ...
    """One liveness probe, dispatched on the peer's transport.

    A peer records how it is reachable, and the probe has to follow that.
    ``check_peer_health`` below speaks HTTP; an SSH peer's URL is
    ``ssh://user@host``, and handing that to ``urlopen`` raises on the unknown
    scheme, so a live node comes back reported as dead. A confidently wrong
    answer is worse than the stale timestamp it replaces, which is why this
    dispatches rather than assuming one transport.

    Both probes already existed. This connects them; it invents no new notion
    of liveness. For SSH the probe is ``axi --version``, which is the same
    cheap reachability check the identity preflight already uses.

    Returns the same shape as :func:`check_peer_health`.
    """
    started = time.time()
    ...
    if transport == "ssh" or url.startswith("ssh://"):
        if not ssh_user or not ssh_host:
            return {
                ...
                "latency_ms": _elapsed(),
                "error": (
                    "peer declares ssh transport but carries no ssh_user/ssh_host, "
                    "so there is nothing to probe"
                ),
                "checked_at": datetime.now(UTC).isoformat(),
            }
        if ssh_runner is None:
            from axiom.vega.federation.discovery import _default_ssh_runner

            ssh_runner = _default_ssh_runner
        try:
            rc, out, err = ssh_runner(ssh_user, ssh_host, "axi --version")
        except Exception as exc:  # noqa: BLE001 — a probe never raises at its caller
            return {
                "node_id": node_id,
                "healthy": False,
                "latency_ms": _elapsed(),
        ...
        if rc != 0:
            return {
                ...
                "healthy": False,
                "latency_ms": _elapsed(),
                "error": (err.strip() or out.strip() or f"ssh rc={rc}"),
                "checked_at": datetime.now(UTC).isoformat(),
            }
        return {
            "node_id": node_id,
            "healthy": True,
            "latency_ms": _elapsed(),
            "details": {"peer_version": out.strip()},
            "checked_at": datetime.now(UTC).isoformat(),
        }

    checker = http_checker or check_peer_health
    return checker(node_id, url, timeout)
