...
class KnownNode:
    ...
    last_seen: str = ""  # ISO 8601 — when we last actually REACHED this peer
    state_changed_at: str = ""  # ISO 8601 — when its state last changed
    ...
    def from_dict(cls, data: dict) -> KnownNode:
        """Rebuild a node from stored form, migrating legacy records.

        Every ``last_seen`` written before contact was recorded came from a
        registry ceremony — discover, bind, or a state change — so it is a
        state-change time wearing a contact-shaped name. Reading it back as
        contact would carry the original error forward into the new field, so
        a record that predates the split is read as what it actually was, and
        its contact time is left empty: we have never recorded reaching it.

        Unknown keys are dropped rather than raising. The previous loader
        splatted stored keys straight into the constructor, so a field removed
        in a later version would make every existing registry unreadable.
        """
        data = dict(data)
        state = data.get("state", "unknown")
        data["state"] = state if isinstance(state, NodeState) else NodeState(state)

        if "state_changed_at" not in data and data.get("last_seen"):
            data["state_changed_at"] = data["last_seen"]
            data["last_seen"] = ""
...
class NodeRegistry:
    ...
    def update_state(self, node_id: str, state: NodeState) -> None:
        ...
        if node is None:
            ...
            raise KeyError(msg)
        node.state = state
        # Deliberately NOT last_seen. A state change is something we decided
        # about a peer, not evidence we reached it: recording it as contact is
        # what left a peer last actually contacted in April displaying as though
        # it had been seen in April. Contact is recorded by record_contact().
        node.state_changed_at = _now_iso()

    def record_contact(self, node_id: str, *, at: str | None = None) -> bool:
        """Record that we actually reached this peer, and persist it.

        Called from a successful liveness probe. This is the only writer of
        ``last_seen``, which is what makes the field mean what its name says.
        Persists immediately: the previous liveness lived in a daemon's
        in-memory ``PeerStatus`` that nothing read back, so a probe's answer
        died with the process that took it.

        Returns False for an unknown node rather than raising — a probe result
        for a peer that has since been removed is ordinary, not exceptional.
        """
        node = self._nodes.get(node_id)
        if node is None:
            return False
        node.last_seen = at or _now_iso()
        self.save()
        return True
    ...
    def load(self) -> None:
        ...
        for entry in entries:
            node = KnownNode.from_dict(entry)
            self._nodes[node.node_id] = node
