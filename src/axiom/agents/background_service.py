...
#: Agents dispatched per tick. Re-enabling autonomy makes every agent with no
#: recorded ``last_run`` due at once (see first-tick semantics below), so a fleet
#: that opts in would stampede on the same tick. A small budget spreads that over
#: a few ticks without making the first run feel delayed.
MAX_DISPATCH_PER_TICK = 2


def select_due(names: list[str], *, max_per_tick: int = MAX_DISPATCH_PER_TICK) -> list[str]:
    """Bound one tick's dispatch. Stable order, so the remainder follows next tick."""
    if max_per_tick <= 0:
        return list(names)
    return list(names)[:max_per_tick]
...
def dispatch_due_agents(
    ...
    max_per_tick: int = MAX_DISPATCH_PER_TICK,
) -> list[str]:
    ...
    ``max_per_tick`` bounds one tick's dispatch so re-enabling autonomy ramps
    instead of stampeding; the remainder follows on subsequent ticks.
    """
    if now is None:
        now = time.time()
    state = store.load()
    ...
    # Two passes: decide what is due, then bound how much of it runs this tick.
    # Deciding first is what lets the budget apply to the whole due set rather
    # than to whatever the iteration order happened to reach.
    due: dict[str, Any] = {}
    ...
    for name in select_due(list(due), max_per_tick=max_per_tick):
        ext = due[name]
