...
"""Domain-consumer agent state management — safe concurrent access with hybrid backend.
...
class StateFileCorrupt(RuntimeError):
    """A state file exists but could not be parsed.

    Distinct from a missing file on purpose. "No file" means no data and an
    empty result is honest; "unparseable file" means there is data we cannot
    read, and reporting that as empty is how a read-modify-write destroys it.
    """
...
class LockedJsonFile:
    ...
    def __init__(
        ...
        *,
        ...
        # Callers that must not act on a guess — the approval queue above all —
        # pass strict=True and get an exception rather than an empty dict.
        self._strict = strict
    ...
    def read(self) -> Any:
        ...
        try:
            ...
            # No file means no data. This half was always right.
            return {}
        ...
        try:
            ...
            # There IS data here; we just cannot read it. Returning {} let the
            # caller's read-modify-write overwrite the only copy — three pending
            # approvals became one, with the original bytes unrecoverable.
            quarantine = self._quarantine()
            logger.error(
                "state file %s is not valid JSON (%s). The unreadable content "
                "has been preserved at %s; this read returns empty, so anything "
                "written next will NOT contain what was in it.",
                self._path, exc, quarantine,
            )
            if self._strict:
                raise StateFileCorrupt(
                    f"{self._path} could not be parsed ({exc}). Its contents "
                    f"were preserved at {quarantine}. Refusing to report it as "
                    "empty."
                ) from exc
            return {}

    def _quarantine(self) -> Path:
        """Move the unparseable bytes aside so a later write cannot lose them."""
        stamp = time.strftime("%Y%m%d-%H%M%S")
        target = self._path.with_suffix(self._path.suffix + f".corrupt-{stamp}")
        try:
            target.write_bytes(self._path.read_bytes())
        except OSError as exc:  # noqa: BLE001 - preservation is best-effort
            logger.error("could not preserve corrupt %s: %s", self._path, exc)
        return target
...
class TamperEvidentChain:
    ...
    * **System Audit Log** — routing decisions, sensitivity events, VPN checks.
      Key source: ``AXIOM_AUDIT_HMAC_KEY`` env var (required in controlled mode).
    * **Operations Log** — tamper-evident logbook for auditable operations.
      Key source: ``AXIOM_OPS_LOG_HMAC_KEY`` env var (required for regulated sites).
    ...
    Usage::
        ...
        chain = TamperEvidentChain(key=os.environ["AXIOM_AUDIT_HMAC_KEY"])
...
class PgStateBackend:
    ...
    def _get_store(self):
        if self._store is None:
            from axiom.infra.state_pg import PgStateStore
...
STATE_LOCATIONS: list[StateLocation] = [
    ...
    # Credentials (user-scoped, under the branded user state directory)
    StateLocation(
        f"~/{project_dir_name()}/credentials/teams",
        "credentials",
        "Teams browser session cookies",
        "critical",
    ),
]
