...
def step_up(
    ...
    *,
    ...
    if target == "attested":
        if not interactive:
            raise StepUpRequired(
                "attested", f"run `{_brand_cli()} identity init` to unlock your local identity, then retry"
            )
    ...
    if target in ("sso", "service"):
        raise StepUpRequired(
            target, f"run `{_brand_cli()} auth login --provider entra --tenant <id> --client-id <id>`, then retry"
        )
