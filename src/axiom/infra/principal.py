...
class PrincipalContext:
    ...
    public_bytes: bytes | None = None  # present at attested+/sso
    idp: str | None = None             # the IdP this principal authenticated via (sso)
...
def attested(public_bytes: bytes, *, handle: str | None = None) -> PrincipalContext:
...
#: ADR-020 handle grammar: `@name:context`, single leading `@`, name and
#: context drawn from `[A-Za-z0-9_.-]`. The colon separator is deliberate —
#: it keeps a principal visually distinct from an email address and parses
#: unambiguously against names containing dots.
_HANDLE_PART = re.compile(r"^[A-Za-z0-9_\-\.]+$")


def principal_from_idp_subject(subject: str, context: str = "") -> str:
    """The ADR-020 handle for an identity-provider subject.

    ``a1b2c3@idp.example.edu`` with context ``example`` becomes
    ``@a1b2c3:example``.

    This exists because nothing joined across machines. The principal used to
    be derived from the NODE's federation ``display_name``, which is
    ``node:person`` — so ``@laptop:person`` parsed as name=laptop, context=person
    and put the human in the context slot, while the same person was
    ``@workstation:person`` on the next machine. The federation ``owner`` field
    disagreed with itself too (`flast@` on one, `first.last@` on the other).

    Keying on the IdP subject rather than an email or a display name is the
    consistent lesson from Matrix, OIDC and SCIM: an email is a label that
    changes and can be reassigned; an institutional subject is issued once.
    So the subject's local part becomes the ``name``, and ``context`` is the
    organisational context ADR-020 already makes switchable.

    Raises ``ValueError`` rather than sanitising. Quietly stripping a
    character that the grammar forbids could map two different humans onto
    one owner, and ownership is what gates access to somebody's sessions.
    """
    raw = (subject or "").strip()
    if raw.count("@") > 1:
        raise ValueError(f"subject is not a single-@ identifier: {raw!r}")
    local = raw.split("@", 1)[0].strip()
    if not local:
        raise ValueError(f"identity-provider subject has no local part: {subject!r}")
    if not _HANDLE_PART.match(local):
        raise ValueError(
            f"subject local part is not a valid ADR-020 name: {local!r}"
        )
    if not context:
        return f"@{local}"
    if not _HANDLE_PART.match(context):
        raise ValueError(f"context is not a valid ADR-020 context: {context!r}")
    return f"@{local}:{context}"


#: Environment carrying a verified identity-provider subject. A settings key
#: or an SSO token supersedes this once the resolver is wired; the env var is
#: the seam that lets the canonical principal exist before that lands.
_IDP_SUBJECT_ENV = "AXIOM_IDP_SUBJECT"
_IDENTITY_CONTEXT_ENV = "AXIOM_IDENTITY_CONTEXT"


def canonical_principal(legacy: str = "") -> str:
    """The handle this human owns things under, on any machine.

    Prefers the identity-provider subject, which is the same wherever it is
    asserted. Falls back to ``legacy`` — the node-derived handle — so a
    laptop with no IdP wired keeps working exactly as it did.

    Never raises. A malformed IdP claim degrades to the legacy handle rather
    than taking chat down with it; the person plane keeps reporting ``open``
    and the operator sees an unproven posture instead of a stack trace.
    """
    subject = os.environ.get(_IDP_SUBJECT_ENV, "").strip()
    if not subject:
        return legacy
    context = os.environ.get(_IDENTITY_CONTEXT_ENV, "").strip()
    try:
        return principal_from_idp_subject(subject, context=context)
    except ValueError:
        return legacy


def legacy_principals(display_name: str = "") -> list[str]:
    """Handles THIS node minted before the canonical principal existed.

    Ownership written earlier carries a node-derived handle: the federation
    ``display_name`` is ``node:person``, so this node produced
    ``@laptop:person``. The inverted reading — ``@person:laptop`` — is what
    ADR-020's ``@name:context`` should have produced, and records may carry
    either, so both are offered.

    Only THIS node's handles. Adopting any legacy-shaped handle would let one
    person claim another's fragments by writing the right string; the point
    of adoption is to carry forward what this installation already owned.
    """
