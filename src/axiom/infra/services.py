...
def _platform_env_for_service(svc_env: dict[str, str], binary_path: str) -> dict[str, str]:
    ...
    overrides the platform default; this is intentional and warned here, so
    the operator sees the relaxation). The routine dropping of
    non-allow-listed entries is logged at DEBUG — it is the bound working.
    """
    env: dict[str, str] = {}
    ...
    if dropped:
        # DEBUG, not warning. Dropping non-allow-listed entries is this
        # function working, not a problem with it: any developer machine has
        # a dozen of them — homebrew, conda, cryptexes, a terminal app's own
        # bin — and naming all ten in front of an unrelated command trains
        # the reader to skip platform output entirely. The relaxation below
        # is the event worth interrupting for.
        log.debug(
    ...
    # An extension replacing PATH discards the bound entirely, which is the
    # relaxation the docstring promises to be loud about — and was not: the
    # warning sat on the routine case instead, so the event that actually
    # widens a service's reach passed silently.
    if "PATH" in svc_env and svc_env["PATH"] != bounded:
        log.warning(
            "Service PATH replaced by extension declaration — the allow-list "
            "bound does not apply to this service: %s",
            svc_env["PATH"],
        )
    env.update(svc_env)
    return env
...
def _resolve_exec_binary(svc: ServiceDef) -> str | None:
    """Resolve ``svc.binary`` to a runnable path, or ``None`` if it can't run.

    A service definition may carry either a bare command name (``"axi"``) or an
    absolute path. Every OS service manager fails *at launch* — not at install
    — when the launch target doesn't exist: systemd 203/EXEC, launchd can't
    posix_spawn, Task Scheduler 0x2 (ERROR_FILE_NOT_FOUND). Those failures are
    silent until the timer/agent first fires, which is exactly how a "healthy"
    install turns into a dead service after the next reboot.

    Resolution is cross-platform:
    - ``shutil.which`` honours ``PATH`` and, on Windows, ``PATHEXT`` (so a bare
      ``"foo"`` correctly resolves ``foo.exe`` / ``foo.cmd``).
    - An absolute/relative path that is a real file is accepted directly. The
      executable-bit check is POSIX-only; on Windows the +x bit is meaningless
      (``os.access(_, X_OK)`` just re-tests existence), so we accept an
      existing file outright there.
    """
    resolved = shutil.which(svc.binary)
    if resolved is not None:
        return resolved
    candidate = Path(svc.binary)
...
def _require_exec_binary(svc: ServiceDef) -> str | None:
    """``_resolve_exec_binary`` plus a uniform refusal log on failure.

    Used by every provider's install/start guard so a service whose launch
    target cannot run is refused with one consistent, actionable message
    rather than being written to disk and silently failing later.
    """
    binary_path = _resolve_exec_binary(svc)
    if binary_path is None:
        log.error(
            "Refusing to install service %r: launch target %r is not on PATH "
            "and is not an executable file — the service would fail at launch "
            "(systemd 203/EXEC, launchd spawn error, or Task Scheduler 0x2). "
            "Fix the binary before install.",
            svc.name,
            svc.binary,
        )
    return binary_path
...
class LaunchdProvider(ServiceProvider):
    ...
    def _compute_plist(self, svc: ServiceDef) -> str:
        ...
        idempotence."""
        binary_path = _resolve_exec_binary(svc) or svc.binary
...
{entries}
        ...
        if svc.is_periodic:
            schedule_xml = (
                f"    <key>StartInterval</key>\n    <integer>{svc.interval_secs}</integer>"
            )
        else:
...
"""
    ...
    def install(self, svc: ServiceDef) -> bool:
        ...
        """
        # Reliability contract: a plist whose ProgramArguments[0] can't be
        # spawned loads clean but never runs — refuse rather than write it.
        if _require_exec_binary(svc) is None:
            return False
    ...
    def start(self, svc: ServiceDef) -> bool:
        ...
        # This used to `return True` here, unconditionally. The return code was
        # never read and `capture_output` swallowed the error, so a refused load
        # — a malformed plist, a bad path, a disabled label — was reported to
        # every caller as a successful start.
        if result.returncode != 0:
            log.warning(
                "launchctl load refused %s: %s",
                svc.name, (result.stderr or "").strip() or f"exit {result.returncode}",
            )
            return False

        # And the exit code is only the claim. launchctl has exited 0 while
        # loading nothing; whether the agent is loaded is the fact, and it is
        # one probe away. The systemd provider alongside this one already
        # verifies its own equivalent rather than trusting the call.
        if not self._is_loaded(svc):
            log.warning(
                "launchctl load reported success for %s but the agent is not "
                "loaded — treating as a failed start rather than a false "
                "'running'", svc.name,
            )
            return False
    ...
    def stop(self, svc: ServiceDef) -> bool:
        # Mirrors start(): read the exit code, then confirm the fact. launchctl
        # exits non-zero for a plist that is not loaded, a label it refuses to
        # touch, or a path that no longer resolves — and `capture_output`
        # swallowed every one of those while this returned True.
        try:
            result = subprocess.run(
        ...
        except Exception as e:  # noqa: BLE001 - launchctl missing / timed out
            log.warning("launchctl unload failed: %s", e)
            return False

        if result.returncode != 0:
            log.warning(
                "launchctl unload refused %s: %s",
                svc.name, (result.stderr or "").strip() or f"exit {result.returncode}",
            )
            return False

        # The exit code is the claim; whether the agent is still loaded is the
        # fact. launchctl has exited 0 while unloading nothing.
        if self._is_loaded(svc):
            log.warning(
                "launchctl reported a clean unload of %s but the agent is still "
                "loaded — reporting failure rather than a false 'stopped'",
                svc.name,
            )
            return False
        return True
    ...
    def uninstall(self, svc: ServiceDef) -> bool:
        # stop() returns whether the agent actually came down. Discarding that
        # meant a still-running agent had its plist deleted out from under it
        # while the operator was told it was uninstalled — still running, no
        # plist left to inspect, and nothing to retry against.
        stopped = self.stop(svc)

        # Remove the plist either way: a reported failure is recoverable, but a
        # failure that also left everything in place is hard to tell from
        # having done nothing at all.
        self._plist_path(svc).unlink(missing_ok=True)

        if not stopped:
            log.warning(
                "uninstall %s: the agent did not come down, so it may still be "
                "running with its plist now removed", svc.name,
            )
            return False
        return True
...
class SystemdProvider(ServiceProvider):
    ...
    def install(self, svc: ServiceDef) -> bool:
        # Reliability contract: never register a unit whose ExecStart cannot
        # run. A bare/unresolvable binary yields ``ExecStart=<name>`` which
        # systemd fails at start with status=203/EXEC — silent until the
        # timer/daemon first fires. Refuse rather than write a doomed unit.
        binary_path = _require_exec_binary(svc)
        if binary_path is None:
            return False
...
"""
        ...
        # `enable` is the step that makes the unit come back after a reboot.
        # This used to discard the result and return True regardless, so a
        # refused symlink or a malformed unit produced a service that ran now
        # and vanished at the next restart — invisible for as long as the
        # machine stayed up.
        ...
        if enabled.returncode != 0:
            log.error(
                "install %s: systemctl refused to enable the unit, so it will "
                "not start after a reboot: %s",
                svc.name, (enabled.stderr or "").strip() or "no detail given",
            )
            # The unit file stays on disk: a reported failure should leave
            # something to inspect and retry, not an empty directory.
            return False

        # Ensure linger is enabled so user services survive logout + reboot.
        # Unlike `enable`, this one is advisory — plenty of hosts forbid
        # linger outright, and refusing to install there would be worse than
        # installing a service that only runs while the operator is logged in.
    ...
    def start(self, svc: ServiceDef) -> bool:
        try:
            ...
            return False
        if svc.interval_secs and not self._timer_will_fire(svc):
            log.warning(
                "timer %s is active but has no NextElapse — it will never fire "
                "(stale OnBootSec + never-run service). Reporting start as failed; "
                "one `systemctl --user start %s` kicks the chain, and units "
                "rendered with OnActiveSec (0.42.1+) do not stall.",
                self._primary_unit_name(svc),
                self._primary_unit_name(svc).replace(".timer", ".service"),
            )
            return False
        return True

    def _timer_will_fire(self, svc: ServiceDef) -> bool:
        """A started periodic timer must have a scheduled next elapse.

        `OnUnitActiveSec` alone schedules nothing until the service has run
        once this boot; with `OnBootSec` long past, `systemctl start` succeeds
        while NEXT stays "-" forever. Surface that as a start failure instead
        of a false "running".
        """
        try:
            result = subprocess.run(
            ...
            return True  # can't tell — don't turn a probe error into a failure
        values = [v.strip().lower() for v in (result.stdout or "").splitlines()]
        return any(v and v not in ("0", "n/a", "infinity") for v in values)
    ...
    def uninstall(self, svc: ServiceDef) -> bool:
        # Stop + disable both the service and the timer so we don't leave an
        # orphaned periodic schedule behind.
        #
        # This used to return True without reading a single return code. A
        # disable that failed left a timer still firing on a schedule nobody
        # was watching, against unit files that had been deleted, while the
        # operator was told the service was uninstalled — the failure
        # invisible and the evidence removed.
        disable_failed: list[str] = []
        for unit_suffix in ("timer", "service"):
            ...
            # A unit that was never installed is not a failure to remove it.
            stderr = (result.stderr or "").lower()
            if result.returncode != 0 and "does not exist" not in stderr:
                disable_failed.append(unit)

        # Cleanup happens either way. A half-removed service that reports
        # failure is recoverable; one that reports failure and also left
        # everything in place is hard to tell from having done nothing.
        ...
        if disable_failed:
            log.warning(
                "uninstall %s: systemctl refused to disable %s — the unit files "
                "are gone but the schedule may still be live",
                svc.name, ", ".join(disable_failed),
            )
            return False

        # Deleting the unit file is not uninstalling: systemd holds the loaded
        # unit until a reload, so ask after reloading rather than assuming.
        if self._still_loaded(svc):
            log.warning(
                "uninstall %s: unit still loaded after daemon-reload — "
                "reporting failure rather than a false 'removed'", svc.name,
            )
            return False
        return True

    def _still_loaded(self, svc: ServiceDef) -> bool:
        """Whether systemd still holds either unit after the reload."""
        for unit_suffix in ("timer", "service"):
            unit = f"neut-{svc.name}.{unit_suffix}"
            try:
                result = subprocess.run(
                    ["systemctl", "--user", "list-units", "--all", "--no-legend", unit],
                    capture_output=True,
                    text=True,
                    timeout=10,
                )
            except Exception:  # noqa: BLE001 - cannot check is not still-loaded
                return False
...
class WindowsTaskProvider(ServiceProvider):
    ...
    def install(self, svc: ServiceDef) -> bool:
        # Reliability contract: Task Scheduler happily registers a task whose
        # action target doesn't exist and only fails at run with 0x2
        # (ERROR_FILE_NOT_FOUND) — invisible until the next logon. Refuse up
        # front. Resolving to an absolute path also insulates the task action
        # from a logon PATH that differs from the install-time PATH.
        binary_path = _require_exec_binary(svc)
        if binary_path is None:
            return False
    ...
    def uninstall(self, svc: ServiceDef) -> bool:
        ...
        except Exception as e:  # noqa: BLE001 - schtasks missing / timed out
            log.warning("schtasks delete failed: %s", e)
            return False

        # A refused delete leaves the task registered and firing on logon while
        # the operator is told the service was removed.
        if deleted.returncode != 0:
            log.warning(
                "schtasks refused to delete the task for %s: %s",
                svc.name, (deleted.stderr or "").strip() or f"exit {deleted.returncode}",
            )
            return False
        return True
...
class SubprocessProvider(ServiceProvider):
    ...
    def start(self, svc: ServiceDef) -> bool:
        # Resolve to an absolute path so the launch doesn't depend on the
        # child inheriting the same PATH (honours PATHEXT on Windows). Popen
        # still raises FileNotFoundError if it truly can't run — caught below.
        binary_path = _resolve_exec_binary(svc) or svc.binary
        try:
