...
def get_platform_state_dir() -> Path:
    """The platform's own state directory, the same under every brand.

    :func:`get_user_state_dir` is branding-aware, which is right for state
    that belongs to a *product*: two products installed side by side should
    not tread on each other. It is wrong for state that belongs to the
    *machine*.

    A CLI failure diagnosis is machine state. Filing it under the brand whose
    alias happened to be typed means `axi` warns about three pending
    diagnoses and `neut` — the same install, same user, same failures —
    reports none, because it is reading a different directory.

    Honours ``AXI_STATE_DIR`` like its branded sibling, so tests and isolated
    runtimes redirect both together.
    """
    ...
    state_dir = Path(override) if override else Path.home() / f".{BrandingConfig.cli_name}"
