...
class ToolPostInvokePayload(TypedDict, total=False):
    ...
    Every field is optional because publishers narrow. The chat surface
    fires the full set; the CLI and protocol capability surfaces
    (``axiom.infra.skill_dispatch``) fire the content-free subset below,
    because their arguments and results can carry a credential. A
    subscriber reads what is there rather than assuming a field.
    """
    ...
    model_id: str
    tokens: int
    # Narrowed-surface fields (see axiom.infra.skill_dispatch.TELEMETRY_FIELDS).
    surface: str
    ok: bool
    errors_count: int
    args_digest: str
