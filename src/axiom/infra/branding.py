...
class BrandingConfig:
    ...
    mascot_name: str = "Axi"
    #: One-line micro-mascot drawn beside the chat header, above the placard.
    #: Must contain no ``|`` or ``│`` — prompt-toolkit reads those as column
    #: separators and shreds multi-row art into detached vertical bars, which
    #: is why the full ASCII mascot was abandoned for the chat TUI in the
    #: first place. A consumer layer supplies its own creature; the platform
    #: knows only that it is a string of that shape.
    mascot_art: str = "(◉)─(◉)"
    #: Multi-row mascot for the framed first-touch banner. Unlike
    #: :attr:`mascot_art` this is never handed to prompt-toolkit, so it may
    #: use ``│`` freely. Empty means "no override": the platform draws its
    #: own Axi. A consumer that sets this gets its own creature in the same
    #: frame, which is what keeps `neut --help` from greeting anyone as Axi.
    banner_art: str = ""
    #: Optional callable returning ``[(art, seconds), ...]`` — a short
    #: routine the mascot performs on first run. The platform plays the
    #: frames and knows nothing about what is in them; a consumer that sets
    #: only :attr:`banner_art` simply gets the resting pose.
    banner_frames: Callable[[], list[tuple[str, float]]] | None = None
    #: Glyph → rich style for the banner art. Lets a consumer paint its
    #: creature in more than one colour without the platform knowing what any
    #: glyph depicts. Unmapped glyphs fall back to the platform's accent.
    banner_style_map: dict[str, str] | None = None
