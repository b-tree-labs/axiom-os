...
CLI_SURFACE = "cli"
MCP_SURFACE = "mcp"
#: A background runner executing an action a human already approved. Its own
#: surface rather than a borrowed ``cli``, because a refusal recorded against
#: it means something different: nobody was at a terminal to be told.
RUNNER_SURFACE = "runner"
#: A capability invoked from inside a conversation. Its own surface for the same
#: reason ``runner`` is: a refusal here is read by a person mid-conversation, not
#: by an operator at a prompt or by another agent over a protocol, and the three
#: are not interchangeable when someone later asks where a capability was used.
#: Chat keeps publishing its full payload to its own per-agent bus — the renderer
#: needs the content and that bus has a narrow blast radius — and additionally
#: emits this redacted projection, so the surfaces can be compared at all.
CHAT_SURFACE = "chat"
#: A capability invoked from a browser by a signed-in person — the Decide
#: button on a case page. Its own surface for the same reason the two
#: above are: a refusal here is read by someone looking at the thing they
#: asked about, in a session the gate authenticated, and later "where was
#: this capability used" must be able to tell that apart from an operator
#: at a terminal or an agent over a protocol.
WEB_SURFACE = "web"

#: Topic the redacted projection is published on, one series across surfaces.
TELEMETRY_TOPIC: Final[str] = "capability.telemetry"
...
def publish_capability_telemetry(
    *,
    tool_name: str,
    principal: str,
    surface: str,
    ...
    """Publish the redacted telemetry projection for one capability invocation.

    For surfaces that dispatch BENEATH :func:`invoke_capability` — chat reaches
    ``tool_gateway.dispatch_tool`` directly, because its turn loop needs the
    gateway's richer payload on its own per-agent bus. That bus is deliberately
    not widened: the renderer needs argument and result content, and a per-agent
    bus is a far narrower blast radius than the process-wide one every CLI verb
    and protocol call reaches.

    What this adds is the OTHER half: the same content-free projection the CLI
    and MCP surfaces publish, on the shared bus, so an invocation from a
    conversation lands in the same series as one from a terminal. Without it,
    chat usage can only be inferred, and "is the assistant better with the
    platform configured" is unanswerable for the surface it matters most on.

    Never raises. Telemetry is an observation about a turn, not part of it; a
    sink that is down must not fail somebody's conversation.
    """
    try:
        bus = _resolve_default_bus() if eventbus is UNSET else eventbus
        if bus is None:
            return
        payload = telemetry_projection(surface)(
            {
                "tool_name": tool_name,
                "principal": principal,
                "args": args,
        ...
        if session_id:
            # Without this the per-session metric cannot match: a run can record
            # every invocation and still report that no session used the
            # platform, which is precisely the number we exist to get right.
            payload["session_id"] = session_id
        bus.publish(TELEMETRY_TOPIC, payload)
    except Exception:  # noqa: BLE001 - see the docstring: never raise into a turn
        log.debug("capability telemetry not published", exc_info=True)


def _resolve_default_bus() -> Any:
    ...
    *,
    ...
    Args:
        ...
        hookbus: Bus the ``tool.pre_invoke`` chain fires on. ``None`` uses the
            process default.
        hold_on_approval: When a hook demands approval, place the call on the
            durable approval queue and report where it went, instead of
            discarding it. ``False`` reports the refusal without queueing, and
            is for the one caller that must not: :class:`
            ~axiom.infra.orchestrator.runner.ActionRunner`, re-invoking an
            action a human already approved. Queueing there would mint a fresh
            pending action on every pass, forever.
    ...
    try:
        ...
        if not hold_on_approval:
            return SkillResult(
                ok=False,
                errors=[
                    f"approval required ({source}): {approval.reason}. "
                    f"{capability} was not run: the {surface} surface cannot prompt "
                    f"for approval. Approve it from an interactive surface, or add "
                    f"a site rule that permits it."
                ],
            )

        # The work is held, not discarded. This branch used to end the story:
        # it told the caller to go and find an interactive surface, and threw
        # the call away. On a scheduled agent or an MCP client that is not
        # advice, it is a refusal with extra steps, and the pause it describes
        # had nowhere to live.
        held = _hold_for_approval(capability, params, ctx, reason=approval.reason)
        cli = _brand_cli()
        if held is None:
            return SkillResult(
                ok=False,
                errors=[
                    f"approval required ({source}): {approval.reason}. "
                    f"{capability} was not run, and could not be queued for a "
                    f"human either. Nothing is waiting; re-run it once the "
                    f"approval store is reachable."
                ],
            )
        return SkillResult(
            ok=False,
            errors=[
                f"approval required ({source}): {approval.reason}. "
                f"{capability} is held as {held} awaiting a human. "
                f"Answer it with '{cli} approve ok {held}', or reject it with "
                f"'{cli} approve no {held} --reason ...'."
            ],
            value={"held_action_id": held, "status": "pending_approval"},
        )
...
def _hold_for_approval(
    ...
    *,
    ...
    """Put a refused call on the durable approval queue. Returns its id.

    Imported here rather than at module scope because the orchestrator's runner
    imports this module, and a top-level import would close the cycle.

    Returns ``None`` when the store cannot be written. The caller then says so
    plainly. Reporting a held id for something that was never recorded would be
    the worst outcome available: a person waiting on a queue entry that does
    not exist.
    """
    from axiom.infra.orchestrator.actions import Action, ActionCategory
    ...
    path = (
        Path(state_dir) / "orchestrator" / "approvals.json"
        if state_dir
        else default_approval_path()
    )
    action = Action(
        name=capability,
        params=dict(params),
    ...
    try:
        ApprovalGate(FileActionStore(path)).submit(action)
    except Exception:  # noqa: BLE001 — an unwritable queue is reported, not raised
        log.exception("could not hold %s for approval at %s", capability, path)
        return None
