...
def ensure_managed_gitignore(
    repo_root: Path | str, *, marker: str, patterns: list[str]
) -> tuple[Path, bool]:
    """Idempotently maintain a visible, clearly-marked block in the repo's
    top-level ``.gitignore`` (ADR-112 §D5).

    The block is fenced by ``# >>> {marker} (managed) >>>`` / ``# <<< … <<<`` so
    a person *sees and understands* it (unlike a hidden ``.git/info/exclude``).
    Writing it is idempotent: an identical block is left untouched; a block whose
    patterns drifted is rewritten in place; an absent block is appended after a
    blank line. Returns ``(gitignore_path, changed)``.

    The file is the repo top-level's ``.gitignore`` so glob patterns
    (``*.conflict``) apply repo-wide regardless of how deep the mirror sits.
    """
    top = Path(run_git(Path(repo_root), "rev-parse", "--show-toplevel").strip())
    begin = f"# >>> {marker} (managed) >>>"
    end = f"# <<< {marker} (managed) <<<"
    ...
    if i != -1 and j != -1 and j > i:
        new = existing[:i] + block + existing[j + len(end):]
    elif existing.strip():
        new = existing.rstrip("\n") + "\n\n" + block + "\n"
    else:
        new = block + "\n"
