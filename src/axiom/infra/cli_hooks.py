...
# ---------------------------------------------------------------------------
#: How much of a diagnosis summary belongs in a banner that is standing in
#: front of somebody else's command. The rest is one command away.
_SUMMARY_WIDTH = 96
...
def surface_pending_diagnoses(command: str = "") -> None:
    ...
    ``command`` is the subcommand about to run, so the banner can stay out of
    the way of the surface that exists to read it.
    """
    if os.environ.get("AXI_DIAGNOSES_QUIET"):
        return
    # Not on the triage surface itself. The banner ends with "Read the remedy:
    # axi triage pending", which on `axi triage pending` instructs the reader
    # to run the command they are already running — and then the command
    # prints the same diagnoses again underneath it.
    if command.split()[:1] == ["triage"]:
        return
    try:
        from axiom.extensions.builtins.diagnostics import cli_listener

        # Fold in anything filed under a brand home before the paths were
        # unified, so those failures are not stranded where only one alias
        # could see them.
    ...
    if not pending:
        return
    # One line per diagnosis, and the remedy stays where it can be read on
    # purpose. This banner prints in front of whatever the operator actually
    # typed: pasting a thirty-line Docker-and-Postgres remedy ahead of an
    # unrelated `update` is how a signal worth reading trains people to skip
    # it. It also has to say the failure was an EARLIER command, because
    # appearing here reads as though this one just broke.
    count = len(pending)
    print(
        f"\n[TRIAGE] {count} pending diagnos{'is' if count == 1 else 'es'} "
        f"from an earlier command (not this one):",
        file=sys.stderr,
    )
    for d in pending:
        ...
        if len(summary) > _SUMMARY_WIDTH:
            summary = summary[: _SUMMARY_WIDTH - 1].rstrip() + "…"
        confidence = d.get("confidence", 0.0)
        print(f"  • {fp}  {summary} ({confidence:.2f})", file=sys.stderr)
    first = pending[0].get("fingerprint", "<fingerprint>")
    print(
        f"  Read the remedy: {_brand_cli()} triage pending   ·   Dismiss: {_brand_cli()} triage clear {first}",
        file=sys.stderr,
    )
    print(
        "  Silence all of them for one command with AXI_DIAGNOSES_QUIET=1\n",
        file=sys.stderr,
    )
