...
class UploadResult:
    ...
    ``version`` is the platform-wide name for "which stored bytes" (ADR-112
    §D4) — the same field name as the connector file-store's
    ``PutReceipt.version`` and the mirror's ``RemoteDoc.version``. The
    storage families stay separate; only the name is shared."""
