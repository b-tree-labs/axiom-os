...
class AuthMode(str, Enum):
    """How a configured app authenticates — the app-type axis (ADR-110 §Decision-2).

    The field that was always implicit. It is what makes a fleet of external app
    registrations legible: a service identity, a per-user delegated app, and a
    staff sign-in app are three ``auth_mode`` values, not an unlabelled zoo — and
    the same three map onto every cloud (Entra / Google / AWS)."""

    SERVICE = "service"                # unattended service identity (application perms)
    DELEGATED_USER = "delegated_user"  # acts as the signed-in user (per-user tokens)
    USER_SIGNIN = "user_signin"        # people signing INTO the product (OIDC RP)
    UNSPECIFIED = "unspecified"        # not yet declared (backward-compatible default)
...
class ConnectionInstance:
    ...
    ``auth_mode`` / ``tenancy`` / ``capability_scope`` are the app-type profile
    (ADR-110): what kind of app this is, single- or multi-tenant, and what it may
    touch. New with defaults, so every existing call site keeps working."""
    ...
    meta: dict = field(default_factory=dict)
    auth_mode: AuthMode = AuthMode.UNSPECIFIED
    tenancy: Tenancy = Tenancy.SINGLE
    capability_scope: str = ""  # what the app may touch: site | mailbox | user-own | tenant
    ...
    def __post_init__(self) -> None:
        ...
        # Coerce persisted string values back to enums (raises on an invalid one).
        if not isinstance(self.auth_mode, AuthMode):
            self.auth_mode = AuthMode(self.auth_mode)
        if not isinstance(self.tenancy, Tenancy):
            self.tenancy = Tenancy(self.tenancy)
