...
def _render_skill_md(spec: SkillSpec, ext_version: str) -> str:
    """Render SKILL.md for ``spec``.

    The frontmatter contract lives in ``skill_md`` — the module that also parses
    it — so the writer and the reader cannot drift apart on field names or
    escaping. This function only decides the PROSE.
    """
...
def emit_md_for_spec(spec: SkillSpec, out_dir: Path, ext_version: str) -> Path:
    ...
    Refuses to traverse. The skill name becomes a directory component upstream
    (``<ext>/skills/<leaf>/``), so a name carrying ``..`` or a separator turns
    emission into a write-anywhere primitive. The reader validates names too;
    this is the second lock, because a single validation point is exactly the
    brittleness this contract is meant to remove.
    """
    if ".." in out_dir.parts:
        raise ValueError(
            f"refusing to emit into {out_dir} — a path that traverses upward "
            "cannot be a skill directory"
        )
    validate_skill_name(spec.name, where="the registered skill")
...
class _ExtTarget:
    ...
    root: Path
    version: str
    # Dotted import path to the ext's ``skills`` package, e.g.
    # ``<pkg>.extensions.builtins.<name>.skills``. ``None`` for
    # project-/user-level extensions (loaded from a file path, not importable
    # by dotted name) and for the test override tree.
    skills_pkg: str | None = None
...
def _skills_pkg_for_root(root: Path) -> str | None:
    """Derive an extension's ``skills`` package dotted path from its root.

    An installed/builtin extension lives at ``<pkg>/extensions/builtins/<name>``
    (works for Axiom's own builtins — ``pkg == "axiom"`` — and for any
    consumer package). Mirrors
    ``discovery._extension_source_package`` so the generator loads consumer
    extensions the same way discovery does at runtime, instead of assuming an
    ``axiom.extensions.builtins.*`` prefix. Returns ``None`` when the root is
    not under an ``extensions/builtins`` tree (project-/user-level extensions).
    """
    parts = root.parts
    try:
    ...
    if i >= 2 and parts[i - 1] == "extensions":
        return f"{parts[i - 2]}.extensions.builtins.{root.name}.skills"
    return None
...
def _discover_ext_targets(only: str | None, root_override: Path | None) -> list[_ExtTarget]:
    ...
    if root_override:
        # Single-tree override (used in tests): specs come from ctx.registry,
        # so no skills_pkg is needed.
        name = only or root_override.name
        targets.append(_ExtTarget(
            name=name,
            root=root_override,
            version=_read_version(root_override / "axiom-extension.toml"),
        ))
        return targets

    # Real discovery: walk EVERY extension root (Axiom builtins + consumer
    # packages (a domain consumer's own) + project/user), not just
    # Axiom's own builtins — the authoring tooling must reach consumer
    # extensions the same way the runtime already does.
    from axiom.extensions.discovery import discover_extensions

    for ext in discover_extensions():
        if only and ext.name != only:
            continue
        if not (ext.root / "skills").is_dir():
            continue
        targets.append(_ExtTarget(
            name=ext.name,
            root=ext.root,
            version=_read_version(ext.root / "axiom-extension.toml"),
            skills_pkg=_skills_pkg_for_root(ext.root),
        ))
    return targets
...
def _load_specs_for_ext(ext: _ExtTarget, root_override: Path | None) -> list[SkillSpec]:
    ...
    Imports by the ext's own dotted path (``ext.skills_pkg``) — for a consumer
    package that is ``<pkg>.extensions.builtins.<name>.skills``, not the old
    hardcoded ``axiom.extensions.builtins.<name>.skills``. Specs are attributed
    to their owning extension by module prefix, so emitting across many
    extensions never cross-assigns one ext's skills to another (they all share
    the process-wide default registry).

    When ``root_override`` is set (tests), the caller pre-populates the
    registry; this path is unused there.
    """
    from axiom.infra.skills import default_registry

    if root_override is not None or not ext.skills_pkg:
        return []
    try:
        mod = importlib.import_module(ext.skills_pkg)
    except Exception:
        return []
    if hasattr(mod, "bind_default"):
        try:
            ...
            pass
    # e.g. "<pkg>.extensions.builtins.<name>." — a spec belongs to
    # this ext iff its fn is defined under the ext's package.
    prefix = ext.skills_pkg.rsplit(".skills", 1)[0] + "."
    reg = default_registry()
    return [
        s for s in reg.specs().values()
        if getattr(s.fn, "__module__", "").startswith(prefix)
    ]
...
def run(params: dict[str, Any], ctx: SkillContext) -> SkillResult:
    ...
    for ext in targets:
        ...
        # _load_specs_for_ext already attributes specs to THIS ext by module
        # prefix (namespaces needn't equal the ext name — press lives in
        # publishing), so each spec is emitted under its owning extension even
        # when many are discovered at once.
        ...
        for spec in specs:
            ...
            # ADR-118 split: an existing doc is classified before anything
            # touches it. Parses without the generator marker → authored,
            # owned by a human — never overwritten, never called drift.
            # Fails to parse → it cannot be a valid authored doc (AEOS061
            # requires parse), so write mode repairs it and says so.
            existing_state = "missing"
            if target_md.exists():
                try:
                    existing_state = (
                        "generated"
                        if read_skill_md(target_md).is_generated
                        else "authored"
                    )
                except SkillDocumentError:
                    existing_state = "broken"

            if existing_state == "authored":
                actions.append(
                    f"kept authored SKILL.md for {spec.name} at {target_md} "
                    "(no generator marker; regenerate-only-where-generated)"
                )
                continue
            ...
            if check:
                ...
                if existing_state == "broken":
                    actions.append(f"repaired unparseable SKILL.md: {target_md}")
                else:
