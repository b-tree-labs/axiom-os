...
# --- required marker + temporal formats (WS-C) ---------------------------- #
def test_split_shape_reads_required_marker():
    assert split_shape("date!") == ("date", True)
    assert split_shape(" int ") == ("int", False)
    assert split_shape("!") == ("", True)


def test_required_is_opt_in_and_ordered():
    schema = inputs_to_json_schema({"start": "date!", "end": "date", "limit": "int!"})
    assert schema["required"] == ["start", "limit"]
    # The marker never leaks into the property itself.
    assert schema["properties"]["start"]["description"] == "start (date)"
    assert schema["properties"]["limit"]["type"] == "integer"


def test_no_marker_means_no_required_key():
    # Existing capabilities keep their exact shape: no `required` at all.
    assert "required" not in inputs_to_json_schema({"source": "Path", "copies": "int"})


def test_temporal_shapes_carry_json_schema_formats():
    schema = inputs_to_json_schema({"d": "date", "t": "timestamp", "dt": "datetime", "s": "str"})
    assert schema["properties"]["d"] == {"type": "string", "format": "date", "description": "d (date)"}
    assert schema["properties"]["t"]["format"] == "date-time"
    assert schema["properties"]["dt"]["format"] == "date-time"
    assert "format" not in schema["properties"]["s"]
