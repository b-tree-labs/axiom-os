...
class ActionEnvelope:
    ...
    """The ADR-084 governance actor (roles/tenant/assurance).

    Additive on purpose: ``actor`` stays the bare ``Principal`` so receipts and
    capability signatures are byte-identical. Promoting ``actor`` itself is a
    separate sweep of its 37 consumers and does not belong inside a core change
    that touches signing.
    """

    surface: str = ""
    """The dispatch surface this action crossed — ``"cli" | "mcp" | "agent_tool"``
    (ADR-114 §3). Empty when the call did not come through a surface-aware
    gateway. Lets a ``Rule`` scope to one surface (e.g. require approval for a
    tool *on mcp* without gating the CLI). Additive: a rule that declares no
    surface still matches every surface, so existing rules are unchanged.
    """
