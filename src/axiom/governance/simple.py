...
def get_current_actor(*, dev_mode: bool = False) -> Principal:
    ...
    if handle:
        return principal_from_handle(handle)

    if dev_mode:
        host = socket.gethostname().split(".")[0] or "localhost"
        return principal_from_handle(f"@dev:{host}")
...
def principal_from_handle(handle: str) -> Principal:
    ...
    Public because binding an actor from a handle is something callers outside
    this module genuinely need, and the alternative was worse than a private
    name: ``set_current_actor`` accepts anything, and ``get_current_actor``
    silently ignores whatever is not a ``Principal``. A caller holding a
    ``PrincipalContext`` — which every skill has, and which carries a handle —
    would bind it, read back nothing, and lose attribution with no error. Worse,
    the bad binding also defeats the ``AXIOM_ACTOR`` fallback for everything
    inside its scope, so it does not merely fail to help, it takes something
    away.
    """
...
class ExtensionAuthnContext:
    ...
    """The context's own capability. Time-limited (1h TTL at mint); see
    ``capability_factory`` for why that is safe in a long-lived host."""
    capability_factory: object = None
    """Zero-arg callable re-minting ``default_capability`` through the same
    path that minted it at setup. Without this, a host that outlives the
    token's TTL (the data-platform orchestrator) presents an expired token
    to every action and is floor-denied EXPIRED_CAPABILITY — four nightly
    backups were lost to exactly that before the fleet console surfaced it.
    Renewal applies ONLY to the context's own default: an explicitly passed
    capability is the caller's authority and never silently renews."""
    registered_verbs: list[str] = field(default_factory=list)

    #: Renew when the default token has less than this long to live, so an
    #: action started just before expiry doesn't race the floor check.
    _RENEWAL_MARGIN = timedelta(seconds=60)

    def _fresh_default_capability(self) -> CapabilityToken | None:
        """The context's own capability, re-minted if (nearly) expired.

        Late-binds authority at action time instead of freezing it at
        setup — the runtime twin of the def-time-default rule. The
        factory re-runs the SAME mint path setup used (same subject,
        same scope), so renewal is continuation of the original grant,
        never escalation.
        """
        ...
        if cap.is_valid_at(now + self._RENEWAL_MARGIN):
            return cap
        factory = self.capability_factory
        if factory is None:
            return cap  # nothing to renew with; the floor will judge it
        try:
            renewed = factory()
        except Exception:  # noqa: BLE001 — a broken factory must not mask the deny
            _log.warning(
                "extension %r: capability renewal failed; presenting the "
                "expired token to the floor",
                self.extension_name,
            )
            return cap
        self.default_capability = renewed
        return renewed
    ...
    def action(
        ...
        *,
        ...
        cap = capability
        if cap is None:
            cap = self._fresh_default_capability()
        if cap is None:
            raise RuntimeError(
...
def _resolve_actor_arg(
    ...
    if isinstance(actor, str):
        return principal_from_handle(actor)
    ...
    *,
    ...
    if wire_authz:
        try:
            ...
            # ADR-083: point GUARD at OpenFGA when a node configures one
            # (AXIOM_OPENFGA_* / authz.openfga.*). Unset → NullSubstrate, so
            # behaviour is unchanged until a server is named.
            try:
                from axiom.extensions.builtins.authz.openfga_http import (
                    maybe_register_substrate,
                )

                maybe_register_substrate(ctx.authz_ctx)
            except ValueError:
                # Half-configured substrate: surface it, never run without
                # the authorization a deployment asked for.
                raise
    ...
    if wire_vault:
        try:
            ...
            if dev_mode:
                # Issue a wildcard-scope capability for this process. The
                # factory re-runs this exact mint so a long-lived host can
                # renew the same grant when the TTL lapses (see
                # ExtensionAuthnContext.capability_factory).
                dev_principal = get_current_actor(dev_mode=True)

                def _remint_via_keep(
                    _vctx=vctx, _subject=dev_principal
                ) -> CapabilityToken:
                ...
                ctx.default_capability = _remint_via_keep()
                ctx.capability_factory = _remint_via_keep
        except Exception:
            pass

    if ctx.default_capability is None and dev_mode:
        # Even without KEEP wired, give the extension a usable token — and
        # the factory to renew it, so a host outliving the 1h TTL keeps
        # the same grant instead of being floor-denied EXPIRED_CAPABILITY.
        dev_principal = get_current_actor(dev_mode=True)

        def _remint_unscoped(_subject=dev_principal) -> CapabilityToken:
            return CapabilityToken.unscoped_test_token(subject=_subject)

        ctx.default_capability = _remint_unscoped()
        ctx.capability_factory = _remint_unscoped
