...
from .users import InMemoryUserStore, User, WritableUserStore, _norm_email, validate_site
...
_USER_FIELDS = frozenset({"user_id", "email", "password_hash", "name", "roles", "attributes", "site"})

# An account may carry no password ONLY when it is bound to an identity provider
# (``attributes.idp`` set by the gate's OIDC sign-in). Such an account can never
# pass the password form (``authenticate`` refuses passwordless accounts) — it
# enters through SSO alone. A null hash without that binding is a broken record.
_SSO_BINDING_KEY = "idp"
...
def _validate_records(raw: object, *, where: str) -> list[dict]:
    ...
    for i, entry in enumerate(raw):
        ...
        if not isinstance(attributes, dict):
            raise AccountsFileError(f"{at}: 'attributes' must be an object")
        pw_hash = entry.get("password_hash")
        sso_bound = bool(attributes.get(_SSO_BINDING_KEY))
        if pw_hash is None and sso_bound:
            pass  # SSO-only account: no password, enters via the gate's OIDC route
        elif not (isinstance(pw_hash, str) and pw_hash.strip()):
            raise AccountsFileError(
                f"{at}: 'password_hash' is required (non-empty) unless the account "
                f"is bound to an identity provider (attributes.{_SSO_BINDING_KEY})"
            )
        if email in seen:
        ...
        try:
            site = validate_site(entry.get("site"))
        except ValueError as e:
            raise AccountsFileError(f"{at}: {e}") from e
        records.append(
            {
...
def upsert_user_record(
    ...
    *,
    ...
    On update, ``name``/``roles``/``attributes`` left as ``None`` keep the
    existing values (so a password reset does not have to restate a user's
    roles). ``password_hash=None`` keeps an existing hash, and for a new account
    means SSO-only — which validation permits only when ``attributes`` binds it
    to an identity provider. Refuses to overwrite an existing email unless
    ``overwrite=True``.
    ...
    record = {
        ...
        "email": norm,
        "password_hash": password_hash if password_hash is not None else prior.get("password_hash"),
        "name": name if name is not None else prior.get("name", ""),
        "roles": tuple(roles) if roles is not None else prior.get("roles", ()),
        "attributes": dict(attributes) if attributes is not None else dict(prior.get("attributes") or {}),
        "site": validate_site(site) if site is not None else prior.get("site"),
    }
    if created:
...
class JsonFileUserStore:
    ...
    def upsert(self, user: User) -> User:
        """Create or replace ``user`` in the accounts file and re-read it.

        Matches an existing record by ``user_id`` first (an SSO subject stays
        stable across email changes), then by email. Raises when the store has
        no file to write to.
        """
        if not self._path:
            raise AccountsFileError("this user store has no accounts file to write")
        records = load_user_records(self._path) if self._path.is_file() else []
        idx = next((i for i, r in enumerate(records) if r["user_id"] == user.user_id), None)
        if idx is None:
            idx = next((i for i, r in enumerate(records) if r["email"] == user.email), None)
        record = {
        ...
        if idx is None:
            ...
            records[idx] = record
        save_user_records(self._path, records)
        self.reload()
        found = self.get_by_id(user.user_id)
        if found is None:  # pragma: no cover — save validated the record we just wrote
            raise AccountsFileError(f"account {user.user_id} did not persist")
        return found
...
# A JsonFileUserStore satisfies the (writable) UserStore protocol.
_: WritableUserStore = JsonFileUserStore(None)
