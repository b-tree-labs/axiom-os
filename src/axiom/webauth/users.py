...
#: The ``:context`` half of a ``@name:context`` principal handle — the SITE the
#: account belongs to (ADR-050 tenancy). Same grammar as the handle context.
_SITE_RE = re.compile(r"^[A-Za-z0-9_\-\.]+$")


def validate_site(site) -> str | None:
    """Normalize a site id (``None``/empty → ``None``); raise on a bad shape."""
    if site is None:
        return None
    s = str(site).strip()
    if not s:
    ...
    if not _SITE_RE.match(s):
        raise ValueError(f"invalid site {site!r}: use letters, digits, '_', '-', '.'")
    return s


def principal_site(handle: str) -> str | None:
    """The site a ``@name:context`` handle names, or ``None`` when it has none."""
    h = (handle or "").strip()
    if not h.startswith("@") or ":" not in h:
        return None
    return h.split(":", 1)[1] or None
...
class User:
    ...
    #: The site (tenant) this account belongs to — the ``:context`` of its
    #: principal handle. ``None`` = the node's default site (single-site install).
    site: str | None = None
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "site", validate_site(self.site))

    @property
    def handle(self) -> str:
        """The account as a ``@name:context`` principal handle (``:site`` when set)."""
        name = _handle_name(self.user_id)
        return f"@{name}:{self.site}" if self.site else f"@{name}"
...
@runtime_checkable
class WritableUserStore(UserStore, Protocol):
    """A store that can also create/replace an account (SSO just-in-time
    provisioning, account linking). Read-only stores simply don't implement it,
    and callers fail closed on ``isinstance``."""
...
class InMemoryUserStore:
    ...
    def add(self, user: User) -> None:
        # Replacing an account whose email changed must drop the stale email key,
        # or a lookup by the old address would still resolve.
        prior = self._by_id.get(user.user_id)
        if prior is not None and _norm_email(prior.email) != _norm_email(user.email):
            self._by_email.pop(_norm_email(prior.email), None)
...
def _handle_name(value: str) -> str:
    """Fold any user id (email, GUID) into the ``@name`` grammar."""
    out = re.sub(r"[^A-Za-z0-9_\-\.]", "_", value or "").strip("_")
    return out or "anon"
