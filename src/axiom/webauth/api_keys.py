...
class ApiKeyIdentity:
    ...
    scopes: tuple[str, ...]
    name: str = ""
    #: The site the key acts within — always the principal's ``:context``.
    site: str | None = None
...
def bind_principal_to_site(principal: str, site: str | None) -> tuple[str, str | None]:
    """Reconcile a handle with a site: ``@svc`` + ``acu`` → ``@svc:acu``; a handle
    that already names a context must agree with ``site`` or minting refuses —
    a key can never act in one site under another site's name."""
    from .users import principal_site, validate_site

    handle = _validate_principal(principal)
    wanted = validate_site(site)
    have = principal_site(handle)
    if wanted is None:
        return handle, have
    if have is None:
        return _validate_principal(f"{handle}:{wanted}"), wanted
    if have != wanted:
        raise ValueError(
            f"principal {handle!r} names site {have!r} but --site is {wanted!r}"
        )
    return handle, have
...
def mint_api_key(
    *,
    ...
    auto-generated (callers never invent identifiers on create). ``site`` binds
    the key to a tenant: it becomes (or must equal) the handle's ``:context``.
    """
...
def _validate_records(raw: object, *, where: str) -> list[dict]:
    ...
    for i, entry in enumerate(raw):
        ...
        # The site is the handle's context; a stored 'site' must agree with it.
        from .users import principal_site, validate_site

        handle_site = principal_site(principal)
        try:
            stored_site = validate_site(entry.get("site"))
        except ValueError as e:
            raise ApiKeysFileError(f"{at}: {e}") from e
        if stored_site is not None and handle_site is not None and stored_site != handle_site:
            raise ApiKeysFileError(
                f"{at}: 'site' {stored_site!r} disagrees with principal context {handle_site!r}")
        records.append({
            "key_id": key_id,
            "principal": principal,
            ...
            "site": stored_site or handle_site,
        })
    return records
