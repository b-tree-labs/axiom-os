...
class ProbeResult:
    ...
    rag_endpoint: str | None = None  # the sibling RAG resource's URL (for `rag.database_url`)
...
def _attach_rag_companions(results: list[ProbeResult], registry: ConnectionRegistry) -> None:
    """If a probed LLM endpoint has a sibling RAG resource on the same host,
    attach the corpus descriptor and its URL so the prompt can surface it.

    Matched by hostname, not host:port: a node's served RAG face commonly
    lives on a *different port* of the same host (e.g. the LLM gateway on
    :8766 and the retrieve endpoint on :8767), so a host:port match would
    never pair the real siblings.
    """
    rag_conns = {c.endpoint: c for c in registry.by_category("rag")}
    for r in results:
        host = _host_of(r.connection.endpoint or "")
        if not host:
            continue
        for rag_ep, rag_conn in rag_conns.items():
            if host == _host_of(rag_ep):
                r.rag_corpus = getattr(rag_conn, "display_name", rag_conn.name)
                r.rag_endpoint = rag_conn.endpoint
                break
...
def render_prompt(result: ProbeResult) -> str:
    ...
    c = result.connection
    # Report the TIER, never a compliance verdict.
    #
    # This said "EC-safe" whenever a provider's `routing_tier` was configured
    # as export-controlled — a value someone typed, rendered to a reader as an
    # assurance. Routing tier says where this deployment sends the content. It
    # says nothing about whether an environment is AUTHORISED to hold it: that
    # is an attestation by an export-control office, covering who may log in,
    # what may rest on disk, and what happens to backups. A self-hosted model
    # with no egress satisfies none of those by itself.
    #
    # Naming the tier is useful and true. Naming it safety is neither.
    tier = (getattr(c, "routing_tier", "") or "").strip()
    ec_text = (
        "routing tier: export-controlled (routing only — not an authorisation)"
        if tier == "export_controlled"
        else "routing tier: public (this gateway cloud-routes; avoid sensitive content)"
    )
