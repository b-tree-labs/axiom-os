...
def install_docker_linux() -> bool:
    ...
    if user:
        ...
        if added.returncode == 0:
            print(
                f"  Added {user} to docker group. Log out and back in, "
                "or run: newgrp docker"
            )
        else:
            # Announcing a group membership that was refused sends the operator
            # to `newgrp docker`, which cannot help, and the real error — no
            # sudo, no docker group — stays hidden in captured output.
            print(
                f"  Could not add {user} to the docker group: "
                f"{(added.stderr or '').strip() or f'exit {added.returncode}'}\n"
                "  Docker is installed but will need sudo until this is fixed."
            )
        ...
        try:
            ...
            for _ in range(30):
                ...
                if result.returncode == 0:
                    ...
                    return True
            # 60 seconds of `docker info` failing is not a slow start, it is
            # a Docker that is not up. Saying otherwise sends the caller on to
            # a compose/k3d step that will fail further from the cause.
            print(
                "Docker did not become ready after 60s. Open Docker Desktop and "
...
"""
    ...
    # The poll above is the only evidence available, and it never saw Running.
    # Returning True here reported a deployed LLM server to a caller that will
    # then try to use it.
    print(
        "  LLM server did not reach Running in time. Check: "
...
def provision_infrastructure(
    callback=None, model: str | None = None, provision_llm: bool = True
) -> str:
    ...
    *model* is the local LLM profile name (``"qwen"`` or ``"small"``).
    If ``None``, the llamafile module's default applies (qwen).

    *provision_llm* — when False, skip the llamafile download/start entirely
    (the caller's size-confirmation gate was declined). The database side
    still provisions; a multi-GB model pull requires explicit consent.
    """
    ...
    if path == "k3d":
        ...
        if provision_llm:
            ...
            callback("Local LLM skipped (download declined).")
    else:
        if callback:
    ...
    if system == "Darwin":
        renderer.numbered_steps(
            [
                ...
                f"Run '{_get_branding().cli_name} infra' again",
            ]
        )
...
def main(args: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        prog=f"{_get_branding().cli_name} infra",
    )
    parser.add_argument(
        "--check",
        "-c",
    ...
    if result.success:
        ...
        renderer.text("\nYou can now use:")
        renderer.text(f"  {_get_branding().cli_name} db status    # Check database connection")
        renderer.text(f"  {_get_branding().cli_name} db migrate   # Run schema migrations")
        renderer.text(f"  {_get_branding().cli_name} signal ...    # Run sense commands")
        return 0
