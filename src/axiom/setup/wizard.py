...
class SetupWizard:
    ...
    def resolve_local_model(self) -> str:
        ...
        renderer.info(
            f"Found a cached Bonsai-1.7B model at {cached}. Bonsai is retired: "
            "it produced degenerate output in the field and is kept only so an "
            "existing cache can be migrated off it."
        )
        upgrade = renderer.prompt_yn(
            "Upgrade to qwen2.5:7b (4.7GB download)? "
            "[Y = upgrade, n = use gemma2-2b instead (~1.6GB)]",
            default=True,
        )
        # Declining the large download must not resolve to the retired model.
        # The choice on offer is size, not whether to keep something the
        # codebase itself documents as producing degenerate output.
        self._resolved_model = "qwen" if upgrade else "small"
        return self._resolved_model

    def _confirm_model_download(self, model: str) -> bool:
        """Explicit, size-stating consent gate before any multi-GB model pull.

        Sandbox audit 2026-09-18, gap 2: the default 4.7GB GGUF used to start
        downloading behind a default-Yes infra prompt, with no size stated
        until a progress callback after the fetch had begun. This gate names
        the size, advertises the lighter profile and `--dry-run`, and defaults
        to No — EOF or a plain Enter skips the download.

        Returns True when the local LLM should be provisioned now (already
        on disk, or the user explicitly said yes).
        """
        from axiom.setup.llamafile import SMALL_MODEL, is_llamafile_installed

        if is_llamafile_installed(model=model):
            return True  # nothing left to download

        cli = _get_branding().cli_name
        profile = resolve_model(model)
        size = profile.get("size_gb")
        size_label = f"{size}GB" if size else "a large"
        renderer.text(
            f"The local LLM profile '{model}' is a {size_label} download "
            f"({profile['gguf']}).\n"
        )
        if model != SMALL_MODEL:
            small_size = resolve_model(SMALL_MODEL).get("size_gb")
            renderer.text(
                f"Lighter option ({small_size}GB):     {cli} config --model {SMALL_MODEL}\n"
                f"Preview without downloading: {cli} config --dry-run\n"
            )
        if renderer.prompt_yn(f"Download {size_label} now?", default=False):
            return True
        renderer.info(
            f"Skipped the model download — set up a local LLM later with "
            f"'{cli} config --model {model}' (or a cloud API key via '{cli} config')."
        )
        return False
    ...
    def _phase_infra(self) -> None:
        ...
        if not renderer.prompt_yn("Set up local database infrastructure now?", default=True):
            renderer.info(f"Skipped — you can set this up later with: {_get_branding().cli_name} infra")
        ...
        if infra_path == "k3d":
            ...
            if result.success:
                ...
                renderer.text(f"You can complete setup later with: {_get_branding().cli_name} infra")
                self.state.infra_configured = False
        else:
            ...
            # No multi-GB pull without an explicit, size-stating yes.
            provision_llm = self._confirm_model_download(chosen_model)
            used_path = provision_infrastructure(
                callback=_cb, model=chosen_model, provision_llm=provision_llm
            )
    ...
    def _phase_services(self) -> None:
        ...
        for r in results:
            if r.ok:
                ...
                renderer.warning(f"  Heal later with: {_get_branding().cli_name} agents start {r.agent_name}")
