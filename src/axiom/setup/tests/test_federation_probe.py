...
def test_render_prompt_includes_all_required_fields():
    ...
    assert "routing tier: public" in text, (
        "a non-export-controlled tier must be named as a routing tier"
    )
    assert "EC-safe" not in text, (
        "the probe must not render a compliance verdict from a config value"
    )
...
def test_render_prompt_names_the_tier_without_claiming_safety():
    result = fp.ProbeResult(
    ...
    text = fp.render_prompt(result)
    # The tier is reported; safety is not asserted. `routing_tier` says where
    # this deployment SENDS content, not whether an environment is authorised
    # to hold it — that is an attestation, not a setting.
    assert "routing tier: export-controlled" in text
    assert "not an authorisation" in text
    assert "EC-safe" not in text
...
def test_attach_rag_companions_sets_rag_endpoint(monkeypatch):
    """A sibling RAG connection at the same host:port attaches BOTH the corpus
    label and its endpoint URL (the latter drives `rag.database_url`)."""
    from axiom.setup.federation_probe import ProbeResult, _attach_rag_companions

    llm = type("C", (), {"name": "qwen", "endpoint": "https://node:8766/v1",
                         "display_name": "Qwen"})()
    rag = type("C", (), {"name": "corpus", "endpoint": "https://node:8767",
                         "display_name": "Reactor Corpus"})()

    class _Reg:
        def by_category(self, cat):
            return [rag] if cat == "rag" else []

    r = ProbeResult(connection=llm, reachable=True, latency_ms=10, rag_corpus=None)
    _attach_rag_companions([r], _Reg())
    assert r.rag_corpus == "Reactor Corpus"
    assert r.rag_endpoint == "https://node:8767"
