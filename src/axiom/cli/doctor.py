    ...
    *,
    ...
    if not providers:
        return CheckResult(
            ...
            fix_hint=f"{_brand_cli()} connect <preset>  # to wire an LLM endpoint",
            detail={"providers": []},
        )
    ...
    return CheckResult(
        ...
        fix_hint=f"{_brand_cli()} connect status  # to inspect; verify VPN / endpoint URL",
        detail=detail,
    )
...
def check_rag_corpus(*, rag_root: Path | None = None) -> CheckResult:
    ...
    if not health.corpora:
        return CheckResult(
            ...
            fix_hint=f"{_brand_cli()} rag ingest <document>  # to populate a corpus",
            detail=detail,
        )
...
def check_extension_manifests() -> CheckResult:
    ...
    if invalid:
        ...
        return CheckResult(
            ...
            fix_hint=f"{_brand_cli()} ext lint  # to inspect failing manifests",
            detail=detail,
        )
    return CheckResult(
        status=CheckStatus.OK,
...
def check_runtime_config(*, config_dir: Path | None = None) -> CheckResult:
    ...
    if not cfg.exists():
        return CheckResult(
            ...
            fix_hint=f"mkdir -p {cfg} && {_brand_cli()} connect <preset>",
            detail=detail,
        )
    ...
    if missing:
        return CheckResult(
            ...
            fix_hint=f"{_brand_cli()} config  # to walk the onboarding wizard",
            detail=detail,
        )
    return CheckResult(
        status=CheckStatus.OK,
...
def check_memory_store(*, data_root: Path | None = None) -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            ...
            fix_hint=f"pip install -e .  # then re-run `{_brand_cli()} doctor`",
            detail={
                "data_root": str(root),
...
def check_axiom_memory_mcp_registered() -> CheckResult:
    ...
    interpreter the registrar itself resolves (the pinned service venv when
    present, else the python this doctor runs under) — so a deliberately
    pinned registration is never flagged. Common cause: venv switch / repo
    move; fix is the same as missing.
    """
    try:
    ...
    if not status.get("registered"):
        ...
        return CheckResult(
            ...
            fix_hint=f"{_brand_cli()} memory register-mcp",
            detail={
    ...
    if status.get("stale"):
        return CheckResult(
            ...
            summary=(
                ...
                f"(registered={status.get('command')}, expected={expected})"
            ),
            fix_hint=f"{_brand_cli()} memory register-mcp",
            detail={
                "config_path": status.get("config_path", ""),
...
def check_unified_mcp_installed() -> CheckResult:
    ...
    if detected:
        return CheckResult(
            ...
            fix_hint=f"{_brand_cli()} mcp install",
...
def summarize_principal_distribution(
    owner_counts: dict[str, int], *, pinned: str = ""
) -> dict[str, Any]:
    """Owner distribution across a whole ledger, and whether it has drifted.

    The reconciliation check below sampled the newest 25 fragments. That is the
    right instrument for drift *starting* and blind to drift that already
    happened: measured against a real ledger with 105 fragments stranded across
    five sibling identities over five months, the newest 25 were clean and the
    check passed every time while a twentieth of the ledger sat unreachable.

    A whole-ledger distribution is one pass over data the check already loads,
    so the wider window costs nothing and sees the tail.

    Works without a pin. The old behaviour was to skip entirely when
    ``memory.default_principal`` was unset, but a ledger holding six spellings
    of one person is worth reporting to anyone, pin or no pin — in that case
    the largest holder stands in as the reference.
    """
    # Owners that are legitimately not the user. Counting them would leave the
    # check permanently warning, and a warning that can never clear is one
    # people learn to scroll past — the same failure as a gate that can never
    # pass being a gate that is always bypassed.
    non_human = {"axiom-system", "smoketest@example.org"}
    counts = {k: v for k, v in (owner_counts or {}).items() if k not in non_human}
    total = sum(counts.values())
    if not counts:
        return {
            "has_drift": False, "drift_total": 0, "total": 0,
            "dominant": "", "others": [], "counts": {},
        }

    dominant = pinned if pinned in counts else max(counts, key=lambda k: counts[k])
    # An empty principal counts as drift rather than being skipped: a write
    # that recorded no owner at all is its own bug, and dropping it hides a
    # resolver that produced nothing.
    drift_total = sum(v for k, v in counts.items() if k != dominant)
    others = sorted(
        (k for k in counts if k != dominant),
        key=lambda k: counts[k], reverse=True,
    )
    return {
        "has_drift": drift_total > 0,
...
def check_axiom_memory_principal_reconciliation() -> CheckResult:
    ...
    if not pinned:
        return CheckResult(
            ...
            fix_hint=(
                f"{_brand_cli()} settings --global set memory.default_principal "
                "<your-principal-id>"
            ),
            detail={"pinned": ""},
        )
    ...
    # The recent sample answers "is drift starting". It cannot answer "has
    # drift already happened" — measured on a real ledger, 105 fragments across
    # five sibling identities spanning five months sat behind a newest-25 window
    # that was entirely clean. The whole-ledger pass below is over artifacts
    # already loaded, so the wider window is free.
    ledger_owners: dict[str, int] = {}
    for art in artifacts:
        data = art.data or {}
        owner = (data.get("ownership") or {}).get("master") or ""
        ledger_owners[owner] = ledger_owners.get(owner, 0) + 1
    ledger = summarize_principal_distribution(ledger_owners, pinned=pinned)
    ...
    if drift == 0 and ledger["has_drift"]:
        # Recent writes are clean and the history is not. This is the case the
        # sample was blind to, and the one that strands memories: they are
        # present, signed, and unreachable from a default read.
        return CheckResult(
            status=CheckStatus.WARNING,
            summary=(
                f"recent writes are clean, but {ledger['drift_total']} of "
                f"{ledger['total']} fragments in the ledger are owned by "
                f"{len(ledger['others'])} other identity/identities and will "
                f"not surface in reads as {pinned}"
            ),
            fix_hint=(
                f"{_brand_cli()} identity drift   # then: "
                f"{_brand_cli()} identity merge <from> --into {pinned}"
            ),
            detail={
                "pinned": pinned,
                "recent_sample_clean": True,
                "ledger_total": ledger["total"],
                "ledger_drift": ledger["drift_total"],
                "ledger_owners": ledger["counts"],
            },
        )
    ...
    if drift == 0:
        return CheckResult(
            status=CheckStatus.OK,
            summary=(
                f"pin matches all {total} recent writes and all "
                f"{ledger['total']} ledger fragments (principal={pinned})"
            ),
            fix_hint=None,
            detail={
                "pinned": pinned,
...
def check_axiom_memory_heartbeat_freshness() -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            ...
            fix_hint=f"{_brand_cli()} memory heartbeat",
            detail={"exception": type(exc).__name__, "message": str(exc)},
        )
    ...
    if state == "warn":
        return CheckResult(
            ...
            fix_hint=(
                f"{_brand_cli()} memory heartbeat  # cron/launchd may have stalled; "
    ...
    if status.get("reason") == "no_heartbeat":
        return CheckResult(
            ...
            fix_hint=(
                f"{_brand_cli()} memory heartbeat  # one-shot test that the write path works; "
    ...
    return CheckResult(
        ...
        fix_hint=(
            f"{_brand_cli()} memory heartbeat  # then check launchd/systemd cadence"
        ),
        detail=detail,
    )
...
def check_herald_channels(*, env: dict[str, str] | None = None) -> CheckResult:
    ...
    if not declared_raw:
        return CheckResult(
            status=CheckStatus.SKIPPED,
            summary=(
                f"no channels declared — run `{_brand_cli()} notifications setup <channel>` "
    ...
    if missing:
        ...
        return CheckResult(
            ...
            fix_hint=(
                ...
                f"`{_brand_cli()} notifications channels`"
            ),
            detail=detail,
        )
    return CheckResult(
...
def cmd_run(*, argv: Sequence[str], deps: DoctorCliDeps) -> int:
    ...
    parser = argparse.ArgumentParser(prog=f"{_brand_cli()} doctor")
    parser.add_argument(
        "--json",
        action="store_true",
        dest="emit_json",
    ...
    parser.add_argument(
        "--check",
        default=None,
        help=f"Run only the named check (use `{_brand_cli()} doctor list` to see names).",
    )
    ...
    if args.check is not None:
        ...
        if not matched:
            ...
            print(
                f"{_brand_cli()} doctor: unknown check: {args.check}",
                file=sys.stderr,
            )
...
def cmd_list(*, argv: Sequence[str], deps: DoctorCliDeps) -> int:
    ...
    parser = argparse.ArgumentParser(prog=f"{_brand_cli()} doctor list")
    ...
    for category in sorted(by_cat):
        ...
        for check in by_cat[category]:
            print(f"    - {check.name}")
        print()
    print(f"Run a single check with:  {_brand_cli()} doctor --check <name>")
    return 0
