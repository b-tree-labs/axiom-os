...
def main(argv: list[str] | None = None) -> int:
    ...
    ctx = SkillContext(
        ...
        # The user state dir, not cwd — the action audit (and anything else
        # keyed off ctx.state_dir) must never land inside whatever repo the
    ...
    if args.verb == "emit-md":
        ...
        result = invoke_capability(
            default_registry(), "skills.emit_md", params, ctx, surface=CLI_SURFACE
        )
