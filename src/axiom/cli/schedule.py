...
def main(args: list[str] | None = None) -> int:
    ...
    if parsed.action == "create":
        try:
            ...
            parser.exit(2, f"{_brand_cli()} schedule create: {exc}\n")
        ...
        if not report.outcomes:
            ...
            if mismatched:
                ...
                msg_lines.append(
                    ...
                    f"or recreate the schedule with `{_brand_cli()} schedule create "
    ...
    if parsed.action == "uninstall":
        ...
        if not artifact_path.exists():
            ...
            print(f"{_brand_cli()} schedule uninstall: nothing to remove{hint}")
            return 1
