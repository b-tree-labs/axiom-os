...
def cmd_new(argv: Sequence[str], deps: PlanCliDeps) -> int:
    parser = argparse.ArgumentParser(prog=f"{_brand_cli()} plan new")
...
def cmd_show(argv: Sequence[str], deps: PlanCliDeps) -> int:
    parser = argparse.ArgumentParser(prog=f"{_brand_cli()} plan show")
    ...
    try:
        ...
        print(f"{_brand_cli()} plan: {exc}", file=sys.stderr)
...
def cmd_edit(argv: Sequence[str], deps: PlanCliDeps) -> int:
    parser = argparse.ArgumentParser(prog=f"{_brand_cli()} plan edit")
    parser.add_argument("plan_id")
    args = parser.parse_args(list(argv))
    ...
    try:
        ...
        print(f"{_brand_cli()} plan edit: parse failed: {exc}", file=sys.stderr)
        return exc.exit_code
...
def cmd_import(argv: Sequence[str], deps: PlanCliDeps) -> int:
    parser = argparse.ArgumentParser(prog=f"{_brand_cli()} plan import")
    ...
    src = Path(args.path)
    if not src.exists():
        print(f"{_brand_cli()} plan import: file not found: {src}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"{_brand_cli()} plan import: parse failed: {exc}", file=sys.stderr)
        return exc.exit_code
...
def cmd_refresh(argv: Sequence[str], deps: PlanCliDeps) -> int:
    parser = argparse.ArgumentParser(prog=f"{_brand_cli()} plan refresh")
    parser.add_argument("plan_id")
    parser.add_argument("--md-out", default=None)
    args = parser.parse_args(list(argv))
...
def _record_decision(
    *,
    ...
    if not raci.can_approve(deps.current_user):
        print(
            f"{_brand_cli()} plan: principal {deps.current_user!r} not authorized "
...
def cmd_approve(argv: Sequence[str], deps: PlanCliDeps) -> int:
    parser = argparse.ArgumentParser(prog=f"{_brand_cli()} plan approve")
    parser.add_argument("plan_id")
...
def cmd_reject(argv: Sequence[str], deps: PlanCliDeps) -> int:
    parser = argparse.ArgumentParser(prog=f"{_brand_cli()} plan reject")
    parser.add_argument("plan_id")
    parser.add_argument("--rationale", required=True)
    args = parser.parse_args(list(argv))
    return _record_decision(
...
def cmd_list(argv: Sequence[str], deps: PlanCliDeps) -> int:
    parser = argparse.ArgumentParser(prog=f"{_brand_cli()} plan list")
    parser.add_argument("--scope", default=None)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if handler is None:
        print(f"{_brand_cli()} plan: unknown action {action!r}", file=sys.stderr)
