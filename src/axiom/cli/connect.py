...
def _expand_env(text: str | None) -> str:
    ...
    if missing:
        ...
        raise ConnectCliError(
            ...
            f"Set them before running '{_brand_cli()} connect <preset>' "
...
def cmd_list(*, argv: Sequence[str], deps: ConnectCliDeps) -> int:
    parser = argparse.ArgumentParser(prog=f"{_brand_cli()} connect list")
    ...
    print()
    print(f"Apply with:  {_brand_cli()} connect <preset>")
    return 0
...
def cmd_apply(*, argv: Sequence[str], deps: ConnectCliDeps) -> int:
    parser = argparse.ArgumentParser(prog=f"{_brand_cli()} connect <preset>")
    parser.add_argument("preset", help=f"Preset name (see '{_brand_cli()} connect list').")
    parser.add_argument(
        "--no-test",
        action="store_true",
    ...
    if matched is None:
        ...
        print(
            f"{_brand_cli()} connect: preset not found: {args.preset}",
            file=sys.stderr,
        )
    ...
    try:
        ...
        print(f"{_brand_cli()} connect: {exc}", file=sys.stderr)
    ...
    if not args.no_test:
        ...
        if failed:
            print(
                f"{_brand_cli()} connect: preset '{matched.name}' has unreachable endpoints:",
                file=sys.stderr,
            )
    ...
    # ── Confirmation ───────────────────────────────────────────────────────
    print(f"{_brand_cli()} connect: applied preset '{matched.name}'")
...
def cmd_status(*, argv: Sequence[str], deps: ConnectCliDeps) -> int:
    parser = argparse.ArgumentParser(prog=f"{_brand_cli()} connect status")
    ...
    if args.preset is not None:
        ...
        matched = next((p for p in presets if p.name == args.preset), None)
        if matched is None:
            print(f"{_brand_cli()} connect status: preset not found: {args.preset}", file=sys.stderr)
            return 2
        try:
            resolved = [_resolve_provider(p) for p in matched.providers]
        except ConnectCliError as exc:
            print(f"{_brand_cli()} connect status: {exc}", file=sys.stderr)
            return exc.exit_code
    ...
    if not triples:
        print(f"{_brand_cli()} connect status: no providers currently configured.")
        print(
            f"  (looked in {deps.runtime_config_dir}/llm-providers.toml + "
            f"{deps.runtime_config_dir}/rag-packs.toml)"
        )
        print(f"  Run '{_brand_cli()} connect list' to see available presets.")
        return 0
...
def _legacy_connect_main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"{_brand_cli()} connect: legacy connection setup unavailable: {exc}", file=sys.stderr)
        return 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    apply_p = sub.add_parser(
        "apply",
        help=f"Apply a preset (alias for `{_brand_cli()} connect <name>`).",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if argv[0] in ("-h", "--help"):
        ...
        print(
            ...
            f"legacy `{_brand_cli()} connect` (connection-credential setup)."
        )
        return 0
