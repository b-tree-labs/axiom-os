...
def _builtin_templates() -> list[Template]:
    from axiom.cli.ext.templates import lifecycle as _lifecycle
    from axiom.cli.ext.templates import scaffold as _scaffold

    _KIND_DESC = {
        "conform": "ADR-005 conform normalizer — one bronze->silver pure fn + a can-fire test",
        "alerting": "ADR-005 alerting monitor — a check(client) rule + a can-fire test",
        "analytics": "ADR-005 analytics job — read->compute->emit + a can-fire test",
        "ingestion": "ADR-005 ingestion provider — a batch/push read() + a can-fire test",
    }
    ...
    return [
        ...
        *[
            Template(id=k, description=_KIND_DESC[k], create=_lifecycle.make_create(k))
            for k in _lifecycle.list_kinds()
        ],
    ]
