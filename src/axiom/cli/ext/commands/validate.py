    ...
    if not manifest_path.exists() or not pyproj_path.exists():
        results.append(
            ValidationResult(
                ...
                remediation=f"run `{_brand_cli()} ext lint` first and resolve file-presence errors",
            )
        )
        return results
...
def run_standard_tests(ext_path: Path) -> ValidationResult:
    ...
    return ValidationResult(
        ...
        remediation=(
            "" if ok else f"run `{_brand_cli()} ext test` locally and fix the failures"
        ),
    )
...
class ValidateProvider:
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument("--env", choices=["staging", "prod"], default=None,
                            help="Run the in-env production-validation smoke (ADR-005 rung 5) instead of static validation")
        parser.add_argument(
            "--skip-tests",
            action="store_true",
    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        ...
        if getattr(args, "env", None):
            try:
                return self._smoke(target, args.env)
            except RuntimeError as exc:
                con = console()
                con.print(f"{_brand_cli()} ext validate --env {args.env}: cannot run here — {exc}")
                con.print(
                    "         → This is the rung-5 production-validation smoke; run it in the "
                    "target env (staging/prod) with DATABASE_URL set, not on a dev box."
                )
                return 1
        ...
        con = console()
        con.print(f"{_brand_cli()} ext validate: {target.name}")
        con.print("")
        for r in results:
        ...
        next_steps(
            [
                f"{_brand_cli()} ext test                 # Full test suite",
                f"{_brand_cli()} ext publish --yes        # Sign + register locally",
            ]
        )
        return 0
...
def _validate_env_smoke(target, env: str) -> int:
    """ADR-005 rung 5: per-kind production-validation smoke, HERALD-reported."""
    from axiom.cli.ext._output import console
    from axiom.cli.ext.lifecycle import real_deps, validate as _lifecycle_validate
    deps = real_deps()
    res = _lifecycle_validate(target, env, prober=deps["prober"], herald=deps["herald"])
    con = console()
...
ValidateProvider._smoke = staticmethod(_validate_env_smoke)
