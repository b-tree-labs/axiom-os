...
# Lower case, like every other table's headings. This one shouted while its
# neighbours did not, which reads as emphasis nobody intended.
...
def _format_table(rows: list[ListRow]) -> str:
    """Render the extension list as a bordered grid.

    Sized to the real terminal rather than to its own content: an extension
    installed from a long path used to push the status column off the edge,
    because a table that only measures itself cannot know how much room it has.
    """
    if not rows:
        return ""
    ...
    return "\n".join(
        table(
            [(r.name, r.version, r.source, r.status) for r in rows],
            [
                Column(_COLUMNS[0]),
                Column(_COLUMNS[1]),
                # The source is a path or an index name and is the value most
                # likely to be enormous; it gives up width before the others.
                Column(_COLUMNS[2], wrap=True),
                Column(_COLUMNS[3]),
            ],
...
class ListProvider:
    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        ...
        if not rows:
            ...
            next_steps(
                [
                    f"{_brand_cli()} ext init my-extension    # Create a new extension",
                    f"{_brand_cli()} ext search <query>       # Find an extension to install",
                    f"{_brand_cli()} ext install <name>       # Install from the registry",
                ],
                header="Get started:",
            )
            return 0
