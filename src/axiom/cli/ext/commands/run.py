...
class RunProvider:
    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        ...
        try:
            ...
            con.print(f"{_brand_cli()} ext run: {exc}")
            return 2

        ext_path = _installed_extension_path(ext_name)
        if ext_path is None:
            con.print(
                f"{_brand_cli()} ext run: extension {ext_name!r} is not installed. "
                f"Run `{_brand_cli()} ext list` to see installed extensions."
            )
            return 1
        ...
        if kind is None and name is None:
            ...
            if sole is None:
                if not all_cmds:
                    con.print(
                        f"{_brand_cli()} ext run: no runnable cmd declared on "
                ...
                con.print(
                    f"{_brand_cli()} ext run: {ext_name!r} declares multiple cmds; "
                    f"pick one — {labels}"
                )
                return 1
            kind = "cmd"
            ...
            if not name:
                con.print(
                    f"{_brand_cli()} ext run: {ext_name!r} cmd block has no name/noun"
                )
                return 1

        if kind not in _SUPPORTED_KINDS:
            con.print(
                f"{_brand_cli()} ext run: does not yet support kind={kind!r} "
        ...
        if entry is None:
            con.print(
                f"{_brand_cli()} ext run: capability {args.capability!r} not declared in "
        ...
        try:
            ...
            con.print(f"{_brand_cli()} ext run: failed to import entry {entry!r}: {exc}")
            return 1
        ...
        try:
            ...
            con.print(f"{_brand_cli()} ext run: capability raised {type(exc).__name__}: {exc}")
            return 1
