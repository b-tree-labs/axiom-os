    ...
    if key_resolved is None:
        return VerifyResult(
            ...
            detail=(
                ...
                f"run `{_brand_cli()} ext sign` on this host to self-trust, or drop "
...
class VerifyProvider:
    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        ...
        con = console()
        if result.ok:
            con.print(f"{_brand_cli()} ext verify: OK — {artifact.name}")
            if result.publisher:
        ...
        con.print(f"{_brand_cli()} ext verify: FAIL — {artifact.name}")
        if result.detail:
