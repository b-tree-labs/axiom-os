    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        if args.registry_override is not None:
            if not args.registry_override.startswith("file://"):
                print(
                    f"{_brand_cli()} ext show: --registry must use the file:// scheme "
        ...
        try:
            ...
            print(f"{_brand_cli()} ext show: {exc}")
            return 2
        ...
        if args.installed:
            ...
            if view is None:
                print(
                    f"{_brand_cli()} ext show: {name} is not axi-installed; "
                    f"run `{_brand_cli()} ext list` to see installed extensions, or "
            ...
            if version is not None and view.version != version:
                print(
                    f"{_brand_cli()} ext show: {name} is installed at "
            ...
            if view is None:
                ...
                print(
                    f"{_brand_cli()} ext show: {where} not found in the registry; "
        ...
        if not view.installed and view.source == "registry":
            ...
            next_steps(
                [
                    f"{_brand_cli()} ext install {view.name}    # Install the latest",
                ]
            )
        return 0
