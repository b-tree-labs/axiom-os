...
class QuickstartProvider:
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--publish",
            action="store_true",
            help=f"Compose `{_brand_cli()} ext publish --yes` as a final step",
        )
        parser.add_argument(
            "--yes",
            action="store_true",
    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        ...
        if rc != 0 or ext_path is None:
            error(
                f"{_brand_cli()} ext quickstart: init failed; see message above.",
        ...
        # --- lint ---------------------------------------------------------
        con.print(f"{_brand_cli()} ext quickstart: linting {ext_path.name}")
        ...
        if lint_errors:
            ...
            error(
                f"{_brand_cli()} ext quickstart: lint found {len(lint_errors)} error(s)",
                hint=f"run `{_brand_cli()} ext lint {ext_path}` for full details",
            )
            return 1
        status("pass", "lint", "no errors")

        # --- validate (skip tests to keep the chain fast) -----------------
        con.print(f"{_brand_cli()} ext quickstart: validating {ext_path.name}")
        ...
        if val_failures:
            ...
            error(
                f"{_brand_cli()} ext quickstart: validate found {len(val_failures)} failure(s)",
                hint=f"run `{_brand_cli()} ext validate {ext_path}` for the full report",
            )
            return 1
        status("pass", "validate", f"{len(val_results)} checks passed")

        # --- scan ---------------------------------------------------------
        con.print(f"{_brand_cli()} ext quickstart: scanning {ext_path.name}")
        ...
        if scan_result.hard_failure:
            ...
            error(
                f"{_brand_cli()} ext quickstart: scan found hard failure(s)",
                hint=f"run `{_brand_cli()} ext scan {ext_path}` for details",
            )
            return 1
        ...
        if args.publish or args.yes:
            con.print(f"{_brand_cli()} ext quickstart: publishing {ext_path.name}")
            try:
                publish_extension(
                    ext_path,
                    yes=True,
                ...
                error(
                    f"{_brand_cli()} ext quickstart: publish failed: {exc}",
                    hint=f"run `{_brand_cli()} ext publish {ext_path} --yes` once fixed",
                )
                return 1
            ...
            next_steps(
                [
                    f"{_brand_cli()} ext install {args.name}    # Install the just-published version",
                    f"{_brand_cli()} ext list                       # Confirm registry + install state",
                ]
            )
            return 0
        ...
        next_steps(
            [
                f"{_brand_cli()} ext publish {args.name} --yes   # Sign and register locally",
