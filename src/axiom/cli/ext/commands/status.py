...
def _render_welcome(d: StatusDashboard) -> None:
    con = console()
    heading(f"Welcome to {_brand_cli()} ext")
    con.print("")
    con.print(
    ...
    next_steps(
        [
            f"{_brand_cli()} ext quickstart <name> # Scaffold + lint + validate + scan in one",
            f"{_brand_cli()} ext init <name>       # Scaffold only (wizard with --interactive)",
            f"{_brand_cli()} ext search <query>    # Find extensions in the registry",
        ],
...
def _render_dashboard(d: StatusDashboard) -> None:
    con = console()
    heading(f"{_brand_cli()} ext status")
    con.print("")
