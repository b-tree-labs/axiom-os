    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        ...
        if args.from_version is not None or args.to_version is not None:
            if args.from_version is None or args.to_version is None:
                con.print(
                    f"{_brand_cli()} ext migrate: both --from-version and --to-version "
            ...
            if hop not in _SUPPORTED_VERSION_HOPS:
                con.print(
                    f"{_brand_cli()} ext migrate: no migration registered for "
                    f"{args.from_version} → {args.to_version}"
                )
                return 1
            if _SUPPORTED_VERSION_HOPS[hop] == "no-op":
                con.print(
                    f"{_brand_cli()} ext migrate: no migration between {args.from_version} "
                    f"and {args.to_version} (AEOS spec versions are identical)"
                )
                return 0
            # Future spec versions plug in their handler here.
            con.print(f"{_brand_cli()} ext migrate: version handler not implemented")
            return 1
        ...
        if _already_aeos_compound(target):
            con.print(
                f"{_brand_cli()} ext migrate: {target} already matches the AEOS compound "
        ...
        try:
            ...
            con.print(f"{_brand_cli()} ext migrate: {exc}")
            return 1
        except Exception as exc:  # noqa: BLE001
            con.print(f"{_brand_cli()} ext migrate: failed to build plan — {exc}")
            return 1

        con.print(f"{_brand_cli()} ext migrate: detected pre-AEOS layout at {target}")
        ...
        if args.dry_run:
            con.print(f"{_brand_cli()} ext migrate: dry-run — disk untouched.")
            return 0
        ...
        if not proceed:
            ...
            if sys.stdin.isatty() and sys.stdout.isatty():
                ...
                if resp.strip().lower() not in {"", "y", "yes"}:
                    con.print(f"{_brand_cli()} ext migrate: aborted by user.")
                    return 1
                proceed = True
            else:
                con.print(
                    f"{_brand_cli()} ext migrate: use --yes to confirm or "
        ...
        try:
            ...
            con.print(
                f"{_brand_cli()} ext migrate: failed — {exc}. "
        ...
        con.print(
            f"{_brand_cli()} ext migrate: moved {len(summary['moves'])} files into "
            f"{summary['new_root']}"
        )
        if summary["other"]:
            con.print(
                f"{_brand_cli()} ext migrate: {len(summary['other'])} file(s) had no "
