...
class ConfigProvider:
    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        ...
        if not tokens:
            print(f"{_brand_cli()} ext config: missing operation (get/set/list/unset)")
            return 2
        ...
        if tokens[0] in _OPS:
            ...
            if ext_name is None:
                print(
                    f"{_brand_cli()} ext config: no extension specified and current directory "
            ...
            if len(tokens) < 2 or tokens[1] not in _OPS:
                print(
                    f"{_brand_cli()} ext config: unknown operation "
    ...
    def _op_get(self, ext_name: str, rest: list[str]) -> int:
        if len(rest) != 1:
            print(f"{_brand_cli()} ext config: `get` requires exactly one <key> argument")
            return 2
        data = _load_config(ext_name)
        key = rest[0]
        if key not in data:
            print(f"{_brand_cli()} ext config: {ext_name}: key {key!r} is not set")
            return 1
        print(data[key])
        return 0
    ...
    def _op_set(self, ext_name: str, rest: list[str]) -> int:
        if len(rest) != 2:
            print(f"{_brand_cli()} ext config: `set` requires <key> <value>")
            return 2
        key, value = rest
        data = _load_config(ext_name)
        data[key] = value
        _save_config(ext_name, data)
        print(f"{_brand_cli()} ext config: {ext_name}: set {key}={value}")
        return 0
    ...
    def _op_list(self, ext_name: str, *, as_json: bool) -> int:
        ...
        if as_json:
            ...
            return 0
        if not data:
            print(f"{_brand_cli()} ext config: {ext_name}: (no keys set)")
            return 0
        from axiom.infra.cli_format import SQUARE, Column, table, terminal_width

        for line in table(
            [(k, str(data[k])) for k in sorted(data)],
            [Column("key"), Column("value", wrap=True)],
    ...
    def _op_unset(self, ext_name: str, rest: list[str]) -> int:
        if len(rest) != 1:
            print(f"{_brand_cli()} ext config: `unset` requires exactly one <key> argument")
            return 2
        key = rest[0]
        data = _load_config(ext_name)
        if key in data:
            del data[key]
            _save_config(ext_name, data)
            print(f"{_brand_cli()} ext config: {ext_name}: unset {key}")
        else:
            # Idempotent — unsetting a missing key is a no-op.
            print(f"{_brand_cli()} ext config: {ext_name}: {key} was not set")
        return 0
