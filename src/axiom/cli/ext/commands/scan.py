    ...
    if not manifest_path.exists():
        return (
            ScanCheck(
                ...
                remediation=f"run `{_brand_cli()} ext init` or create the manifest",
            ),
            None,
        )
    try:
    ...
    if errors:
        return (
            ScanCheck(
                ...
                remediation=f"run `{_brand_cli()} ext lint` for the full findings list",
            ),
            None,
        )
    return (
...
class ScanProvider:
    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        ...
        con = console()
        con.print(f"{_brand_cli()} ext scan: {target.name}")
        con.print("")
        ...
        if result.hard_failure:
            ...
            next_steps(
                [
                    f"{_brand_cli()} ext sign                 # Build + sign the artifact",
                    f"{_brand_cli()} ext publish --yes        # Sign + register in one step",
                ]
            )
        return exit_code
