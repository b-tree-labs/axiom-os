    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        ...
        if args.installed:
            ...
            if not (target / "axiom-extension.toml").exists():
                con.print(
                    f"{_brand_cli()} ext graph: {target} is not an extension "
        ...
        if args.svg_path or args.render:
            ...
            if rc != 0:
                return rc
            con.print(f"{_brand_cli()} ext graph: wrote {svg_target}")
            if args.render:
        ...
        if args.output:
            ...
            con.print(f"{_brand_cli()} ext graph: wrote {out_path}")
            return 0
...
def _resolve_svg_target(explicit: str | None) -> Path | None:
    ...
    try:
        ...
        error(f"{_brand_cli()} ext graph: cannot create {target.parent}: {exc}")
        return None
    return target
...
def _render_svg(mermaid: str, target: Path) -> int:
    ...
    if mmdc is None:
        error(
            f"{_brand_cli()} ext graph: mmdc (mermaid-cli) not found on PATH",
    ...
    try:
        ...
        error(f"{_brand_cli()} ext graph: failed to invoke mmdc")
        return 2
    finally:
        try:
            os.unlink(mmd_path)
    ...
    if proc.returncode != 0:
        error(
            f"{_brand_cli()} ext graph: mmdc exited {proc.returncode}",
