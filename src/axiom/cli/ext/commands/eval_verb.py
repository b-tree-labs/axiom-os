...
class EvalProvider:
    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        ...
        if runner == "none":
            ...
            con.print(f"{_brand_cli()} ext eval: {target.name}: no eval suite configured")
            return 0

        if runner == "promptfoo":
            cmd = ["npx", "promptfoo", "eval", "-c", str(rpath)]
            con.print(f"{_brand_cli()} ext eval: running promptfoo ({rpath})")
            proc = subprocess.run(cmd, cwd=str(target))
            return proc.returncode

        if runner == "pytest":
            cmd = [sys.executable, "-m", "pytest", str(rpath), "-v"]
            con.print(f"{_brand_cli()} ext eval: running pytest ({rpath})")
            proc = subprocess.run(cmd, cwd=str(target))
            return proc.returncode

        # Defensive — _detect_eval_runner should not return anything else.
        con.print(f"{_brand_cli()} ext eval: unknown runner {runner!r}")
        return 1
