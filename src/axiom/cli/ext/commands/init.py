...
class InitProvider:
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--template",
            default=None,
            help=f"Template id from `{_brand_cli()} ext templates` (default: the registry default)",
        )
        parser.add_argument(
            "--interactive",
            action="store_true",
    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        ...
        if wants_wizard:
            if not sys.stdin.isatty():
                if getattr(args, "interactive", False):
                    error(
                        f"{_brand_cli()} ext init: interactive mode requires a TTY",
                        hint=f"pass a name positionally, e.g. `{_brand_cli()} ext init <name>`",
                    )
                    return 2
                # No --interactive and no name in a non-TTY context: match the
                # pre-wizard behavior with a clear argparse-like message.
                error(
                    f"{_brand_cli()} ext init: the following arguments are required: name",
        ...
        if err_msg is not None:
            error(f"{_brand_cli()} ext init: {err_msg}")
            return 2

        resolved_license = resolve_spdx(args.license)
        if resolved_license is None:
            error(
                f"{_brand_cli()} ext init: unknown license {args.license!r}",
        ...
        if args.template is None:
            ...
            if template is None:
                error(
                    f"{_brand_cli()} ext init: unknown template {args.template!r}",
                    hint=f"run `{_brand_cli()} ext templates` to see registered templates",
                )
                return 2
    ...
    def _run_wizard(
        ...
        con = console()
        heading(f"{_brand_cli()} ext init — guided scaffold")
        con.print("")
        ...
        try:
            ...
            con.print("")
            con.print(f"{_brand_cli()} ext init: aborted.")
            return 0
        ...
        if ext_dir.exists():
            error(
                f"{_brand_cli()} ext init: refusing to overwrite existing directory {ext_dir}"
            )
            return 1
    ...
    def _prompt_template(self, *, explicit: str | None):
        if explicit is not None:
            ...
            if template is None:
                error(
                    f"{_brand_cli()} ext init: unknown template {explicit!r}",
                    hint=f"run `{_brand_cli()} ext templates` to see registered templates",
                )
...
def _scaffold_and_report(
    ...
    *,
    ...
    try:
        ...
        error(f"{_brand_cli()} ext init: failed to scaffold {ext_dir}: {exc}")
        return 1
    ...
    next_steps(
        [
            f"cd {ext_dir}",
            f"{_brand_cli()} ext lint         # Bronze conformance check",
            f"{_brand_cli()} ext test         # Run the standard tests",
            f"{_brand_cli()} ext publish --yes # Sign and register locally",
        ]
    )
    return 0
