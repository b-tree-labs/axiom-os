...
def _resolve_or_generate_keypair(
    *,
    ...
    announce(
        f"{_brand_cli()} ext sign: created publisher identity\n"
...
class SignProvider:
    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        ...
        if not manifest_path.exists():
            print(
                f"{_brand_cli()} ext sign: {target} does not look like an extension "
        ...
        try:
            ...
            print(f"{_brand_cli()} ext sign: {exc}")
            return 1
        ...
        next_steps(
            [
                f"{_brand_cli()} ext verify              # Confirm the signature resolves",
                f"{_brand_cli()} ext publish             # Register with the local file:// registry",
            ]
        )
        return 0
