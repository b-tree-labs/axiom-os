        ...
        if not (layout_root / dirname).is_dir():
            findings.append(
                _warn(
                    code="AEOS040",
                    message=(
                        f"manifest provides {kind!r} but the {dirname}/ "
                        f"subdirectory is missing"
                    ),
                    remediation=(
...
def _secret_dependency_findings(manifest: dict[str, Any]) -> list[Finding]:
    ...
    if not isinstance(block, dict) or not isinstance(requires, list) or not all(
        isinstance(n, str) for n in (requires or [])
    ):
        return [
            _error(
                code="AEOS080",
...
class LintProvider:
    ...
    def run(self, args: argparse.Namespace, context: CliContext) -> int:
        ...
        if not findings:
            con.print(
                f"{_brand_cli()} ext lint: {target.name}: "
