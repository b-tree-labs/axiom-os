...
def table(title: str, columns: list[str], rows: list[list[str]]) -> None:
    ...
    # Plain-text fallback — a bordered grid sized to the real terminal.
    # The previous version sized columns to their own content and drew a
    # separator as long as the header, so a wide row ran past the edge and the
    # rule stopped short of the thing it was supposed to be under.
    if not rows:
        if title:
            con.print(title)
    ...
    from axiom.infra.cli_format import SQUARE, Column, terminal_width
    from axiom.infra.cli_format import table as _grid

    if title:
        con.print(title)
    # The last column is the one that gives up width: in every caller it is
    # the description or path, and the ones before it are identifiers.
    specs = [Column(c) for c in columns[:-1]] + [Column(columns[-1], wrap=True)]
    for line in _grid(
