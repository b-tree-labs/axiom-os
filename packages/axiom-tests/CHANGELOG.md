...
- `axiom_tests.fixtures.tui`: a headless harness for prompt_toolkit
  applications (`headless_terminal`, the `headless_terminal_factory`
  fixture) and a filter-aware key-binding lint (`find_binding_conflicts`)
  that reports bindings shadowing each other in some reachable UI state and
  single-key bindings that are also the prefix of a longer sequence.
  `prompt_toolkit` is imported lazily; install the new `tui` extra to use it.
