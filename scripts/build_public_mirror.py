...
FORBIDDEN_TERMS = (
    ...
    r"/home/ben\b",
    r"ben-macbook",
    r"\bMPACT\b",         # export-controlled neutronics code (RSICC) — never name it
)
...
#: Token shapes **observed** to make GitHub push protection refuse the mirror
#: push. Evidence-driven on purpose, not a catalogue of every provider format.
#:
#: GitHub's detector is not shape-alone — it also weighs entropy and allowlists
#: well-known documentation values, so this tree already carries
#: ``AKIAIOSFODNN7EXAMPLE``, ``ghp_0123…`` and ``xoxb-1234…`` without ever
#: blocking a push. Guarding on shape alone produced 20 false positives here on
#: the first run, and a guard that cries wolf is a guard someone deletes.
#:
#: Add an entry when a push is actually refused, and cite the run. Do not guess.
#:
#:   - GitLab PAT: refused 2026-09-07, run 34094337851. A test fixture used a
#:     realistic-length value where every sibling test used ``glpat-SENTINEL``.
SCANNER_TRIPPING_PATTERNS = (
    (r"glpat-[A-Za-z0-9_-]{20,}", "GitLab personal access token"),
)

_SCANNER_RE = [
    (re.compile(pattern), label) for pattern, label in SCANNER_TRIPPING_PATTERNS
]


def scan_scanner_tripping(paths: list[str] | None = None) -> list[tuple[str, int, str, str]]:
    """Find strings in the public surface that would block the mirror push.

    The mirror publishes a single squashed commit built from the public tree, so
    anything matching a provider token shape — even an invented fixture — makes
    GitHub refuse the push. That failure is silent from the inside: the mirror
    job goes red, the public repo simply stops updating, and because the
    pre-push hook keys on "last CI run failed", every push to this repo is
    blocked until someone reads the log.

    Returns (path, line number, label, matched prefix). The matched text is
    truncated so a real credential is never echoed into CI output.
    """
    hits: list[tuple[str, int, str, str]] = []
    for rel in paths if paths is not None else public_files():
        path = REPO_ROOT / rel
        try:
        ...
        for lineno, line in enumerate(text.splitlines(), start=1):
            for regex, label in _SCANNER_RE:
                match = regex.search(line)
                if match:
                    hits.append((rel, lineno, label, match.group(0)[:12] + "…"))
    return hits
