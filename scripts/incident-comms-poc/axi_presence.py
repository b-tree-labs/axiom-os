...
def main() -> int:
    ...
    def on_message(msg: ChannelMessage) -> None:
        ...
        if not seeded["done"]:
            ...
            seeded["done"] = True
        try:
            # NOT raw: it empties the system prompt (where the content
            # safeguards live) and drops history, so the presence brief
            # seeded on turn one would not survive to turn two.
            reply = agent.turn(text, stream=False)
