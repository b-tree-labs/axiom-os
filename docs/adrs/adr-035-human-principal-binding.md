...
**Status:** Accepted — implemented (2026-04-27 decision; `accountable_human_id` + `delegation_chain` in `memory/fragment.py`, fragment signing in `memory/attest.py`)
...
Third-party extensions installed via Vyzier (planned / not yet built) or external sources cannot write through CompositionService without honoring this contract. The CompositionService check is the enforcement point; AEOS conformance gates extension publication on it.
