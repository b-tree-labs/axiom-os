...
### 5.10 Alert-directed delegation

**Core, not optional** (Ben, 2026-09-08): *"a key aspect of alerts in an
agentic system is that the agent can help the alert viewer take action."* An
alert that only tells you something happened leaves the hardest part — what to
do — to a person under time pressure, which is precisely when a runbook goes
unread. The delivery mechanism is shipped; this is what makes it worth having.

An alert says something happened. The next question is always *what do I do
about it*, and today the answer lives in a person's head or a runbook nobody
opens under pressure.

The pieces already exist: an extension declares its capabilities in
`axiom-extension.toml`, chat discovers them, and every invocation passes the
`invoke_capability` authority chokepoint. What is missing is the binding from
*this alert* to *those capabilities*.

```
alert:
  summary:  "Feed pump 2 offline — flow at 12% of setpoint"
  link:     https://.../studio/feed-pump-2
  respond_with:                    # capability REFERENCES
    - triga.pump_history
    - triga.scram_procedure
  corpus_hint: "netl/ops/pump-2"   # retrieval scope, not content
```

**The load-bearing rule: an alert REFERENCES capabilities, it never CARRIES
them.** If an alert could supply a tool, the inbox becomes an
authority-injection channel — anything able to write a row could widen what the
agent may do. This is the same rule that already governs alert TEXT (data, not
instruction) applied one level up, to alert STRUCTURE.

So resolution happens at delivery: a named capability must already be declared
by an installed extension and already be permitted for this principal, or it is
dropped with a note. **The alert narrows what is OFFERED; it can never widen
what is ALLOWED.**

Pre-approved operator choices then fall out of that rather than needing their
own mechanism: the offered set is the intersection of what the alert suggests
and what the principal may already do, and selecting one routes through the
same chokepoint and leaves the same receipt as any other invocation.

Open, and worth settling before this is built:

- **Does an unresolvable reference stay visible?** Silently dropping it hides
  that the responder's toolkit is not installed, which is exactly what someone
  needs to know mid-incident.
- **Does the corpus hint scope retrieval, or merely bias it?** Scoping is
  stronger and can hide a relevant document from an adjacent system.
- **Acknowledgement vs action.** Acking records that a person took an alert up
  (§ shipped). Whether invoking a suggested capability should imply
  acknowledgement is a policy question, not a technical one.

### 5.11 Group broadcast

An alert today is addressed to one principal. Real operations address a ROLE —
the shift crew, the reactor supervisors, whoever is on call — and the membership
changes without the monitor knowing.

So a recipient may be a GROUP handle (`@shift-crew:netl`), resolved to members
at delivery. The monitor names the role; the roster is not its business.

**The hard question is acknowledgement, and it must be answered before this is
built.** Two shapes, and they are not interchangeable:

| | fan-out (a row per member) | shared (one row, many readers) |
|---|---|---|
| who saw it | recorded per person | unknown |
| "somebody has this" | not expressible | natural |
| one person acks | others still see it | closes it for everyone |
| suits | *everyone must be aware* | *someone must respond* |

A reactor alert is usually the second — one qualified person taking it up is the
outcome, and five people acknowledging separately is noise. But "everyone must
be aware" is a real category too (a scheduled outage), and picking one shape for
both would make the other silently wrong.

**Proposal:** the group declares which it is, because the group knows and the
monitor does not. `@shift-crew:netl` is a respond-once group;
`@all-operators:netl` is an inform-everyone group. The alert names the group and
inherits its semantics — a monitor should never have to think about this.

Consequences worth stating:

- **"Only the recipient may acknowledge" needs extending**, not replacing: for a
  respond-once group it becomes "any current member", and the acknowledger's
  identity must then be RECORDED, since it is no longer implied by the row. That
  is a schema change (`acknowledged_by`), and it is the reason to settle this
  before the column set hardens.
- **Membership at delivery, or at acknowledgement?** Someone who joins the crew
  after an alert fires arguably should still be able to take it up; someone who
  left should not. Resolving membership only at send time answers neither.
- **Escalation is the natural sequel** and is deliberately out of scope here: if
  nobody acknowledges within N minutes, tell someone else. That needs this
  section settled first.
