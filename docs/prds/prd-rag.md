...
**Tech Specs:** [spec-rag-core.md](../tech-specs/spec-rag-core.md) · [spec-rag-community.md](../specs/spec-rag-community.md) · [spec-rag-knowledge-maturity.md](../specs/spec-rag-knowledge-maturity.md)
...
**Platform-surface enforcement (the MCP retrieve primitive).** The always-on
`axiom_rag__retrieve` MCP tool (`spec-builtin-mcp-server.md` §8) is a broad surface,
so it applies this principle by construction: it runs the real hybrid retrieve over
the configured store with an `AccessContext(max_access_tier="public")` filter and a
DB-backed per-document tier lookup (`RAGStore.access_tier_for`, tightest-tier-wins).
A local store that cannot report tiers **fails closed** (refuses rather than returning
unfiltered chunks); an unknown path is denied, never defaulted to public; and the MCP
client-seam EC gate runs downstream regardless. Per-principal tier expansion (widening
`max_access_tier` for an authorized caller, per §4.2 Session Tier Propagation) is the
documented follow-up — the safe default is public until that lands.
...
|-------|----------------|-----------|-------------|
| ML model federation | Model parameters (gradients/weights) | Flower AI | External ontology/catalog provider |
| Knowledge federation | Validated knowledge facts | Flower AI secure aggregation | Axiom (intelligence/operations) |

Axiom and an external ontology/engineering platform are **peer systems, not
a hierarchy**. The external system owns ontology and lineage. Axiom federates
knowledge and owns operational knowledge, retrieval, and the promotion
pipeline. Neither is subordinate to the other.
...
## Part 11: Corpus storage is bounded (added 2026-09-23)

### 11.1 The problem, measured

A production corpus of 5,304,997 chunks occupies **73 GB**: 13.8 KB per chunk,
for text that averages a small fraction of that. Measured on a live node:

| Component | Size | Note |
|---|---|---|
| Vector index (IVFFlat) | 36 GB | a second full copy of every vector |
| TOAST (text + vectors) | 25 GB | `vector` is `attstorage='e'`, so vectors live here |
| Heap | 7.9 GB | |
| Duplicate GIN index | 1.8 GB | byte-identical to another index on the same expression |
| Nine btree indexes | 0.8 GB | two of them never scanned |

**Roughly 52 of the 73 GB is the embedding**, stored as `vector(768)` — float32,
3072 bytes per row, plus a full copy inside the index. The text is a minority of
its own corpus.

This was discovered as a node outage, not as a finding. Nothing measured it,
nothing bounded it, and no agent owned it.

### 11.2 Requirements

**R-STORE-1 — A corpus declares a size budget.** Every corpus carries a bound.
Exceeding it is a finding on a heartbeat, not a discovery during an incident.

**R-STORE-2 — Embedding precision is a declared choice, not a default.** The
platform stores embeddings at the smallest precision that holds retrieval
quality. float32 is a deliberate selection, never an accident of the default.

**R-STORE-3 — Dimensionality is a declared choice.** Where an embedding model is
Matryoshka-trained, the stored dimensionality is chosen against measured recall
rather than inherited from the model's maximum.

**R-STORE-4 — No quality claim without a measurement.** Any reduction in
precision or dimensionality is accompanied by a recall comparison on a fixed
query set, before and after. A storage win that silently costs recall is a
regression, and the grounding battery already exists to detect it.

**R-STORE-5 — The corpus is reaped.** Generations that no longer serve retrieval
are removed, and content that is byte-identical within a source is stored once.

Byte-identity is judged on the chunk's own text. The existing `chunks.checksum`
column is **not** that: it holds the source file's digest, repeated on every
chunk of the file. Reading it as a chunk identity overstated duplication in
this corpus by more than an order of magnitude. The same text appearing in two
different documents is two facts about two documents and is not reaped.

**R-STORE-6 — Indexes justify themselves.** An index that is never scanned, or
that duplicates another index's expression, is a defect. The platform reports
both.

**R-STORE-7 — An agent owns the bound.** Corpus size has a named owner whose
check runs unattended. An unowned cost is discovered as an outage; this has now
happened once and should not happen twice.

**R-STORE-8 — Ingestion declines what retrieval cannot use.** Content is
assessed before it is embedded, not after it is stored. A chunk that is
measurement data, a repeated reading, a failed text extraction, or a duplicate
of a chunk already offered does not enter the index and does not cost an
embedding call.

The guard is **reported, bounded and switchable**:

- Every refusal states its threshold and the measured value behind it. A
  refusal a human cannot check is one they will switch the guard off over.
- Ingest reports chunks refused, by reason, alongside chunks created. "We
  indexed 40 chunks" and "we indexed 40 of 3,000" are different facts.
- A single switch restores the pre-guard behaviour exactly. Anything that only
  half-disables the guard is worse than no switch at all.
- Thresholds are calibrated against a real corpus and the calibration is
  recorded with its false-positive rate. A guard tuned only against fixtures
  has not been tested against the documents it will refuse.

**R-STORE-9 — Refusing everything is a failure mode with equal standing.** A
guard that admits nothing looks cautious and silently empties the corpus. Tests
covering refusal must be paired with a control of comparable size that must
pass, in the same run, so a threshold change fails at the commit rather than in
a quiet corpus months later.

**R-STORE-10 — The store is watched after it is installed, not only when it is
installed.** Correctness at install time and correctness three months later are
different properties, and only the second matters to the person asking a
question. The platform reports, on its existing unattended heartbeat:

- indexes whose expression duplicates another's under a different name;
- indexes the planner has never chosen;
- a vector index whose parameters were chosen for a corpus it no longer has;
- **a single source file supplying a disproportionate share of the index** —
  the shape signal, which total size cannot show;
- a corpus over its declared bound, or without one, or without an owner;
- the ingestion guard left disabled;
- a database whose shared memory is too small for maintenance to run.

Each of these existed unobserved in a live corpus. None was hidden; nothing was
looking.

**R-STORE-11 — Every check has a negative control.** A check that cannot stay
silent is ignored exactly as fast as a check that cannot fire, and the
difference between them is invisible in a green report. A healthy store
produces no findings at all, and a fresh empty install produces none either —
a report that opens with a page of findings is discredited before it is ever
right.

**R-STORE-12 — The platform provisions the database it needs.** Defaults that
are adequate at installation and fail at scale are the platform's
responsibility, not the operator's to discover. Shared memory sized for
maintenance is the worked example: the container default is enough until a
table is large, and then the table that most needs maintenance is the one that
cannot get it, with the failure appearing only when somebody finally tries.

**R-STORE-13 — Deployment shape is part of the store's correctness.** How the
database is deployed can destroy it in ways no schema check sees. The platform
reports a database that could run two writers against one volume, and one
capped far below the data it serves. Both were live, and one of them aborted a
maintenance operation mid-run.

The corollary matters more than the checks: **the platform's own manifests
being correct is not the same property as the cluster matching them.** The
charts here already had the rollout strategy right; the node was running a
hand-applied manifest that predated them, and nothing compared the two. Only a
check that reads the cluster can tell those apart.

### 11.3 What is explicitly not required

**Refusing binary analysis artifacts at the extractor.** R-STORE-8 governs
chunks, not file types. A 10 MB binary workspace that text-extracts into 817 MB
of numeric mush is a defect in what the extractor accepts, upstream of the
chunk guard and not addressed by it.

**Embedding everything.** The largest storage lever is not compression, it is
declining to index content that retrieval does not need. A corpus with
generations but no reaping only grows. Some harnesses store no vectors at all
and retrieve agentically; that trade is available to a consumer and the platform
does not presume against it.

**A single global policy.** A community corpus, a site corpus and a personal
corpus have different budgets, retention and precision. The bound is per corpus.
