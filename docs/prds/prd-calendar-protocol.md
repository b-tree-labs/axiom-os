...
- The Microsoft 365 Graph OAuth foundation itself. M365 Graph is a Tier-A blocker tracked separately; this PRD commits only to the M365 calendar adapter that rides on top of it once the foundation lands. Note (ADR-110): that foundation is a `service` **Connection Identity Profile** (`auth_mode=service`) — the same app-type model the document mirror exercises at the `delegated_user` pole; the calendar build proceeds against the `fake`/CalDAV/Google vendors and swaps in the service profile at cutover, changing credentials, not architecture.
