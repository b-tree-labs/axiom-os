...
4. **No canonical resolution of the *local* principal.** The three holes above are about which external account a write belongs to. This one is about the substrate disagreeing with itself. Several independent resolvers — `infra/principal.py`, `memory/session.py`, `memory/session_capture.py`, the MCP platform primitives, and `connect/identity_link.py` — each carry their own fallback chain, so the spelling of the *same* local principal depends on which code path performs the write. No persona is involved and no vendor account is involved; the substrate simply answers "who is this" differently depending on where you ask.

   **This is observed, not hypothetical.** A real install carried 11,170 fragments under one identity and 105 more stranded across five other spellings of the same person: a work address, a personal address, a git commit address, a machine-derived handle (`@laptop:<user>`), and one malformed value that welded a `@name:context` prefix onto an email address — a shape no resolver intended to produce. A sixth partition came from branding rather than identity: a second physical ledger under a consumer-branded state directory, holding writes the primary ledger's readers never saw.

   Nothing reported any of it. It surfaced months later because a query returned empty and someone went looking by hand. That is the characteristic failure of this hole and the reason it is worth its own section: **fragmentation degrades recall silently.** A rejected write is visible; a write filed under a spelling nobody queries is not. The memories are present, attributable, correctly signed, and unfindable.
...
axi identity drift                        # owner-identity drift in the local ledger (hole 4)
axi identity merge <from> --into <to>     # fold one spelling into another; re-signs, never rewrites history silently
```
...
### 5.6 Canonical local-principal resolution (hole 4)

§5.5 resolves *which external account* a write belongs to, and falls back to
`active_persona.principal_id`. That fallback assumes the local principal has one
spelling. It does not today, which is what produced the fragmentation in §2
hole 4 — so this section is upstream of §5.5, not a variant of it.

**One resolver.** A single `resolve_local_principal()` is the only function
permitted to answer "who is writing". The existing resolvers become callers or
are deleted. The requirement is not that the answer be clever; it is that there
be exactly one answer, because the failure mode is disagreement rather than
error.

**Validation at the write boundary.** A principal must match one of the two
declared conventions — `name@domain` or `@name:context` — and nothing else. The
observed `@user@example.org` is not a judgement call: it is both conventions at
once, and it minted an identity nobody chose. Malformed values are rejected at
write time rather than stored and discovered later.

**Normalization, narrowly.** Only unambiguous repair: a stray `@` before an
address (the address form takes no prefix; the handle form takes no second
`@`), and case folding for addresses, which are case-insensitive by
specification. Handles are left untouched — reinterpreting them is a different
decision and belongs to whoever owns the convention.

**Aliases are declared, never inferred.** A human may legitimately write under a
git commit address, a machine handle, or a personal address. The substrate
cannot know those are one person, and guessing would merge people who are
genuinely distinct — a worse error than the one it fixes, and an unrecoverable
one. Aliases therefore live in the persona config (§5.3) as an explicit
`aliases = [...]` list, and resolution folds a declared alias to the persona's
principal at write time. An undeclared spelling is reported by drift detection,
never silently absorbed.

**Drift is reported, not prevented.** Two valid spellings of one person are
indistinguishable from two people. `axi identity drift` therefore reports: the
dominant identity, the minority spellings, how many fragments are stranded under
each, and any malformed values. The number that makes someone act is the count
of memories that cannot be found and by whom — not a boolean.

**One ledger per install.** The second physical store in §2 hole 4 came from
branding choosing a different state directory, not from any identity decision. A
consumer's brand must not fork the memory store: the state directory is a
platform property, and a persona wanting genuine separation gets it through the
mechanism in open question 5, deliberately.

### 5.7 Migration of an already-fragmented ledger

The condition exists in the field, so the design owes it a path rather than only
a prevention.

`axi identity merge <from> --into <to>` folds one spelling into another. Two
properties are non-negotiable, both learned from performing this by hand:
...
- **Back up, dry-run, then verify the whole ledger.** Not only the migrated
  ...
  and the only evidence that has not happened is a full verification pass after
  the fact.
...
5. **Per-persona ledger or single ledger?** A user with strict work-vs-personal separation may want physically separate ledgers; today personas share one. Open question whether persona = scope or persona = principal-within-scope. Lean: principal-within-scope (one ledger; queries filter by persona). Multi-ledger is a federation question, not a persona question. **Evidence since this was written:** multi-ledger already happens by accident — a consumer-branded state directory produced a second physical store whose writes the primary ledger's readers never saw. Whatever this resolves to, the accidental path must be closed first (§5.6, last paragraph): a second ledger should be a decision, not a side effect of branding.
