...
## Generic answering surface (2026-09-18)

**Directive:** the data platform maximizes what a **base (domain-agnostic) install** can do. The
medallion is not only a store consumers query by hand — the platform itself ships a small set of
**schema-introspecting answering capabilities** over the gold tier, so an assistant (or any
caller) can answer quantitative questions about *whatever data the operator has loaded* with no
domain code installed:

- `gold.tables` / `gold.describe` — what exists (tables, columns, types, coverage window).
- `gold.aggregate` — deterministic aggregate (sum/mean/min/max/std/count) of a column over an
  optional time window and filter.
- `gold.series` — a bucketed series from a table (the input shape the analytics tool consumes).

These are **capabilities** (ADR-056 skills) projected to CLI/MCP/agent-tool via ADR-072/073 and
gated by ADR-114 (identity + site rules; access tiers enforced in the verb, fail-closed). Domain
packs then **declare** curated, named verbs (units, exclusion rules, provenance boilerplate) that
the platform executes over the same machinery — a specialization layer, not parallel hand-written
per-quantity SQL. Contract: [spec-medallion-answering.md](../specs/spec-medallion-answering.md);
decision: ADR-115. This is what makes the assistant's three install modes real
([prd-deterministic-quantitative-answering.md](prd-deterministic-quantitative-answering.md) §7).
...
- Apache Kafka (KRaft mode, streaming) + Apache Flink (stream processing) — see [ADR-007](../adrs/adr-007-streaming-first-architecture.md)
