...
- **DeepLynx handles ontology, not operational knowledge.** Per the architecture, DeepLynx Nexus owns model parameters and digital twin lineage; Axiom owns operational intelligence. The graph layer fills the gap between raw RAG chunks and formal ontology. Axiom consumes the Nexus ontology (1:1 metatype mapping) when available, but runs fully without it — proving interoperability with INL's stack while maintaining independence.
...
8. **External ontology integration (OntologyProvider)** — Consume domain entity/relationship types from external ontology systems (DeepLynx Nexus via REST/MCP, or any conforming provider). 1:1 mapping: Nexus metatype → EntityType, metatype relationship → RelationshipType. Fully optional — system operates with core types + extension hardcoded fallbacks when no provider is configured. Cached locally for offline use. Validates INL interoperability.
...
- ~~**Open question:** How does the graph layer interact with DeepLynx Nexus ontology?~~ **Resolved.** Nexus is the preferred (but optional) ontology source. 1:1 mapping: Nexus metatype → Axiom EntityType, Nexus metatype relationship → Axiom RelationshipType. Axiom runs fully without Nexus (hardcoded fallback + offline cache). Bidirectional sync (Axiom→Nexus entity push) deferred to Phase 1.0. See `spec-knowledge-graph.md` §8.
