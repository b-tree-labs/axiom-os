...
A publisher may narrow the payload it fires. `tool.post_invoke` from the CLI and protocol capability surfaces (`axiom.infra.skill_dispatch`) carries `{tool_name, principal, surface, ok, errors_count, error, latency_ms, args_digest}` instead: those surfaces run the platform's credential verbs, so `args` and `result` would put a stored value, a revealed value or a minted token on whatever sink the process default bus is wired to. `args_digest` is the action audit's own digest of the same invocation, so the two records join. The chat surface, which passes its own per-agent bus, fires the full payload above. Subscribers read these payloads as `total=False` TypedDicts and must not assume a field is present.
