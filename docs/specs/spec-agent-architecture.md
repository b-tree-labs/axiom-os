...
> TIDY, PRESS, Doctor) are shipped. SECUR-T guardian capability is implemented, its agent role folded into TRIAGE (content
...
| **SECUR-T (Guardian)** | Content verification, anomaly detection (5 rules), trust scoring, alert lifecycle, escalation path verification | ✅ Implemented (30 tests); agent role folded into TRIAGE |
...
All agents follow the design principles and safety guardrails defined in [prd-agents.md](../prds/prd-agents.md).
...
**This is the flagship capability.** Digital Twin agents automate the lifecycle of any modeled system's digital twin — from data acquisition through ROM deployment. The pattern applies broadly: a wind turbine, a jet/gas turbine engine, a chemical process plant, a power-grid substation, a building HVAC system, a climate/weather model, a manufacturing line, a hydro dam, or any other instrumented physical system. These agents coordinate with the digital-twin infrastructure specified by the consuming domain portfolio, whose twin-architecture spec is maintained in that domain's own repository rather than in this domain-neutral substrate.
...
|-------|------------|-------------------|
| Time synchronization | Vibration, temperature, and flow readings time-aligned (e.g. turbine vibration + bearing temp; or grid voltage + line frequency) | Alert data team, delay Shadow |
| Correlation | Control input → predictable response (e.g. valve open → pressure rise; throttle → RPM; damper open → airflow response) | Flag for manual review |
...
system = "turbine-a"       # any modeled system: turbine | engine | hvac | grid | process | ...
...
See [Agents PRD — RACI Framework](../prds/prd-agents.md) for
the full design.
...
> **Building on existing work:** A consumer layer may already run a nightly high-fidelity solver (e.g. a turbine CFD run, a grid power-flow study, or a building-energy HVAC simulation) and email daily predictions. Phase 1 extracts this workflow into the Axiom agent framework, generalizing for multi-system support.
...
3. Calibration target tracking — domain-specific tunable inputs (e.g. blade aero coefficients and material fatigue limits for a turbine; reaction-rate constants and heat-transfer coefficients for a chemical plant; or, for a generic physical model, the governing-equation coefficients, initial-state parameters, and geometry)
