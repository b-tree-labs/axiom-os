...
6. **A page bounds what the caller may SEE, never the raw table.** Ownership is part of the *query*, not a filter applied to its result. A `LIMIT` applied before the ownership predicate pages rows the caller may not read and then discards them, so the list comes back short — or empty — while the caller's own conversations plainly exist. On a shared install another principal's newer conversations can fill the page entirely. The ownership predicate (unowned, own, or an adoptable legacy handle) is therefore ANDed into the statement ahead of the limit, with the in-process readability check retained as a backstop so the two failing to agree is loud rather than silently permissive.
