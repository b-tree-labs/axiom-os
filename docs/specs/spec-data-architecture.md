...
> **Note:** Domain-specific Bronze tables (e.g., sensor readings, operations logs, simulation outputs) are defined by downstream products. See [Data Platform PRD](../prds/prd-data-platform.md) for the generic framework; axiom defines nuclear-specific Bronze schemas in its own Data Platform PRD.
...
See [Data Platform PRD § Domain Extension Points](../prds/prd-data-platform.md#domain-extension-points) for the extension model.
...
### 7.3 External System Interoperability

Axiom exposes one standard **pull** data-access interface: query/SQL over the
served store plus registerable callbacks. Any external system (e.g. a
graph-based engineering data platform) is an optional consumer of that
interface. Axiom builds no system-specific push adapter and takes no
dependency on any external API; concrete bindings to a particular external
product live in the consuming domain extension, not in Axiom core.
