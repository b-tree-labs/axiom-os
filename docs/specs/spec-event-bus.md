...
### 6a. In-memory history is bounded; the durable log is not

Two things look like "the event history" and only one of them is complete.

- **`bus.history`** is a bounded ring of the most recent `history_limit`
  events (default 1000, `EventBus(history_limit=...)` to change it), oldest
  first. It exists for interactive debugging and post-incident inspection.
  Once the bus passes the cap the oldest events are evicted, so a
  long-lived process retains a flat number of events instead of one per
  publish for as long as it runs. Bus events retain their `payload` dict by
  reference and a `tool.post_invoke` payload carries the whole tool result,
  so an unbounded history grows with traffic and with payload size at once.
- **`bus.replay()`** reads the transport's durable JSONL log, so it stays
  complete for any transport that keeps one. Events evicted from
  `bus.history` are still replayed.

`EventBus(history_limit=UNBOUNDED_HISTORY)` opts back into a history that
never evicts. It is for short-lived processes that want a complete
in-memory tape; a long-lived process that passes it is choosing unbounded
growth. Unbounded is never the default.
...
- **`test_history_bounded.py`**: history retains the newest `history_limit` events oldest-first on both the publish and the `bus.errors` append path; the retained count stays flat as the publish count grows; `history` is a fresh plain `list` a caller may mutate; unbounded is opt-in; `replay()` still returns every logged event.
