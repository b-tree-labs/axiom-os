...
## §11a. Deferred — alert-directed delegation

Tracked in PRD §5.10. Recorded here so the schema question is settled before
someone adds the field ad hoc.

An inbox row would gain two optional fields:

| field | type | meaning |
|---|---|---|
| `respond_with` | `list[str]` | capability names the responder should consider |
| `corpus_hint` | `str` | retrieval scope for answering about this alert |

Both are REFERENCES. Resolution happens at delivery against the installed
extensions and the recipient's own permissions: a name that does not resolve,
or that the recipient may not invoke, is not offered. An alert must never be
able to widen authority — it is written by a monitor, and the inbox is
reachable by anything that can write a row.

Note the existing `body_ref` column already carries the operator-facing link
(shipped), so a link needs no schema change; these two would.

## §11b. Deferred — group recipients

Tracked in PRD §5.11. Recorded here because it changes the inbox schema and the
acknowledgement rule, and both are cheaper to settle now than to migrate.

A recipient may be a group handle. Delivery resolves it to members; the monitor
names a role and never sees a roster.

**The acknowledgement rule does not survive unchanged.** Today "only the
recipient may acknowledge" is what makes `acknowledged_at` unambiguous with no
`acknowledged_by` column — the row's recipient IS the acknowledger. For a
respond-once group that stops being true, so the schema gains:

| field | type | when |
|---|---|---|
| `acknowledged_by` | `str \| null` | required once a group can be a recipient |
| `group_ref` | `str \| null` | the group named, retained for audit after membership changes |

Both are null for the individual case, so the shipped behaviour is unaffected —
but adding `acknowledged_by` later means backfilling rows whose acknowledger can
no longer be determined. Hence recording it here rather than after the fact.
