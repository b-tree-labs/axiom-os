...
display_name = "INL Axiom"
kind = "a2a"
endpoint = "https://axiom.inl.gov/api/v1"
transport = "https"
credential_type = "mtls"
credential_file = "a2a/inl-client.pem"
health_check = "http_get"
health_endpoint = "https://axiom.inl.gov/api/v1/health"
category = "federation"
extension = "core"
required = false
```
...
engineering backbone used for domain-specific projects. If partnership proceeds, Axiom
could integrate as either an MCP client or via data exchange.

**Option A: MCP Client** — Axiom queries DeepLynx's MCP server for ontology/graph data
...
- Query system component ontology for AI agent context
- Fetch safety limits and classifications from DeepLynx schema
- Exchange timeseries data via CSV/Parquet (both use DuckDB)
- Cross-facility digital twin coordination (PartnerLab ↔ INL facility)
