...
**`actor`** — A `Principal` (ADR-020 handle grammar): `handle` + `public_bytes`, the minimal cryptographic identifier and nothing more — its bytes are load-bearing in capability signatures, so it stays minimal (ADR-084). Names a human or an agent. An agent acting on behalf of a human is *not* stamped onto the principal — there is no `on_behalf_of` field. That relationship lives on the `CapabilityToken` the agent presents, whose `parent_capability` + `delegation_depth` chain the authority back to the human's token (§2.2, ADR-086). The richer governance view of the actor — roles, tenant, and assurance/posture — is `ActorContext` (ADR-084), which rides alongside the bare principal on the envelope rather than bloating `Principal`. Federation: a peer-forwarded action carries `actor = peer's principal` and `federation_origin = peer_id`.
...
Cross-cohort capability presentation: a peer presents a token issued by their KEEP, validated against the local trust graph (ADR-028 score must exceed a per-resource threshold), classification-checked against the local `cohort_policy`, and admitted with a transitive identity chain — the token's `subject` (the peer principal) mapped to its local equivalent, with the capability's own delegation chain (`parent_capability` + `delegation_depth`, ADR-086) preserved across the hop. The chain rides on the `CapabilityToken`, not on any `on_behalf_of` field of `Principal` (which carries only `handle` + `public_bytes`).
