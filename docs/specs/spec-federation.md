...
| **A2A Server** | HTTP server on configurable port (default: 8443) | Serves `/.well-known/agent-card.json`, `/.well-known/axiom-manifest.json`, A2A task endpoints. **Transport substrate = the `serve` extension** (see `spec-serve.md`): the A2A + `.well-known` routes mount on the composed serve app as a `MountSpec`, and serve's auth middleware calls this Identity Manager's `verify()` for inbound peer auth. The A2A endpoints themselves remain to be built (this is the unbuilt transport that serve provides). |
...
policy_endpoint = "https://axiom-policy.partner.example/v1"
...
axi federation trust propose fed-inl-nrad \
    --policy-endpoint https://axiom-policy.partner.example/v1 \
