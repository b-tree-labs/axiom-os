...
```
Priority 1: External ontology provider (any OntologyProvider implementation)
             ↓ (if available)
...
Axiom runs fine with **only Priority 3** — no external dependencies required. An external ontology provider is optional; it enriches the type system when configured.
...
class OntologyProvider(Protocol):
    ...
    The canonical domain ontology lives in the external system (an
    external ontology system). Axiom maps external types to its
    EntityType / RelationshipType model.
    """
...
### 8.3 External Ontology Provider (optional)

A consumer/domain extension implements `OntologyProvider` against its own
external ontology system. The implementation:

- Maps that system's types **1:1** to Axiom `EntityType` / `RelationshipType`
  (each external metatype → one entity label; each external relationship →
  one edge type; property definitions map directly).
- Caches the fetched ontology to `~/.axi/ontology/{provider}.json`.
- Falls back to the cached snapshot when the external system is unreachable,
  and to extension-hardcoded defaults when there is no cache.

Concrete providers (binding a specific external ontology product) live in the
consuming domain extension, not in Axiom core. Axiom core ships only the
`OntologyProvider` seam and the registry that consumes it. A provider sets
`source="<provider>"` and may carry an `external_type_id` on each mapped type
...
class EntityTypeRegistry:
    ...
    Sources (applied in order):
      ...
      2. OntologyProvider (external ontology provider — if configured and reachable)
...
### 8.5 Extension Lifecycle (with an external ontology provider)
...
def on_activate(self, ctx: ExtensionContext):
    ...
    # Try the configured ontology provider first (if configured in connections.toml)
    conn = ctx.get_connection("ontology")
    if conn:
        provider = ExternalOntologyProvider(conn)  # implemented in the consumer repo
        loaded = graph.registry.load_from_provider(provider, domain="example")
        if loaded > 0:
            log.info("Ontology loaded from external provider (%d types)", loaded)
            return  # the provider is authoritative — no hardcoded overrides

    # Fallback: register types directly (works without an external provider)
...
In the initial phases, the flow is **provider → Axiom** (Axiom consumes the ontology). In Phase 1.0+, the flow becomes bidirectional:

- **Provider → Axiom:** Ontology types, property definitions, relationship vocabulary
- **Axiom → Provider:** Discovered entities and relationships from operational content (new components found in procedures that aren't yet in the formal ontology)

This creates a feedback loop: operators working with documents surface entities that engineers formalize in the ontology. The protocol for the Axiom→provider push is deferred to Phase 1.0 when both systems are running in production.

### 8.7 Offline / No External Provider Operation

The graph layer MUST operate fully without an external ontology provider:

| Scenario | Behavior |
|---|---|
| Provider configured + reachable | Load ontology from the provider, cache locally |
| Provider configured + unreachable | Load from `~/.axi/ontology/{provider}.json` cache |
| Provider configured + no cache + unreachable | Fall back to extension-hardcoded types + warning |
| Provider not configured | Extension-hardcoded types only (default) |
...
3. ~~**External ontology handoff.**~~ **Resolved.** An external ontology provider is an optional ontology source via the `OntologyProvider` interface. External metatypes map 1:1 to Axiom EntityTypes. The Axiom→provider push is deferred to Phase 1.0. See §8.
