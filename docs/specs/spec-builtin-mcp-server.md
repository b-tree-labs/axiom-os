...
| `input_schema_module` | string | freeform | Dotted `pkg.mod:attr` (or `pkg.mod`, resolved via the conventional `INPUT_SCHEMA` attribute), imported lazily. The target may be a JSON Schema dict, a `SkillSpec.inputs` shape map (projected through `capability_projection.inputs_to_json_schema`, the same call the CLI/chat/registry surfaces make), a callable returning either, a pydantic model, or an object exposing `to_json_schema()` (e.g. a `CapabilityInputs` from `axiom.infra.sql_capability`). A missing or broken target logs a warning and the tool advertises the freeform `{"type": "object", "additionalProperties": true}` schema; the surface never breaks. |
| `input_schema` | table | none | The same, declared inline as a TOML table (JSON Schema or shape map). Wins over `input_schema_module`. |
...
| `axiom_memory__list` | `axiom.memory.composition.CompositionService` | Enumerate principals and scopes for which the node has memory. |
| `axiom_federation__node_status` | `axiom.vega.federation` | This node's federation status (cohorts, peer count, trust state). No args. |
| `axiom_rag__retrieve` | `axiom.rag.retriever` | Real hybrid retrieval (vector + BM25, RRF-fused) over the node's configured store, with a **public-tier access filter**: defaults to the public tier and **fails closed** if a local store cannot report per-document tiers (never returns restricted/EC chunks unfiltered); the client-seam EC gate runs downstream. Per-principal tier expansion is a follow-up. Args: `query`, `k?`, `corpora?`. |
| `axiom_signals__brief` | `axiom.infra.signals` | Run signal-briefing pipeline; returns latest brief. Args: `since?`, `kinds?`. |
| `axiom_node__hooks_list` | `axiom.infra.hooks` | List hooks installed on this node (diagnostics). No args. |
| `axiom_mcp__client_capabilities` | `axiom.extensions.builtins.mcp` | Chart of supported agent harnesses and whether each is OK to route to an export-controlled model. A harness is EC-routable iff its model can be put in-enclave; the server withholds EC tool output from any client that is not. |

> This table is checked against `PLATFORM_TOOL_NAMES` by
> `test_spec_section_8_matches_the_platform_surface`. It had drifted three
> phantom tools (`axiom_federation__list_peers`, `axiom_federation__send`,
> `axiom_node__status`) and was missing four real ones, which is what a table
> maintained by hand does; the test is the reason it cannot do so again.
