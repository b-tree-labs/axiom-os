...
def test_lint_warns_for_missing_dir_of_a_provided_kind(good_ext: Path) -> None:
    # AEOS040 applies only to kinds the manifest PROVIDES: the scaffold
    # provides a cmd, so a missing commands/ dir warns (not errors).
    import shutil

    shutil.rmtree(good_ext / "good_ext" / "commands")
    findings = lint_extension(good_ext)
    assert any(f.code == "AEOS040" and f.severity == "warning" for f in findings)


def test_lint_silent_for_missing_dir_of_an_unprovided_kind(good_ext: Path) -> None:
    # The scaffold provides no tool capability, so a missing tools/ dir is
    # not a finding — empty ceremony dirs are not required (AEOS §5.3).
    import shutil

    shutil.rmtree(good_ext / "good_ext" / "tools")
    findings = lint_extension(good_ext)
    assert not any(f.code == "AEOS040" for f in findings)
...
# ---------------------------------------------------------------------------
# Secret dependencies — [extension.secrets] requires (issue #667)
# ---------------------------------------------------------------------------


def _declare_secrets(ext: Path, requires) -> None:
    data = tomllib.loads((ext / "axiom-extension.toml").read_text())
    data.setdefault("extension", {})["secrets"] = {"requires": requires}
    _write_toml(ext / "axiom-extension.toml", data)


@pytest.fixture
def secret_state(tmp_path: Path, monkeypatch) -> Path:
    """Isolated state dir holding one stored foreign credential."""
    ...
    state = tmp_path / "lint-state"
    monkeypatch.setenv("AXI_STATE_DIR", str(state))
    ForeignCredentialStore(
        state, value_store=InMemoryValueStore(),
    ).set("present-secret", b"v")
    return state


def test_declared_secret_present_in_store_passes(
    good_ext: Path, secret_state: Path
) -> None:
    _declare_secrets(good_ext, ["present-secret"])
    findings = lint_extension(good_ext)
    assert not any(f.code in ("AEOS080", "AEOS081") for f in findings), findings


def test_declared_secret_missing_from_store_warns(
    good_ext: Path, secret_state: Path
) -> None:
    _declare_secrets(good_ext, ["present-secret", "absent-secret"])
    findings = lint_extension(good_ext)
    hits = [f for f in findings if f.code == "AEOS081"]
    assert len(hits) == 1
    assert "absent-secret" in hits[0].message
    assert "axi secrets set" in hits[0].remediation
    # warning, not error — a missing local secret must not fail Bronze
    assert hits[0].severity == "warning"


def test_malformed_secrets_block_errors(
    good_ext: Path, secret_state: Path
) -> None:
    _declare_secrets(good_ext, "not-a-list")
    findings = lint_extension(good_ext)
    assert any(f.code == "AEOS080" and f.severity == "error" for f in findings)


def test_bad_secret_name_errors(good_ext: Path, secret_state: Path) -> None:
    _declare_secrets(good_ext, ["ok-name", "bad name!"])
    findings = lint_extension(good_ext)
    assert any(f.code == "AEOS080" for f in findings)


def test_no_secrets_block_emits_no_secret_findings(
    good_ext: Path, secret_state: Path
) -> None:
    findings = lint_extension(good_ext)
    assert not any(f.code in ("AEOS080", "AEOS081") for f in findings)
