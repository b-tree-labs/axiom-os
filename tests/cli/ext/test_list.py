...
def test_cli_table_has_columns_header(
    ...
    assert rc == 0
    # Lower case: every other table on the platform spells its headings that
    # way, and this one shouting read as emphasis nobody intended. Compared
    # case-insensitively so the assertion is about the COLUMN being present,
    # not about its capitalisation.
    lowered = out.lower()
