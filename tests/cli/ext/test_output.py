...
class TestTable:
    ...
    def test_table_has_a_rule_under_the_header(self, capsys):
        _output.table(
            title="T",
            columns=["X"],
            rows=[["y"]],
        ...
        out = capsys.readouterr().out
        # The plain-text fallback draws a bordered grid now, so the rule is a
        # box-drawing run rather than a row of ASCII hyphens.
        assert "─" in out, f"no rule in:\n{out}"

    def test_table_is_a_closed_grid(self, capsys):
        _output.table(title="T", columns=["X", "Y"], rows=[["y", "z"]])
        out = capsys.readouterr().out
        assert "┌" in out and "┘" in out, f"grid not closed:\n{out}"
