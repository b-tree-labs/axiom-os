...
class TestEvidenceTableScaling:
    ...
    def test_repeated_writes_to_same_edge_stay_bounded(self, graph):
        """Write 200 edges with the same (from, to, edge_type) and
        increasing evidence. Time per write must stay roughly
        constant — not grow linearly with prior evidence count."""
        import time
        ...
        for i in range(50):
            graph.upsert_edge(ConceptEdge(
                ...
                evidence=[f"frag-warm-{i}"],
            ))

        # Measure next 100
        t_first_50 = time.perf_counter()
        for i in range(100):
            graph.upsert_edge(ConceptEdge(
                from_concept=a, to_concept=b, edge_type="co_occurs",
                evidence=[f"frag-mid-{i}"],
            ))
        mid_window = time.perf_counter() - t_first_50

        # Then last 100 — at this point evidence has 250 items if O(N)
        t_second_window = time.perf_counter()
        for i in range(100):
            graph.upsert_edge(ConceptEdge(
                from_concept=a, to_concept=b, edge_type="co_occurs",
                evidence=[f"frag-late-{i}"],
            ))
        late_window = time.perf_counter() - t_second_window

        # With O(1) writes, late_window should be roughly equal to
        # mid_window (within 2× slack for jitter). With the old
        # JSON-blob upsert this ratio was ~3-4× because each upsert
        # deserialized a 150+ item array and re-serialized 250.
        assert late_window < mid_window * 2.5, (
            f"Edge upsert appears to scale with prior evidence count: "
            f"mid_window={mid_window:.3f}s, late_window={late_window:.3f}s"
        )
