...
class TestMcpStdioConformance:
    def test_recall_discoverable_callable_serves_own_and_gates(
        ...
        schema = out["tools"]["axiom_memory_recall"].input_schema
...
class TestMcpStdioServerIdentity:
    def test_all_ten_tools_and_instructions_advertised(self, stdio_env):
        pytest.importorskip("mcp")
        out = asyncio.run(_recall_over_stdio(
            stdio_env, harness="claude-code", query="anything",
        ))
        # The recall path is one of the ten tools every client discovers
        # (five memory tools + two action-ledger query tools + the three
        # #667 secrets/vault lifecycle tools).
        assert set(out["tools"]) == {
            "axiom_memory_append", "axiom_memory_show", "axiom_memory_recent",
            "axiom_memory_search", "axiom_memory_recall",
            "axiom_actions_recent", "axiom_actions_search",
            "axiom_secrets_list", "axiom_vault_audit",
