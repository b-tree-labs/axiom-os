...
class TestResourcePattern:
    ...
    def test_prefix_wildcard(self):
        # trailing-star prefix form — lets a rule/capability scope to one
        # mount of the composed HTTP app (extension://<mount><path>)
        pat = ResourcePattern("extension://gateway/*")
        assert pat.matches(ResourceRef.parse("extension://gateway/v1/chat"))
        assert pat.matches(ResourceRef.parse("extension://gateway/"))
        assert not pat.matches(ResourceRef.parse("extension://gateway2/v1"))
        assert not pat.matches(ResourceRef.parse("extension://rag/query"))

    def test_prefix_wildcard_does_not_match_bare_prefix_sibling(self):
        pat = ResourcePattern("memory://host/frag/*")
        assert pat.matches(ResourceRef.parse("memory://host/frag/abc"))
        assert not pat.matches(ResourceRef.parse("memory://host/fragments"))
