...
# ---------------------------------------------------------------------------
# Long-lived hosts: the context renews its own capability (2026-09-21 incident)
# ---------------------------------------------------------------------------


class TestLongLivedContextRenewsCapability:
    """The default capability is minted once with a 1-hour TTL, and the
    context is cached module-global in every extension's _authz wiring.
    A host that lives longer than the TTL (the data-platform orchestrator)
    presented an expired token to every scheduled dispatch: four
    consecutive nightly backups floor-denied EXPIRED_CAPABILITY on a
    production node before the fleet console surfaced it. The context's
    OWN default renews at action time; an explicitly passed capability is
    the caller's authority and must never silently renew.
    """

    def _expire_default(self, ext):
        import dataclasses
        from datetime import datetime, timedelta, timezone

        old = ext.default_capability
        assert old is not None
        expired = dataclasses.replace(
            old,
            not_before=datetime.now(timezone.utc) - timedelta(hours=3),
            not_after=datetime.now(timezone.utc) - timedelta(hours=2),
        )
        ext.default_capability = expired
        return expired

    def test_expired_default_capability_renews_at_action_time(self):
        with mock.patch.dict(os.environ, {"AXIOM_ACTOR": "@dev:test"}):
            ext = setup_extension("test_ext_renew", verbs=["tick"], dev_mode=True)
            expired = self._expire_default(ext)
            with ext.action(
                verb="tick", resource="extension://test_ext_renew/nightly"
            ) as act:
                assert act.receipt_id
                cap = act.envelope.capability
                from datetime import datetime, timezone

                assert cap.is_valid_at(datetime.now(timezone.utc))
                assert cap.not_after > expired.not_after

    def test_renewed_capability_replaces_the_cached_default(self):
        """One renewal serves subsequent actions — no re-mint per call."""
        with mock.patch.dict(os.environ, {"AXIOM_ACTOR": "@dev:test"}):
            ext = setup_extension("test_ext_renew2", verbs=["tick"], dev_mode=True)
            self._expire_default(ext)
            with ext.action(
                verb="tick", resource="extension://test_ext_renew2/a"
            ) as act1:
                first = act1.envelope.capability
            with ext.action(
                verb="tick", resource="extension://test_ext_renew2/b"
            ) as act2:
                assert act2.envelope.capability is first

    def test_renewal_preserves_subject(self):
        with mock.patch.dict(os.environ, {"AXIOM_ACTOR": "@dev:test"}):
            ext = setup_extension("test_ext_renew3", verbs=["tick"], dev_mode=True)
            before = ext.default_capability.subject
            self._expire_default(ext)
            with ext.action(
                verb="tick", resource="extension://test_ext_renew3/x"
            ) as act:
                assert act.envelope.capability.subject.handle == before.handle

    def test_explicitly_passed_expired_capability_is_still_denied(self):
        """Caller-owned authority never silently renews — the floor must
        still be able to fail (a renewal that covered explicit tokens
        would make EXPIRED_CAPABILITY undetectable)."""
        from axiom.governance.simple import AuthorizationDenied

        with mock.patch.dict(os.environ, {"AXIOM_ACTOR": "@dev:test"}):
            ext = setup_extension("test_ext_renew4", verbs=["tick"], dev_mode=True)
            expired = self._expire_default(ext)
            import pytest as _pytest

            with _pytest.raises(AuthorizationDenied):
                with ext.action(
                    verb="tick",
                    resource="extension://test_ext_renew4/x",
                    capability=expired,
                ):
                    raise AssertionError("must not execute under an expired explicit token")
