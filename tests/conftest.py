...
#
# ``git`` exports these into every hook it runs, so a suite launched from
# ``pre-push`` inherits them. ``GIT_DIR`` short-circuits *all* repo discovery:
# it beats ``cwd=``, ``-C <path>`` and ``GIT_CEILING_DIRECTORIES``. A test that
# builds a scratch repo under ``tmp_path`` and then runs ``git remote add
# origin`` does not touch its scratch repo — it reaches into the real one.
#
# That is the mechanism behind "never run the full suite from a worktree":
# the suite was not unsafe in a worktree, it was unsafe when *launched by a
# git hook*, which is exactly how a pre-push run starts. Strip them once,
# before collection, so the suite is indifferent to who launched it and from
# which worktree.
#
# Preferences (GIT_EDITOR, GIT_PAGER, GIT_ASKPASS) are the developer's and are
# left alone — they do not redirect discovery.
GIT_DISCOVERY_VARS = (
    "GIT_DIR",
    "GIT_WORK_TREE",
    "GIT_COMMON_DIR",
...
def pytest_configure(config):
    ...
    """
    # Drop any git discovery context inherited from whoever launched us,
    # before anything can shell out to git. See GIT_DISCOVERY_VARS.
    for _v in GIT_DISCOVERY_VARS:
    ...
    # Action-provenance ledger (#665): guarded_act journals every decision.
    # Default the test runtime to the JSONL backend so the many existing
    # guarded_act call sites in tests never probe (or write to) a developer's
    # live Postgres. Tests that exercise the SQL backend inject an engine or
    # override this env var explicitly. Production unaffected (var unset).
    os.environ.setdefault("AXIOM_ACTION_LEDGER_BACKEND", "jsonl")

    # Declared authority (P5): every chat tool call consults decide(). Default
    # the test runtime to no receipt persistence so the chat suites never probe
    # (or write verdict rows to) a developer's live Postgres. Tests that
    # exercise receipts pass their own DecideContext. Production unaffected.
    os.environ.setdefault("AXIOM_AUTHORITY_RECEIPTS", "off")
...
def no_slow_llm_calls(monkeypatch, request):
    """A test must never wait three minutes on a model that is busy.

    Several code paths call ``Gateway.complete()`` in passing — ``classroom
    ask``, RIVET's heartbeat — without asserting anything about what the model
    said. The transport gives each attempt a 60 s read timeout and retries three
    times, so one incidental call can burn 180 s. The local model answers fine on
    an idle machine and stalls while eleven thousand tests run beside it, which
    is exactly when the suite runs it, and pytest-timeout kills the test at 90 s.
    The failures looked like flakes: they moved between runs and passed whenever
    anyone re-ran them alone.

    The first attempt at this stubbed ``Gateway.complete`` outright and broke
    fifteen tests whose subject *is* the gateway — they mock the HTTP layer and
    assert on routing, and a stub above them bypasses the thing they came to
    test. So this shortens the clock instead: an incidental call fails in a
    couple of seconds rather than three minutes, and every test that drives the
    gateway deliberately still drives the real one.

    ``@pytest.mark.live_llm`` opts a test back into the production timings.
    """
    if request.node.get_closest_marker("live_llm"):
        return
    try:
        import axiom.llm.gateway as gw

        # A test run must not adopt whatever the developer happens to be
        # running. Gateway._discover_local_llm() probes localhost:8080 and
        # registers any OpenAI-compatible server it finds as a provider, so a
        # laptop with the local model up gets a provider and CI does not. That
        # is the whole reason these tests "passed in CI and failed locally":
        # the same code took a network path on one machine and no path at all
        # on the other. Disabled here so every run looks like the hermetic one.
        # Done by making the probe find nothing, not by removing the method:
        # tests/test_gateway.py::TestLocalLLMAutoDiscovery calls
        # _discover_local_llm() directly with socket.create_connection mocked to
        # succeed, and a no-op method registers no provider and fails it. This
        # refuses only the probe to localhost:8080 and delegates every other
        # connection, so that test's own patch still wins inside its with-block.
        import socket as _socket

        _real_conn = _socket.create_connection

        def _no_local_model(address, *args, **kwargs):
            try:
                host, port = address
            except Exception:
                return _real_conn(address, *args, **kwargs)
            if str(host) in ("localhost", "127.0.0.1") and int(port) == 8080:
                raise OSError("local model probe disabled for tests")
            return _real_conn(address, *args, **kwargs)

        monkeypatch.setattr(_socket, "create_connection", _no_local_model, raising=False)

        # Only the wait is shortened. Retry counts and backoff are semantics —
        # tests/infra/test_gateway_resilience.py asserts on exactly those, and
        # overriding them here broke all eight of its cases. Call sites pass timeout=120 / 180 as literals, so capping the default
        # is not enough. Wrap the transport and override the kwarg, leaving its
        # retry policy, error mapping and return value exactly as they are.
        _real_post = gw._post_with_rate_limit_retry

        def _capped(requests_mod, url, payload, headers, timeout=60, **kwargs):
            return _real_post(
                requests_mod, url, payload, headers, timeout=2, **kwargs
            )

        monkeypatch.setattr(gw, "_post_with_rate_limit_retry", _capped, raising=False)
    except Exception:  # pragma: no cover - gateway import is not universal
        pass



# --- subprocess source pinning ---------------------------------------------
#
# The workspace venv installs axiom EDITABLE from ONE checkout. Every other
# worktree therefore runs its in-process tests against its own source (pytest
# puts the rootdir's `src` first) while every SUBPROCESS it launches imports
# the anchor checkout instead. The two disagree silently, and the subprocess
# side wins for exactly the tests the CLI convention says must exist.
#
# Found 2026-09-08 the only way this ever gets found: two faithful mutants to
# `release/_legacy_rivet_cli.py` both SURVIVED a subprocess smoke run from a
# second worktree, because the smoke was exercising a different branch. A test
# that survives every mutant is not a test.
#
# Pinning it here rather than in each test fixes the class. Before this, of the
# 30 test modules that launch `sys.executable`, 17 had hand-rolled their own
# PYTHONPATH line — seventeen bespoke solutions to one problem — and 13 had
# none at all.
#
# This does NOT weaken the install-path suite: those tests run `pip install`
# inside a fresh `python:3.12-slim` CONTAINER, which does not inherit the
# host's environment, so they still prove the published artifact works.
@pytest.fixture(scope="session", autouse=True)
def _pin_subprocess_to_this_checkout() -> None:
    """Put THIS checkout's ``src`` first on ``PYTHONPATH`` for the session.

    Subprocesses that inherit the environment (or splat ``os.environ``) then
    import the source under test. A test that builds its env from scratch still
    has to pass ``PYTHONPATH`` itself — there is no way to reach those from
    here, which is why `test_subprocess_source_pinning.py` exists to prove this
    one works rather than assuming it.
    """
    src = Path(__file__).resolve().parent.parent / "src"
    if not src.is_dir():  # pragma: no cover - non-src layout
        return
    existing = os.environ.get("PYTHONPATH", "")
    parts = [p for p in existing.split(os.pathsep) if p and p != str(src)]
    os.environ["PYTHONPATH"] = os.pathsep.join([str(src), *parts])
