...
class TestDiscoverExtensions:
    ...
    def test_malformed_manifest_skipped(self, ext_base):
        ...
        bad.mkdir()
        (bad / "axiom-extension.toml").write_text("this is not valid toml [[[")
        exts = discover_extensions(ext_base)
        assert exts == []
...
class TestLoadChatTools:
    ...
    def test_no_tools_module(self, ext_base):
        ...
        d.mkdir()
        (d / "axiom-extension.toml").write_text('[extension]\nname = "empty-ext"\n')
        exts = discover_extensions(ext_base)
        tools = load_chat_tools(exts[0])
        assert tools == []
...
class TestExecuteExtensionTool:
    def test_execute_scaffolded_tool(self, scaffolded_ext):
        result = execute_extension_tool("data_query", {"query": "power"}, scaffolded_ext)
        assert result is not None
        assert "results" in result

    def test_unknown_tool_returns_none(self, scaffolded_ext):
        result = execute_extension_tool("nonexistent_tool", {}, scaffolded_ext)
...
class TestGenerateContractDocs:
    def test_generates_markdown(self):
        ...
        assert "# Extension Contracts" in docs
        assert "axiom-extension.toml" in docs
        assert "ToolDef" in docs
        assert "SKILL.md" in docs
        assert "GenerationProvider" in docs


class TestProjectWalkStaysQuiet:
    """The ancestor walk probes directories; it must never warn about them."""

    def test_walking_past_a_legacy_dir_emits_nothing(self, tmp_path, monkeypatch, capsys):
        # An ancestor that still holds the retired state dir, with no current
        # one beside it, and a working directory two levels below. The walk
        # passes straight through it while looking for <root>/.<cli>/extensions.
        from axiom.extensions.discovery import _project_extensions_dir
        from axiom.infra import brand_migration
        from axiom.infra.brand_migration import LEGACY_PROJECT_DIR

        (tmp_path / LEGACY_PROJECT_DIR).mkdir()
        cwd = tmp_path / "repos" / "checkout"
    ...
    def test_an_explicit_root_still_warns(self, tmp_path, monkeypatch, capsys):
        # AXIOM_ROOT names *the* project; a legacy dir there is the operator's
        # own state going unread, which is exactly what the notice is for.
        from axiom.extensions.discovery import _project_extensions_dir
        from axiom.infra import brand_migration
        from axiom.infra.brand_migration import LEGACY_PROJECT_DIR
        ...
        assert "no longer reads it" in capsys.readouterr().err
