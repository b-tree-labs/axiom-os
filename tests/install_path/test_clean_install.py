...
def test_python_multipart_is_a_core_dependency():
    """webgate's login router (`webgate/api/routers.py`) declares FastAPI
    ``Form(...)`` fields, which FastAPI refuses to serve unless python-multipart
    is importable — raising RuntimeError at router-build time. It must be a
    *core* dependency, not arrive transitively via the ``[mcp]`` extra: 0.37.0
    shipped the webgate as a core builtin but declared multipart nowhere, so a
    plain ``pip install axiom-os-lm`` that mounts the gate crashed. No docker
    needed — this reads the manifest directly.
    """
    import tomllib

    deps = tomllib.loads(PYPROJECT.read_text())["project"]["dependencies"]
    assert any("multipart" in d.lower() for d in deps), (
        "python-multipart missing from core [project.dependencies] — the "
        "webgate Form login will RuntimeError on a clean install"
    )
