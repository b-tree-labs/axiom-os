...
class TestL2DefenseInDepth:
    ...
    def test_access_denied_blocks_read(self, composition_a):
        ...
        # u1 owns this fragment; u2 is a different principal with no
        # delegated access edges to it.
        frag = composition_a.write(
            content={"fact": "x"}, cognitive_type="semantic",
            principal_id="u1", agents={"a1"}, resources={"r1"},
        )
        # A DIFFERENT principal with no access edges → read denied. This is
        # the genuine breach the access layer must block. (An owner reading
        # their OWN memory is not a breach: ADR-026 grants the master an
        # intrinsic read right — defense-in-depth gates the unauthorized
        # cross-principal read, not self-recall.)
        results = composition_a.read(
            fragment_ids=[frag.id], user="u2", agent="a2",
        )
        assert results == []
        denial_entries = [
