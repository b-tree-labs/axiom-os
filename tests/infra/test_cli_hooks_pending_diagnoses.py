...
def _seed_diagnosis(state_dir: Path, *, fingerprint: str = "abc123") -> None:
    cli_listener.append_diagnosis(
        state_dir,
        {
            "pattern_id": "provider-missing",
            "fingerprint": fingerprint,
            "summary": "gemma gguf missing",
            "remedy": "Edit llm-providers.toml: point the local provider at gemma2-2b-it.",
            "confidence": 0.95,
...
class TestSurfacePendingDiagnoses:
    ...
    def test_pending_diagnosis_writes_to_stderr(
        ...
        assert "gemma" in captured.err, "the summary identifies the diagnosis"
        # The remedy is deliberately NOT inlined any more. This banner prints
        # in front of whatever the operator actually typed, and a multi-line
        # remedy pasted there is how a signal worth reading trains people to
        # skip it. It lives one command away instead.
        assert "llm-providers.toml" not in captured.err, "the remedy is not pasted inline"
        assert "triage pending" in captured.err, "and the banner says where it is"

    def test_includes_clear_hint(self, state_dir: Path, capsys: pytest.CaptureFixture) -> None:
