...
- PATH = intersect(os.environ["PATH"], curated_allow_list).
  `.`, `~` (literal), and any world-writable dir are dropped. Dropping is
  logged at DEBUG: it is the bound working, and every developer machine has a
  dozen such entries. The LOUD case is an extension replacing PATH outright,
  which discards the bound.
...
def _launchd_svc(env=None, interval_secs=300):
    return ServiceDef(
        name="release-agent",
        # Absolute, always-resolvable binary — LaunchdProvider.install() now
        # refuses a plist whose ProgramArguments[0] can't be spawned, and these
        # tests restrict PATH to exercise env-bounding, so a bare name would not
        # resolve. sys.executable is a real interpreter, resolvable regardless.
...
def _systemd_svc(env=None, interval_secs=300):
    return ServiceDef(
        name="tidy-agent",
        # Absolute, always-resolvable binary. SystemdProvider.install() now
        # refuses to register a unit whose ExecStart can't run (the 203/EXEC
        # guard), and these tests restrict PATH to exercise env-bounding, so a
        # bare name would not resolve. sys.executable is a real venv python —
        # resolvable regardless of the patched PATH.
        binary=sys.executable,
...
class TestTheLoudMessageIsOnTheEventThatDeservesIt:
    """Bounding the PATH is the feature working; replacing it is not.

    The warning sat on the routine case — "dropped 10 non-allow-listed
    entries", naming all ten — which fired in front of unrelated commands on
    any machine with homebrew, conda and a cryptex on its PATH. Meanwhile an
    extension REPLACING PATH, which discards the bound entirely, passed
    silently despite a docstring promising it would be loud.
    """

    @staticmethod
    def _compute(svc_env, caplog):
        import logging

        from axiom.infra.services import _platform_env_for_service

        with caplog.at_level(logging.DEBUG, logger="axiom.infra.services"):
            env = _platform_env_for_service(svc_env, "/usr/bin/python3")
        return env, caplog.records

    def test_dropping_entries_does_not_warn(self, caplog, monkeypatch):
        monkeypatch.setenv("PATH", "/usr/bin:/opt/homebrew/bin:/some/random/place")
        _, records = self._compute({}, caplog)
        warnings = [r for r in records if r.levelname == "WARNING"]
        assert not warnings, (
            f"routine bounding should not warn: {[r.getMessage() for r in warnings]}"
        )

    def test_dropping_entries_is_still_recorded_at_debug(self, caplog, monkeypatch):
        """Quiet is not the same as gone — an operator asking why a service
        cannot find a binary needs this."""
        monkeypatch.setenv("PATH", "/usr/bin:/some/random/place")
        _, records = self._compute({}, caplog)
        assert any(
            "non-allow-listed" in r.getMessage() and r.levelname == "DEBUG"
            for r in records
        ), [f"{r.levelname}: {r.getMessage()}" for r in records]

    def test_an_extension_replacing_path_warns(self, caplog, monkeypatch):
        monkeypatch.setenv("PATH", "/usr/bin")
        _, records = self._compute({"PATH": "/anything/at/all"}, caplog)
        warnings = [r for r in records if r.levelname == "WARNING"]
        assert warnings, "replacing the PATH discards the bound and must be loud"
        assert "allow-list" in warnings[0].getMessage()

    def test_an_extension_setting_the_same_path_does_not_warn(
        self, caplog, monkeypatch
    ):
        """Declaring exactly what the bound already produced relaxes nothing."""
        from axiom.infra.services import _platform_env_for_service

        monkeypatch.setenv("PATH", "/usr/bin")
        bounded = _platform_env_for_service({}, "/usr/bin/python3")["PATH"]
        _, records = self._compute({"PATH": bounded}, caplog)
