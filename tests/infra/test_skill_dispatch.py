...
class TestApprovalRequired:
    ...
    def test_the_message_says_where_the_work_went(self, tmp_path, hookbus, no_db_context):
        """This used to say "the cli surface cannot prompt for approval" and
        throw the call away.

        Naming the surface was the best it could do while the work was being
        discarded. Now the work is held, so the useful thing to say is the id a
        human answers, and the command that answers it.
        """
        from axiom.infra.skill_dispatch import invoke_capability
        ...
        assert out.value["held_action_id"] in joined
        assert "axi approve ok" in joined

    def test_the_surface_is_still_named_when_holding_is_off(
        self, tmp_path, hookbus, no_db_context
    ):
        """``hold_on_approval=False`` keeps the old message, and needs it.

        Its one caller is the runner re-invoking an already-approved action,
        where "the runner surface cannot prompt" is exactly the fact to record.
        """
        from axiom.infra.skill_dispatch import invoke_capability
        ...
        joined = " ".join(out.errors)
        assert "cannot prompt" in joined
