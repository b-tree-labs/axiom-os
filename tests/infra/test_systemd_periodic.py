...
class TestPeriodicInstall:
    def test_periodic_writes_service_and_timer(self, tmp_path, monkeypatch):
        ...
        assert "OnActiveSec=300" in timer_file.read_text(), (
            "Without OnActiveSec a mid-uptime install never fires until a manual kick"
        )
...
class TestStartVerifiesNextElapse:
    def _run_side_effect(self, next_elapse_lines):
        def side_effect(cmd, **kw):
            ...
            r = R()
            if "show" in cmd:
                r.stdout = next_elapse_lines
            return r

        return side_effect

    def test_start_fails_when_timer_will_never_fire(self, tmp_path, monkeypatch):
        prov = SystemdProvider()
        monkeypatch.setattr(prov, "_unit_dir", lambda: tmp_path)
        with patch("axiom.infra.services.subprocess.run") as run:
            run.side_effect = self._run_side_effect("0\nn/a\n")
            assert prov.start(_svc(interval_secs=300)) is False

    def test_start_succeeds_when_next_elapse_scheduled(self, tmp_path, monkeypatch):
        prov = SystemdProvider()
        monkeypatch.setattr(prov, "_unit_dir", lambda: tmp_path)
        with patch("axiom.infra.services.subprocess.run") as run:
            run.side_effect = self._run_side_effect("1788225601000000\n0\n")
            assert prov.start(_svc(interval_secs=300)) is True

    def test_daemon_start_skips_the_probe(self, tmp_path, monkeypatch):
        prov = SystemdProvider()
        monkeypatch.setattr(prov, "_unit_dir", lambda: tmp_path)
        with patch("axiom.infra.services.subprocess.run") as run:
            run.return_value.returncode = 0
            run.return_value.stdout = ""
            assert prov.start(_svc(interval_secs=0)) is True
            assert not any("show" in c.args[0] for c in run.call_args_list)
