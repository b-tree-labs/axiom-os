...
def _isolate_site_policy(tmp_path_factory, monkeypatch):
    """ADR-114 §2 hermeticity: point the site-policy loader at an absent file so
    a dev machine's ``~/.axi/authority/site_rules.toml`` can't leak in, and
    reset the cached default context + site-policy flag around every test so §2
    module state never bleeds between tests."""
    missing = tmp_path_factory.mktemp("no_site_policy") / "absent.toml"
    monkeypatch.setenv("AXIOM_AUTHORITY_SITE_RULES", str(missing))
    authority._reset_default_tool_context()
    yield
    authority._reset_default_tool_context()


def _boom(env, ctx=None):
...
class TestSitePolicyFailClosed:
    """ADR-114 §2 — a loaded site policy adds rules and flips the error path
    from fail-open (pass-through) to fail-closed (deny)."""

    def _policy(self, tmp_path, monkeypatch, body: str):
        f = tmp_path / "site_rules.toml"
        f.write_text(body, encoding="utf-8")
        monkeypatch.setenv("AXIOM_AUTHORITY_SITE_RULES", str(f))
        monkeypatch.setenv("AXIOM_AUTHORITY_RECEIPTS", "off")
        authority._reset_default_tool_context()
        return f

    def test_no_policy_keeps_the_error_path_fail_open(self, monkeypatch, caplog):
        # autouse fixture already points at an absent file
        monkeypatch.setattr(authority, "decide_tool_call", _boom)
        with caplog.at_level(logging.WARNING, logger="axiom.infra.authority"):
            assert authority.authority_pre_invoke_hook(_payload(), "@alice:axiom") is None
        assert any("fail-open" in r.getMessage() for r in caplog.records)

    def test_present_policy_makes_the_error_path_deny(self, tmp_path, monkeypatch, caplog):
        self._policy(tmp_path, monkeypatch, '[[rule]]\ntool="press.publish"\ndisposition="propose"\n')
        monkeypatch.setattr(authority, "decide_tool_call", _boom)
        with caplog.at_level(logging.ERROR, logger="axiom.infra.authority"):
            result = authority.authority_pre_invoke_hook(_payload(), "@alice:axiom")
        assert result is not None and result.decision == "deny"
        assert any("fail-closed" in r.getMessage() for r in caplog.records)

    def test_malformed_policy_is_present_and_fails_closed(self, tmp_path, monkeypatch):
        self._policy(tmp_path, monkeypatch, "not valid toml [[[")
        monkeypatch.setattr(authority, "decide_tool_call", _boom)
        result = authority.authority_pre_invoke_hook(_payload(), "@alice:axiom")
        assert result is not None and result.decision == "deny"

    def test_default_context_applies_a_loaded_deny_rule(self, tmp_path, monkeypatch):
        self._policy(tmp_path, monkeypatch, '[[rule]]\ntool="press.publish"\ndisposition="deny"\n')
        env = authority.build_tool_envelope("press.publish", {}, "@alice:axiom")
        verdict = authority.decide_tool_call(env)  # uses default_tool_context()
        assert verdict.decision == Decision.DENY

    def test_loaded_propose_rule_holds_a_tool_for_approval(self, tmp_path, monkeypatch):
        self._policy(tmp_path, monkeypatch, '[[rule]]\ntool="press.publish"\ndisposition="propose"\n')
        result = authority.authority_pre_invoke_hook(
            _payload(tool_name="press.publish"), "@alice:axiom"
        )
        assert result is not None and result.decision == "approval_required"

    def test_unlisted_tool_still_permits_under_a_site_policy(self, tmp_path, monkeypatch):
        # Additive: a policy that only gates press.publish leaves other tools open.
        self._policy(tmp_path, monkeypatch, '[[rule]]\ntool="press.publish"\ndisposition="deny"\n')
        assert (
            authority.authority_pre_invoke_hook(_payload(tool_name="data.list"), "@alice:axiom")
            is None
        )


class TestEgressWriteMcpFloor:
    """ADR-114 §3 — external-egress writes return to MCP behind a built-in
    propose-on-mcp approval floor; the CLI/agent path is untouched."""

    @pytest.fixture(autouse=True)
    def _receipts_off(self, monkeypatch):
        monkeypatch.setenv("AXIOM_AUTHORITY_RECEIPTS", "off")
        authority._reset_default_tool_context()

    def test_press_publish_holds_for_approval_on_mcp(self):
        result = authority.authority_pre_invoke_hook(
            _payload(tool_name="press.publish", surface="mcp"), "@alice:axiom"
        )
        assert result is not None and result.decision == "approval_required"

    def test_press_publish_permits_on_cli(self):
        assert (
            authority.authority_pre_invoke_hook(
                _payload(tool_name="press.publish", surface="cli"), "@alice:axiom"
            )
            is None
        )

    def test_press_mirror_sync_holds_for_approval_on_mcp(self):
        result = authority.authority_pre_invoke_hook(
            _payload(tool_name="press.mirror_sync", surface="mcp"), "@alice:axiom"
        )
        assert result is not None and result.decision == "approval_required"

    def test_a_non_egress_tool_is_unaffected_on_mcp(self):
        assert (
            authority.authority_pre_invoke_hook(
                _payload(tool_name="data.list", surface="mcp"), "@alice:axiom"
            )
            is None
        )

    def test_surface_less_egress_call_is_not_gated_by_the_mcp_floor(self):
        # A call with no surface (e.g. legacy path) doesn't match the mcp-scoped
        # floor; it falls through to the open permit.
        assert (
            authority.authority_pre_invoke_hook(
                _payload(tool_name="press.publish"), "@alice:axiom"
            )
    ...
    def test_the_floor_names_the_two_egress_writes(self):
        assert set(authority.EGRESS_WRITE_MCP_APPROVAL_TOOLS) == {
            "press.publish",
            "press.mirror_sync",
        }
        rules = authority.egress_write_mcp_rules()
        assert all(r.surface_pattern == "mcp" and r.disposition == "propose" for r in rules)
