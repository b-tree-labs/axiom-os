...
def _pg_available() -> bool:
    try:
        ...
        url = os.environ.get("AXIOM_DB_URL", "postgresql://axiom:axiom@localhost:5432/axiom_db")
...
class TestSchemaPerExtension:
    def test_ensure_schema_idempotent(self, monkeypatch):
        # Asserts the production mapping (no suffix); same rationale as
        # TestNormalizeExtensionName._no_worker_suffix. CREATE SCHEMA IF NOT
        # EXISTS is safe for two workers to run unsuffixed.
        monkeypatch.delenv("AXIOM_TEST_SCHEMA_SUFFIX", raising=False)
    ...
    def test_two_extensions_cannot_see_each_others_tables(self):
        ...
        try:
            ...
            with session_for("test_db_b") as b:
                seen = b.execute(text("SELECT to_regclass('only_in_a') IS NOT NULL")).scalar()
    ...
    def test_engine_for_returns_shared_engine_and_schema(self, monkeypatch):
        # Production mapping again: no suffix, no DB write — engine_for only
        # resolves, so clearing the worker suffix cannot collide workers.
        monkeypatch.delenv("AXIOM_TEST_SCHEMA_SUFFIX", raising=False)
...
class TestEnginePrePing:
    """The shared engine must pre-ping so long-running services survive a
    Postgres-dropped idle connection instead of 500-ing on first reuse."""

    def test_shared_engine_enables_pool_pre_ping(self, monkeypatch):
        # Point at an in-memory URL so no real DB is needed; the pool option
        # is set the same way regardless of dialect.
        monkeypatch.setenv("AXIOM_DB_URL", "sqlite://")
        import axiom.infra.db as db

        # Force a rebuild keyed on this URL so we read a fresh engine.
        db._engine = None
        db._engine_url = None
        try:
            ...
            # Don't leak this throwaway engine into other tests' global cache.
            db._engine = None
            db._engine_url = None
