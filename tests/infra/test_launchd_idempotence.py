...
class TestStartIdempotence:
    ...
    def test_start_loads_when_not_loaded(self, tmp_path, monkeypatch):
        ...
        def fake_run(args, **kwargs):
            ...
            from types import SimpleNamespace
            if "list" in calls[-1]:
                # Not loaded before the load; loaded after it. The original
                # fake answered "not loaded" even after a successful load,
                # which was fine while start() trusted the exit code and is not
                # now that it confirms the agent is actually running.
                rc = 0 if loaded["yes"] else 113
