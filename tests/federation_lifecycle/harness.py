...
def docker_available() -> tuple[bool, str]:
    ...
    Every probe here is guarded, because this function exists to let a test
    skip and a guard that raises does the opposite of its job. Both probes
    carry a short timeout, and a short timeout is a statement about a quiet
    machine: these tests run inside a parallel suite that is itself the load,
    so an unresponsive-looking Docker is an ordinary outcome, not an
    exceptional one. The first probe was already guarded; the second was not,
    which turned a would-be skip into a failed run whenever the daemon
    answered slowly.
    """
    ...
    try:
        ...
        return False, f"docker compose not responsive: {exc}"
    if r2.returncode != 0:
