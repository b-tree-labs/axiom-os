...
"""Tests for axiom.setup.state."""

from datetime import UTC, datetime, timedelta

from axiom.infra.paths import project_dir_name
from axiom.setup.state import (
    SetupState,
    clear_state,
    load_state,
    save_state,
...
class TestPersistence:
    ...
    def test_load_corrupt_file(self, tmp_path):
        state_path = tmp_path / project_dir_name() / "setup-state.json"
    ...
    def test_load_stale_state(self, tmp_path):
        ...
        assert not (tmp_path / project_dir_name() / "setup-state.json").exists()

    def test_clear_state(self, tmp_path):
        state = SetupState()
        save_state(state, tmp_path)
        assert (tmp_path / project_dir_name() / "setup-state.json").exists()
