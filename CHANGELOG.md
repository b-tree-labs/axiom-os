...
### A long-lived host no longer loses its authority after an hour

Every extension context minted its capability once at setup with a
1-hour TTL and cached it for the life of the process. A short-lived CLI
never noticed; the data-platform orchestrator outlived the TTL and every
scheduled dispatch after the first hour was floor-denied
EXPIRED_CAPABILITY — four nightly backups were lost to it before the
fleet console's effect-checked status surfaced the gap on its first day.
The context now re-mints its own capability through the same path setup
used when it nears expiry: renewal is continuation of the original grant
(same subject, same scope), never escalation, and an explicitly passed
capability never silently renews — an expired caller token still denies.

### The checks printer fires only on its own shape

`axi data backup-validate` ran all five checks, wrote its audit receipt,
then crashed printing the result: the connector-preflight pretty-printer
triggered on any value containing `checks`, including shapes it was
never built for. It now requires the preflight shape; everything else
falls through to the generic renderers. A printer must never hide the
effect it was asked to report.

## 0.58.0 — 2026-09-21

### ADR-109 was claimed twice; the tenancy control plane is now ADR-120

Two ADRs carried number 109 — `build-provenance-attestation` on main and
`tenancy-control-plane` on a long-lived branch that predated it. The branch's
document was renumbered out of that collision — briefly to 119, which a
parallel branch had independently claimed for the fleet console push model —
and lands as **ADR-120**; the build-provenance ADR keeps 109 and the fleet
console keeps 119.

Caught both times by the pre-push ADR-collision lint — the only layer that
runs it, which is why the number a branch computes locally is not settled
until the push that carries it wins. (This entry originally rode into the
released 0.57.0 section by mistake; it belongs here.)

### One owner for the SKILL.md contract

The frontmatter was written by f-strings and read by a YAML parser, and
the two drifted: on the 178 descriptions shipped in builtin manifests,
the old writer produced 42 unreadable documents — a colon in an unquoted
value took the whole frontmatter with it. Render and read now live in
one module (`skill_md`), rendering through `yaml.safe_dump`, and
generated documents declare themselves (`generator:` marker,
`SkillDocument.is_generated`). Also fixed, demonstrated not theorised: a
skill name carrying `../` became a directory component and emitted a
file OUTSIDE the skills tree; names are validated as dotted identifiers
and emission independently refuses traversing paths. (#917, ADR-117)

### Every declared skill carries a parsing SKILL.md

Before: 4 of 58 SkillSpec-declared skills across 19 extensions had a
parsing SKILL.md — 54 declared capabilities were invisible to any
assistant that discovers capability by reading SKILL.md. After the
sweep: 58/58 parse (54 generated, 4 authored kept byte-untouched), and
the generator now applies the authored/generated split itself: a
document without the generator marker is never overwritten and never
called drift; an unparseable one is repaired and the repair reported.
(#939, ADR-118)

### The fleet console: nodes push out, green cites evidence

A new `fleet` extension. Nodes push signed reports (heartbeat, service
health, backup + validate outcomes, canary, versions) to one
authenticated front door, `POST /api/v1/fleet/reports`, behind a
site-bound key — site from the credential, never the payload; the
console never opens a connection toward a node. `axi fleet status`
evaluates at read time: GREEN requires cited observed effect (dump bytes
with a fresh timestamp, `restore_live` PASS, measured latency), a claim
without evidence renders UNPROVEN — distinct from FAILED — and anything
past 3x its declared cadence renders STALE. A silent node can never look
green. (#940, ADR-119)

### Retrieval enforces access, ranks honestly, and serves one path

The RAG integrity core lands whole: deterministic anti-fabrication gates
on the answer path (G4), access enforced at retrieval — columns
projected, honored, and the context threaded per-principal through one
seam, fail-closed (G2), the heuristic reranker folded onto the live
retrieval path (G1), and the org serving endpoint converged onto the
hardened `/rag` path (G5). The MCP retrieve primitive keeps its
hardened store mechanics (a store that cannot report tiers is refused;
unknown paths deny) with the context now derived through the same
per-principal seam. (#889, #942)

### Chat usage is measured, not inferred

Chat becomes a named capability surface and publishes the same
content-free telemetry projection that the CLI and MCP surfaces already
emit, so an invocation from a conversation lands in the same series as
one from a terminal. Built at the shared dispatch chokepoint rather than
in any one protocol, so `neut chat` is measurable on exactly the same
terms as an assistant reaching the platform from outside. Until this,
chat was the one surface whose usage could only be inferred — the one
surface where "is the assistant better with the platform configured"
was unanswerable. (#941)

### Partners sign in with a magic link

The partner-facing half of auth: magic-link sign-in core with a
cross-component end-to-end test. (#934)

### Site reads have a first-class authorization model

The per-site (tenant) read-authorization model for the OpenFGA
substrate, with a productized model test. (#935)

### Connectors refresh on a cadence, and "ran" is not "landed"

Every enrolled connector projects onto a daily CDC-refresh cadence
instead of hand-run pulls (#936). Two deliberately distinct questions:
`data.refresh` asks whether the pull RAN; `data.ingest_freshness` asks
whether rows LANDED — a connector can refresh on time and deliver
nothing, which is the case that looks healthiest and matters most. The
new `axi data ingest-freshness` verb asks the second question from the
terminal. (#936, #945)

### A push connector must name its site

Registered without `--site`, a push connector accepted rows with 2xx
forever while conformance silently skipped every one (found live: all
eleven push connectors on a node). A connector that can ACCEPT rows must
be able to LAND them: registration now refuses, with `--allow-unmapped`
as the explicit escape hatch. (#945)

### Extension lifecycle verbs reconciled

The extension lifecycle verbs consolidate into one noun, and `--env`
validation fails with a clean message instead of a traceback. (#937)

## 0.57.0 — 2026-09-20

### `kind = "api"` was legal in the code and illegal in the schema

A verb joins the composed `/api/v1` surface by declaring `kind = "api"` with a
subpath, which the API shell discovers and resolves lazily — that is how a
client gets one base URL and one auth posture regardless of which verbs are
installed. The manifest schema never learned the kind, so an extension that
followed the documented mechanism failed its own conformance test, while
another extension shipped the same declaration unnoticed, having no schema test
of its own. The schema and AEOS §4.10 now define it.

### Chat over HTTP

`/api/v1/chat` serves conversation CRUD, streaming (SSE) and message feedback,
with site/tenant scoping ANDed with principal isolation so one account's
history cannot be read through another's scope. Performance guards cover
list-at-scale, create, stream and throughput.

### A base install could not answer a question about its own data

The data platform shipped ingest, backup and diagnose — and nothing that could
answer "what is the mean of this column". Every producing tool lived as a
hand-written SQL function per quantity, at the consumer layer, which is the most
specific layer owning the most reusable machinery.

`data.gold_tables`, `data.gold_describe`, `data.gold_aggregate` and
`data.gold_series` are that generic half, in the platform: schema-introspecting,
read-only, provenance-stamped, and projected to CLI, MCP and the agent surface
so an assistant discovers them instead of carrying a hardcoded dispatch table.
A node that has ingested one table can answer aggregate and series questions
about it with no domain code installed.

The filter grammar is small on purpose (`= != < > <= >= in`, AND-joined).
Identifiers are checked against the introspected schema and then quoted; every
literal is a bound parameter; a bare identifier is refused so it cannot smuggle
a column reference. `between` is absent because `>=` plus `<=` says the same
thing without an AND-ambiguity in the parser, and a smaller grammar is a smaller
attack surface. Access tiers fail closed: a table declared restricted that
carries no tier column refuses to answer rather than answering unfiltered.

An empty window returns `data: null` with a provenance note — never a
fabricated zero, so the answer layer abstains instead of inventing.

### Answers are grounded or withheld

The provenance gate verifies every value an answer states against that turn's
evidence and suppresses what it cannot ground, now including honest aggregates
of a tool series in any format. The analytics tool does the arithmetic
deterministically; the model narrates it. Coverage reconciles gated quantities
against the tools installed to produce them, so a missing tool reads as a
coverage gap rather than a wrong number.

### Authority is enforced, not just declared

`allowed_principals` was declared, linted and never checked at call time. It is
now enforced at the MCP dispatch chokepoint, alongside a site-rule effect gate
(a policy file can hold or refuse a specific tool) and surface-scoped rules, so
"require approval for this tool on MCP" no longer means gating the CLI too. The
error path flips from fail-open to fail-closed once a site policy is present;
a malformed policy still counts as present, so a typo cannot silently reopen
the surface.

### Installing now checks what publishing signs

ADR-109 gave every published artifact a signed PEP 740 provenance attestation,
and named its own gap: we signed what we published and never checked what we
installed. `axi ext install --from-pypi` closes that half — it fetches the
package's Sigstore attestations and verifies them against the expected
publisher identity before anything unpacks, failing closed with no
trust-on-first-use fallback. AEOS §9.2's sentence is now true instead of
half-true.

### The stranger path works, and CI walks it

A stranger with a bare install hit four dead ends the sandbox audit named:
`axi config` was a no-op that pointed back at itself, the small-model path was
unreachable, `axi db compose` assumed credentials a stranger doesn't have, and
the SQLite store wasn't reachable from setup. All four now work using assets
the repo already shipped. A container-smoke CI job replays that stranger path
from a clean image on every pull request, so the first-run experience can no
longer rot silently.

### A stale example template can no longer shadow a corrected builtin

The prompt registry loads builtins, then example files, then site overrides —
later layers winning by id. Rewriting a builtin template therefore changed
nothing on any install that had materialized the old example file: the stale
copy silently won. Shipped example templates now defer to a newer builtin of
the same id, the base template is renamed `agent_base` (`neut_agent_base`
resolves as an alias), and a drift guard fails the build when a shipped
example and its builtin disagree.

### Every builtin carries the standard it ships

Three AEOS conformance moves. AEOS040 now demands a capability-kind directory
only when the manifest *provides* that kind, taking the fleet from 307
warnings to 70 real findings. All 48 builtin extensions carry the §8.2
standard test base, so every manifest is exercised by the same suite. And
manifest conformance now covers the schema itself, MCP declarations, and
personas.

### A principal need not be a person

`Principal` always described itself as "human, agent, node, org"; the identity
layer now records the kind as a first-class discriminator instead of assuming
a person. `@room-scheduler:site` is a valid principal with no new naming
scheme, no interview, and no consent flow — profiles key on the handle, so a
service principal costs nothing extra.

### A tenant is now a record, so subscriptions have a home

`tenant_corpora` accepted subscriptions that nothing could ever supply — a
tenant was implied by scoping context but had no configuration of its own.
There is now one tenant record (TOML beside the connector registry, the
pattern already proven there), and corpus subscriptions live in it.

### The gateway serves the embedding model RAG was already calling

The rendered LiteLLM gateway config carried only the generation model, so a
fresh deployment's RAG embedded against nothing: `axi rag add` failed and
retrieval timed out against a stray local server with no error on the chat
path. The IaC now emits the embedding model alongside generation by default,
capturing what was previously a live hot-fix.

### Ingest has a reference model

`axi data register --site` closes a silent drop — a connector without a site
was skipped by conformance and its rows sat in bronze forever, and the natural
registration verb had no way to set one. Ingest freshness views make
bronze→silver lag observable, and the RECIPE kit is the single-source model
partner loops copy.

### Governance overhead is now a number

A benchmark measures the governed-minus-bare delta on the real seams, no
mocks: the full governed path costs 8.15 ms p50, and the decision logic itself
is 19 µs — the cost is durability (fsync), not judgment. A calibration-audit
harness measures what a calibrated typed-decision endpoint actually delivers.

### The tier classifier keeps score against itself

A new `graduation` extension shadows the LLM tier-routing decision and logs
outcomes — changing no live behavior; a raising or slow observer cannot alter
what `classify()` returns — so a 30-day record accumulates of what the rule
decided and what actually happened.

### `axi agents status` states the master gate

Status could read "running" with consented agents, intervals and last-run
times while autonomy was off — every line true, the whole misleading. It now
leads with an explicit `Autonomy: ON/OFF` line in both states.

### Also

- `axi analytics` no longer hardcodes the platform CLI name in its usage line.

## 0.56.0 — 2026-09-18

### Nine skills, nine answers to "which database?"

`axi data backup` could not find the database on a node where
`axi db migrate upgrade head` had just succeeded. data.backup is the skill a
nightly PULSE cadence invokes, so arming the schedule against it would have
registered a row that fires at 2am, fails, and leaves a policy reading
`enabled = true` with no backup behind it — and the verb would have reported
`PULSE SAYS: armed`, truthfully. Found by running a backup BEFORE arming.

0.54.0 fixed `ensure_schema` and called it a class fix. It was one of nine:
every data-platform skill resolved a DSN by hand and they disagreed.
`data_platform._dsn.resolve_dsn` is now the only answer — explicit param, the
env names deployments set, then `axiom.infra.db.platform_db_url`. A static
guard rejects any skill reading those names directly; it found two sites
beyond the seven already changed.

The fallback is the platform's own connection, not a guess, but it earns
visibility: `data.backup` reports which SOURCE answered, never the value. A DSN
carries a password, and `axi data backup-policy` now redacts any `_dsn` field
it prints.

### `scratch_db`, so a restore check needs no credential in the policy

`backup-validate`'s restore check needs a disposable database. Naming it as a
full `scratch_dsn` would put a password in a file the policy verb prints back.
The policy now takes `scratch_db` — a bare name — and derives the DSN from the
live one at use time. `backup_validate` also resolves the live DSN through the
shared resolver, so the row-count comparison runs on a scheduled invocation;
it was previously skipped unless someone passed `--dsn` by hand.

### A result printer that hid the failure it was asked to print

`axi data backup-validate` died with `KeyError: 'connector'` — `_emit` treated
any result carrying "checks" as the connector-preflight shape. The skill had
produced the right answer ("no scratch_dsn provided"); the printer threw it
away. Now the connector branch requires both keys and a generic checks result
prints its checks.

## 0.55.0 — 2026-09-18

### The gold views could not be replaced, because a column was inserted

    ERROR: schema not ensured: cannot change name of view column "ts" to "role"

`role`, `uncertainty` and `derivation` were written into the gold view SELECT
interleaved — `channel, role, ts, value, unit, uncertainty, ...` — which reads
better and is unusable. `CREATE OR REPLACE VIEW` may only APPEND a column; it
cannot insert, reorder or rename one. Every live install has the original ten,
so the deploy step has stopped here since the columns were added, refusing to
restart services onto a database behind the code.

The column order is now data rather than prose: `GOLD_SIGNALS_BASE_COLUMNS` is
the frozen ten, new columns append after it, and the SQL is built from the
concatenation so the two cannot drift. A guard checks the base is a literal
prefix of the whole, so inserting a column anywhere but the end fails in CI
instead of on a node mid-deploy.

`ensure_schema` also recovers a view whose shape already drifted: on that one
recognisable conflict it drops and recreates, and reports which views it
recreated. `DROP` without `CASCADE` on purpose — if something depends on the
view, that is a decision for a person, not a deploy step. The recovery is
narrow: an unrelated view error still fails the deploy and drops nothing.

## 0.54.0 — 2026-09-17

Two more places where the thing reporting success was not watching the thing
that mattered. Both were found by trying to arm a nightly backup and refusing
to claim it was armed.

### ensure-schema could not find the database the process was already using

The v1.6.65 deploy, one step, two lines apart:

    ✅ Upgrade complete                      <- axi db migrate reached the DB
    ERROR: no DSN: pass dsn= or set DP1_RAG_DSN / DATABASE_URL / AXIOM_DB_URL

`get_engine` — and therefore every `session_for`, `engine_for` and Alembic
`env.py` — reads `AXIOM_DB_URL` and falls back to `DEFAULT_DB_URL`.
`data.ensure_schema` read three env names and failed when none was set. On a
node whose environment carries none of them, one door connects and the other
refuses, in the same breath.

The skill's own error message already named this ("these three names all
address the same database and are not interchangeable across skills — that
inconsistency is its own bug"). Naming a bug in an error string is not fixing
it. `axiom.infra.db.platform_db_url()` is now the single definition both use.

A test was protecting the old behaviour: it asserted the skill FAILS on an
empty environment and checked the error listed all three names. A nicely-worded
refusal to find a database the process is already connected to is still a
refusal.

### backup-policy reports what PULSE will actually run

`ensure_backup_cadences` is called in exactly one place: orchestrator startup.
Nothing re-runs it when the policy file changes, and there was no CLI or skill
door to it. Its docstring says it "reschedules on policy edits" — true of the
function, false of the system.

So `enabled = true` armed nothing until the service restarted, and until then
the file said the backup was on while the scheduler held no rows, with nothing
anywhere reporting the disagreement. Arming the nightly backup and confirming
it were the same unverifiable gesture.

The policy is an intention; a PULSE row is the effect. They are now two lines:

    POLICY ASKS FOR: data.backup on '0 2 * * *', data.backup_validate on '30 6 * * *'
    PULSE SAYS: armed
       data.backup            active, next 2026-09-18 02:00:00+00:00
       data.backup_validate   active, next 2026-09-18 06:30:00+00:00

Three disagreements are errors rather than green reports: enabled with no rows;
only one of the two cadences active (a backup with no validation is the state
that produced an 83 MB dump nobody could restore); and rows still active after
a disable. An unreadable schedule store reports `unknown` rather than guessing
`not armed`.

`--apply` projects the policy onto PULSE immediately, so arming does not need a
service restart and can be confirmed in the same breath.

The first draft of that reader used `getattr(row, "next_run_at", None)`. The
column is `next_fire_at`; `next_run_at` does not exist, so it would have
reported "no next run" forever. The guard checks the reader against the real
SQLAlchemy model rather than a fixture, because a fixture agrees with whatever
name the reader invented.

## 0.53.0 — 2026-09-17

Three failures with one shape: a distinction that existed in the code but could
not reach the place that needed it.

### The backup policy loader dropped four fields it was never taught

`BackupPolicy` gained `exclude_table_data`, `timeout_s`, `validate_restore` and
`scratch_dsn`. Both the TOML reader and the writer enumerate every field by
hand, and neither was updated. The policy file on the node sets three excluded
tables and a 3600s timeout; the node loaded `None` and `600`.

A setting that reads correctly on disk and does nothing is worse than one that
is missing, because nothing about it looks wrong. Arming the nightly schedule
against it would have dumped the whole 368 GB database against the old timeout
and left a truncated artifact that still passes `exists`, `fresh` and
`nonempty` — the exact failure `exclude_table_data` was added in 0.51.0 to
prevent.

The guard is reflective rather than another hand-written list: round-trip a
policy whose every field differs from its default, then diff
`dataclasses.fields(BackupPolicy)` against what survives. It caught the writer
immediately after the reader was fixed.

### An absent optional extra was a deploy failure

`run_migrations` ended in `except Exception` and returned `False`, so
`ModuleNotFoundError` died one frame down and arrived at callers as a bool.
`axi db migrate upgrade head` already had the skip branch for it — `signals`
needs `pgvector`, which lives in the `signal` extra — and that branch could
never fire:
...
0.52.0 claimed this skip. It shipped unreachable. The v0.52.0 deploy failed on
it and correctly refused to restart services, leaving the node running old code
while every other step reported success.

All three call sites (`db`, `update`, `signals`) now treat an absent extra as a
skip and name the missing module. A genuinely broken migration still fails.

### `axi data backup-policy`

Arming the nightly backup *is* editing the policy file, so the arming gesture
and the configuration were the same gesture with nothing in between. This verb
is that step: it reports the policy as the node **loads** it, and names anything
the file said that did not survive —

  * a key the loader does not know, and
  * settings stranded at the top of the file instead of under
    `[backup_policy]`. Those parse as valid TOML, load an empty table and yield
    pure defaults, so an `enabled = true` written that way arms nothing and
    looks armed.

## 0.52.0 — 2026-09-17

Two classes, both found by a deploy step running migrations on a node for the
first time and refusing to restart services when it could not finish.

### Migrations that stay in their own schema

`schedule` failed with `relation "schedule_definition" does not exist` on an
`ALTER` — after Alembic reported creating that very table two revisions
earlier. On a clean database the cause is visible:

    search_path: "$user", public          <- not "schedule"

    public      schedule_time_slot        <- 0002 landed in the WRONG schema
    schedule    schedule_definition       <- 0001 landed correctly

`0001` passes `schema="schedule"` on all six of its ops. `0002`–`0006` pass it
on none — **28 ops across five revisions** — so they resolve through
`search_path`, which does not include the extension's schema, and land in
`public`. The unqualified `ALTER` then looks in `public` and dies.

`session_for` sets `search_path`; `engine_for`, which every Alembic `env.py`
uses, does not. The two doors to the same database disagree about where an
unqualified name lives, and a migration is always behind the door that
disagrees. That extension had **never once migrated successfully**, anywhere.

All 28 are qualified, and `tests/infra/test_migrations_stay_in_their_schema.py`
closes the class with an AST walk over every revision of every extension —
static, no database, naming the file and line. It immediately found two more:
`signals` (26 unqualified ops, which is exactly why its tables are in `public`)
and `expman`. Both are `xfail(strict=True)` with the decision recorded rather
than exempted: their tables already exist in `public` with live data, so
qualifying them would leave every install with two copies. The marker fails as
XPASS the moment someone does the data migration.

### "Not installed" is not "broken"

The same run failed on `signals` with `No module named 'pgvector'`. Nothing was
broken: `pgvector` lives in the `signal` extra and a node may legitimately
install without it. Counting that as failure means a deploy can never complete
on a node that does not ship every extension — and a step that cannot go green
is a step people route around.

`ProvisionResult.skipped` is a third outcome in its own field: `ok` is True so
the caller proceeds, `migrated` stays False so a skip is never read as "up to
date", and it is kept apart from `error` because "not installed" and "broken"
want different reactions. Narrow on purpose — only `ModuleNotFoundError` takes
that path.

    signals:  skipped — 'pgvector' is not installed
              (pip install 'axiom-os-lm[signal]' to provision it here)

## 0.51.0 — 2026-09-17

Three checks that reported success without observing the effect. All three were
found by running the thing rather than reading it.

### Backups that restore

A backup taken on a live node passed four checks and could not be restored:

    backup_exists    PASS   1 artifact
    backup_fresh     PASS   0.2h old
    backup_nonempty  PASS   87,292,980 bytes
    toc_parses       PASS   TOC parses (268 entries)
    restore_live     FAIL   relation "public.documents" does not exist
                            CREATE VIEW bronze.documents AS SELECT ... FROM public.documents

The dump was scoped by schema to keep it small. **`pg_dump` will happily dump a
view whose underlying table sits in an excluded schema**, so the artifact looked
complete by every structural measure and died on its first view. The
dependencies on that node run all the way down — `bronze.documents` selects from
`public`, all seven `gold` views select from `silver` — so there is no
self-contained subset to scope to.

- **`exclude_table_data`** is the right lever: keep every DEFINITION so views
  resolve, skip the DATA of the few enormous re-derivable tables. Measured:
  368 GB → 95 MB, `pg_restore` exit 0, no errors.
- **`timeout_s`** is configurable. It was hard-coded at 600s, which a large dump
  blows through — leaving a truncated file that passes exists/fresh/nonempty
  exactly like the one above.
- `schemas` remains, with a docstring that now says why it is usually wrong.

### The nightly check now actually restores

`validate_restore` was a params-only flag. Nothing in the policy carried it and
nothing scheduled passed it, so the nightly validation ran the four structural
checks and **never added a restore check at all** — not FAIL, not skipped:
absent, from a report that reads as complete.

It moves into the policy, defaulted **on**, and `restore_live` is always
reported: `PASS` restored and row counts sane, `FAIL` restore attempted and
failed, `WARN` not attempted so the backup is unproven. That third state exists
because "unverified" and "broken" are different conditions, and one colour for
both is how a red report becomes background noise.

### `data ensure-schema` — the step a deploy was missing

A release added three columns to `silver.signals`. The deploy installed it,
restarted the services, went green across every step, and the columns were
absent. A deploy is not a provision: the Alembic-versioned extensions have
`axi db migrate upgrade head`, while the conformance tier is raw DDL applied
only by a conform pass — which is not scheduled on every node. The installed
upsert then named three columns the table did not have.

`ensure-schema` reports the **difference**, not the outcome: it reads
`information_schema` before and after and returns the columns it actually added,
so a log says `added silver.signals.role` or says nothing changed, and both are
checkable. It skips index creation loudly (`CREATE INDEX` blocks writes; a
deploy is the wrong place for it), holds a 5-second lock timeout so it never
queues in front of readers, and fails rather than warns — restarting services
onto a database behind the code they run is the condition it exists to prevent.

## 0.50.0 — 2026-09-17

Silver now says what it holds. Every change here fixes the same defect wearing
a different hat: something knew a fact and discarded it without saying so.

### The provenance vocabulary

- **`simulated` joins `source_class`.** Three flow-loop sites carried a
  `loop.instrument` stream stamped `measured` whose own `schema_ref` read
  `flowloop-sim/instrument-v0` — one generator with three seeds, presented as
  three reactors' measurements. The vocabulary had nowhere honest to put
  synthetic data, so it took the default, and the default asserts a
  measurement. It is deliberately **not** folded into `predicted`: a prediction
  is a model's statement about a real system's state, while a simulator's
  output has no referent, and conflating them is how a demo stream gets
  averaged into a comparison between two real loops. Like any modelled class it
  must name its generator.
- `skills.enroll` and `daq.envelope` held separate copies of that tuple — one
  edit from a source that enrolls but whose rows are refused at ingest. Now one
  definition.

### Three columns silver was dropping

- **`uncertainty`**, in the same unit as `value` and with no unit column of its
  own: two would drift, and every chart would have to ask which it got before
  drawing a band. NULL, never 0 — zero uncertainty is a claim of perfect
  precision.
- **`role`** — what a channel *means*, beside what it is *called*. Channel names
  stay exactly as acquired, because a rename is a decision the raw record
  cannot justify; the role is what lets two sites be compared without either
  being renamed.
- **`derivation`** (`raw` | `derived`) — specified in the ingest contract so
  "gold rollups don't double-count", never carried. A loop reporting a log-mean
  temperature beside the thermocouples it was computed from was contributing
  the same physics twice to any rollup treating channels as independent.

All three arrive on an existing table in place; deployed rows survive as NULL.

### Runs, and labels that keep their provenance

A run is a bounded operation with configuration in normalised keys — keys are
to runs what roles are to channels. `similar_runs` reports **why** each
candidate matched, since a match on two keys of six is a different statement
from two of two; a reference with no configuration matches nothing, because two
runs are not alike merely because nothing is known about either.

`label_source` separates three claims that must never merge: `curated` (a
person marked it, and must be named), `recorded` (the plant declared it — no
curator to invent, nothing estimated), and `inferred` (an algorithm's
hypothesis, which must state a confidence or it reads with a person's
authority). The first two are ground truth; the third is scored against them,
which a single column would make impossible.

`collapse_states` turns a mode channel into segments — on a live console feed,
hundreds of millions of rows become a few thousand. A state with no mapping is
**kept** as `unknown` carrying its raw value, so a gap in the vocabulary is
visible rather than indistinguishable from silence.

### The chart catalog

Displays described rather than drawn: a `kind` from a closed vocabulary, and a
renderer decides what it looks like. Provenance is part of the display, not a
caption — a chart claiming to be measured whose inputs carry modelled rows is
refused, because that rule cannot live in the renderer without every renderer
having to remember it. Charts group by the question they answer, and
`comparable()` reports which sites a chart can honestly be drawn for.

### Two compound reads

`series_compare` gathers one role across sites and names what it excluded and
why — a site missing from a comparison reads as "no data" when it may mean
"excluded as simulated", and those call for opposite fixes.

`parity` is `hybrid` by construction, with no way to ask for "measured", and
**counts unpaired points** instead of dropping them: a residual computed only
where both sides happen to have data is the most flattering possible view of a
model, and it is what subtracting two arrays gives you for free.

### Fixed

- The serving catalog stops advertising channels the source no longer has. It
  offered three channels at 978 rows each while silver held none for them — a
  partner clicking one got an empty chart, which reads as their data being
  broken rather than as a channel that is gone.

## 0.49.0 — 2026-09-16

One platform serves several sites. This release stops it handing every site to
whoever asks, and gives the catalog a schema it can actually evolve.

### Site scope

- `axiom.infra.site_scope` bounds every site-scoped resource to what the request
  is allowed to see. It is a **platform primitive**, not an API filter, because
  charts, models, logs and documents all belong to a site — if each surface
  grows its own filter, the one that forgets is the leak.
- The bound is **deployment ∩ grant**: `AXIOM_SERVED_SITES` names what this
  deployment hosts, and a principal's grant narrows it further. Deployment
  applies first, so a misconfigured grant cannot reach a site the node does not
  host.
- An out-of-scope site is **404, not 403**. 403 confirms existence — it would
  tell one partner that another is hosted beside them. Listings are filtered for
  the same reason: naming a site that would then be refused discloses exactly
  what the refusal withholds.
- **Site is the only axis enforced**, and deliberately generic — a reactor, a
  flow loop, a factory, a data centre. Org, fleet and role are how a grant is
  *computed*, not separate filters; two filters that disagree is how a resource
  becomes reachable by one path and hidden by another.
- **Undeclared stays unbounded.** A development box must work, and defaulting to
  "serve nothing" turns a missing environment variable into an outage that reads
  like missing data.

Before this, `/api/v1/sites` returned every site in the catalog to anyone who
could reach the endpoint.

### Catalog schema

- The catalog now ships an Alembic revision instead of calling `create_all`.
  `create_all` adds what is missing and says nothing about what exists and
  differs, so a column added later would never appear on a node that already had
  the table — the schema drifts from the code that reads it, silently.
- The revision carries the `site` index the scoped listing needs before a fleet
  turns the listing into a table scan.

## 0.48.0 — 2026-09-15

One versioned API, and three capabilities a domain consumer no longer has to
build for itself.

### One API surface

- `/api/v1` is the single versioned surface. Verbs **contribute routers** —
  declared as `kind = "api"` in their own manifest and resolved lazily — so the
  API shell imports no verb and installing one does not drag in the rest.
  Subpath collisions are refused rather than resolved by import order, and a
  contribution that fails to load costs its own verb while saying so.
- Serving catalog on that surface: `/api/v1/sites`, `/sites/{site}/channels`
  from an indexed projection, and `/sites/{site}/series` reading gold directly.
  Series points carry `min`/`max`/`n` alongside `avg`, because a mean hides the
  excursion a reader is usually looking for.

### Retrieval reaches the rows

- `rag.harvest` renders database records to cards and upserts them into the
  corpus as they are written, which is the ingest axis artifact-based ingest
  never covered. Rendering happens at flush (the only moment a deleted record
  is still describable), queueing at commit (a rolled-back record was never
  real), and the durable queue reuses `locked_append_jsonl` rather than adding a
  table.

### A library whose vocabulary is the product's

- New `library` extension: intake, classification and placement, with document
  types and categories supplied as a registered taxonomy. Domain-extracted
  detail rides in an `attributes` mapping the platform never interprets, so a
  second product is a registration rather than a fork.

### Prompts can be named and tried

- `prompt_stack_id` gives a prompt *design* a stable id so turns that shared one
  can be grouped and compared. Only the cacheable layers are hashed; per-turn
  context differs by construction and would make every turn unique.
- `prompt_managed` allows a wording to be tried without a release. The shipped
  text is a required fallback, so no lookup can return silence, and a local
  override still outranks a managed one.

### Provenance

- The daq envelope now refuses `source_class = "measured"` on a record that
  names a model — the missing half of a rule that only caught the harmless
  direction — and a new check reports channels whose coverage is identical
  across sites, which is how simulator output gets found masquerading as
  measurement.

### Auth

- `webauth` verifies legacy bcrypt hashes alongside scrypt, so a product with
  existing users can adopt Axiom's auth without invalidating every password made
  before the switch. `needs_rehash` upgrades one on successful login.

## 0.44.1 — 2026-09-04

### Added
- **Prediction provenance on canonical silver.** `silver.signals` gains
  `model_ref` (which model/parameter set produced a predicted row) and
  `basis` (`live` vs historical/retrodiction backfill), migrated in place on
  deployed tables and exposed through both gold views. Upserts default
  `basis='live'`; normalizers pass provenance through untouched.

## 0.44.0 — 2026-09-04

### Added
- **Canonical silver conformance (ADR-023).** `data_platform.conformance`:
  one conformed `silver.signals` shape for every site and reactor type, a
  schema_ref-keyed `NormalizerRegistry` (the provider-registry pattern, one
  hop later), a bronze `_rows` walker with per-line error quarantine and an
  unknown-schema funnel, idempotent upserts by `(row_hash, channel)`, and the
  uniform `gold.signals` / `gold.signals_latest` views. Domain normalizers
  live in downstream platform extensions; deployments contribute only their
  connector→site mapping.

## 0.43.2 — 2026-09-03

### Fixed
- **The catalog reads with either postgres driver.** The Dagster asset graph's
  lakehouse-catalog mirror required `psycopg` (v3), but `dagster-k8s` images ship
  only `psycopg2` — so deployed graphs silently showed no catalog assets.
  `_catalog_rows` now falls back to `psycopg2` when `psycopg` is absent.
- **The graph honors catalog curation.** Datasets marked `visibility='system'`
  in the lakehouse catalog (staging schemas, scratch tables) are filtered from
  the asset graph, along with any lineage edges touching them — casual explorers
  see the medallion, not the plumbing. Pre-curation catalogs (no visibility
  column) still load everything.

## [0.43.1] — 2026-09-03 — The asset graph reads the catalog

### Added

- **Catalog-driven Dagster assets (#741)** — when a deployment's database
  carries a lakehouse `catalog` schema (`datasets` + `table_lineage`), the
  Dagster definitions mirror it as external assets at every code-server
  load: tier groups, live view-dependency edges, and the seeded external
  hops (object stores, push faces, streaming bridges, producers). The
  graph tracks deployments instead of drifting; environments without a
  catalog load unchanged.

## [0.43.0] — role bundles and the universal action audit

### Added

- **Role bundles (webgate)** — a role can now *mean* something mechanical:
  `ScopeBundle` names a role and the gate scopes (`<mount>[:read|invoke|access]`)
  it grants, and a `BundleRegistry` resolves any set of roles to the union of
  their scopes. Axiom ships only two generic bundles as examples — `admin`
  (`*`) and `viewer` (`*:read`); domain consumers register their own role
  vocabulary at startup via `register_default_bundles`. Deployments may narrow
  (never widen) what a role grants through a TOML overrides file
  (`load_overrides`): removing scopes or defining a new role inside existing
  grants is fine, while any scope the defaults cannot already grant raises
  `OverrideWidensError` naming the offender — a config file is never a
  privilege-escalation path.
- **`--apply-bundle` on `gate.adduser` / `gate.issue`** — opt-in wiring from
  roles to grants: `adduser --role viewer --apply-bundle` pins the bundle's
  resolved scopes onto the account record (`attributes.bundle_scopes`), and
  `issue --role viewer --apply-bundle` mints the key with the union of the
  bundles' scopes (plus any explicit `--scope`). Without the flag, nothing
  changes — roles stay the free-form strings they always were.
- **JIT default role** — an SSO account created just-in-time with no roles
  claim from the IdP now lands with a role instead of a role-less limbo:
  `$AXIOM_GATE_JIT_ROLE`, defaulting to `viewer` (set it empty to opt out).
  An IdP roles claim still always wins.
- **Universal action audit** — every mutating skill invocation, from every
  surface that routes through `SkillRegistry.invoke` (CLI verbs, MCP tools,
  agent personas — ADR-056), now leaves one tamper-evident record in an
  HMAC-SHA256 hash chain at `<state_dir>/audit/actions.jsonl`: who
  (`SkillContext.actor`, `@cli:local` by default), what (skill + a digest of
  the params with secret-shaped values redacted *before* hashing), when, and
  how it ended. The chain uses the platform's existing scheme and key
  (`$AXIOM_AUDIT_HMAC_KEY`; the documented `unkeyed` sentinel before a key is
  provisioned), `verify_chain` proves integrity, and skills can opt out only
  by declaring themselves read-only (`register(..., mutating=False)`). An
  audit-write failure never fails the skill — swallowed, counted, warned.

## [0.42.4] — 2026-09-01 — Release umbrellas tell their story too

### Fixed

- **Releases items carry the commit body (#726)** — repos that squash-merge
  everything under `release:` subjects got a terse Releases list with the
  story stuck in the bodies. Release items now render like Features:
  subject — first body paragraph.

## [0.42.3] — 2026-09-01 — Features tell the story, fixes stay direct

### Changed

- **Changelog speaks human (#723)** — sections are now **Features** (title plus
  a benefit-bearing description — deterministically the commit body\'s first
  paragraph; via `--ai`, a written 1–2 sentence "what you can do now" naming
  the user type), **Bug fixes** (one direct sentence: broken → works), and a
  quiet "Other changes" tail. Commit bodies feed both paths; the AI schema
  separates features from fixes explicitly.

## [0.42.2] — 2026-09-01 — The deploy alert tells the story

### Added

- **`axi release changelog` (#722)** — a human-readable changelog between any
  two refs, out of the box: curated `CHANGELOG.md` sections when the repo
  keeps them, conventional-prefix-grouped commit subjects when it doesn't.
  `--format text|markdown|json`, `--since-state`/`--update-state` for deploy
  pipelines that must diff "what actually shipped last".
- **Teams cards render the body (#722)** — the notifications dispatcher already
  passed `body_text` to adapters that accept it; the Teams webhook adapter now
  does, as a subtle wrapped block under the headline.

## [0.42.1] — 2026-09-01 — Fresh timers fire without a kick

### Fixed

- **systemd timers gain `OnActiveSec` (#721)** — a timer installed mid-uptime
  had only `OnBootSec` (long past) and `OnUnitActiveSec` (needs a prior service
  run), so `axi agents register` reported "running" while NEXT stayed "-" until
  someone started the service by hand. `OnActiveSec` schedules from timer
  activation, so the chain starts itself.

## [0.42.0] — 2026-08-31 — The gate knows the neighborhood

### Added

- **Issuer presets (#719)** — `AXIOM_GATE_OIDC_<NAME>_PROVIDER=https://<issuer>`
  already accepted any discovery-compliant IdP; now well-known issuers are
  recognized (Google, Microsoft/Entra, AWS Cognito, Cloudflare Access, Okta,
  Auth0, GitLab, Keycloak realms) and get the right glyph, a branded default
  label ("Continue with Okta"), and the right default subject claim — zero
  extra configuration. Unknown issuers keep working with the generic mark.
- Webgate README grows recipes for the popular issuers.

## [0.41.2] — 2026-08-31 — Subjects travel fully quoted

### Fixed

- **mailto subjects URL-quote the product name (#718)** — a product name with
  spaces produced `subject=UT TRIGA Reactor%20account%20request` (raw spaces in
  the href). Now `urllib.parse.quote` encodes the whole subject.

## [0.41.1] — 2026-08-31 — The administrator is one click away

### Added

- **`LoginBrand.support_email` (#717)** — when set, "Ask your administrator"
  on the login card and the forgot-password page become `mailto:` links with a
  pre-filled subject; the SPA payload carries `supportEmail`. Unset keeps the
  plain text.

## [0.41.0] — 2026-08-31 — The gate offers every door at once

### Added

- **Multiple OIDC providers on the gate (#716)** — Portkey-style sign-in:
  the primary `AXIOM_GATE_OIDC_*` block (the institution button, historical
  routes) plus `AXIOM_GATE_OIDC_PROVIDERS=google,microsoft,…`, each reading its
  own `AXIOM_GATE_OIDC_<NAME>_*` keys and routing at
  `/gate/oidc/<name>/login|callback`. One button per provider with Google /
  Microsoft / generic marks, then the password form. `providers_from_env()`;
  per-provider JWKS caches; a half-configured block still refuses loudly.
- **The login card says how an account comes to exist** — `LoginBrand.login_hint`
  (custom line) > `signup_url` ("Create an account") > a JIT provider ("First
  time here? Use <label> and your account is created automatically.") > "ask
  your administrator". Server-rendered page and the SPA payload
  (`providers`, `loginHint`, `signupUrl`) both carry it.

## [0.40.3] — 2026-08-27 — The stuck-value probe ignores the clock

### Fixed

- **DAQ stuck-value detection (#715)** — `ConsolidatedRecord.record_hash()`
  excluded nothing, so a source re-emitting the same reading under a fresh
  receive timestamp never looked stuck (spec §11 "unchanged content with
  advancing seq"). Found on the first live producer with its source idle: 30
  identical readings, `stuck_run` 0. The hash now covers what the record says
  (values, tags, quality, sensitivity, schema) and not when it was received.

## [0.40.2] — 2026-08-27 — The Journal lets go of what it has delivered

### Fixed

- **DAQ Journal compaction (#714)** — `DAQJournal.compact()` releases whole
  segments that lie behind *every* cursor's committed position (Transmitter
  and subscribers alike); a lagging cursor pins its segments. The Producer
  compacts on a cadence (`compact_every`, default every 50 steps). Found on
  the first live producer: a healthy stream was on course to fill `max_bytes`
  with delivered history and then `block_producer` on itself within a day.
  Health gains `compacted`.

## [0.40.1] — 2026-08-27 — The ingest face answers on the composed app

### Fixed

- **`/ingest` mount is one flat router (#713)** — the two lanes were nested
  through `include_router`; under FastAPI ≥ 0.138's lazy include a lane nested
  inside a lazily included router never materialised on the composed app: the
  middleware named the mount, the hook admitted the caller, FastAPI answered
  404. The lanes' routes are appended directly, and a test now drives both
  lanes through `compose_app` + the middleware with an env token.

## [0.40.0] — 2026-08-26 — Identity: roles that reach `decide()`; the push-ingest gateway

The identity release, and the signal-ingest release. Membership resolution
becomes a pluggable seam with a reconciler that removes as well as adds, the
governance actor gains roles and assurance, a browser signs in once at the
gate (password or an upstream IdP) and every decision downstream knows who is
acting — and under which **site**. On the data side, the push-ingest gateway
(ADR-106/107) lands: a row lane on the served face whose tenancy comes from
the credential, and a generic producer core that journals first and delivers
at-least-once.

### Added

- **Directory seam, ADR-103 phases 1–3 (#690, #698, #692)** — `directory`
  builtin: a `DirectoryProvider` protocol with capabilities, `entra`
  (Graph `/memberOf`, injected client, paginated) / `oidc_claims` / `local`
  adapters, `GroupRoleMap` (first-match rules, empty default), a claims-first
  `MembershipResolver` whose degradation only ever *removes* authority, a
  `MembershipReconciler` that reconciles rather than appends (an empty desired
  set removes everyone; a stale projection raises `StaleProjectionError`
  instead of driving deletions), the recently-revoked fast path (`RevokedSet`),
  and `subject_from_membership` projecting groups onto `SubjectContext` as
  contextual tuples.
- **`ActorContext`, the assurance ladder and the `STEP_UP_REQUIRED` verdict
  (ADR-084, partial)** — `governance.actor.resolve_actor` composes handle,
  tenant, roles, attributes and an `Assurance` (posture → AAL/acr) from
  verified claims; out-of-band roles win over the token's claim.
- **Credential discovery sweep + KEEP's heartbeat to run it (#696)** —
  `axi secrets discover` finds credential material living outside the store
  (recursive, and now matching self-generated secrets, not only vendor shapes).
- **Memory capture across tools (#695)** — the silent cross-tool capture
  failure is fixed; Cursor and Hermes clients; the at-turn capture hook and
  `axi memory capture-sweep` with its staleness signal.
- **`TabularIngestSink` — the row-lane push core (#681)**, test-proven.
- **Teams: single-tenant Bot Connector authority (#686)** — a single-tenant
  registration no longer 700016s against the multi-tenant endpoint.
- **`webgate` OIDC sign-in — "Sign in with <your institution>" (#701)** — on the
  `auth` PKCE + JWKS building blocks; accounts keyed on the immutable subject
  (Entra `oid`), password accounts linked not duplicated, just-in-time
  creation; `WritableUserStore`; SSO-bound records in the accounts file.
- **`ActorContext` + the directory seam on every HTTP request (#702)** —
  `http/actor.py`; the gate's session cookie as a first-class credential
  (`sso` vs `attested`); `chain_resolvers`; role-scoped `Rule.roles`.
- **OpenFGA HTTP client (#704)** — `FgaCheckClient` *and* `TupleStore` in one
  object (check / paginated read / chunked write), starter model as JSON,
  `AXIOM_OPENFGA_*` / `authz.openfga.*` registration into `DecideContext`.
- **`axi directory sync|status` and the cadenced projection (#705)** — the
  900 s `[agent]` heartbeat, `JsonFileRevokedSet`, `JsonFileTupleStore`,
  `AXIOM_DIRECTORY_*` config.
- **The site on the principal (#706)** — `User.site`, session `site` claim,
  `X-Axiom-User-Site`, `axi gate adduser|issue --site`, API keys bound to a
  site (a key can never act under another site's name).
- **Push-ingest gateway — `POST /ingest/rows` with tenancy from the credential
  (#710, ADR-106)** — the row lane of the served ingest face; `site` and the
  access-tier ceiling are resolved from the principal, never the payload
  (foreign site → 403, tier above ceiling → 403, unknown tier → 422, site
  stamped on every batch); one `/ingest` mount carries both lanes.
- **DAQ producer core + the `push` source kind (#711, ADR-106/107)** —
  `data_platform.daq`: `DAQReader → DAQConsolidator → DAQJournal →
  {DAQTransmitter, LiveDispatcher}`; the Journal is written first, every
  consumer is a cursor into it, each stream carries a gapless `seq` and a
  `prev_hash`/`content_hash` chain, overflow policy is gated by sensitivity and
  delivery class, the Transmitter delivers at-least-once and dead-letters
  definitive refusals, health carries silence detection and a credited-stream
  guard. `axi data register --kind push` names a producer's landing zone.

### Fixed

- **Hooks clear stale artifacts before the gate judges, and hook installs
  refresh (#694).**
- **Action-ledger tests are isolated from an exported
  `AXIOM_AUDIT_HMAC_KEY` (#689).**

### Docs

- ADR-102 builder runbooks standard (#687); ADR-103 directory providers
  (#688) → Accepted, ADR-075 status refreshed (#707); the spec template (#680);
  ADR-106 push ingest gateway, ADR-107 signal protocol strategy, and the
  signal-ingest PRD + spec (#710).

### Upgrade notes

- The accounts file gains optional `site` and `attributes` fields and permits a
  `null` `password_hash` only for IdP-bound records; the API-keys file gains
  `site`. Existing files load unchanged.
- New environment: `AXIOM_DIRECTORY_{PROVIDER,LOCAL_FILE,ROLE_MAP,SYNC_GROUPS,
  TUPLE_STORE,REVOKED_FILE,ENTRA_*}`, `AXIOM_OPENFGA_{URL,STORE_ID,MODEL_ID,
  TOKEN|TOKEN_FILE,ON_ERROR}`, `AXIOM_GATE_OIDC_*`, `AXIOM_GATE_PRINCIPAL_CONTEXT`,
  `AXIOM_SITE`, and for the ingest face `AXIOM_ACCESS_TIER_LADDER`,
  `AXIOM_INGEST_MAX_TIER`, `AXIOM_INGEST_SITE_MAX_TIERS`. All unset ⇒ behaviour
  identical to 0.39.0 — except that a pushed batch may no longer assert a
  `site` (or request an `access_tier`) its credential does not grant.
- The `directory` extension declares a daemon heartbeat; the Background
  Service dispatches it only where autonomy is enabled.

## [0.39.0] — 2026-08-10 — Fail-closed serving, grounded output, MCP 2.0

A serving-hardening release. The provenance gate that keeps a model from
citing quantities its evidence does not support moves into the platform, the
dev-mode anonymous convenience stops leaking onto network binds, and the MCP
surface is ported to the 2.0 SDK.

### Added

- **RAG output-provenance gate (#675)** — post-synthesis gate blocking
  fabricated tool citations and quantities unsupported by the turn's evidence,
  deciding support on parsed *values* rather than strings (display rounding,
  config-gated unit scales, and numeric closure over tool-result arrays all
  match honestly; an unrelated value matches nothing). Ships with
  `context_budget` (deterministic overflow detection + conversation trimming,
  so an over-limit request retries smaller instead of surfacing a misleading
  "please retry") and `tabular` (whole-file table parsing with honest
  truncation notes and deterministic IQR/z-score outliers). Domain-agnostic:
  consumer layers inject their own quantity patterns and constants.
- **Remote-peer retrieval credentials (#668)** — `create_store` threads
  `AXIOM_RAG_REMOTE_SIGNATURE` / `AXIOM_RAG_REMOTE_NODE_ID` from the
  environment into `RemoteRetrievalStore`, so a peer enforcing auth gets a real
  `X-Signature` and the secret never rides the store URL or committed settings.
  Absent → the previous dev-mode defaults, unchanged.
- **Vendor-API rotation wiring (#656)** — `VendorHttpClient` (stdlib `urllib`,
  no new dependency) plus `RotationConfig`/`build_vendor_strategy` resolve the
  vendor admin credential through the SecretStore seam, so
  `axi secrets rotate --strategy <vendor>` runs end to end. The credential and
  every response body are excluded from errors and logs — a mint response is
  itself the new secret.

### Fixed

- **Dev anonymous permit-all is loopback-only (#663, SRV-022)** — dev mode is
  the *default* on an unconfigured box, and `compose_app` was auto-wiring a
  permit-all authz hook regardless of bind address, so an auth-required mount
  could be served anonymously on a network interface. The dev convenience is
  now gated on a loopback bind; unknown or network binds fail closed. A
  credentialed serve is unaffected on any bind, with no anonymous fallback
  off-loopback.
- **MCP 2.0 SDK migration (#677)** — `mcp` 2.0 replaced the server decorator
  registration with constructor callbacks (`on_list_tools`/`on_call_tool`/…,
  taking `(ctx, params)` and returning `*Result` models) and renamed every wire
  field's Python attribute to snake_case. All three servers (root, classroom,
  memory) are ported. The JSON wire format is unchanged — 2.0 keeps camelCase
  via pydantic aliases.

### Changed

- **`mcp` floor raised to `>=2.0`** — the 2.0 call style does not run on 1.x.
  Environments installing this release must upgrade the SDK; the previous
  unbounded `mcp>=1.0` pin is what allowed 2.0 to break CI silently.

## [0.38.0] — 2026-07-24 — Guardrail track: provenance, hygiene, foreign credentials

The autonomous-agent guardrail release: every guarded agent action is now
journaled to a queryable ledger, the hygiene agent detects the git-cruft
classes that previously accumulated silently, and foreign credentials get a
keychain-backed store with issuer-side rotation.

### Added

- **Action-provenance ledger (#666, closes #665)** — every `guarded_act`
  action or refusal from every agent journals to Postgres
  (`policy.agent_actions`; JSONL fallback), queryable via `axi audit actions`,
  MCP tools `axiom_actions_recent`/`axiom_actions_search` on the memory
  server, and a Neut Chat tool. Always-on in standard mode; refusals are
  first-class records.
- **Foreign-credential store + KEEP rotation (#672, refs #667)** —
  `axi secrets set/list/get/rm/audit` backed by the macOS keychain
  (provider factory), `secrets rotate` with issuer-side rotation
  (`gitlab-pat` self-rotate API, guided fallback), a git
  credential-helper (`secrets git-credential` + `wire-git`), expiry audit
  findings, and manifest `[extension.secrets] requires` with AEOS080/081
  lint. Secret values never transit MCP or the ledger.
- **Category-level git hygiene (#664)** — squash-aware landed-branch
  detection (`content_landed` via merge-tree), upstream-gone branches with
  stranded-content protection, 7-day stash dormancy + guarded
  stash→archive-branch executor, primary-checkout untracked-file findings,
  open-PR drift escalation at 50+ behind, all live behind the new
  `axi hygiene stat git` verb with an end-to-end wiring test.
- **Memory session plane + port (#660)** — `axi memory port` one-command
  onboarding, bundle v3 harness session stores (claude-code/codex) with
  fail-closed secret gate and never-overwrite restore; ADR-097 (gateway
  serving face), ADR-098 (session plane).
- **Tabular source lane P0–P3 (#658)** and **productized DB backups +
  PULSE production dispatch (#659)** in data_platform.

### Fixed

- **Fragment-side secret gate (#661)** — memory fragments are scanned before
  riding bundles; `--skip-fragment` escape hatch; claude-code headless
  no-API-key gateway provider; ADR-099 harness-shim contract.
